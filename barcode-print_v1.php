...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* Simple / generic layouts: fill mm box — not 82×38 (uses JS mm sizing). */
        body:not(.barcode-print--82x38-2box) .barcode-svg-wrap:not(.barcode-svg-wrap--qr) svg,
        body:not(.barcode-print--82x38-2box) .barcode-print-wrap svg {
            width: 100% !important;
            height: 100% !important;
            margin: 0;
            display: block;
        ...
        /* 82×38: dimensions come from auragoldFitBoxBarcodeSvgToGraphic inline mm — never auto/100%. */
        .barcode-print--82x38-2box .barcode-print-wrap svg {
            display: block !important;
            margin: 0 !important;
            padding: 0 !important;
            shape-rendering: crispEdges;
        ...
        @media screen {
            body.barcode-print--82x38-2box .barcode-container {
                margin: 16px auto;
        ...
        .barcode-print--82x38-2box .barcode-box-inner .barcode-print-wrap svg,
        .barcode-print--82x38-2box .barcode-print-wrap svg {
            display: block;
            margin: 0;
            padding: 0;
        ...
        @media print {
            ...
            * {
            ...
            .barcode-print--82x38-2box .barcode-print-wrap {
                line-height: 0 !important;
            }
...
<body class="barcode-print <?php echo $use_design ? 'barcode-print--design' : 'barcode-print--simple'; ?><?php echo $is_120x50_double_barcode ? ' barcode-print--120x50-double' : ''; ?><?php echo $is_82x38_2box ? ' barcode-print--82x38-2box' : ''; ?><?php echo $item_count === 1 ? ' barcode-print--single-page' : ''; ?>">
    ...
    <script>
        var auragoldPrintBarW = <?php echo (int) max(2, min(4, $barcode_bar_width_px)); ?>;
        var auragoldPrintMinBarHmm = 8;
        ...
        function auragoldFitBoxBarcodeSvgToGraphic(el, graphicSelector) {
            ...
            if (!graphic) return;
            var pxPerMm = auragoldMeasureBrowserPxPerMm();
            if (!(pxPerMm > 0)) pxPerMm = auragoldCssPxPerMm;
            var boxWMm = parseFloat(el.getAttribute('data-barcode-width-mm') || '0');
            var boxHMm = parseFloat(el.getAttribute('data-barcode-height-mm') || '0');
            if (!(boxWMm > 0)) boxWMm = auragoldMmFromStyle(graphic, 'width');
            if (!(boxHMm > 0)) boxHMm = auragoldMmFromStyle(graphic, 'height');
            var gRect = graphic.getBoundingClientRect();
            if (!(boxWMm > 0) && gRect.width > 0) boxWMm = gRect.width / pxPerMm;
            if (!(boxHMm > 0) && gRect.height > 0) boxHMm = gRect.height / pxPerMm;
            boxHMm = Math.max(auragoldPrintMinBarHmm, boxHMm);
            boxWMm = Math.max(4, boxWMm);
            var maxWPx = Math.max(8, Math.round(boxWMm * pxPerMm));
            var targetBarHPx = Math.max(Math.round(auragoldPrintMinBarHmm * pxPerMm), Math.round(boxHMm * pxPerMm));
            if (gRect.height > 4) targetBarHPx = Math.max(targetBarHPx, Math.round(gRect.height));
            if (maxWPx <= 0 || targetBarHPx <= 0) return;
            var code = String(el.getAttribute('data-barcode') || '').trim();
            if (!code) return;
            var modulePx = Math.max(2, Math.min(4, Math.round(auragoldPrintBarW)));
            var quietPx = Math.max(3, Math.min(8, modulePx * 3));
            function draw(modW) {
                modW = Math.max(2, Math.min(4, Math.round(modW)));
                el.innerHTML = '';
                JsBarcode(el, code, {
                    format: 'CODE128',
                    renderer: 'svg',
            ...
            var sz = readSvgSize();
            if (sz.w > maxWPx) {
                draw(Math.max(2, Math.floor(modulePx * (maxWPx / sz.w))));
                sz = readSvgSize();
            }
            if (!(sz.w > 0)) sz.w = maxWPx;
            sz.h = targetBarHPx;
            var svgWMm = Math.min(boxWMm, Math.round((sz.w / pxPerMm) * 100) / 100);
            var svgHMm = Math.round((targetBarHPx / pxPerMm) * 100) / 100;
            svgHMm = Math.max(auragoldPrintMinBarHmm, svgHMm);
            el.setAttribute('width', String(sz.w));
            el.setAttribute('height', String(targetBarHPx));
            el.removeAttribute('viewBox');
            el.removeAttribute('preserveAspectRatio');
            el.setAttribute('data-barcode-width-mm', String(svgWMm));
            el.setAttribute('data-barcode-height-mm', String(svgHMm));
            el.style.setProperty('width', svgWMm + 'mm', 'important');
            el.style.setProperty('height', svgHMm + 'mm', 'important');
            el.style.setProperty('max-width', boxWMm + 'mm', 'important');
            el.style.setProperty('max-height', svgHMm + 'mm', 'important');
            el.style.setProperty('min-width', svgWMm + 'mm', 'important');
            el.style.setProperty('min-height', svgHMm + 'mm', 'important');
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (document.body.classList.contains('barcode-print--82x38-2box') && !printQr) {
                ...
                requestAnimationFrame(function() {
                    auragoldRenderAll82x38DesignBarcodes();
                    requestAnimationFrame(auragoldRenderAll82x38DesignBarcodes);
                });
                setTimeout(auragoldRenderAll82x38DesignBarcodes, 120);
                setTimeout(auragoldRenderAll82x38DesignBarcodes, 400);
                setTimeout(auragoldRenderAll82x38DesignBarcodes, 800);
            }
