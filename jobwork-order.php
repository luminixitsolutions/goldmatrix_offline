...
$edit_payments = [];
$current_jwo_id = 0;
/** Used by includes/voucher_diamond_stone_panels.php (must be set before payment-area markup). */
$auragold_voucher_ds_kind = 'jobwork_order';
...
if ($edit_order_id > 0) {
    ...
    if ($jwo_row) {
        ...
        /** Payments (metal exchange, cash, etc.) are stored on tbl_sale_order_payments — JWO row has no payment lines. */
        $jwo_linked_so_id = (int)($jwo_row['sale_order_id'] ?? 0);
        if ($jwo_linked_so_id > 0) {
            $edit_payments = getList('SELECT * FROM tbl_sale_order_payments WHERE order_id = ' . $jwo_linked_so_id);
            ...
            if ($jwo_ro) {
                ...
                if ($existing_rjwo && !empty($existing_rjwo['id'])) {
                    ...
                    if (isset($existing_rjwo['status']) && (string)$existing_rjwo['status'] !== '') {
                        $edit_order['jwo_status'] = $existing_rjwo['status'];
                    }
...
$jwo_departments = getList("SELECT id, dept_name AS name, process_type FROM tbl_departments WHERE status = 1 ORDER BY dept_name ASC");
...
if ($ensure_jwo_dept_id > 0) {
    ...
    if (!$have_dept) {
        $extra_dept = getRecord("SELECT id, dept_name AS name, process_type FROM tbl_departments WHERE id = $ensure_jwo_dept_id LIMIT 1");
        if ($extra_dept) {
...
// Department is required on JWO from sale order; do not auto-select "Main Office" so the Name list can show all ledgers until a department is chosen.
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-4">
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control form-control-sm" id="jwoDepartment" required>
                                                        <option value="">--Select--</option>
                                                        <?php foreach ($jwo_departments as $d):
                                                            $dept_process = trim((string)($d['process_type'] ?? ''));
                                                            $dept_suffix = ($dept_process === 'Manufacturing Outsource') ? 'Outsource' : 'Insource';
                                                        ?>
                                                        <option value="<?php echo (int)$d['id']; ?>" data-process-type="<?php echo htmlspecialchars($dept_process !== '' ? $dept_process : 'Manufacturing Inhouse', ENT_QUOTES, 'UTF-8'); ?>"<?php echo ($ensure_jwo_dept_id > 0 && (int)$d['id'] === $ensure_jwo_dept_id) ? ' selected' : ''; ?>><?php echo htmlspecialchars($d['name'] . ' (' . $dept_suffix . ')'); ?></option>
                                                        <?php endforeach; ?>
                                                    </select>
                                                </div>
                                            </div>
...
<script>
    ...
    if (!empty($edit_order_id) && $edit_order_id > 0 && !empty($edit_order) && is_array($edit_order)) {
        /** Refresh payment rows from source order before embed (fixes empty cards when opening JWO by ?id=). Skip OJ scrap enrich overwrite. */
        if (!empty($is_jobwork_from_sale_order) && $is_jobwork_from_sale_order && empty($jwo_from_oj_scrap_refinery) && empty($is_jobwork_from_repair_order)) {
            $__reload_pay_so = (int)($jwo_sale_order_id_param ?? 0);
            if ($__reload_pay_so <= 0 && !empty($edit_url_id_is_jobwork_order) && !empty($current_jwo_id) && (int)$current_jwo_id > 0) {
                $__rso = getRecord('SELECT sale_order_id FROM tbl_jobwork_orders WHERE id = ' . (int)$current_jwo_id . ' LIMIT 1');
                $__reload_pay_so = (int)($__rso['sale_order_id'] ?? 0);
            }
            if ($__reload_pay_so <= 0 && empty($edit_url_id_is_jobwork_order) && !empty($edit_order['id'])) {
                $__reload_pay_so = (int)$edit_order['id'];
            }
            if ($__reload_pay_so > 0) {
                $edit_payments = getList('SELECT * FROM tbl_sale_order_payments WHERE order_id = ' . $__reload_pay_so);
                if (!is_array($edit_payments)) {
                    $edit_payments = [];
                }
                if (!empty($edit_url_id_is_jobwork_order)) {
                    $so_pay_hdr = getRecord(
                        'SELECT metal_amt, paid_amt, balance_amt FROM tbl_sale_orders WHERE id = ' . $__reload_pay_so . ' LIMIT 1'
                    );
                    if ($so_pay_hdr && is_array($so_pay_hdr)) {
                        foreach (['metal_amt', 'paid_amt', 'balance_amt'] as $__ph) {
                            if (array_key_exists($__ph, $so_pay_hdr) && $so_pay_hdr[$__ph] !== null && $so_pay_hdr[$__ph] !== '') {
                                $edit_order[$__ph] = $so_pay_hdr[$__ph];
                            }
                        }
                    }
                }
    ...
    function jwoSetPendingDepartmentForNewJobWorker() {
    ...
    /** Job Work Order (+): Ledger Details modal, default Customer Type = Job Worker (new ledger). */
    ...
    /**
     * (+) next to Name: edit selected / matched job worker ledger, else add with name prefilled.
     */
    async function openJwoJobWorkerLedgerModalFromUi() {
        ...
        if (term.length >= 1) {
            ...
            try {
                ...
                const res = await fetch('ajax/search-jwo-department-users.php?' + params.toString(), {
                    method: 'GET',
                    credentials: 'same-origin'
                });
                const data = await res.json();
                const users = (data && data.status === 'success' && Array.isArray(data.users)) ? data.users : [];
                ...
                users.forEach(function (u) {
                    ...
                    if (String(u.user_name || '').trim().toLowerCase() === termLc) {
                        pick = u;
                    }
                });
                ...
                console.error('openJwoJobWorkerLedgerModalFromUi', err);
            }
    ...
    function setCustomerModalMode(mode) {
        ...
        if (mode === 'edit') {
            if (label) {
                label.textContent = window.isJobworkFromSaleOrder ? 'Ledger Details' : 'Edit Customer';
            }
    ...
    function saveCustomer() {
        ...
        .then(data => {
            if (data.status === 'success' || data.success === true) {
                ...
                if (!window.isJobworkFromSaleOrder && typeof window.auragoldApplySavedCustomerToPartyDropdown === 'function') {
    ...
    function addEmptyProductRow() {
        ...
        // Generate the row HTML with all columns (empty values) — order matches #productListTable thead (common-modal-product-selection.php)
        row.innerHTML = `
    ...
    (function() {
        ...
        function reorderModalRowCellsToMatchHeader(row) {
            ...
            // Saved column order in DB can omit newer thead columns (e.g. item-code, product-category).
            // Merge persisted order with live header keys so no td is dropped — fixes header/body mismatch.
            var domKeys = [];
            headerRow2.querySelectorAll('th[data-column]').forEach(function(th) {
                var dk = th.getAttribute('data-column');
                if (dk && dk !== 'actions') domKeys.push(dk);
            });
            var mergedOrder = [];
            order.forEach(function(k) {
                if (k && k !== 'actions' && mergedOrder.indexOf(k) === -1) mergedOrder.push(k);
            });
            domKeys.forEach(function(k) {
                if (k !== 'actions' && mergedOrder.indexOf(k) === -1) mergedOrder.push(k);
            });
        ...
        function initProductSelectionModalColumnDrag() {
            ...
            function applySavedModalColumnOrder(orderedKeys) {
                ...
                headerRow2.querySelectorAll('th[data-column]').forEach(function(th) {
                    var k = th.getAttribute('data-column');
                    if (k && k !== 'actions') domKeys.push(k);
                });
                var mergedKeys = [];
                orderedKeys.forEach(function(k) {
                    if (headerMap[k] && mergedKeys.indexOf(k) === -1) mergedKeys.push(k);
                });
                domKeys.forEach(function(k) {
                    if (headerMap[k] && mergedKeys.indexOf(k) === -1) mergedKeys.push(k);
                });
                ...
                bodyRows.forEach(function(row) {
                    ...
                    Object.keys(map).forEach(function(k) {
                        if (mergedKeys.indexOf(k) === -1 && map[k]) row.appendChild(map[k]);
                    });
                    if (actionsCell) row.appendChild(actionsCell);
                });
                syncGroupHeaderOrderToColumnOrder(mergedKeys);
            }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (addBtn) {
                addBtn.addEventListener('click', function(e) {
                    ...
                    } else if (typeof openJwoJobWorkerLedgerModal === 'function') {
                        openJwoJobWorkerLedgerModal();
                    }
                });
            }
    ...
    function saveJobWorkOrder() {
        ...
        if (window.isJobworkFromSaleOrder && deptEl && (!deptEl.value || deptEl.value.trim() === '')) {
            ...
            if (groupItemsJson) {
                try {
                    ...
                    if (groupItems && groupItems.length > 0) {
                        ...
                        if (batch.length) itemBatches.push(batch);
                        return;
                    }
                } catch (e) { console.error('Parse data-group-items:', e); }
            }
        ...
        // New JWO from sale order: one tbl_jobwork_orders per Product List row (merged diamond group = one JWO with all components).
        var splitMultiJwo = window.isJobworkFromSaleOrder && !window.jwoFromRepair && currentJwoId < 1 && itemBatches.length > 1;
        if (splitMultiJwo) {
            var splitIdx = 0;
            var lastJwoId = '';
            ...
            function saveSplitNext() {
                if (splitIdx >= itemBatches.length) {
                    isSaving = false;
    ...
    function jwoCaptureManufacturingRedirect() {
        var dEl = document.getElementById('jwoDepartment');
        var wEl = document.getElementById('jwoDepartmentUser');
        ...
        if (dEl && dEl.selectedIndex >= 0) {
            ...
            if (opt) {
                proc = String(opt.getAttribute('data-process-type') || '').trim();
            }
        }
        window._jwoRedirectMpProcessType = proc;
    }
    function jwoManufacturingRedirectUrl() {
        var deptId = window._jwoRedirectMpDeptId ? String(window._jwoRedirectMpDeptId).trim() : '';
        if (!deptId) {
            return 'sale-order-process.php';
        }
        var processType = window._jwoRedirectMpProcessType ? String(window._jwoRedirectMpProcessType).trim() : '';
        var page = (processType === 'Manufacturing Outsource')
            ? 'manufacturing-outsource.php'
            : 'manufacturing-process.php';
        var qs = 'department_id=' + encodeURIComponent(deptId);
        var userId = window._jwoRedirectMpUserId ? String(window._jwoRedirectMpUserId).trim() : '';
        if (userId) {
            qs += '&user_id=' + encodeURIComponent(userId);
        }
        return page + '?' + qs;
    }
    ...
    function closePrintJWOModal() {
        ...
        if (window.jwoFromOjScrapRefinery) {
            ...
            window._jwoRedirectMpProcessType = null;
        ...
        var mpUrl = jwoManufacturingRedirectUrl();
        window._jwoRedirectMpDeptId = null;
        window._jwoRedirectMpUserId = null;
        window._jwoRedirectMpProcessType = null;
        window._jwoSavedJobworkNo = null;
    ...
    function confirmPrintJWO() {
        if (window.jwoFromOjScrapRefinery) {
            if (savedJwoId) {
                window.open('jobwork-order-print.php?id=' + encodeURIComponent(savedJwoId), '_blank');
            }
            closePrintJWOModal();
            return;
        }
        ...
        if (printId) {
            window.open('jobwork-order-print.php?id=' + encodeURIComponent(printId), '_blank');
        }
        var modal = document.getElementById('printJWOModal');
        if (modal) modal.style.display = 'none';
        savedJwoId = null;
    ...
    function addProductRowToSelectionTable(item, product) {
        ...
        // Generate the row HTML with all columns (populated with saved values) — match common-modal thead order (same as sale-order.php)
        row.innerHTML = `
            <td data-column="checkbox" style="text-align: center; background: #fff;">
        ...
        var isDiamondCatRestore = (item.category || '').toString().trim();
        var useDiamondOptions = (isDiamondCatRestore === 'Jewellery' || isDiamondCatRestore === 'Diamonds' || isDiamondCatRestore === 'GemStones');
    ...
    /**
     * Split flat sale-order / JWO line items into one array per Product List row.
     * Uses merge_group_index when saved; else Jewellery-boundary; else same design_no name-cycle.
     */
    ...
    function populateOrderForm(order, items, loadedPayments) {
        ...
        // Load items: merge diamond group lines into one Product List row (same as sale order).
        ...
        if (itemList.length > 0) {
            ...
            try {
                ...
                if (!added && typeof addProductToTableFromModalRow === 'function') {
                    itemList.forEach(function(item) {
                        addProductToTableFromModalRow(savedItemToModalRowData(item));
                    });
                    added = productTableBody && productTableBody.querySelectorAll('tr:not(.no-drag)').length > 0;
                }
                if (!added) {
                ...
                rowGroups.forEach(function(groupItems, gIdx) {
                    var r = dataRows[gIdx];
                    var oneItem = groupItems && groupItems[0];
