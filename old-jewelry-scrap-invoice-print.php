...
// Language from Invoice Print Settings when not passed in URL
$invoice_secondary = function_exists('getInvoicePrintSettingsForDocument') ? (getInvoicePrintSettingsForDocument('sale_invoice')['invoice_secondary_language'] ?? '') : '';
...
try {
    $invoice = getRecord("SELECT * FROM tbl_old_jewelry_scrap_invoices WHERE id = $invoice_id");
...
$items = getList("
    SELECT sii.*,
        COALESCE(NULLIF(TRIM(sii.description), ''), CONCAT('Item #', sii.id)) AS product_name,
        sii.gross_wt AS gross_weight,
        sii.final_wt AS final_weight,
        sii.net_wt AS net_weight,
        sii.less_wt AS less_weight,
        sii.making AS making_amount,
        sii.tax AS tax_amount,
        sii.net_amt AS net_amount,
        sii.net_amt AS net_amt_with_tax,
        sii.diamond_wt AS stone_weight,
        '' AS category_name
    FROM tbl_old_jewelry_scrap_invoice_items sii
    WHERE sii.invoice_id = $invoice_id ORDER BY sii.id ASC
");

$payments = getList("SELECT * FROM tbl_old_jewelry_scrap_invoice_payments WHERE invoice_id = $invoice_id ORDER BY id ASC");
...
foreach ($items as $item) {
    $total_weight += (float) ($item['gross_weight'] ?? $item['gross_wt'] ?? 0);
    $total_stone_weight += (float) ($item['stone_weight'] ?? $item['diamond_wt'] ?? $item['carat'] ?? 0);
    $total_net_wt += (float) ($item['net_weight'] ?? $item['net_wt'] ?? $item['final_weight'] ?? $item['final_wt'] ?? 0);
}

$customer_ref = $invoice['ref_no'] ?? ($invoice['customer_id'] ?? '');
if (empty($customer_ref) && !empty($invoice['customer_id'])) {
    $cust = getRecord("SELECT ref_no, mobile_no FROM tbl_customers WHERE id = " . (int)$invoice['customer_id'] . " LIMIT 1");
    $customer_ref = $cust['ref_no'] ?? $cust['mobile_no'] ?? $invoice['customer_id'];
}
...
$print_settings = (function_exists('getInvoicePrintSettingsForDocument') ? getInvoicePrintSettingsForDocument('sale_invoice') : null) ?: (function_exists('getInvoicePrintSettingsDefaults') ? getInvoicePrintSettingsDefaults() : [
...
// Override header/footer content from invoice print settings when set
...
$back_url = 'old-jewelry-scrap-invoice.php';
$document = $invoice;

// Structural template: from settings or template_preview GET (validated)
$template = isset($_GET['template_preview']) ? trim((string)$_GET['template_preview']) : (function_exists('getInvoiceTemplateForDocument') ? getInvoiceTemplateForDocument('sale_invoice') : 'template_classic');
