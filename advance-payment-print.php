...
if (!$voucher) {
    die('Advance payment voucher not found');
}
...
$items = getList("
    ...
    FROM tbl_advance_payment_items api
    LEFT JOIN tbl_products p ON api.product_id = p.id
    LEFT JOIN tbl_metal m ON api.metal_id = m.id
    WHERE api.voucher_id = $voucher_id
    ORDER BY api.id ASC
");
...
<head>
    ...
    <title><?php echo h($voucher['voucher_no'] ?? 'AP'); ?> — Advance Payment</title>
    <style>
...
<body<?php if (function_exists('invoicePrintBodyPaddingAttr')) echo invoicePrintBodyPaddingAttr($print_settings); ?>>
    ...
    <div class="invoice <?php echo h($design_template); ?>">
        ...
        <div class="inv-tax-badge"><?php echo h(!empty($print_settings['invoice_title']) ? $print_settings['invoice_title'] : 'Advance Payment'); ?> — <?php echo h($voucher['voucher_no'] ?? ''); ?></div>
        <?php endif; ?>

        <div class="sheet">
            <?php if (($print_settings['header_company_name'] ?? '1') !== '1'): ?>
            <h1 style="margin: 0 0 8px; font-size: 22px; color: #11294b;"><?php echo h($company_name); ?></h1>
            <p class="sub">Advance Payment — <?php echo h($voucher['voucher_no'] ?? ''); ?></p>
            <?php endif; ?>
            ...
            <div class="grid">
                ...
                <?php endif; ?>
                <?php if (!empty($voucher['receipt_no'])): ?>
                <div><strong>Receipt No.</strong> <?php echo h($voucher['receipt_no']); ?></div>
                <?php endif; ?>
                ...
                <?php endif; ?>
                <?php if (!empty($voucher['against_of'])): ?>
                <div><strong>Against</strong> <?php echo h($voucher['against_of']); ?></div>
                <?php endif; ?>
            </div>
                ...
                <tbody>
                    ...
                    foreach ($items as $row) {
                        ...
                        $detail_parts = array_filter([
                            ...
                            (float)($row['purity_wt'] ?? 0) > 0 ? 'Pure wt: ' . number_format((float)$row['purity_wt'], 3) : '',
                            (float)($row['weight'] ?? 0) > 0 ? 'Gross wt: ' . number_format((float)$row['weight'], 3) : '',
                        ]);
                    ...
                    <tr>
                        ...
                        <td style="text-align:right"><?php echo ($ptype === 'Metal' || $ptype === 'Scrap') && $amt <= 0 ? '—' : number_format($amt, 2); ?></td>
                    </tr>
                    <?php } ?>
                    <?php if (empty($items)): ?>
