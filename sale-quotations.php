...
$auragold_voucher_runtime_client = auragold_voucher_runtime_bootstrap($conn, $metals, 'Sales Quotation');
...
// Next quotation no: Bill Series for voucher "Sales Quotation" (tbl_bill_series + tbl_voucher_types); else legacy SQ-1, SQ-2
$next_order_no = function_exists('getNextSalesQuotationNo') ? getNextSalesQuotationNo($conn) : 'SQ-1';

// Sales Person dropdown (stored in tbl_sale_quotations.sales_person) — branch-scoped
$sales_person_users = auragold_sales_person_user_display_names($conn_master);

// Load quotation for editing if ID provided (tbl_sale_quotations)
$edit_order_id = filter_input(INPUT_GET, 'id', FILTER_VALIDATE_INT);
$edit_order = null;
$edit_items = [];
$edit_payments = [];

$auragold_voucher_ds_kind = 'sale_quotation';
$auragold_voucher_ds_db_id = (int) ($edit_order_id ?? 0);

if (!empty($edit_order_id)) {
    $edit_order = getRecord("SELECT * FROM tbl_sale_quotations WHERE id = " . intval($edit_order_id));
    if ($edit_order) {
        try {
            auragold_branch_require_document_access($conn, 'tbl_sale_quotations', (int) $edit_order_id);
        } catch (Exception $e) {
            $edit_order = null;
        }
    }
    if ($edit_order) {
        ...
        $edit_items = getList("
            ...
            FROM tbl_sale_quotation_items i
            LEFT JOIN tbl_product_characteristics pc ON pc.id = i.product_characteristic_id AND pc.product_id = i.product_id
            WHERE i.quotation_id = " . intval($edit_order_id));
        $edit_payments = getList("SELECT * FROM tbl_sale_quotation_payments WHERE quotation_id = " . intval($edit_order_id));
        $next_order_no = $edit_order['quotation_no'] ?? '';
        if (!isset($edit_order['order_no'])) {
            $edit_order['order_no'] = $edit_order['quotation_no'] ?? '';
        }
        ...
        $edit_order['order_date'] = $edit_order['quotation_date'] ?? $edit_order['order_date'] ?? '';
        $edit_order['invoice_date'] = $edit_order['quotation_date'] ?? '';
...
// if (defined('SQ_EDIT_DEBUG') && SQ_EDIT_DEBUG) { echo '<pre>edit_order_id=' . var_export($edit_order_id, true) . "\ngetRecord result=" . print_r($edit_order, true) . '</pre>'; }

// Allowed Sale Quotation print languages (English + optional one from Invoice Print Settings)
...
text-transform: uppercase;
    ...
    /* —— Sale Quotations: mobile / tablet (≤991.98px) —— */
    @media (max-width: 991.98px) {
        ...
        .billing-form .sq-quotation-billing-grid > [class*="col-"] {
            flex: 0 0 100% !important;
            max-width: 100% !important;
        }
        .billing-form .sq-quotation-billing-grid > [class*="col-"] > .form-group {
        ...
        .billing-form .sq-quotation-billing-grid > [class*="col-"] > .form-group > label:first-of-type {
            position: absolute;
            top: 0;
            left: 10px;
            transform: translateY(-50%);
        ...
        .billing-form .sq-quotation-billing-grid > [class*="col-"] > .form-group > .d-flex,
        .billing-form .sq-quotation-billing-grid > [class*="col-"] > .form-group > div:not(.input-group) {
            width: 100%;
            flex: 1 1 auto;
            min-width: 0;
        }
        .billing-form .sq-quotation-billing-grid .add-customer-icon,
        .billing-form .sq-quotation-billing-grid .feather.icon-search {
            top: 50% !important;
        }
                        ...
                        <div class="col-lg-8" >
                            ...
                            <div class="card mb-4">
                                <div class="card-body billing-form">
                                        <div class="row sq-quotation-billing-grid">
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label>Search Sale Quotation</label>
                                                    <div style="position: relative;">
                                                        <input type="text" class="form-control form-control-sm" id="searchSaleInvoice" placeholder="Search by customer name or quotation number..." autocomplete="off" style="padding-right: 35px;">
                                        ...
                                        <?php
                                        $sq_order_date_val = date('Y-m-d');
                                        $sq_due_date_val = '';
                                        $sq_sp_selected = '';
                                        if (!empty($edit_order) && is_array($edit_order)) {
                                            if (!empty($edit_order['quotation_date'])) {
                                                $sq_order_date_val = substr($edit_order['quotation_date'], 0, 10);
                                                $sq_due_date_val = auragold_due_date_value($edit_order['due_date'] ?? null);
                                            } else {
                                                $sq_due_date_val = auragold_due_date_value($edit_order['due_date'] ?? null);
                                            }
                                            $sq_sp_selected = trim((string)($edit_order['sales_person'] ?? $edit_order['purchase_person'] ?? ''));
                                        }
                                        $sq_sp_in_list = false;
                                        if ($sq_sp_selected !== '') {
                                            foreach ($sales_person_users as $_spn) {
                                                if (strcasecmp($sq_sp_selected, $_spn) === 0) {
                                                    $sq_sp_in_list = true;
                                                    break;
                                                }
                                            }
                                        ...
                                        <div class="col-md-3">
                                            <div class="form-group">
                                                <label>Date</label>
                                                <input type="date" class="form-control form-control-sm" id="orderDate" value="<?php echo htmlspecialchars($sq_order_date_val); ?>">
                                            </div>
                                        </div>
                                        <div class="col-md-3">
                                            <div class="form-group">
                                                <label>Due Date</label>
                                                <input type="date" class="form-control form-control-sm" id="dueDate" value="<?php echo htmlspecialchars($sq_due_date_val); ?>">
                                            </div>
                                        </div>
                                        <div class="col-md-3">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control form-control-sm" id="salesPerson" data-placeholder="Sales Person">
                                                    ...
                                                    <option value="<?php echo htmlspecialchars($sp_name); ?>"<?php echo ($sq_sp_selected !== '' && strcasecmp($sq_sp_selected, $sp_name) === 0) ? ' selected' : ''; ?>><?php echo htmlspecialchars($sp_name); ?></option>
                                                    <?php endforeach; ?>
                                                    <?php if ($sq_sp_selected !== '' && !$sq_sp_in_list): ?>
                                                    <option value="<?php echo htmlspecialchars($sq_sp_selected); ?>" selected><?php echo htmlspecialchars($sq_sp_selected); ?></option>
                                                    <?php endif; ?>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="col-md-3">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control form-control-sm" id="fixingType">
                                                    <?php $sq_fix = (!empty($edit_order) && is_array($edit_order)) ? ($edit_order['fixing_type'] ?? 'Standard') : 'Standard'; ?>
                                                    <option value="Standard"<?php echo ($sq_fix === 'Standard') ? ' selected' : ''; ?>>Standard</option>
                                                    <option value="Hedging"<?php echo ($sq_fix === 'Hedging') ? ' selected' : ''; ?>>Hedging</option>
                                                </select>
                                            </div>
                                        </div>
                            ...
                            <?php
                            $product_list_prefs_page = 'sale-quotation-product-table';
                            ...
                            <div class="summary-panel" >
                                ...
                                <div class="invoice-header" style="margin: -1rem -1rem 1rem -1rem; border-radius: 10px 10px 0 0; display: flex; align-items: center; justify-content: space-between;">
                                    <div style="display: flex; align-items: center; gap: 0.5rem; flex: 1;">
                                        <i class="feather icon-printer" id="printInvoiceIcon" style="font-size: 12px; color: #94a3b8; opacity: 0.5; cursor: not-allowed; pointer-events: none;" title="Save quotation first to print"></i>
                                        <h5 class="mb-0" style="font-size: 0.9rem;">Sale Quotation No: <span id="currentOrderNo"><?php echo htmlspecialchars($next_order_no); ?></span></h5>
                                    </div>
    ...
    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p class="text-muted small mb-2" id="againstOrderModalHint">Showing documents for the selected customer (or supplier, for purchase documents).</p>
...
<?php if (function_exists('auragold_echo_product_modal_spec_masters_js')) { auragold_echo_product_modal_spec_masters_js(); } ?>
    ...
    window.isSaleQuotationEditMode = <?php echo (!empty($edit_order_id) && $edit_order_id > 0) ? 'true' : 'false'; ?>;
</script>
<script>
window.PB_PAGE_CONFIG = {
    ...
    onAfterLoad: function () {
        if (window.isSaleQuotationEditMode) return;
    ...
    skipAutoLoad: function () { return !!window.isSaleQuotationEditMode; }
};
</script>
...
<script>
    ...
    if (!empty($edit_order_id) && $edit_order_id > 0 && !empty($edit_order) && is_array($edit_order)) {
        $qno = $edit_order['quotation_no'] ?? $edit_order['order_no'] ?? '';
        ...
        $embed_order = [
            ...
            'against_of_ref' => $edit_order['against_of'] ?? '',
            ...
            'quotation_date' => $edit_order['quotation_date'] ?? $edit_order['order_date'] ?? '',
            'invoice_date' => $edit_order['quotation_date'] ?? $edit_order['invoice_date'] ?? $edit_order['order_date'] ?? '',
            'order_date' => $edit_order['quotation_date'] ?? $edit_order['order_date'] ?? $edit_order['invoice_date'] ?? '',
        ...
        $embed_order['diamond_issues'] = auragold_voucher_list_diamond_issue_rows_for_kind($conn, 'sale_quotation', (int) ($edit_order['id'] ?? 0));
        $embed_order['stone_issues'] = auragold_voucher_list_stone_issue_rows_for_kind($conn, 'sale_quotation', (int) ($edit_order['id'] ?? 0));
    ...
    const PRODUCT_MODAL_COLUMNS_PAGE = 'sale-quotation-product-modal';
    ...
    (function initJewelryCatalogSaleQuotationImport() {
        var jcatStorageKey = 'auragold_jcat_sale_quotation_items';
        ...
        function runJewelryCatalogImport() {
            try {
                ...
                loadJewelryCatalogItemsToSaleQuotation(items).then(function (res) {
                    res = res || { added: 0, failed: [] };
                    if (res.added > 0 && typeof swal === 'function') {
                        swal({
                            title: fromDass ? 'Stock items added' : 'Catalogue items added',
                            text: res.added + ' item(s) added to the quotation product list. Complete customer and other details, then save.',
                            type: 'success',
                            timer: 2800,
                ...
                console.error('Stock/catalog quotation import failed', e);
            }
        }
    ...
    $(document).ready(function() {
        ...
        $(document).on('input', '#customerName', function() {
            ...
            if (searchTerm.length < 2) {
                ...
                var cbsClrSq = document.getElementById('customerBillingState');
                if (cbsClrSq) cbsClrSq.value = '';
                    ...
                    success: function(response) {
                        if (response.status === 'success' && response.customers && response.customers.length > 0) {
                            ...
                            response.customers.forEach(function(customer) {
                                var _bsSq = (customer.billing_state != null ? String(customer.billing_state) : '').replace(/"/g, '&quot;');
                                html += `
                                    <div class="customer-suggestion-item" 
                                         data-customer-id="${customer.id}" 
                                         data-customer-name="${customer.name}"
                                         data-billing-state="${_bsSq}"
        ...
        $(document).on('click', '.customer-suggestion-item', function() {
            ...
            var cbsSq = document.getElementById('customerBillingState');
            if (cbsSq) cbsSq.value = billingState;
            ...
            if (document.getElementById('againstId')) document.getElementById('againstId').value = '';
            if (document.getElementById('againstOfRef')) document.getElementById('againstOfRef').value = '';
            if (document.getElementById('againstOf')) document.getElementById('againstOf').value = '';
        ...
        // ================== AGAINST OF: source documents (modal) ==================
        var sqCurrentAgainstType = '';
        var sqAgainstSearchTimeout = null;
        var SQ_AGAINST_TYPES_WITH_LIST = ['Consignment Out', 'Delivery Note', 'Repair Order', 'Sale Order', 'Sale Invoice', 'Sale Quotation', 'Purchase Quotation', 'Purchase Order'];
        window.SQ_AGAINST_TYPES_WITH_LIST = SQ_AGAINST_TYPES_WITH_LIST;

        function sqClearAgainstIdOnly() {
        ...
        function sqClearProductListAndAgainstId() {
            sqClearAgainstIdOnly();
        ...
        function sqLoadAgainstOrderList() {
            var type = sqCurrentAgainstType;
            ...
            try {
                ...
                excludeQid = parseInt(_up.get('id') || '0', 10) || 0;
            } catch (e0) { excludeQid = 0; }
            var url = 'ajax/list-quotation-against-documents.php?type=' + encodeURIComponent(type) + '&customer_id=' + encodeURIComponent(cid) + (q ? '&q=' + encodeURIComponent(q) : '') + (excludeQid > 0 && type === 'Sale Quotation' ? '&exclude_quotation_id=' + encodeURIComponent(excludeQid) : '');
            fetch(url)
                .then(function(r) { return r.json(); })
                .then(function(data) {
        ...
        $('#againstOrderModal').on('hidden.bs.modal', function() {
            ...
            if (aid && !String(aid.value || '').trim() && $('#againstOf').length) {
                ...
                if (SQ_AGAINST_TYPES_WITH_LIST.indexOf(selVal) >= 0) {
                    $('#againstOf').val('');
                }
            }
        });
        ...
        $('#againstOrderModal').on('show.bs.modal', function() {
            ...
            if (el) el.value = '';
            sqLoadAgainstOrderList();
        });
        $(document).on('input', '#againstOrderSearch', function() {
            clearTimeout(sqAgainstSearchTimeout);
            sqAgainstSearchTimeout = setTimeout(sqLoadAgainstOrderList, 300);
        });
        ...
        $('#againstOf').on('change', function() {
            ...
            if (val === 'Task / Event') {
                alert('No document list is available for Task / Event.');
                $(this).val('');
                sqClearAgainstIdOnly();
                return;
            }
            ...
            if (SQ_AGAINST_TYPES_WITH_LIST.indexOf(val) >= 0) {
                sqCurrentAgainstType = val;
        ...
        saleInvoiceSearchInput.on('input', function() {
            ...
            saleInvoiceSearchTimeout = setTimeout(function() {
                $.ajax({
                    url: 'ajax/search-sale-quotations.php',
                    type: 'GET',
                    data: { q: searchTerm },
                    dataType: 'json',
                    success: function(response) {
                        if (response.status === 'success' && response.quotations && response.quotations.length > 0) {
                            let html = '';
                            response.quotations.forEach(function(q) {
                                html += `
                                    <div class="invoice-suggestion-item" 
                                         data-invoice-id="${q.id}" 
                                         data-invoice-no="${q.quotation_no}"
                                         style="padding: 0.75rem; cursor: pointer; border-bottom: 1px solid #f1f5f9; transition: background 0.2s;"
                                         onmouseover="this.style.background='#f8fafc'" 
                                         onmouseout="this.style.background='#fff'">
                                        <div style="font-weight: 600; color: #1e293b; font-size: 0.9rem;">${q.quotation_no}</div>
                                        <div style="font-size: 0.85rem; color: #64748b; margin-top: 0.25rem;">${q.customer_name || ''}</div>
                                        ${q.formatted_date ? '<div style="font-size: 0.75rem; color: #94a3b8; margin-top: 0.15rem;"><i class="feather icon-calendar" style="font-size: 0.7rem;"></i> ' + q.formatted_date + '</div>' : ''}
                                        ${q.grand_total ? '<div style="font-size: 0.75rem; color: #10b981; margin-top: 0.15rem; font-weight: 500;"><i class="feather icon-dollar-sign" style="font-size: 0.7rem;"></i> ' + q.currency + ' ' + parseFloat(q.grand_total).toFixed(2) + '</div>' : ''}
                                    </div>
                                `;
                            });
                            ...
                            saleInvoiceSuggestions.html('<div style="padding: 0.75rem; color: #94a3b8; text-align: center;">No sale quotations found</div>').show();
                        }
                    },
                    error: function() {
                        saleInvoiceSuggestions.hide();
        ...
        // Handle quotation selection from suggestions → open in edit mode (?id=)
        $(document).on('click', '#saleInvoiceSuggestions .invoice-suggestion-item', function() {
            ...
            saleInvoiceSuggestions.hide();
            if (invoiceId > 0) {
                window.location.href = 'sale-quotations.php?id=' + invoiceId;
            }
        });
    ...
    function filterProductsByMetal(metalId) {
        ...
        if (visibleCount === 0) {
            ...
            if (!placeholderRow) {
                ...
                tr.innerHTML = '<td colspan="103" class="text-center text-muted py-4">No products found for this category</td>';
    ...
    function initCategoryTabs() {
        ...
        if (tbody && tbody.querySelectorAll('tr.product-row').length === 0) {
            ...
            if (!placeholder) {
                tbody.innerHTML = '<tr class="no-category-products-placeholder"><td colspan="103" class="text-center text-muted py-4">Click "Add Product" or select a category tab to add products</td></tr>';
            }
        }
    }
...
<td data-column="sale-amount-with"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" readonly style="width: 130px; font-size: 0.7rem;"></td>
        ...
        if (typeof jQuery !== 'undefined' && jQuery.ajax) {
            ...
            success: function(response) {
                ...
                <i class="feather icon-edit" style="cursor: pointer; font-size: 0.8rem; color: #c5a864; margin-right: 8px;" onclick="editProductRowInTable(this)" title="Edit Row"></i>
                    ...
                    var prRowsSq = tbody.querySelectorAll('.product-row');
                    response.products.forEach(function(p, i) {
                        if (!prRowsSq[i]) return;
                        var pcp = prRowsSq[i].querySelector('.product-category-select');
            ...
            fetch(url)
                ...
                .then(data => {
                    if (data.success && data.products.length > 0) {
                        ...
                        var prRowsSqFetch = tbody.querySelectorAll('.product-row');
                        data.products.forEach(function(p, i) {
                            if (!prRowsSqFetch[i]) return;
                            var pcp = prRowsSqFetch[i].querySelector('.product-category-select');
                            var pcid0 = p.category_id != null && p.category_id !== '' ? p.category_id : '';
                            if (pcp && pcid0) try { pcp.value = String(pcid0); } catch (e) {}
                        });
    ...
    function addMergedProductsToTable(modalRowsData, metalIdForImage, loadOpts) {
        ...
        var firstRowGiSq = '';
        for (var _agiSq = 0; _agiSq < modalRowsData.length; _agiSq++) {
            var rgi = modalRowsData[_agiSq].group_image;
            if (rgi && String(rgi).trim()) { firstRowGiSq = rgi; break; }
        }
        var groupImagePayload = tabPayloadMerge || ((typeof window.auragoldCoParseGroupImageAttr === 'function') ? window.auragoldCoParseGroupImageAttr(firstRowGiSq) : '') || '';
        var groupImageAttr = (typeof groupImagePayload === 'object' && groupImagePayload != null) ? JSON.stringify(groupImagePayload) : (groupImagePayload || '');
        row.setAttribute('data-group-image', groupImageAttr);
        var primaryUrl = typeof getGroupImagePrimary === 'function' ? getGroupImagePrimary(groupImagePayload) : (typeof groupImagePayload === 'string' ? groupImagePayload : '');
        var actionCell = '<td><div class="action-btns"><button type="button" class="btn-edit" onclick="editProductRow(\'' + rowId + '\')" title="Edit"><i class="feather icon-edit"></i></button><button type="button" class="btn-delete" onclick="deleteProductRow(\'' + rowId + '\')" title="Delete"><i class="feather icon-trash-2"></i></button></div></td>';
    ...
    function updateSummaryPanel() {
        ...
        // Overpayment shows as negative Balance Amt (customer credit/advance). save-sale-quotation posts this to balance_amt
        ...
        // For sale quotations: if remaining is negative (customer has credit), treat as 0 for "amount to pay"
        const remainingForDisplay = remainingPreviousBalance < 0 ? 0 : remainingPreviousBalance;
        
        // Show original previous balance (e.g. -102600) with same formatting as sale invoice; do NOT overwrite with remaining
        if (previousBalanceEl) {
    ...
    (function() {
        var PAGE_MODAL_COLUMN_PREFS = 'sale-quotation-product-modal';
        var TAB_MAIN = 'main';
        ...
        if (modalEl && typeof jQuery !== 'undefined') {
            jQuery(modalEl).off('shown.bs.modal.saleQuotationColumnDrag').on('shown.bs.modal.saleQuotationColumnDrag', function() {
                initProductSelectionModalColumnDrag();
            });
        }
    })();
    ...
    function saveOrder() {
        ...
        const currentOrderNoText = document.getElementById('currentOrderNo')?.textContent || <?php echo json_encode(isset($next_order_no) ? $next_order_no : 'SQ-1'); ?>;
        ...
        const orderData = {
            ...
            against_of: document.getElementById('againstOfRef')?.value || '',
        ...
        if (typeof jQuery !== 'undefined' && jQuery.ajax) {
            jQuery.ajax({
                url: 'ajax/save-sale-quotation.php',
                type: 'POST',
                data: postData,
                dataType: 'json',
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        if (invoiceId) {
                            ...
                            window.pendingRedirectUrl = 'sale-quotations.php';
                            setTimeout(function() {
                                showPrintInvoiceModal(invoiceId);
                            }, 100);
                        } else {
                            alert('Sale Quotation saved successfully! Quotation No: ' + (response.quotation_no || response.order_no || 'N/A'));
                            window.location.href = 'sale-quotations.php';
                        }
                    } else {
                        alert('Error: ' + (response.message || 'Failed to save sale quotation'));
                    }
                },
            ...
            fetch('ajax/save-sale-quotation.php', {
                method: 'POST',
                body: formData
            })
            .then(response => response.json())
            .then(data => {
                ...
                if (data.status === 'success') {
                    ...
                    if (invoiceId) {
                        ...
                        alert('Sale Quotation saved successfully! Quotation No: ' + (data.quotation_no || data.order_no || 'N/A'));
                        window.location.href = 'sale-quotations.php';
                    }
                } else {
                    alert('Error: ' + (data.message || 'Failed to save sale quotation'));
                }
            })
            .catch(error => {
                // Reset saving flag
    ...
    function printPurchaseInvoice(invoiceId) {
        ...
        if (!invoiceId || invoiceId <= 0) {
            alert('Please save the quotation first before printing.');
            return;
        }
        
        // Open print page in new window
        window.open('sale-quotation-print.php?id=' + invoiceId, '_blank', 'width=1200,height=800');
    }
    ...
    function updatePrintIcon(invoiceId) {
        ...
        if (printIcon) {
            if (invoiceId && invoiceId > 0) {
                ...
                printIcon.setAttribute('title', 'Print Quotation');
            } else {
                ...
                printIcon.setAttribute('title', 'Save quotation first to print');
            }
        }
    }
    ...
    function loadOrder(orderId) {
        ...
        loadingMsg.innerHTML = '<p>Loading quotation...</p>';
        document.body.appendChild(loadingMsg);
        
        // Fetch order data (cache-bust so we always get fresh data)
        var getUrl = 'ajax/get-sale-quotation.php?id=' + orderId + '&_=' + (Date.now ? Date.now() : '');
        if (typeof jQuery !== 'undefined' && jQuery.ajax) {
            jQuery.ajax({
                url: getUrl,
                type: 'GET',
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        // Update URL without reload and set edit mode so Save updates this quotation
                        window.history.pushState({}, '', 'sale-quotations.php?id=' + orderId);
                        window.isSaleQuotationEditMode = true;
                    } else {
                        alert('Error loading quotation: ' + (response.message || 'Unknown error'));
                    }
                },
                error: function(xhr, status, error) {
                    const msg = document.getElementById('orderLoadingMsg');
                    if (msg) document.body.removeChild(msg);
                    alert('Error loading quotation: ' + error);
            ...
            fetch(getUrl, { cache: 'no-store' })
                ...
                .then(data => {
                    ...
                    if (data.status === 'success') {
                        ...
                        alert('Error loading quotation: ' + (data.message || 'Unknown error'));
                    }
                })
                .catch(error => {
                    const msg = document.getElementById('orderLoadingMsg');
    ...
    function populateOrderForm(order, items, loadedPayments) {
        ...
        if (typeof window.__pbLoadGeneration === 'number') {
            window.__pbLoadGeneration += 1;
        } else {
            window.__pbLoadGeneration = 1;
        }
        ...
        if (document.getElementById('currentOrderNo')) {
            document.getElementById('currentOrderNo').textContent = order.quotation_no || order.invoice_no || order.order_no;
        }
        ...
        if (document.getElementById('againstOf')) {
            var typeVal = (order.against_type != null && order.against_type !== '') ? String(order.against_type) : '';
            if (!typeVal && order.against_of) {
                typeVal = String(order.against_of);
            }
            var sel = document.getElementById('againstOf');
            var hasOption = false;
            for (var i = 0; i < sel.options.length; i++) {
                if (sel.options[i].value === typeVal) { hasOption = true; break; }
            }
            if (typeVal && !hasOption) {
        ...
        if (document.getElementById('againstOfRef')) {
            var refOnly = (order.against_of_ref != null && order.against_of_ref !== '') ? String(order.against_of_ref) : '';
            var sqTypes = window.SQ_AGAINST_TYPES_WITH_LIST || [];
            if (!refOnly && order.against_of && sqTypes.indexOf(String(order.against_type || '')) >= 0) {
                refOnly = String(order.against_of);
            }
            document.getElementById('againstOfRef').value = refOnly;
        }
        ...
        if (itemList.length === 1) {
            var hdrNet = parseFloat(order.net_total || order.grand_total || 0) || 0;
            var lineNet = parseFloat(itemList[0].net_amt_with_tax || itemList[0].net_amount || itemList[0].amount || 0) || 0;
            if (hdrNet > lineNet + 0.05) {
                itemList[0].net_amt_with_tax = hdrNet;
                if (!(parseFloat(itemList[0].net_amount) > 0)) itemList[0].net_amount = hdrNet;
                if (!(parseFloat(itemList[0].amount) > 0)) itemList[0].amount = hdrNet;
            }
        }
        ...
        var _pfClearSq = document.getElementById('paymentTableFooter');
        if (_pfClearSq) _pfClearSq.style.display = 'none';
        ...
        if (loadedPayments && loadedPayments.length > 0) {
            loadedPayments.forEach(function(payment) {
                ...
                var _prevBalLoadSq = parseFloat(payment.previous_balance_amount || 0) || 0;
                var _dbTotalLoadSq = parseFloat(payment.amount || 0) || 0;
                var _dbCurLoadSq = payment.current_order_amount != null && payment.current_order_amount !== '' ? parseFloat(payment.current_order_amount) : NaN;
                var _curOrderLoadSq = !isNaN(_dbCurLoadSq) ? _dbCurLoadSq : Math.max(0, _dbTotalLoadSq - _prevBalLoadSq);
                const paymentData = {
                    ...
                    amount: _curOrderLoadSq,
                    previous_balance_amount: _prevBalLoadSq,
            ...
            var _pfEmptySq = document.getElementById('paymentTableFooter');
            if (_pfEmptySq) _pfEmptySq.style.display = 'none';
        }
        ...
        if (document.getElementById('summaryGrandTotal')) {
            document.getElementById('summaryGrandTotal').setAttribute('data-base-amount', savedGrandTotal.toFixed(2));
        }

        // Re-apply after any delayed customer-balance fetch so edit keeps saved quotation totals
        setTimeout(function () {
            if (document.getElementById('summaryTotal')) {
                document.getElementById('summaryTotal').textContent = savedSubtotal.toFixed(2);
            }
            ...
            var pbEl = document.getElementById('previousBalanceAmount');
            if (pbEl) {
                formatSalePreviousBalanceAmount(pbEl, prevAmt);
                pbEl.setAttribute('data-original-balance', prevAmt.toFixed(2));
            }
        }, 400);
    ...
    (function() {
        ...
        function doPopulateFromEmbed() {
            ...
            try {
                ...
                if (window.history && window.history.replaceState) window.history.replaceState({}, '', 'sale-quotations.php?id=' + editId);
                window.isSaleQuotationEditMode = true;
                return true;
            } catch (e) {
...
<div id="printInvoiceModal" class="print-invoice-modal" style="display: none;">
    <div class="print-invoice-modal-content">
        ...
        <p class="print-invoice-modal-message">Do you want to print this quotation?</p>
...
<script>
    ...
    function showPrintInvoiceModal(invoiceId) {
        ...
        setTimeout(function() {
            ...
            if (modal) {
                ...
                if (confirm('Sale Quotation saved successfully! Do you want to print the quotation?')) {
                    window.open('sale-quotation-print.php?id=' + invoiceId, '_blank', 'width=1200,height=800');
                }
    ...
    function confirmPrintInvoice() {
        if (savedInvoiceId) {
            window.open('sale-quotation-print.php?id=' + savedInvoiceId, '_blank', 'width=1200,height=800');
        }
