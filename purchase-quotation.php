...
$auragold_voucher_runtime_client = auragold_voucher_runtime_bootstrap($conn, $metals, 'Purchase Quotation');
...
// Product Selection modal — full tbody row template (all columns aligned with thead)
$pq_empty_row_path = __DIR__ . '/includes/fragments/product_modal_empty_row_inner.html';
$pq_modal_empty_row_html = '';
if (is_file($pq_empty_row_path)) {
    $pq_modal_empty_row_html = trim((string) file_get_contents($pq_empty_row_path));
}
...
// Next quotation no: Bill Series for voucher "Purchase Quotation" (tbl_bill_series + tbl_voucher_types); else legacy PQ-1
$next_order_no = function_exists('getNextPurchaseQuotationNo') ? getNextPurchaseQuotationNo($conn) : 'PQ-1';

// Purchase person dropdown (stored in tbl_purchase_quotations.purchase_person) — branch-scoped
$sales_person_users = auragold_sales_person_user_display_names($conn_master);

// Load quotation for editing if ID provided (tbl_purchase_quotations)
$edit_order_id = filter_input(INPUT_GET, 'id', FILTER_VALIDATE_INT);
$edit_order = null;
$edit_items = [];
$edit_payments = [];

$auragold_voucher_ds_kind = 'purchase_quotation';
$auragold_voucher_ds_db_id = (int) ($edit_order_id ?? 0);

if (!empty($edit_order_id)) {
    $edit_order = getRecord("SELECT * FROM tbl_purchase_quotations WHERE id = " . intval($edit_order_id));
    if ($edit_order) {
        try {
            auragold_branch_require_document_access($conn, 'tbl_purchase_quotations', (int) $edit_order_id);
        } catch (Exception $e) {
            $edit_order = null;
        }
    }
    if ($edit_order) {
        ...
        $edit_items = getList("
            ...
            FROM tbl_purchase_quotation_items i
        ...
        $edit_payments = getList("SELECT * FROM tbl_purchase_quotation_payments WHERE quotation_id = " . intval($edit_order_id));
        ...
        $edit_order['customer_name'] = $edit_order['supplier_name'] ?? $edit_order['customer_name'] ?? '';
        $edit_order['customer_id'] = isset($edit_order['supplier_id']) ? (int) $edit_order['supplier_id'] : (int) ($edit_order['customer_id'] ?? 0);
        $edit_order['sales_person'] = $edit_order['purchase_person'] ?? $edit_order['sales_person'] ?? '';
...
// Allowed Purchase Quotation print languages (English + optional one from Invoice Print Settings)
...
$pq_saved_flash = isset($_GET['saved']) && $_GET['saved'] === '1';
?>
<!DOCTYPE html>
...
text-transform: uppercase;
    ...
    /* —— Purchase Quotation: mobile / tablet (≤991.98px) —— */
                ...
                <div class="container-fluid flex-grow-1" style="padding-top: 0; padding-bottom: 0;">
                    ...
                    <div class="row invoice-content-row">
                        ...
                        <div class="col-lg-8" >
                                ...
                                <div class="card-body billing-form">
                                        <div class="row auragold-mq-billing-grid">
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label>Search Purchase Quotation</label>
                                        ...
                                        <div class="col-md-3">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control form-control-sm" id="salesPerson" data-placeholder="Purchase Person">
                            ...
                            <?php
                            $product_list_prefs_page = 'purchase-quotation-product-table';
                            ...
                            <div class="summary-panel" >
                                ...
                                <div class="invoice-header" style="margin: -1rem -1rem 1rem -1rem; border-radius: 10px 10px 0 0; display: flex; align-items: center; justify-content: space-between;">
                                    <div style="display: flex; align-items: center; gap: 0.5rem; flex: 1;">
                                        ...
                                        <h5 class="mb-0" style="font-size: 0.9rem;">Purchase Quotation No: <span id="currentOrderNo"><?php echo htmlspecialchars($next_order_no); ?></span></h5>
                                    </div>
...
<script>
    ...
    window.PQ_MODAL_EMPTY_ROW_HTML = <?php echo json_encode($pq_modal_empty_row_html, JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT); ?>;
...
<script>
    ...
    if (!empty($edit_order_id) && $edit_order_id > 0 && !empty($edit_order) && is_array($edit_order)) {
        ...
        $embed_order['diamond_issues'] = auragold_voucher_list_diamond_issue_rows_for_kind($conn, 'purchase_quotation', (int) ($edit_order['id'] ?? 0));
        $embed_order['stone_issues'] = auragold_voucher_list_stone_issue_rows_for_kind($conn, 'purchase_quotation', (int) ($edit_order['id'] ?? 0));
    ...
    /** Align tbody cells with thead (creates missing column placeholders). Uses shared product-modal-add-item-common.js. */
    function pqAlignProductModalRow(row) {
        if (!row) return;
        var tbl = document.getElementById('productListTable');
        if (typeof window.reorderModalRowCellsToMatchHeader === 'function') {
            window.reorderModalRowCellsToMatchHeader(row, tbl);
        }
        if (typeof window.stampProductModalDataGroupOnCells === 'function') {
            window.stampProductModalDataGroupOnCells(row);
        }
    }
    function pqAlignAllProductModalRows() {
    ...
    window.pqAlignProductModalRow = pqAlignProductModalRow;
    window.pqAlignAllProductModalRows = pqAlignAllProductModalRows;
    ...
    const PRODUCT_MODAL_COLUMNS_PAGE = 'purchase-quotation-product-modal';
    ...
    function fetchProductByBarcodeAndAdd(barcode) {
        ...
        fetch('ajax/get-product-by-barcode.php?barcode=' + encodeURIComponent(trimmed) + '&allow_non_stock_barcode=1')
    ...
    $(document).ready(function() {
        ...
        $(document).on('input', '#customerName', function() {
            ...
            if (searchTerm.length < 2) {
                ...
                var cbsClrPq = document.getElementById('customerBillingState');
                if (cbsClrPq) cbsClrPq.value = '';
                    ...
                    success: function(response) {
                        if (response.status === 'success' && response.customers && response.customers.length > 0) {
                            ...
                            response.customers.forEach(function(customer) {
                                var _bsPq = (customer.billing_state != null ? String(customer.billing_state) : '').replace(/"/g, '&quot;');
                                html += `
                                    <div class="customer-suggestion-item" 
                                         data-customer-id="${customer.id}" 
                                         data-customer-name="${customer.name}"
                                         data-billing-state="${_bsPq}"
        ...
        $(document).on('click', '.customer-suggestion-item', function() {
            ...
            var cbsPq = document.getElementById('customerBillingState');
            if (cbsPq) cbsPq.value = billingState;
        ...
        var SQ_AGAINST_TYPES_WITH_LIST = ['Purchase Order', 'Task / Event'];
        ...
        function sqLoadAgainstOrderList() {
            ...
            var url = 'ajax/list-quotation-against-documents.php?type=' + encodeURIComponent(type) + '&customer_id=' + encodeURIComponent(cid) + (q ? '&q=' + encodeURIComponent(q) : '') + (excludeQid > 0 && type === 'Purchase Quotation' ? '&exclude_quotation_id=' + encodeURIComponent(excludeQid) : '');
            fetch(url)
        ...
        saleInvoiceSearchInput.on('input', function() {
            ...
            saleInvoiceSearchTimeout = setTimeout(function() {
                $.ajax({
                    url: 'ajax/search-purchase-quotations.php',
                    type: 'GET',
                    data: { q: searchTerm },
                    dataType: 'json',
                    success: function(response) {
                        if (response.status === 'success' && response.quotations && response.quotations.length > 0) {
                            ...
                            saleInvoiceSuggestions.html('<div style="padding: 0.75rem; color: #94a3b8; text-align: center;">No purchase quotations found</div>').show();
                        }
                    },
                    error: function() {
                        saleInvoiceSuggestions.hide();
        ...
        $(document).on('click', '#saleInvoiceSuggestions .invoice-suggestion-item', function() {
            ...
            saleInvoiceSuggestions.hide();
            if (invoiceId > 0) {
                window.location.href = 'purchase-quotation.php?id=' + invoiceId;
            }
        });
    ...
    function addEmptyProductRow() {
        ...
        // Generate the row HTML with all columns (shared fragment — matches modal thead)
        var emptyHtml = (window.PQ_MODAL_EMPTY_ROW_HTML || window.JCC_MODAL_EMPTY_ROW_HTML || '').trim();
        if (!emptyHtml) {
            console.error('PQ_MODAL_EMPTY_ROW_HTML missing — cannot add product row');
            return;
        }
        ...
        // Reorder row cells to match current modal header order (creates placeholders for missing columns)
        pqAlignProductModalRow(row);
            ...
            .then(data => {
                if (data.success && data.products && data.products.length > 0) {
                    ...
                    if (typeof window.auragoldBindProductSearchResultItems === 'function') {
                        ...
                        bindProductSearchResultItems(resultsDiv);
                    }
                } else {
    ...
    function bindProductSearchResultItems(container) {
        if (!container) return;
        container.querySelectorAll('.product-search-item').forEach(function(item) {
            if (item._paProductSearchBound) return;
            item._paProductSearchBound = true;
    ...
    function populateRowWithProduct(row, product, opts) {
        ...
        if (!fromBarcode && typeof window.auragoldEnsureModalRowMetalQtyDefault === 'function') {
            setTimeout(function () {
                window.auragoldEnsureModalRowMetalQtyDefault(row);
            }, 80);
        }
            ...
            productSelectionModal.addEventListener('shown.bs.modal', function() {
                ...
                loadProductModalColumnPreferences(function() {
                    setTimeout(function() {
                        ...
                        if (typeof window.pqAlignAllProductModalRows === 'function') {
                            window.pqAlignAllProductModalRows();
                        }
                        var modalBody = document.querySelector('#productSelectionModal .modal-body');
                        if (modalBody) modalBody.scrollLeft = 0;
                        var scrollWrap = document.getElementById('productListTableScrollWrapper');
    ...
    function addMergedProductsToTable(modalRowsData, metalIdForImage) {
        ...
        var firstRowGiPq = '';
        for (var _agiPq = 0; _agiPq < modalRowsData.length; _agiPq++) {
            var rgi = modalRowsData[_agiPq].group_image;
            if (rgi && String(rgi).trim()) { firstRowGiPq = rgi; break; }
        }
        var groupImagePayload = tabPayloadMerge || ((typeof window.auragoldCoParseGroupImageAttr === 'function') ? window.auragoldCoParseGroupImageAttr(firstRowGiPq) : '') || '';
        var groupImageAttr = (typeof groupImagePayload === 'object' && groupImagePayload != null) ? JSON.stringify(groupImagePayload) : (groupImagePayload || '');
        row.setAttribute('data-group-image', groupImageAttr);
        var primaryUrl = typeof getGroupImagePrimary === 'function' ? getGroupImagePrimary(groupImagePayload) : (typeof groupImagePayload === 'string' ? groupImagePayload : '');
        var actionCell = '<td><div class="action-btns"><button type="button" class="btn-edit" onclick="editProductRow(\'' + rowId + '\')" title="Edit"><i class="feather icon-edit"></i></button><button type="button" class="btn-delete" onclick="deleteProductRow(\'' + rowId + '\')" title="Delete"><i class="feather icon-trash-2"></i></button></div></td>';
    ...
    (function() {
        var PAGE_MODAL_COLUMN_PREFS = 'purchase-quotation-product-modal';
        var TAB_MAIN = 'main';
    ...
    function saveOrder() {
        ...
        const currentOrderNoText = document.getElementById('currentOrderNo')?.textContent || <?php echo json_encode(isset($next_order_no) ? $next_order_no : 'PQ-1'); ?>;
        ...
        if (typeof jQuery !== 'undefined' && jQuery.ajax) {
            jQuery.ajax({
                url: 'ajax/save-purchase-quotation.php',
                type: 'POST',
                data: postData,
                dataType: 'json',
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        if (typeof window.auragoldVoucherDiamondStoneOnSaveSuccess === 'function') {
                            window.auragoldVoucherDiamondStoneOnSaveSuccess(response.invoice_id || response.order_id || response.quotation_id);
                        }
                        const invoiceId = response.invoice_id || response.order_id;
                        if (invoiceId) {
                            // After save: go to create page (not edit) when user closes print modal
                            window.pendingRedirectUrl = 'purchase-quotation.php?saved=1';
                            setTimeout(function() {
                                showPrintInvoiceModal(invoiceId);
                            }, 100);
                        } else {
                            alert('Purchase Quotation Created Successfully! Quotation No: ' + (response.quotation_no || response.order_no || 'N/A'));
                            window.location.href = 'purchase-quotation.php?saved=1';
                        }
                    } else {
                        alert('Error: ' + (response.message || 'Failed to save purchase quotation'));
                    }
                },
            ...
            fetch('ajax/save-purchase-quotation.php', {
                method: 'POST',
                body: formData
            })
            .then(response => response.json())
            .then(data => {
                ...
                if (data.status === 'success') {
                    if (typeof window.auragoldVoucherDiamondStoneOnSaveSuccess === 'function') {
                        window.auragoldVoucherDiamondStoneOnSaveSuccess(data.invoice_id || data.order_id || data.quotation_id);
                    }
                    ...
                    if (invoiceId) {
                        ...
                        alert('Purchase Quotation Created Successfully! Quotation No: ' + (data.quotation_no || data.order_no || 'N/A'));
                        window.location.href = 'purchase-quotation.php?saved=1';
                    }
                } else {
                    alert('Error: ' + (data.message || 'Failed to save purchase quotation'));
                }
            })
            .catch(error => {
                // Reset saving flag
    ...
    function resetOrder() {
        if (confirm('Are you sure you want to create a new invoice? All unsaved data will be lost.')) {
            window.location.href = 'purchase-quotation.php';
        }
    }
    ...
    function printPurchaseInvoice(invoiceId) {
        ...
        window.open('purchase-quotation-print.php?id=' + invoiceId, '_blank', 'width=1200,height=800');
    }
    ...
    function loadOrder(orderId) {
        ...
        var getUrl = 'ajax/get-purchase-quotation.php?id=' + orderId + '&for_edit=1&_=' + (Date.now ? Date.now() : '');
        if (typeof jQuery !== 'undefined' && jQuery.ajax) {
            jQuery.ajax({
                url: getUrl,
                type: 'GET',
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        window.history.pushState({}, '', 'purchase-quotation.php?id=' + orderId);
    ...
    (function() {
        ...
        function doPopulateFromEmbed() {
            ...
            try {
                ...
                if (window.history && window.history.replaceState) window.history.replaceState({}, '', 'purchase-quotation.php?id=' + editId);
                window.isSaleQuotationEditMode = true;
                return true;
            } catch (e) {
...
<script>
    ...
    function showPrintInvoiceModal(invoiceId) {
        ...
        setTimeout(function() {
            ...
            if (modal) {
                ...
                if (confirm('Purchase Quotation saved successfully! Do you want to print the quotation?')) {
                    window.open('purchase-quotation-print.php?id=' + invoiceId, '_blank', 'width=1200,height=800');
                }
    ...
    function confirmPrintInvoice() {
        if (savedInvoiceId) {
            window.open('purchase-quotation-print.php?id=' + savedInvoiceId, '_blank', 'width=1200,height=800');
        }
