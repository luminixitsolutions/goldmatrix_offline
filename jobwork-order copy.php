...
$auragold_voucher_runtime_client = auragold_voucher_runtime_bootstrap($conn, $metals, 'Jobwork Order');
...
$next_order_no = function_exists('getNextJobworkOrderNo') ? getNextJobworkOrderNo($conn) : 'JWO-1';
...
if ($edit_order_id > 0) {
    ...
    if ($jwo_row) {
        ...
        $next_order_no = $jwo_row['jobwork_no'];
...
$jwo_from_oj_scrap_refinery = isset($_GET['from_oj_scrap']) && (string) $_GET['from_oj_scrap'] === '1';
$jwo_oj_scrap_from_date = isset($_GET['oj_from_date']) ? trim((string) esc($_GET['oj_from_date'])) : '';
$jwo_oj_scrap_to_date = isset($_GET['oj_to_date']) ? trim((string) esc($_GET['oj_to_date'])) : '';
...
if ($jwo_sale_order_id_param > 0 && $current_jwo_id === 0) {
        ...
        if ($ok_ro) {
            ...
            if ($jwo_ro) {
                ...
                if (!$jwo_from_process_selection) {
                    $rj_chk = @mysqli_query($conn, "SHOW TABLES LIKE 'tbl_repair_jobwork_orders'");
                    if ($rj_chk && mysqli_num_rows($rj_chk) > 0) {
                        mysqli_free_result($rj_chk);
                        $existing_rjwo = getRecord("SELECT * FROM tbl_repair_jobwork_orders WHERE repair_order_id = " . (int)$jwo_sale_order_id_param . " ORDER BY id DESC LIMIT 1");
                    } elseif ($rj_chk) {
                        mysqli_free_result($rj_chk);
                    }
                }
                if ($existing_rjwo && !empty($existing_rjwo['id'])) {
                    ...
                    $jw_no_cmp = isset($existing_rjwo['jobwork_no']) ? trim((string)$existing_rjwo['jobwork_no']) : '';
                    if ($jw_no_cmp !== '') {
                        $next_order_no = $existing_rjwo['jobwork_no'];
                    }
                    if (isset($existing_rjwo['department_id']) && $existing_rjwo['department_id'] !== '' && $existing_rjwo['department_id'] !== null) {
                        $edit_order['department_id'] = (int)$existing_rjwo['department_id'];
                    }
                    if (isset($existing_rjwo['department_user_id']) && $existing_rjwo['department_user_id'] !== '' && $existing_rjwo['department_user_id'] !== null) {
                        $edit_order['department_user_id'] = (int)$existing_rjwo['department_user_id'];
                    }
                    if (isset($existing_rjwo['priority']) && (string)$existing_rjwo['priority'] !== '') {
                        $edit_order['priority'] = $existing_rjwo['priority'];
                    }
                    if (isset($existing_rjwo['status']) && (string)$existing_rjwo['status'] !== '') {
                        $edit_order['jwo_status'] = trim((string)$existing_rjwo['status']);
                    }
                    $rj_items = getList("SELECT * FROM tbl_repair_jobwork_order_items WHERE repair_jobwork_order_id = " . $jwo_current_rjwo_id);
                    $edit_items = [];
    ...
    if ($jwo_so) {
        ...
        if ($existing_jwo && !empty($existing_jwo['id'])) {
            ...
            $jw_no_cmp = isset($existing_jwo['jobwork_no']) ? trim((string)$existing_jwo['jobwork_no']) : '';
            ...
            if ($jw_no_cmp !== '' && $so_order_no_cmp !== '' && strcasecmp($jw_no_cmp, $so_order_no_cmp) === 0) {
                $cfg_jwo_fix = function_exists('getJobworkOrderBillSeriesConfig')
                    ? getJobworkOrderBillSeriesConfig($conn)
                    : ['prefix' => 'JWO-', 'suffix' => '', 'start_count' => 1, 'from_series_table' => false];
                $fix_no = function_exists('getNextJobworkOrderNo') ? getNextJobworkOrderNo($conn) : 'JWO-1';
                $fix_esc = mysqli_real_escape_string($conn, $fix_no);
                $existing_chk = getRecord("SELECT id FROM tbl_jobwork_orders WHERE jobwork_no = '$fix_esc'");
                $guard_fix = 0;
                while ($existing_chk && $guard_fix < 5000) {
                    $fix_no = function_exists('bumpJobworkOrderNo') ? bumpJobworkOrderNo($conn, $fix_no, $cfg_jwo_fix) : ($fix_no . '-1');
                    $fix_esc = mysqli_real_escape_string($conn, $fix_no);
                    $existing_chk = getRecord("SELECT id FROM tbl_jobwork_orders WHERE jobwork_no = '$fix_esc'");
                    $guard_fix++;
                }
                mysqli_query($conn, "UPDATE tbl_jobwork_orders SET jobwork_no = '$fix_esc' WHERE id = $current_jwo_id");
                $existing_jwo['jobwork_no'] = $fix_no;
                $next_order_no = $fix_no;
            } elseif ($jw_no_cmp !== '') {
                $next_order_no = $existing_jwo['jobwork_no'];
            }
            ...
            $jw_st_existing = isset($existing_jwo['status']) ? trim((string)$existing_jwo['status']) : '';
            $edit_order['jwo_status'] = $jw_st_existing !== '' ? $jw_st_existing : 'Processing';
            ...
            // else: keep sale order lines already loaded in $edit_items (JWO header exists but no lines saved yet)
        } else {
        ...
        if (!empty($jwo_from_oj_scrap_refinery)) {
            $bridge = __DIR__ . '/includes/auragold_oj_scrap_sale_order_bridge.php';
            if (is_file($bridge)) {
                require_once $bridge;
                if (function_exists('auragold_jobwork_enrich_sale_order_embed_from_oj_scrap')) {
                    auragold_jobwork_enrich_sale_order_embed_from_oj_scrap($conn, $edit_order, $edit_items, $edit_payments, (int) $current_jwo_id, !empty($jwo_from_oj_scrap_refinery));
                }
            }
        }
        $is_jobwork_from_sale_order = true;
...
if ($selected_item_ids_param !== '') {
    ...
    if (!empty($selected_item_ids)) {
        ...
        if (!empty($bulk_items)) {
            ...
            // All selected process-list lines appear on one JWO draft; save inserts one tbl_jobwork_orders row with multiple tbl_jobwork_order_items.
...
// Sales Person for Job Work: same as sale-invoice.php — branch-scoped tbl_users (not department-wise)
...
<head>
<title>Job Work Order — Gold Matrix</title>
                ...
                <div class="container-fluid flex-grow-1" style="padding-top: 0; padding-bottom: 0;">
                    ...
                    <div class="row invoice-content-row">
                        ...
                        <div class="col-lg-8" >
                            ...
                            <div class="card mb-4">
                                <div class="card-body billing-form">
                                        <div class="row">
                                            ...
                                            <?php endif; ?>
                                            <div class="col-md-<?php echo !empty($is_jobwork_from_sale_order) ? '4' : '6'; ?>">
                                                <div class="form-group">
                                                    <label>Search Job Work Order</label>
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control form-control-sm" id="againstOf">
                                                    ...
                                                    <option value="Job Work Order">Job Work Order</option>
                                        ...
                                        <?php if ($is_jobwork_from_sale_order): ?>
                                        <?php
                                        // Do not use sale/repair order `status` (e.g. Hold) as JWO status — that is order workflow, not job work.
                                        $__jwo_status_val = !empty($edit_order['jwo_status'])
                                            ? (string)$edit_order['jwo_status']
                                            : (!empty($edit_url_id_is_jobwork_order) && !empty($edit_order['status'])
                                                ? (string)$edit_order['status']
                                                : 'Processing');
                            ...
                            <?php
                            $product_list_prefs_page = 'jobwork-order-product-table';
                            ...
                            <div class="summary-panel" >
                                ...
                                <div class="invoice-header" style="margin: -1rem -1rem 1rem -1rem; border-radius: 10px 10px 0 0; display: flex; align-items: center; justify-content: space-between;">
                                    <div style="display: flex; align-items: center; gap: 0.5rem; flex: 1;">
                                        ...
                                        <h5 class="mb-0" style="font-size: 0.9rem;">Job Work Order No: <span id="currentOrderNo"><?php echo htmlspecialchars($next_order_no); ?></span></h5>
                                        <?php if (!empty($is_jobwork_from_sale_order) && $is_jobwork_from_sale_order): ?>
                                        <span id="jwoSaleOrderId" data-sale-order-id="<?php echo (int)$jwo_sale_order_id_param; ?>"></span>
                                        <span id="jwoCurrentId" data-jwo-id="<?php echo !empty($is_jobwork_from_repair_order) ? (int)$jwo_current_rjwo_id : (int)$current_jwo_id; ?>"></span>
                                        <?php endif; ?>
                                    </div>
...
<?php
if (empty($GLOBALS['auragold_common_modal_product_selection_included'])) {
    require __DIR__ . '/includes/common-modal-product-selection.php';
}
if (empty($GLOBALS['auragold_common_modal_payments_included'])) {
    require __DIR__ . '/includes/payment-modals-only.php';
}
?>
...
<?php
$auragold_jobwork_ds_cfg = (!empty($is_jobwork_from_sale_order) && $is_jobwork_from_sale_order)
    ? ['voucherKind' => 'jobwork_order', 'urlIdParam' => 'id', 'windowDbIdKey' => 'jwoCurrentRecordId']
    : ['voucherKind' => 'sale_order', 'urlIdParam' => 'id', 'windowDbIdKey' => '__saleOrderDbId'];
require __DIR__ . '/includes/sale-order-diamond-stock-modal.php';
require __DIR__ . '/includes/sale-order-stone-stock-modal.php';
?>
<script>
window.AURAGOLD_VOUCHER_DS = <?php echo json_encode($auragold_jobwork_ds_cfg, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT); ?>;
</script>
...
<script>
    ...
    /** Sale-order–linked JWO: customer is prefilled/readonly — always allow Add Item + modal. */
    if (window.isJobworkFromSaleOrder) {
        window.AURAGOLD_ADD_ITEM_ALWAYS_ENABLED = true;
        if (typeof window.saleInvoiceHasCustomerSelected === 'function') {
            (function () {
                var _origSaleInvoiceHasCustomer = window.saleInvoiceHasCustomerSelected;
                window.saleInvoiceHasCustomerSelected = function () {
                    if (_origSaleInvoiceHasCustomer()) return true;
    ...
    window.jwoFromOjScrapRefinery = <?php echo !empty($jwo_from_oj_scrap_refinery) ? 'true' : 'false'; ?>;
    window.jwoOjScrapReturnFromDate = <?php echo json_encode((string) ($jwo_oj_scrap_from_date ?? ''), JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT); ?>;
    window.jwoOjScrapReturnToDate = <?php echo json_encode((string) ($jwo_oj_scrap_to_date ?? ''), JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT); ?>;
...
<script>
    ...
    if (!empty($edit_order_id) && $edit_order_id > 0 && !empty($edit_order) && is_array($edit_order)) {
        ...
        if (empty($is_jobwork_from_repair_order) || !$is_jobwork_from_repair_order) {
            $jw_emb_id = !empty($current_jwo_id) ? (int) $current_jwo_id : 0;
            if ($jw_emb_id > 0 && !empty($is_jobwork_from_sale_order) && $is_jobwork_from_sale_order) {
                ...
                $embed_order['diamond_issues'] = auragold_voucher_list_diamond_issue_rows_for_kind($conn, 'jobwork_order', $jw_emb_id);
                $embed_order['stone_issues'] = auragold_voucher_list_stone_issue_rows_for_kind($conn, 'jobwork_order', $jw_emb_id);
            } elseif (empty($is_jobwork_from_sale_order) || !$is_jobwork_from_sale_order) {
                $so_emb_id = (int) ($embed_order['id'] ?? 0);
                if ($so_emb_id > 0) {
                    require_once __DIR__ . '/includes/auragold_voucher_diamond_stock.php';
                    require_once __DIR__ . '/includes/auragold_voucher_stone_stock.php';
                    $embed_order['diamond_issues'] = auragold_voucher_list_diamond_issue_rows_for_kind($conn, 'sale_order', $so_emb_id);
                    $embed_order['stone_issues'] = auragold_voucher_list_stone_issue_rows_for_kind($conn, 'sale_order', $so_emb_id);
                }
            }
        }
        $embed_items = is_array($edit_items) ? $edit_items : [];
        $embed_payments = is_array($edit_payments) ? $edit_payments : [];
        if (!empty($jwo_from_oj_scrap_refinery) && !empty($embed_payments)) {
            $bridge_strip = __DIR__ . '/includes/auragold_oj_scrap_sale_order_bridge.php';
            if (is_file($bridge_strip)) {
                require_once $bridge_strip;
                if (function_exists('auragold_jobwork_embed_strip_scrap_metal_payments')) {
                    auragold_jobwork_embed_strip_scrap_metal_payments($embed_payments);
                }
            }
        }
        ...
        unset($emb_it);
        if (is_array($embed_payments) && count($embed_payments) > 0) {
    ...
    window.jobworkOrderScreenUrl = function(saleOrderId) {
        if (!saleOrderId) return 'jobwork-order.php';
        return 'jobwork-order.php?sale_order_id=' + encodeURIComponent(saleOrderId);
    };
    ...
    const PRODUCT_MODAL_COLUMNS_PAGE = 'jobwork-order-product-modal';
    ...
    $(document).ready(function() {
        ...
        $(document).on('keydown', '#jwoDepartmentUserSearch', function(e) {
            ...
            e.preventDefault();
            $('#jwoDepartmentUserSuggestions').hide();
            if (typeof openJwoJobWorkerLedgerModal !== 'function') return;
        ...
        saleReturnSearchInput.on('input', function() {
            ...
            saleReturnSearchTimeout = setTimeout(function() {
                $.ajax({
                    url: 'ajax/search-jobwork-orders.php',
                    type: 'GET',
                    data: { q: searchTerm },
                    dataType: 'json',
                    success: function(response) {
                        if (response.status === 'success' && response.orders && response.orders.length > 0) {
                            ...
                            response.orders.forEach(function(order) {
                                html += `
                                    <div class="jobwork-order-suggestion-item" 
                            ...
                            saleReturnSuggestions.html('<div style="padding: 0.75rem; color: #94a3b8; text-align: center;">No job work orders found</div>').show();
                        }
                    },
                    error: function() {
                        saleReturnSuggestions.hide();
        ...
        $(document).on('click', '.jobwork-order-suggestion-item', function() {
            const jwoId = $(this).data('order-id');
            const soId = parseInt($(this).attr('data-sale-order-id') || '0', 10);
            saleReturnSearchInput.val('');
            saleReturnSuggestions.hide();
            if (soId > 0) {
                window.location.href = 'jobwork-order.php?sale_order_id=' + soId;
            } else if (jwoId) {
                window.location.href = 'jobwork-order.php?id=' + jwoId;
            }
        });
    ...
    function openJwoJobWorkerLedgerModal(prefillLedgerName) {
        ...
        const deptNum = d && parseInt(d, 10) > 0 ? parseInt(d, 10) : 0;
    ...
    function saveCustomer() {
        ...
        .then(data => {
            if (data.status === 'success' || data.success === true) {
                ...
                var mapDeptId = pendingDept && parseInt(pendingDept, 10) > 0 ? parseInt(pendingDept, 10) : 0;
                if (mapDeptId < 1 && window.isJobworkFromSaleOrder && newCid) {
                    var de = document.getElementById('jwoDepartment');
                    var dv = de && de.value ? parseInt(de.value, 10) : 0;
                    if (dv > 0) {
                        mapDeptId = dv;
                    }
                }
                if (mapDeptId > 0 && newCid) {
                    var fdMap = new FormData();
                    fdMap.append('department_id', String(mapDeptId));
                    ...
                    fetch('ajax/map-jobworker-to-department.php', { method: 'POST', body: fdMap, credentials: 'same-origin' })
                        ...
                        .then(function() {
                            if (typeof refreshJwoDepartmentUsers === 'function') {
                                refreshJwoDepartmentUsers(String(mapDeptId), newCid);
                            }
                        })
                        .catch(function() { /* ignore */ });
                } else if (window.isJobworkFromSaleOrder && newCid && typeof refreshJwoDepartmentUsers === 'function') {
                    var de2 = document.getElementById('jwoDepartment');
                    var dv2 = de2 && de2.value ? String(parseInt(de2.value, 10)) : '';
                    if (dv2 && parseInt(dv2, 10) > 0) {
                        refreshJwoDepartmentUsers(dv2, newCid);
                    }
                }
    ...
    function openProductModal() {
        try {
            ...
            if (typeof showAnyModal === 'function') {
                if (!showAnyModal('#productSelectionModal')) {
                    return;
                }
            } else {
                const modal = document.getElementById('productSelectionModal');
...
<td data-column="sale-amount-with"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" readonly style="width: 130px; font-size: 0.7rem;"></td>
        ...
        if (typeof jQuery !== 'undefined' && jQuery.ajax) {
            ...
            success: function(response) {
                ...
                <i class="feather icon-edit" style="cursor: pointer; font-size: 0.8rem; color: #c5a864; margin-right: 8px;" onclick="editProductRowInTable(this)" title="Edit Row"></i>
                    ...
                    var prRowsJq = tbody.querySelectorAll('.product-row');
                    response.products.forEach(function(p, i) {
                        if (!prRowsJq[i]) return;
                        var pcp = prRowsJq[i].querySelector('.product-category-select');
            ...
            fetch(url)
                ...
                .then(data => {
                    if (data.success && data.products.length > 0) {
                        ...
                        var prRowsFetch = tbody.querySelectorAll('.product-row');
                        data.products.forEach(function(p, i) {
                            if (!prRowsFetch[i]) return;
                            var pcp = prRowsFetch[i].querySelector('.product-category-select');
                            var pcid0 = p.category_id != null && p.category_id !== '' ? p.category_id : '';
                            if (pcp && pcid0) try { pcp.value = String(pcid0); } catch (e) {}
                        });
                        tbody.querySelectorAll('[data-column="category"] select').forEach(function(select) {
    ...
    function addProductToTableFromModalRow(modalRowData, metalIdForImage) {
        ...
        try {
            ...
            if (!groupImagePayload && typeof window.auragoldCoParseGroupImageAttr === 'function') {
                var rowGiJwOrd = window.auragoldCoParseGroupImageAttr(modalRowData.group_image || '');
                if (rowGiJwOrd) groupImagePayload = rowGiJwOrd;
            }
            ...
            var cellsHtml = '';
            if (typeof getProductListRowCells === 'function') {
                cellsHtml = getProductListRowCells(modalData, { groupImage: primaryUrl }).join('');
            } else {
                console.warn('getProductListRowCells missing — load product-list-table-shared.js');
            }
            row.innerHTML = cellsHtml + actionCell;
    ...
    function addMergedProductsToTable(modalRowsData, metalIdForImage) {
        ...
        var firstRowGiJwOrdM = '';
        for (var _agiJwOrdM = 0; _agiJwOrdM < modalRowsData.length; _agiJwOrdM++) {
            var rgi = modalRowsData[_agiJwOrdM].group_image;
            if (rgi && String(rgi).trim()) { firstRowGiJwOrdM = rgi; break; }
        }
        var groupImagePayload = tabPayloadMerge || ((typeof window.auragoldCoParseGroupImageAttr === 'function') ? window.auragoldCoParseGroupImageAttr(firstRowGiJwOrdM) : '') || '';
        var groupImageAttr = (typeof groupImagePayload === 'object' && groupImagePayload != null) ? JSON.stringify(groupImagePayload) : (groupImagePayload || '');
        row.setAttribute('data-group-image', groupImageAttr);
        var primaryUrl = typeof getGroupImagePrimary === 'function' ? getGroupImagePrimary(groupImagePayload) : (typeof groupImagePayload === 'string' ? groupImagePayload : '');
        var actionCell = '<td><div class="action-btns"><button type="button" class="btn-edit" onclick="editProductRow(\'' + rowId + '\')" title="Edit"><i class="feather icon-edit"></i></button><button type="button" class="btn-delete" onclick="deleteProductRow(\'' + rowId + '\')" title="Delete"><i class="feather icon-trash-2"></i></button></div></td>';
    ...
    (function() {
        var PAGE_MODAL_COLUMN_PREFS = 'jobwork-order-product-modal';
        var TAB_MAIN = 'main';
    ...
    /** Read numeric from main product list row: data-field input, or input/text inside data-column (textContent misses input values). */
    function jwoReadProductListCellNumber(row, fieldAttr, columnName) {
        if (fieldAttr) {
            var inp = row.querySelector('[data-field="' + fieldAttr + '"]');
            if (inp && inp.value !== undefined && String(inp.value).trim() !== '') {
                var v0 = parseFloat(inp.value);
                if (isFinite(v0)) return v0;
            }
        }
        var cell = row.querySelector('[data-column="' + columnName + '"]');
        if (!cell) return 0;
        var inner = cell.querySelector('input, select');
        if (inner && inner.value !== undefined && String(inner.value).trim() !== '') {
            var v1 = parseFloat(inner.value);
    ...
    function saveJobWorkOrder() {
        ...
        if (window.isJobworkFromSaleOrder && window.jwoHasDepartmentUserTables && userEl && (!userEl.value || userEl.value.trim() === '')) {
        ...
        productRows.forEach(function(row) {
            ...
            if (productId) {
                ...
                var quantity = jwoReadProductListCellNumber(row, 'quantity', 'quantity');
                var grossWeight = jwoReadProductListCellNumber(row, 'gross_wt', 'gross-wt');
                var lessWeight = jwoReadProductListCellNumber(row, 'less_wt', 'less-wt');
                var finalWeight = jwoReadProductListCellNumber(row, 'final_wt', 'final-wt');
                var netWeight = jwoReadProductListCellNumber(row, '', 'net-wt');
                var pureWeight = jwoReadProductListCellNumber(row, '', 'pure-wt');
                var purity = parseFloat(row.getAttribute('data-purity')) || jwoReadProductListCellNumber(row, 'purity', 'purity');
                var rate = parseFloat(row.getAttribute('data-rate')) || jwoReadProductListCellNumber(row, '', 'rate');
                var makingAmount = jwoReadProductListCellNumber(row, 'making', 'making-amount');
        ...
        // Creating new JWO from sale order with multiple lines: one tbl_jobwork_orders row per line (sequential save).
        var splitMultiJwo = window.isJobworkFromSaleOrder && !window.jwoFromRepair && currentJwoId < 1 && items.length > 1;
        if (splitMultiJwo) {
            var _pdc = (window.__pendingSaleOrderDiamondLines && window.__pendingSaleOrderDiamondLines.length) || 0;
            var _psc = (window.__pendingSaleOrderStoneLines && window.__pendingSaleOrderStoneLines.length) || 0;
            if (_pdc > 0 || _psc > 0) {
                alert('Diamond or stone allocations cannot be saved when creating multiple job work orders at once (one job work order per line). Remove gems/stones or use a single-line job work order.');
                isSaving = false;
        ...
        if (splitMultiJwo) {
            ...
            function appendJwoCommonFields(fd) {
                ...
                fdSplit.append('sales_person', String(spEl0.value || '').trim());
                }
                var _pdJ = Array.isArray(window.__pendingSaleOrderDiamondLines) ? window.__pendingSaleOrderDiamondLines.slice() : [];
                var _psJ = Array.isArray(window.__pendingSaleOrderStoneLines) ? window.__pendingSaleOrderStoneLines.slice() : [];
                fdSplit.append('pending_diamond_allocations', JSON.stringify(_pdJ));
                fdSplit.append('pending_stone_allocations', JSON.stringify(_psJ));
        ...
        if (window.jwoFromRepair) {
            ...
            if (deptEl && deptEl.value && deptEl.value.trim() !== '') {
                fdRep.append('department_id', deptEl.value.trim());
            }
            var statusElRep = document.getElementById('jwoStatus');
            if (statusElRep && statusElRep.value) {
                fdRep.append('jwo_status', statusElRep.value);
            }
            var priorityElRep = document.getElementById('jwoPriority');
            if (priorityElRep && priorityElRep.value) {
                fdRep.append('priority', priorityElRep.value);
            }
            if (userEl) {
                fdRep.append('department_user_id', userEl.value.trim());
            }
            fetch('ajax/save-repair-jobwork-order.php', { method: 'POST', body: fdRep, credentials: 'same-origin' })
                .then(function(r) { return r.json(); })
                .then(function(data) {
                    isSaving = false;
                    if (btn) { btn.disabled = false; btn.textContent = 'Save'; }
        ...
        var pendDimJwo = Array.isArray(window.__pendingSaleOrderDiamondLines) ? window.__pendingSaleOrderDiamondLines.slice() : [];
        var pendStoneJwo = Array.isArray(window.__pendingSaleOrderStoneLines) ? window.__pendingSaleOrderStoneLines.slice() : [];
        fd.append('pending_diamond_allocations', JSON.stringify(pendDimJwo));
        fd.append('pending_stone_allocations', JSON.stringify(pendStoneJwo));
        fetch('ajax/save-jobwork-order.php', { method: 'POST', body: fd, credentials: 'same-origin' })
            ...
            .then(function(data) {
                ...
                if (data.status === 'success') {
                    ...
                    window._jwoSavedJobworkNo = data.jobwork_no || data.job_work_no || '';
                    ...
                    if (rem.length > 0) {
                        ...
                        if (window.jwoFromOjScrapRefinery) {
                            nextQs += '&from_oj_scrap=1';
                            if (window.jwoOjScrapReturnFromDate) {
                                nextQs += '&oj_from_date=' + encodeURIComponent(window.jwoOjScrapReturnFromDate);
                            }
                            if (window.jwoOjScrapReturnToDate) {
                                nextQs += '&oj_to_date=' + encodeURIComponent(window.jwoOjScrapReturnToDate);
                            }
                        }
                        ...
                        window.location.href = 'jobwork-order.php?' + nextQs;
                        return;
                    }
                    window.__pendingSaleOrderDiamondLines = [];
                    window.__pendingSaleOrderStoneLines = [];
                    var jidRefresh = parseInt(String(newJwoId || currentJwoId || ''), 10) || 0;
                    if (jidRefresh > 0) {
                        window.jwoCurrentRecordId = jidRefresh;
                        if (typeof window.refreshSaleOrderDiamondIssuesFromServer === 'function') {
                            window.refreshSaleOrderDiamondIssuesFromServer(jidRefresh);
                        }
                        if (typeof window.refreshSaleOrderStoneIssuesFromServer === 'function') {
                            window.refreshSaleOrderStoneIssuesFromServer(jidRefresh);
                        }
                    }
                    ...
                    if (msg.indexOf('already exists') !== -1 && typeof swal === 'function') {
                        swal({
                            ...
                            if (isConfirm && data.jwo_id) {
                                if (saleOrderId > 0) {
                                    window.location.href = 'jobwork-order.php?sale_order_id=' + saleOrderId;
                                } else {
                                    window.location.href = 'jobwork-order.php?id=' + data.jwo_id;
                                }
                            }
                        });
                    } else {
    ...
    function closePrintJWOModal() {
        ...
        if (window.jwoFromOjScrapRefinery) {
            ...
            window._jwoSavedJobworkNo = null;
            var rqs = 'tab=refine';
            if (window.jwoOjScrapReturnFromDate) {
                rqs += '&from_date=' + encodeURIComponent(window.jwoOjScrapReturnFromDate);
            }
            if (window.jwoOjScrapReturnToDate) {
                rqs += '&to_date=' + encodeURIComponent(window.jwoOjScrapReturnToDate);
            }
            window.location.href = 'old-jewellery.php?' + rqs;
            return;
        }
        window._jwoRedirectMpDeptId = null;
        window._jwoRedirectMpUserId = null;
        window._jwoSavedJobworkNo = null;
        window.location.href = 'sale-order-process.php';
    }
    ...
    function saveOrder() {
        ...
        productRows.forEach(function(row) {
            ...
            if (productId) {
                ...
                const quantity = jwoReadProductListCellNumber(row, 'quantity', 'quantity');
                const grossWeight = jwoReadProductListCellNumber(row, 'gross_wt', 'gross-wt');
                const lessWeight = jwoReadProductListCellNumber(row, 'less_wt', 'less-wt');
                const finalWeight = jwoReadProductListCellNumber(row, 'final_wt', 'final-wt');
                const netWeight = jwoReadProductListCellNumber(row, '', 'net-wt');
        ...
        if (typeof jQuery !== 'undefined' && jQuery.ajax) {
            jQuery.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        var ridNum = parseInt(String(returnId || ''), 10) || 0;
                        if (ridNum > 0) {
                            window.__saleOrderDbId = ridNum;
                            if (typeof window.refreshSaleOrderDiamondIssuesFromServer === 'function') {
                                window.refreshSaleOrderDiamondIssuesFromServer(ridNum);
                            }
                            if (typeof window.refreshSaleOrderStoneIssuesFromServer === 'function') {
                                window.refreshSaleOrderStoneIssuesFromServer(ridNum);
                            }
                        }
            ...
            .then(data => {
                ...
                if (data.status === 'success') {
                    ...
                    var ridNumF = parseInt(String(returnId || ''), 10) || 0;
                    if (ridNumF > 0) {
                        window.__saleOrderDbId = ridNumF;
                        if (typeof window.refreshSaleOrderDiamondIssuesFromServer === 'function') {
                            window.refreshSaleOrderDiamondIssuesFromServer(ridNumF);
                        }
                        if (typeof window.refreshSaleOrderStoneIssuesFromServer === 'function') {
                            window.refreshSaleOrderStoneIssuesFromServer(ridNumF);
                        }
                    }
                    saleReturnAfterSavePrompts(returnId, returnNo, barcodes);
                } else {
    ...
    if (saveInvoiceBtn) {
        ...
        saveInvoiceBtn.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
            if (window.isJobworkFromSaleOrder && typeof saveJobWorkOrder === 'function') {
                saveJobWorkOrder();
            } else {
                saveOrder();
            }
            ...
            jQuery.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        var histPathLoad = 'jobwork-order.php?sale_order_id=' + orderId;
            ...
            fetch(getUrl, { cache: 'no-store' })
                ...
                .then(data => {
                    ...
                    if (data.status === 'success') {
                        ...
                        var histPathLoadF = 'jobwork-order.php?sale_order_id=' + orderId;
                        if (window.isJobworkFromSaleOrder && typeof jobworkOrderCanonicalPath === 'function') {
    ...
    function populateOrderForm(order, items, loadedPayments) {
        ...
        var vkDs = (window.AURAGOLD_VOUCHER_DS && window.AURAGOLD_VOUCHER_DS.voucherKind) || 'sale_order';
        if (order && order.id && vkDs === 'sale_order') {
    ...
    /** Query suffix for scrap refinery deep-link (preserve return dates for Old Jewellery → Refine). */
    function jobworkOrderOjScrapQuerySuffix() {
        if (!window.jwoFromOjScrapRefinery) {
            return '';
        }
        var s = '&from_oj_scrap=1';
        if (window.jwoOjScrapReturnFromDate) {
            s += '&oj_from_date=' + encodeURIComponent(window.jwoOjScrapReturnFromDate);
        }
        if (window.jwoOjScrapReturnToDate) {
            s += '&oj_to_date=' + encodeURIComponent(window.jwoOjScrapReturnToDate);
        }
        return s;
    }
    ...
    function jobworkOrderCanonicalPath(fallbackEditId) {
        ...
        var ojScrapQs = jobworkOrderOjScrapQuerySuffix();
        if (window.jwoSaleOrderIdParam > 0) {
            return 'jobwork-order.php?sale_order_id=' + window.jwoSaleOrderIdParam + ojScrapQs;
        }
        if (window.jwoCurrentRecordId > 0) {
            return 'jobwork-order.php?id=' + window.jwoCurrentRecordId + ojScrapQs;
        }
        if (fallbackEditId) {
            return 'jobwork-order.php?sale_order_id=' + fallbackEditId + ojScrapQs;
        }
        return 'jobwork-order.php' + (ojScrapQs !== '' ? ('?' + ojScrapQs.replace(/^&/, '')) : '');
    }
    ...
    (function() {
            ...
            try {
                ...
                if (window.history && window.history.replaceState) {
                    ...
                    if (jwoPath) {
                        ...
                        window.history.replaceState({}, '', 'jobwork-order.php?sale_order_id=' + editId);
                    }
                }
                window.isPurchaseInvoiceEditMode = true;
                if (typeof window.updateSaleInvoiceAddItemButtonState === 'function') {
        ...
        function doLoadOrder() {
            if (window.editUrlIdIsJobworkOrder) {
                return;
            }
            if (editId && typeof loadOrder === 'function') {
                loadOrder(editId);
            }
        }
        ...
        setTimeout(function() {
            ...
            if (!hasRows || !gtZero) {
                if (window.EDIT_ORDER_DATA && window.EDIT_ORDER_DATA.order) {
                    doPopulateFromEmbed();
                } else if (!window.editUrlIdIsJobworkOrder && editId && typeof loadOrder === 'function') {
                    loadOrder(editId);
                }
            }
        }, 1200);
    ...
    function logJobworkOrderModalPresence() {
        console.log('Product modal exists:', !!document.getElementById('productSelectionModal'));
        console.log('Cash modal exists:', !!document.getElementById('cashPaymentModal'));
        console.log('UPI modal exists:', !!document.getElementById('upiPaymentModal'));
        console.log('Bank modal exists:', !!document.getElementById('bankPaymentModal'));
        console.log('Cheque modal exists:', !!document.getElementById('chequePaymentModal'));
        console.log('Card modal exists:', !!document.getElementById('cardPaymentModal'));
        console.log('Metal exchange modal exists:', !!document.getElementById('metalExchangeModal'));
        console.log('Scrap modal exists:', !!document.getElementById('scrapPaymentModal'));
    }
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', logJobworkOrderModalPresence);
    } else {
        logJobworkOrderModalPresence();
    }
    ...
    function openPaymentModal(type) {
        ...
        if (modalId) {
            ...
            if (typeof showAnyModal === 'function') {
                ...
                console.error('showAnyModal / jQuery modal not available for', modalId);
            }
        }
    }
...
body.jobwork-order-page.stop-scrolling .sweet-overlay {
    background: rgba(15, 23, 42, 0.45);
}
body.jobwork-order-page .sweet-alert.sale-return-swal {
    border-radius: 20px;
    padding: 32px 28px 28px;
    box-shadow: 0 10px 40px rgba(0, 0, 0, 0.18);
    font-family: inherit;
}
body.jobwork-order-page .sweet-alert.sale-return-swal h2 {
    font-size: 1.35rem;
    font-weight: 700;
    color: #1e3a5f;
    margin: 0 0 12px;
...
body.jobwork-order-page .sweet-alert.sale-return-swal p.lead {
    color: #6b7280;
    font-size: 0.95rem;
    line-height: 1.5;
    margin: 0 0 8px;
}
body.jobwork-order-page .sweet-alert.sale-return-swal .sa-button-container {
    margin-top: 22px;
    gap: 14px;
    display: flex;
    flex-wrap: wrap;
...
body.jobwork-order-page .sweet-alert.sale-return-swal button.confirm.btn-sale-return-sw-confirm {
...
body.jobwork-order-page .sweet-alert.sale-return-swal button.confirm.btn-sale-return-sw-confirm:hover {
    filter: brightness(1.06);
}
body.jobwork-order-page .sweet-alert.sale-return-swal button.cancel.btn-sale-return-sw-cancel {
...
body.jobwork-order-page .sweet-alert.sale-return-swal button.cancel.btn-sale-return-sw-cancel:hover {
