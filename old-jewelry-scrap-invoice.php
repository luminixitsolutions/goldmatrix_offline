...
$auragold_voucher_runtime_client = auragold_voucher_runtime_bootstrap($conn, $metals, 'Old Jewelry - Scrap Invoice');
...
// Next invoice no: Bill Series (bill-series.php) for voucher "Old Jewelry Scrap Invoice" (or legacy OJB-1, OJB-2)
$next_order_no = function_exists('getNextOldJewelryScrapInvoiceNo') ? getNextOldJewelryScrapInvoiceNo($conn) : 'OJB-1';

// Sales Person dropdown — branch-scoped (stored in tbl_old_jewelry_scrap_invoices.sales_person)
$sales_person_users = auragold_sales_person_user_display_names($conn_master);

// Load invoice for editing if ID provided (tbl_old_jewelry_scrap_invoices)
...
// Allowed invoice print languages (English + optional one from Invoice Print Settings)
...
<head>
<title>Old Jewellery Scrap Invoice - Gold Matrix</title>
                ...
                <div class="container-fluid flex-grow-1" style="padding-top: 0; padding-bottom: 0;">
                    ...
                    <div class="row invoice-content-row">
                        ...
                        <div class="col-lg-8" >
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="d-flex align-items-stretch" style="gap: 4px;">
                                                    <select class="form-control form-control-sm" id="againstOf" style="flex: 1; min-width: 0;">
                                                        ...
                                                        <!-- <option value="Old Jewellery Scrap Invoice">Old Jewellery Scrap Invoice</option> -->
                                                    </select>
                            ...
                            <?php
                            $product_list_prefs_page = 'old-jewelry-scrap-invoice-product-table';
                            ...
                            <div class="summary-panel" >
                                ...
                                <div class="invoice-header" style="margin: -1rem -1rem 1rem -1rem; border-radius: 10px 10px 0 0; display: flex; align-items: center; justify-content: space-between;">
                                    <div style="display: flex; align-items: center; gap: 0.5rem; flex: 1;">
                                        ...
                                        <h5 class="mb-0" style="font-size: 0.9rem;">Old Jewellery Scrap Invoice No: <span id="currentOrderNo"><?php echo htmlspecialchars($next_order_no); ?></span></h5>
                                    </div>
...
<script>
    ...
    if (!empty($edit_order_id) && $edit_order_id > 0 && !empty($edit_order) && is_array($edit_order)) {
        $embed_order = [
            ...
            'previous_balance' => (float)($edit_order['previous_balance_amt'] ?? $edit_order['previous_balance'] ?? 0),
            'previous_gold' => (float)($edit_order['previous_balance_gold'] ?? $edit_order['previous_gold'] ?? 0),
            'previous_silver' => (float)($edit_order['previous_balance_silver'] ?? $edit_order['previous_silver'] ?? 0),
        ...
        foreach ($embed_items as &$__ei) {
            if (!is_array($__ei)) {
                continue;
            }
            if (!isset($__ei['gross_weight']) && isset($__ei['gross_wt'])) {
                $__ei['gross_weight'] = $__ei['gross_wt'];
            }
            if (!isset($__ei['product_name']) && isset($__ei['description'])) {
                $__ei['product_name'] = $__ei['description'];
            }
            if (!isset($__ei['product_id'])) {
                $__ei['product_id'] = 0;
            }
        }
        unset($__ei);
    ...
    const PRODUCT_MODAL_COLUMNS_PAGE = 'old-jewelry-scrap-invoice-product-modal';
    ...
    $(document).ready(function() {
        ...
        $('#againstOf').on('change', function() {
            ...
            var cid = document.getElementById('customerId') ? String(document.getElementById('customerId').value || '').trim() : '';
            if (!cid) {
                alert('Pick a customer from the name suggestions (so Customer ID is saved). Typed text alone is not enough. Then choose Against Of again.');
                $(this).val('');
                return;
            }
        ...
        $('#againstOfPickDocBtn').on('click', function() {
            ...
            var cid = document.getElementById('customerId') ? String(document.getElementById('customerId').value || '').trim() : '';
            if (!cid) {
                alert('Pick a customer from the name suggestions first (so Customer ID is set), then use this button.');
                return;
            }
            siCurrentAgainstType = val;
            var titleEl = document.getElementById('againstOrderModalTitle');
        ...
        saleInvoiceSearchInput.on('input', function() {
            ...
            saleInvoiceSearchTimeout = setTimeout(function() {
                $.ajax({
                    url: 'ajax/search-old-jewelry-scrap-invoices.php',
                    type: 'GET',
                    data: { q: searchTerm },
                    dataType: 'json',
                    success: function(response) {
                        if (response.status === 'success' && response.invoices && response.invoices.length > 0) {
                            ...
                            saleInvoiceSuggestions.html('<div style="padding: 0.75rem; color: #94a3b8; text-align: center;">No scrap invoices found</div>').show();
                        }
                    },
                    error: function() {
                        saleInvoiceSuggestions.hide();
        ...
        $(document).on('click', '#saleInvoiceSuggestions .invoice-suggestion-item', function() {
            ...
            saleInvoiceSuggestions.hide();
            if (invoiceId > 0) {
                window.location.href = 'old-jewelry-scrap-invoice.php?id=' + invoiceId;
            }
        });
...
<td data-column="sale-amount-with"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" readonly style="width: 130px; font-size: 0.7rem;"></td>
    ...
    function addProductToTableFromModalRow(modalRowData, metalIdForImage) {
        ...
        row.setAttribute('data-rate', parseFloat(modalRowData.rate || modalRowData.metal_rate || 0));
    ...
    function addMergedProductsToTable(modalRowsData, metalIdForImage) {
        ...
        modalRowsData.forEach(function(d) {
            ...
            var lineMr = parseFloat(d.metal_rate) || parseFloat(d.rate) || 0;
            if (lineMr > 0) {
                merged.metal_rate = (merged.metal_rate || 0) > 0 ? merged.metal_rate : lineMr;
            }
        ...
        if ((!merged.rate || merged.rate <= 0) && merged.metal_rate > 0) {
            merged.rate = merged.metal_rate;
        }
        if ((!merged.metal_rate || merged.metal_rate <= 0) && merged.rate > 0) {
            merged.metal_rate = merged.rate;
        }
        ...
        row.setAttribute('data-rate', merged.rate || merged.metal_rate || 0);
        ...
        var firstRowGiOj = '';
        for (var _agiOj = 0; _agiOj < modalRowsData.length; _agiOj++) {
            var rgi = modalRowsData[_agiOj].group_image;
            if (rgi && String(rgi).trim()) { firstRowGiOj = rgi; break; }
        }
        var groupImagePayload = tabPayloadMerge || ((typeof window.auragoldCoParseGroupImageAttr === 'function') ? window.auragoldCoParseGroupImageAttr(firstRowGiOj) : '') || '';
        var groupImageAttr = (typeof groupImagePayload === 'object' && groupImagePayload != null) ? JSON.stringify(groupImagePayload) : (groupImagePayload || '');
        row.setAttribute('data-group-image', groupImageAttr);
        var primaryUrl = typeof getGroupImagePrimary === 'function' ? getGroupImagePrimary(groupImagePayload) : (typeof groupImagePayload === 'string' ? groupImagePayload : '');
        var actionCell = '<td><div class="action-btns"><button type="button" class="btn-edit" onclick="editProductRow(\'' + rowId + '\')" title="Edit"><i class="feather icon-edit"></i></button><button type="button" class="btn-delete" onclick="deleteProductRow(\'' + rowId + '\')" title="Delete"><i class="feather icon-trash-2"></i></button></div></td>';
    ...
    function updateSummaryPanel() {
        ...
        // Scrap payments carry actual metal weights; product lines may be valuation-only — prefer scrap for footer weights.
        function sumScrapPaymentWeights() {
            var o = { gross: 0, less: 0, net: 0, purity: 0, qty: 0, rate: 0 };
            var list = (typeof payments !== 'undefined' && Array.isArray(payments)) ? payments : [];
            list.forEach(function(p) {
                if (!p) return;
                var isScrap = p.type === 'scrap' || (String(p.payment_type || '').toLowerCase().indexOf('scrap') >= 0);
                if (!isScrap) return;
                o.gross += parseFloat(p.scrap_gross_wt) || 0;
                o.less += parseFloat(p.scrap_less_wt) || 0;
                o.net += parseFloat(p.scrap_net_wt) || 0;
                o.purity += parseFloat(p.scrap_purity_wt) || 0;
                o.qty += parseFloat(p.quantity) || 0;
                o.rate += parseFloat(p.scrap_rate) || 0;
            });
            return o;
        }
        var sw = sumScrapPaymentWeights();
        var useScrapWt = sw.net > 0.000001 || sw.gross > 0.000001 || sw.purity > 0.000001;
        var sumLess = 0;
        rows.forEach(function(r) { sumLess += plRowCellFloat(r, 'less-wt'); });
        var footerLessWt = document.getElementById('footerLessWt');
        if (footerLessWt && (rows.length > 0 || useScrapWt)) {
            var dispLess = useScrapWt ? sw.less : sumLess;
            footerLessWt.textContent = dispLess.toFixed(3);
        }
        
        // Update grand total footer
        const footer = document.getElementById('productTableFooter');
        if (footer && (rows.length > 0 || useScrapWt)) {
            footer.style.display = '';
            var dispQty = useScrapWt ? (sw.qty > 0 ? sw.qty : totalQuantity) : totalQuantity;
            var dispGross = useScrapWt ? sw.gross : totalGrossWt;
            var dispNet = useScrapWt ? sw.net : totalNetWt;
            var dispPure = useScrapWt ? sw.purity : totalPureWt;
            var dispMetalWt = useScrapWt ? sw.net : totalMetalWeight;
            var dispFinal = useScrapWt ? sw.gross : totalFinalWt;
            var fq = document.getElementById('footerQuantity'); if (fq) fq.textContent = dispQty.toFixed(2);
            var fg = document.getElementById('footerGrossWt'); if (fg) fg.textContent = dispGross.toFixed(1);
            var ff = document.getElementById('footerFinalWt'); if (ff) ff.textContent = dispFinal.toFixed(1);
            var fn = document.getElementById('footerNetWt'); if (fn) fn.textContent = dispNet.toFixed(1);
            var fp = document.getElementById('footerPureWt'); if (fp) fp.textContent = dispPure.toFixed(3);
            var fmw = document.getElementById('footerMetalWeight'); if (fmw) fmw.textContent = dispMetalWt.toFixed(3);
            ...
            if (fpu) {
                var effP = (dispNet > 0.00001) ? (dispPure / dispNet) : 0;
            ...
            var scrapRateSum = 0, scrapRateN = 0;
            if (typeof payments !== 'undefined' && Array.isArray(payments)) {
                payments.forEach(function(p) {
                    if (!p) return;
                    var isScrap = p.type === 'scrap' || (String(p.payment_type || '').toLowerCase().indexOf('scrap') >= 0);
                    if (!isScrap) return;
                    var rr = parseFloat(p.scrap_rate) || 0;
                    if (rr > 0) { scrapRateSum += rr; scrapRateN++; }
                });
            }
            var dispFooterRate = (useScrapWt && scrapRateN > 0) ? (scrapRateSum / scrapRateN) : totalRate;
            var fr = document.getElementById('footerRate'); if (fr) fr.textContent = dispFooterRate.toFixed(2);
        ...
        // Overpayment shows as negative Balance Amt (customer credit/advance). save-old-jewelry-scrap-invoice posts this to balance_amt
    ...
    (function() {
        var PAGE_MODAL_COLUMN_PREFS = 'old-jewelry-scrap-invoice-product-modal';
        var TAB_MAIN = 'main';
    ...
    function saveOrder() {
        ...
        productRows.forEach(function(row) {
                    ...
                    if (groupItems && groupItems.length > 0) {
                        ...
                        groupItems.forEach(function(d) {
                            ...
                            items.push({
                                ...
                                pure_weight: pureWt,
                                rate: (function () {
                                    var r = parseFloat(d.rate) || 0;
                                    var mr = parseFloat(d.metal_rate) || 0;
                                    return r > 0 ? r : mr;
                                })(),
                                metal_rate: (function () {
                                    var mr = parseFloat(d.metal_rate) || 0;
            ...
            if (productId) {
                ...
                const rateCellVal = parseFloat(row.querySelector('[data-column="rate"] input')?.value || row.querySelector('[data-column="rate"]')?.textContent) || 0;
                const rateAttr = parseFloat(row.getAttribute('data-rate')) || 0;
                ...
                var rate = rateAttr || rateCellVal;
                if (!rate && metalRate) rate = metalRate;
                ...
                const amount = parseFloat(row.querySelector('[data-column="amount"] input')?.value || row.querySelector('[data-column="amount"]')?.textContent) || 0;
                const netAmount = parseFloat(row.querySelector('[data-column="net-amt"] input')?.value || row.querySelector('[data-column="net-amt"]')?.textContent) || 0;
                const netAmtWithTax = parseFloat(row.querySelector('[data-column="net-amt-tax"] input')?.value || row.querySelector('[data-column="net-amt-tax"]')?.textContent) || 0;
        ...
        if (typeof jQuery !== 'undefined' && jQuery.ajax) {
            jQuery.ajax({
                url: 'ajax/save-old-jewelry-scrap-invoice.php',
                type: 'POST',
                data: postData,
                dataType: 'json',
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        if (invoiceId) {
                            ...
                            window.pendingRedirectUrl = 'old-jewelry-scrap-invoice.php';
                            setTimeout(function() {
                                showPrintInvoiceModal(invoiceId);
                            }, 100);
                        } else {
                            alert('Old Jewellery Scrap Invoice saved successfully! Invoice No: ' + (response.invoice_no || response.order_no || 'N/A'));
                            window.location.href = 'old-jewelry-scrap-invoice.php';
                        }
                    } else {
                        alert('Error: ' + (response.message || 'Failed to save scrap invoice'));
                    }
            ...
            fetch('ajax/save-old-jewelry-scrap-invoice.php', {
                method: 'POST',
                body: formData
            })
            .then(response => response.json())
            .then(data => {
                ...
                if (data.status === 'success') {
                    ...
                    if (invoiceId) {
                        ...
                        alert('Old Jewellery Scrap Invoice saved successfully! Invoice No: ' + (data.invoice_no || data.order_no || 'N/A'));
                        window.location.href = 'old-jewelry-scrap-invoice.php';
                    }
                } else {
                    alert('Error: ' + (data.message || 'Failed to save scrap invoice'));
                }
                if (saveBtn && typeof updateSiSaveBlockedByPfd === 'function') {
                    updateSiSaveBlockedByPfd(!!window.siSaveBlockedByPurchaseFixing);
                } else if (saveBtn) {
    ...
    function loadOrder(orderId) {
        ...
        var getUrl = 'ajax/get-old-jewelry-scrap-invoice.php?id=' + orderId + '&_=' + (Date.now ? Date.now() : '');
        if (typeof jQuery !== 'undefined' && jQuery.ajax) {
            jQuery.ajax({
                url: getUrl,
                type: 'GET',
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        window.history.pushState({}, '', 'old-jewelry-scrap-invoice.php?id=' + orderId);
    ...
    function addProductRowToSelectionTable(item, product) {
        ...
        row.innerHTML = `
            ...
            <td data-column="item-code"><input type="text" class="form-control form-control-sm" value="${escapeHtml(item.item_code || '')}" style="width: 100px; font-size: 0.7rem;"></td>
            <td data-column="category"><select class="form-control form-control-sm" style="width: 100px; font-size: 0.7rem;"><option value="">Select</option></select></td>
            <td data-column="product-category"><select class="form-control form-control-sm product-category-select" style="width: 120px; font-size: 0.7rem;"><option value="">Select Category</option></select></td>
            ...
            <td data-column="fc-amount"><input type="text" class="form-control form-control-sm" value="${parseFloat(item.fc_amount || 0).toFixed(2)}" step="0.01" style="width: 90px; font-size: 0.7rem;"></td>
            <td data-column="diamond-line-metal-value"><input type="text" class="form-control form-control-sm" value="${parseFloat(item.diamond_line_metal_value || 0).toFixed(2)}" step="0.01" style="width: 100px; font-size: 0.7rem;"></td>
            <td data-column="rapnet-valuation"><input type="text" class="form-control form-control-sm" value="${parseFloat(item.rapnet_valuation || 0).toFixed(2)}" step="0.01" style="width: 100px; font-size: 0.7rem;"></td>
            ...
            <td data-column="mark-up-amount"><input type="text" class="form-control form-control-sm" value="${parseFloat(item.mark_up_amount || 0).toFixed(2)}" step="0.01" style="width: 100px; font-size: 0.7rem;"></td>
            <td data-column="mark-up-per"><input type="text" class="form-control form-control-sm" value="${parseFloat(item.mark_up_per || 0).toFixed(2)}" step="0.01" style="width: 80px; font-size: 0.7rem;"></td>
        ...
        if (productCategorySelect && typeof populateSelect === 'function' && typeof categories !== 'undefined') {
            populateSelect(productCategorySelect, categories, 'id', 'name', 'Select Category');
            var pcidSaved = item.product_category_id != null && item.product_category_id !== '' ? item.product_category_id : (item.category_id != null && item.category_id !== '' ? item.category_id : '');
            if (pcidSaved) try { productCategorySelect.value = String(pcidSaved); } catch (e) {}
        }
        if (typeof auragoldPopulateModalSpecSelectsForRow === 'function') {
            auragoldPopulateModalSpecSelectsForRow(row);
        }
    ...
    function populateOrderForm(order, items, loadedPayments) {
        ...
        if (order && order.id > 0) {
            var pfdRefreshUrl = 'ajax/get-old-jewelry-scrap-invoice.php?id=' + encodeURIComponent(order.id) + '&_=' + (Date.now ? Date.now() : 0);
        ...
        if (loadedPayments && loadedPayments.length > 0) {
            loadedPayments.forEach(function(rawPayment) {
                var payment = rawPayment;
                if (payment && typeof payment.payment_details === 'string' && payment.payment_details.trim().indexOf('{') === 0) {
                    try {
                        var pmerge = JSON.parse(payment.payment_details);
                        if (pmerge && typeof pmerge === 'object') {
                            var idKeep = payment.id;
                            payment = Object.assign({}, payment, pmerge);
                            payment.id = idKeep;
                        }
                    } catch (ex) {}
                }
    ...
    (function() {
        ...
        function doPopulateFromEmbed() {
            ...
            try {
                ...
                if (window.history && window.history.replaceState) window.history.replaceState({}, '', 'old-jewelry-scrap-invoice.php?id=' + editId);
                window.isPurchaseInvoiceEditMode = true;
                return true;
            } catch (e) {
    ...
    function openPaymentModal(type) {
        ...
        if (modalId) {
            // Edit mode: populatePayment / editPayment sets fields — do not overwrite with summary balance
    ...
    function editPayment(paymentId) {
        ...
        if (payment) {
            if (!payment.type && payment.payment_type) {
                var _ptl = String(payment.payment_type).toLowerCase();
                if (_ptl.indexOf('scrap') >= 0) payment.type = 'scrap';
                else if (_ptl.indexOf('metal') >= 0 || _ptl.indexOf('exch') >= 0) payment.type = 'metal-exchange';
                else if (_ptl.indexOf('cash') >= 0) payment.type = 'cash';
                else if (_ptl.indexOf('bank') >= 0) payment.type = 'bank';
                else if (_ptl.indexOf('cheque') >= 0) payment.type = 'cheque';
                else if (_ptl.indexOf('upi') >= 0 || _ptl.indexOf('mobile') >= 0) payment.type = 'upi';
                else if (_ptl.indexOf('card') >= 0) payment.type = 'card';
            }
            window._editingPaymentId = paymentId;
            ...
            if (payment.type === 'cash') {
                ...
                var _smid = payment.scrap_metal_id != null && payment.scrap_metal_id !== '' ? String(payment.scrap_metal_id).trim() : '';
                if (!_smid && payment.metal_id != null && payment.metal_id !== '') _smid = String(payment.metal_id).trim();
                if (sm && _smid) {
                    sm.value = _smid;
                    try {
                        sm.dispatchEvent(new Event('change', { bubbles: true }));
                    } catch (e) {
                        if (typeof jQuery !== 'undefined' && jQuery(sm).trigger) jQuery(sm).trigger('change');
                    }
                }
                var spi = document.getElementById('scrapProductInput');
                var _spn = payment.scrap_product_name || payment.product_name || '';
                if (spi && _spn) spi.value = _spn;
                var spid = document.getElementById('scrapProductId');
                var _spid = payment.scrap_product_id != null && payment.scrap_product_id !== '' ? String(payment.scrap_product_id) : '';
                if (!_spid && payment.product_id != null && payment.product_id !== '') _spid = String(payment.product_id);
                if (spid && _spid) spid.value = _spid;
                ...
                var _savedScrapAmt = parseFloat(payment.amount) || 0;
                var _gw = parseFloat(payment.scrap_gross_wt) || 0;
                var _lw = parseFloat(payment.scrap_less_wt) || 0;
                var _sw = parseFloat(payment.scrap_stone_wt) || 0;
                if (_gw > 0.00001 || _lw > 0.00001 || _sw > 0.00001) {
                    if (typeof updateScrapCalculations === 'function') updateScrapCalculations();
                } else if (_savedScrapAmt > 0 && typeof updateScrapRateFromAmount === 'function') {
                    updateScrapRateFromAmount();
                    var _saEl = document.getElementById('scrapAmount');
                    if (_saEl && (parseFloat(_saEl.value) || 0) < 0.01) _saEl.value = _savedScrapAmt.toFixed(2);
                } else if (typeof updateScrapCalculations === 'function') {
                    updateScrapCalculations();
                    var _saFix = document.getElementById('scrapAmount');
                    if (_saFix && _savedScrapAmt > 0 && (parseFloat(_saFix.value) || 0) < 0.01) _saFix.value = _savedScrapAmt.toFixed(2);
                }
    ...
    function showPrintInvoiceModal(invoiceId) {
        ...
        setTimeout(function() {
            ...
            if (modal) {
                ...
                if (confirm('Old Jewellery Scrap Invoice saved successfully! Do you want to print the invoice?')) {
                    window.open('old-jewelry-scrap-invoice-print.php?id=' + invoiceId, '_blank', 'width=1200,height=800');
                }
