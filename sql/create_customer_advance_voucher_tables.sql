-- Create Customer Advance Voucher Tables
-- Run this SQL script to create the necessary tables for customer advance vouchers

-- Table: tbl_customer_advance_vouchers
CREATE TABLE IF NOT EXISTS `tbl_customer_advance_vouchers` (
...
-- Table: tbl_customer_advance_voucher_items (Receipt items)
CREATE TABLE IF NOT EXISTS `tbl_customer_advance_voucher_items` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  ...
  CONSTRAINT `fk_customer_advance_voucher_items_voucher` FOREIGN KEY (`voucher_id`) REFERENCES `tbl_customer_advance_vouchers` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
