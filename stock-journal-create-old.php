...
$edit_item_id = isset($_GET['item_id']) ? intval($_GET['item_id']) : 0;
...
$has_sj_voucher = false;
$rc = @mysqli_query($conn, "SHOW COLUMNS FROM tbl_stock_journal LIKE 'voucher_no'");
if ($rc && mysqli_num_rows($rc) > 0) $has_sj_voucher = true;
if ($rc) @mysqli_free_result($rc);

if ($isEdit && $item_id > 0) {
    $voucher_no = null;
    if ($has_sj_voucher) {
        $stmt = $conn->prepare("SELECT voucher_no FROM tbl_stock_journal WHERE id = ? LIMIT 1");
        if ($stmt) {
        ...
        $stmt = $conn->prepare("SELECT sj_invoice_no FROM tbl_stock_journal WHERE id = ? LIMIT 1");
        if ($stmt) {
            $stmt->bind_param("i", $item_id);
            $stmt->execute();
            $result = $stmt->get_result();
            if ($result && $result->num_rows > 0) {
                $row = $result->fetch_assoc();
                $voucher_no = isset($row['sj_invoice_no']) ? $row['sj_invoice_no'] : null;
            }
            $stmt->close();
        }
    }
    if ($voucher_no !== null && $voucher_no !== '') {
        if ($has_sj_voucher) {
            $stmt2 = $conn->prepare("SELECT * FROM tbl_stock_journal WHERE voucher_no = ? ORDER BY id ASC");
        } else {
            $stmt2 = $conn->prepare("SELECT * FROM tbl_stock_journal WHERE sj_invoice_no = ? ORDER BY id ASC");
        }
        if ($stmt2) {
            $stmt2->bind_param("s", $voucher_no);
            $stmt2->execute();
    ...
    if (empty($journalRows) && $voucher_no !== null && $voucher_no !== '') {
        $has_st_voucher = false;
        $rc2 = @mysqli_query($conn, "SHOW COLUMNS FROM tbl_stock LIKE 'voucher_no'");
        if ($rc2 && mysqli_num_rows($rc2) > 0) $has_st_voucher = true;
        if ($rc2) @mysqli_free_result($rc2);
        if ($has_st_voucher) {
            $stmt3 = $conn->prepare("SELECT * FROM tbl_stock WHERE voucher_no = ? AND stock_type = 'outward' ORDER BY id ASC");
            if ($stmt3) {
                $stmt3->bind_param("s", $voucher_no);
                $stmt3->execute();
                $res3 = $stmt3->get_result();
                if ($res3) {
                    while ($r = $res3->fetch_assoc()) {
                        $r['stock_id'] = $r['id'];
                        $r['product_name'] = isset($r['product_name']) ? $r['product_name'] : '';
                        $r['article'] = isset($r['article']) ? $r['article'] : '';
                        $r['metal_name'] = isset($r['metal_name']) ? $r['metal_name'] : '';
                        $r['quantity'] = isset($r['opening_qty']) ? $r['opening_qty'] : (isset($r['current_qty']) ? $r['current_qty'] : 0);
                        $r['gross_weight'] = isset($r['opening_weight']) ? $r['opening_weight'] : (isset($r['current_weight']) ? $r['current_weight'] : 0);
                        $journalRows[] = $r;
                    }
                }
                $stmt3->close();
    ...
    if (empty($journalRows) && $voucher_no === null) {
        echo "NO JOURNAL FOUND FOR ID: " . (int)$item_id;
        exit;
    }
    ...
    foreach ($journalRows as $r) {
        ...
        if (!empty($r['product_id'])) {
            $prec = getRecord("SELECT p.name, p.article FROM tbl_products p WHERE p.id = " . (int)$r['product_id']);
            if ($prec) {
                $pname = $prec['name'] ?? '';
                $article = $prec['article'] ?? '';
            }
            if (!empty($r['metal_id'])) {
                $mrec = getRecord("SELECT display_name FROM tbl_metal WHERE id = " . (int)$r['metal_id']);
                if ($mrec) $metal_name = $mrec['display_name'] ?? '';
            }
        }
        $row['product_name'] = $pname !== null ? $pname : (isset($r['product_name']) ? $r['product_name'] : '');
        $row['article'] = $article !== null ? $article : (isset($r['article']) ? $r['article'] : '');
        $row['metal_name'] = $metal_name !== null ? $metal_name : (isset($r['metal_name']) ? $r['metal_name'] : '');
        $stock_id_row = null;
        if (!empty($row['barcode']) && !empty($row['product_id'])) {
            $b_esc = mysqli_real_escape_string($conn, $row['barcode']);
            $stock_id_row = getRecord("SELECT id FROM tbl_stock WHERE barcode COLLATE utf8mb4_unicode_ci = '$b_esc' AND product_id = " . (int)$row['product_id'] . " AND stock_type IN ('purchase','opening') LIMIT 1");
        }
        $row['stock_id'] = $stock_id_row ? $stock_id_row['id'] : null;
        $edit_stock_journal_items[] = $row;
    }
}

if ($edit_item_id > 0) {
    $purchase_invoice_item = getRecord("
        SELECT pii.id, pii.quantity as total_quantity, pii.gross_weight as total_gross_weight,
...
text-transform: uppercase;
    ...
    body.stock-journal-edit-mode #productListTable th[data-column="category"],
    body.stock-journal-edit-mode #productListTable td[data-column="category"],
    body.stock-journal-edit-mode #productListTable th[data-column="product"],
    body.stock-journal-edit-mode #productListTable td[data-column="product"],
    body.stock-journal-edit-mode #productListTable th[data-column="location"],
    body.stock-journal-edit-mode #productListTable td[data-column="location"],
    body.stock-journal-edit-mode #productListTable th[data-column="calculation"],
    body.stock-journal-edit-mode #productListTable td[data-column="calculation"] {
        display: none !important;
    }
    #productListTable th,
    #productListTable td {
...
<body<?php if (!empty($isEdit)) echo ' class="stock-journal-edit-mode"'; ?>>
<!-- [ Preloader ] Start -->
                        ...
                        <div class="col-lg-12" >
                            ...
                            <div class="card mb-4">
                                <div class="card-body product-entry" style="padding: 1.5rem;">
                                    ...
                                    <!-- Balance Display Section (only show when item_id is provided) - single line, small font -->
                                    <?php if ($edit_item_id > 0 && $purchase_invoice_item): ?>
                                    <div class="balance-info-card" style="background: #11294b; color: #fff; padding: 0.5rem 0.75rem; border-radius: 6px; margin-bottom: 0.5rem; box-shadow: 0 1px 4px rgba(0,0,0,0.1); font-size: 0.9rem; line-height: 1.5;">
                                        <span style="opacity: 0.9;">Purchase Invoice:</span> <strong><?php echo htmlspecialchars($purchase_invoice_item['invoice_no'] ?? 'N/A'); ?></strong>
                                        <span style="margin-left: 0.75rem; opacity: 0.7;">|</span>
                                        <span style="margin-left: 0.75rem; opacity: 0.9;">Total Qty:</span> <strong id="totalQuantityDisplay"><?php echo number_format($purchase_invoice_item['total_quantity'] ?? 0, 2); ?></strong>
                                        <span style="margin-left: 0.5rem; opacity: 0.9;">Used Qty:</span> <strong id="usedQuantityDisplay">0.00</strong>
                                        <span style="margin-left: 0.5rem; opacity: 0.9;">Balance Qty:</span> <strong id="balanceQuantityDisplay" style="color: #a7f3d0;"><?php echo number_format($purchase_invoice_item['total_quantity'] ?? 0, 2); ?></strong>
                                        <span style="margin-left: 0.75rem; opacity: 0.7;">|</span>
                                        <span style="margin-left: 0.75rem; opacity: 0.9;">Total Gross Wt:</span> <strong id="totalGrossWeightDisplay"><?php echo number_format($purchase_invoice_item['total_gross_weight'] ?? 0, 3); ?></strong>
                                        <span style="margin-left: 0.5rem; opacity: 0.9;">Used Gross Wt:</span> <strong id="usedGrossWeightDisplay">0.000</strong>
                                        <span style="margin-left: 0.5rem; opacity: 0.9;">Balance Gross Wt:</span> <strong id="balanceGrossWeightDisplay" style="color: #a7f3d0;"><?php echo number_format($purchase_invoice_item['total_gross_weight'] ?? 0, 3); ?></strong>
                                        <span style="margin-left: 0.75rem; opacity: 0.7;">|</span>
                                        <span style="margin-left: 0.75rem; opacity: 0.9;">Barcodes:</span> <strong id="totalBarcodesDisplay">0</strong>
                                    </div>
                                    <?php endif; ?>
                                        ...
                                        <table class="table table-bordered table-sm mb-0" id="productListTable" style="min-width: 4000px; font-size: 0.75rem;">
                                            <thead style="position: sticky; top: 0; background: #f8fafc;">
                                                ...
                                                <tr style="background: #e2e8f0; font-weight: 600;">
                                                    <th rowspan="2" data-column="checkbox" style="min-width: 50px; position: sticky; left: 0; background: #e2e8f0; vertical-align: middle;">
                                                    ...
                                                    <th rowspan="2" data-column="net-amt-tax" style="min-width: 120px; width: 120px; position: sticky !important; right: 160px !important;     background: #a68a4a !important; vertical-align: middle;">Net Amt+Tax</th>
                                                    <th rowspan="2" data-column="reverse" style="min-width: 80px; width: 80px; position: sticky !important; right: 80px !important;     background: #a68a4a !important; vertical-align: middle;">Reverse</th>
                                                    <th rowspan="2" data-column="actions" style="min-width: 80px; width: 80px; text-align: center; position: sticky !important; right: 0 !important;     background: #a68a4a !important; vertical-align: middle;">Action</th>
                                                </tr>
                                            ...
                                            <tbody id="productListBody">
                                                <tr>
                                                    <td colspan="71" class="text-center text-muted py-4">Click "Add Product" button to add products for billing...</td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
...
<script>
    ...
    function initCategoryTabs() {
        document.querySelectorAll('.category-tab-btn').forEach(function(btn) {
            btn.addEventListener('click', function() {
                ...
                // Load products for the selected metal
                if (metalId) {
                    loadProducts(metalId, '');
                }
        ...
        if (firstTab) {
            ...
            // Load products for initial metal
            if (currentMetalId) {
                loadProducts(currentMetalId, '');
            }
    ...
    // Filter products in the modal table by metal ID
    function filterProductsByMetal(metalId) {
        ...
        if (visibleCount === 0 && !emptyRow) {
            tbody.innerHTML = '<tr><td colspan="70" class="text-center text-muted py-4">No products found for this category</td></tr>';
    ...
    // Fetch next serial barcode from server (prefix + zero-padded serial, no duplicate).
    // Pass existing barcodes in Product List so each new row gets RN00002, RN00003, etc.
    function getNextBarcodeFromServer(prefix, digit) {
        prefix = (prefix && String(prefix).trim()) ? String(prefix).trim() : 'B';
        digit = parseInt(digit, 10) || 5;
        if (digit < 1) digit = 5;
        const usedBarcodes = [];
        document.querySelectorAll('#productTableBody [data-column="barcode"]').forEach(function(cell) {
            const t = (cell.textContent || '').trim();
            if (t) usedBarcodes.push(t);
        });
        document.querySelectorAll('#productTableBody tr[data-barcode]').forEach(function(tr) {
            const b = tr.getAttribute('data-barcode');
            if (b && b.trim() && usedBarcodes.indexOf(b.trim()) === -1) usedBarcodes.push(b.trim());
        });
        const params = new URLSearchParams({ prefix: prefix, digit: digit });
    ...
    function generateUniqueBarcode(productId = null, characteristicId = null, prefix = 'B', digits = 8) {
        ...
        if (productId) {
            ...
            const productRow = document.querySelector(`#productListBody tr[data-product-id="${productId}"]${characteristicId ? `[data-characteristic-id="${characteristicId}"]` : ''}`);
            if (productRow) {
    ...
    function addEmptyProductRow() {
        ...
        // Create a new empty product row (no product_id yet, so use default prefix and digits)
        const uniqueBarcode = generateUniqueBarcode(null, null, 'B', 8);
        const row = document.createElement('tr');
    ...
    function populateRowWithProduct(row, product) {
        ...
        if (barcodeInput) {
            if (!barcodeInput.value || barcodeInput.value.trim() === '') {
                // Get product_id and characteristic_id from row
                const productId = row.getAttribute('data-product-id') || null;
                const characteristicId = row.getAttribute('data-characteristic-id') || null;
                const prefix = row.getAttribute('data-barcode-prefix') || 'B';
                const digits = parseInt(row.getAttribute('data-barcode-digits')) || 8;
                barcodeInput.value = generateUniqueBarcode(productId, characteristicId, prefix, digits);
            }
        }
    ...
    function loadProducts(metalId, search = '') {
        ...
        // Check if item_id is present - if so, load products from purchase invoice items
        ...
        // If item_id is present, load products from purchase invoice items instead
        if (itemId > 0) {
        ...
        if (typeof jQuery !== 'undefined' && jQuery.ajax) {
            ...
            success: function(response) {
                if (response.success && response.products.length > 0) {
                    ...
                    response.products.forEach(function(product) {
                        const productName = product.name + (product.metal_name ? ' - ' + product.metal_name : '');
                        html += `
                            <tr class="product-row" data-product-id="${product.id}" data-characteristic-id="${product.characteristic_id || ''}" data-metal-id="${product.metal_id || metalId || ''}" data-barcode-prefix="${product.barcode_prefix || 'B'}" data-barcode-digits="${product.barcode_digits || 8}">
                                <td data-column="checkbox" style="text-align: center; position: sticky; left: 0; background: #fff; z-index: 1;">
            ...
            // If item_id is present, load products from purchase invoice items instead
            if (itemId > 0) {
                url = 'ajax/get-products-by-item-id.php?item_id=' + itemId;
            }
    ...
    function loadProductModalColumnPreferences(onLoaded) {
        $.ajax({
            ...
            success: function(response) {
                ...
                // Apply for current tab when load completes so Gold tab (and others) show saved columns
    ...
    function applyProductModalColumnVisibilityForTab(tabKey) {
        ...
        var prefs = (window.productModalColumnVisibilityByTab && (window.productModalColumnVisibilityByTab[tk] || window.productModalColumnVisibilityByTab[tabKey]));
        // There are two Product Selection areas (main page + modal); update all dropdowns and all tables
        var dropdowns = document.querySelectorAll('[id="modalTableSettingsDropdown"]');
        var tables = document.querySelectorAll('[id="productListTable"]');
        ...
        dropdowns.forEach(function(settingsDropdown) {
            ...
            checkboxes.forEach(function(checkbox) {
                ...
                var isVisible = (prefs && prefs.hasOwnProperty(columnName)) ? (prefs[columnName] === 1) : true;
    ...
    function saveProductModalColumnPreferencesDebounced(tabKey) {
        ...
        productModalColumnSaveTimeout = setTimeout(function() {
            // Prefer modal's dropdown (user is in modal when using tabs); fallback to first
            var settingsDropdown = document.querySelector('#productSelectionModal [id="modalTableSettingsDropdown"]') || document.querySelector('[id="modalTableSettingsDropdown"]');
            ...
            for (var k in prefs) window.productModalColumnVisibilityByTab[tabKey][k] = prefs[k];
            $.ajax({
    ...
    // Event delegation so both main page and modal Show/Hide buttons work
    document.addEventListener('click', function(e) {
        var btn = e.target.closest('[id="modalTableSettingsBtn"]');
        if (btn) {
            e.stopPropagation();
            e.preventDefault();
            var wrapper = btn.closest('.table-settings-wrapper');
            var drop = wrapper && wrapper.querySelector('.table-settings-dropdown');
    ...
    document.addEventListener('change', function(e) {
        if (!e.target.matches('.table-settings-dropdown input[type="checkbox"][data-column]')) return;
        var columnName = e.target.getAttribute('data-column');
        var isVisible = e.target.checked;
        // Update all Product Selection tables (main page + modal)
        document.querySelectorAll('[id="productListTable"]').forEach(function(table) {
            var headers = table.querySelectorAll('th[data-column="' + columnName + '"]');
            var cells = table.querySelectorAll('td[data-column="' + columnName + '"]');
        ...
        // Sync other dropdown(s) so checkbox state is same everywhere
        document.querySelectorAll('[id="modalTableSettingsDropdown"]').forEach(function(drop) {
            var cb = drop.querySelector('input[data-column="' + columnName + '"]');
            if (cb && cb !== e.target) cb.checked = isVisible;
        });
        // Save for current tab (per user); other tabs keep their own state
    ...
    async function selectProduct(row, closeModal = false) {
        ...
        const modalRowData = {
            ...
            barcode_prefix: row.getAttribute('data-barcode-prefix') || 'B',
            barcode_digits: parseInt(row.getAttribute('data-barcode-digits'), 10) || 5
        };
        ...
        console.log('Extracted modal row data:', modalRowData);
            ...
            await updateProductListRowFromModalRow(currentEditingRowId, row);
            currentEditingRowId = null;
                    } else {
            // Add new row with modal data
            console.log('Adding new row from modal data');
            await addProductToTableFromModalRow(modalRowData);
                    }
    ...
    async function addProductToTableFromModalRow(modalRowData) {
        ...
        if (typeof purchaseInvoiceItem !== 'undefined' && purchaseInvoiceItem) {
        ...
        try {
            // Use existing barcode from modal data if available (for edit mode), otherwise get next serial from server (no duplicate)
            let barcode = '';
            if (modalRowData.barcode && modalRowData.barcode.trim() !== '') {
                barcode = modalRowData.barcode.trim();
            } else {
                const prefix = modalRowData.barcode_prefix || 'B';
                const digit = parseInt(modalRowData.barcode_digits, 10) || 5;
                try {
    ...
    // Get current balance qty and balance gross weight (for item_id context). Returns null if no item_id.
    function getBalanceQtyAndWeight(excludeRow) {
        const urlParams = new URLSearchParams(window.location.search);
        const itemId = urlParams.get('item_id') ? parseInt(urlParams.get('item_id')) : 0;
        if (itemId <= 0 || typeof purchaseInvoiceItem === 'undefined' || !purchaseInvoiceItem) {
            return null;
        }
        const totalQty = parseFloat(purchaseInvoiceItem.total_quantity || 0);
        const totalGrossWt = parseFloat(purchaseInvoiceItem.total_gross_weight || 0);
        const existingUsedQty = parseFloat(purchaseInvoiceItem.existing_used_quantity || 0);
        const existingUsedGrossWt = parseFloat(purchaseInvoiceItem.existing_used_gross_weight || 0);
        const tbody = document.getElementById('productTableBody');
    ...
    // Calculate and update quantity and gross weight balance
    function updateBalance() {
        // Only show balance if item_id is provided
        const urlParams = new URLSearchParams(window.location.search);
        const itemId = urlParams.get('item_id') ? parseInt(urlParams.get('item_id')) : 0;
        
        if (itemId <= 0) {
            return; // No balance tracking if no item_id
        }
        
        // Get purchase invoice item data from PHP (passed via JavaScript variable)
        if (typeof purchaseInvoiceItem === 'undefined' || !purchaseInvoiceItem) {
            return;
        }
    ...
    async function updateProductListRowFromModalRow(productListRowId, modalRow) {
        ...
        // If still blank, get next serial from server (no duplicate)
        if (!barcode || barcode === '') {
            const prefix = modalRow.getAttribute('data-barcode-prefix') || 'B';
            const digit = parseInt(modalRow.getAttribute('data-barcode-digits'), 10) || 5;
            try {
                barcode = await getNextBarcodeFromServer(prefix, digit);
            } catch (err) {
                console.error('getNextBarcodeFromServer failed:', err);
        ...
        if (productListBarcodeCell && barcode) {
            ...
            productListBarcodeCell.setAttribute('onclick', `printBarcode('${escapeHtml(barcode)}', event)`);
        ...
        if (productListPrintCell && barcode) {
            productListPrintCell.innerHTML = `<i class="feather icon-printer" style="cursor: pointer; font-size: 0.9rem; color: #c5a864;" onclick="printBarcode('${escapeHtml(barcode)}', event)" title="Print Barcode"></i>`;
        }
        ...
        fetch(url)
            ...
            .then(data => {
                if (data.success && data.product) {
                    ...
                    setTimeout(function() {
                        // Clear the product list body inside the modal (use modal's tbody so product shows in edit mode)
    ...
    // Modal Add Button - Add all products to table (without checkbox requirement)
    ...
    if (modalAddBtn) {
        modalAddBtn.addEventListener('click', function() {
            ...
            if (window.STOCK_JOURNAL_EDIT_MODE && !currentEditingRowId) {
                alert('Add is disabled in edit mode. You can only update existing records.');
                return;
            }
            const tbody = document.getElementById('productListBody');
            ...
            console.log('Found product rows:', allProductRows.length);
            
            // Check if we're in edit mode (editing a row in the table)
            if (currentEditingRowId) {
                console.log('Edit mode - updating row:', currentEditingRowId);
            ...
            if (typeof purchaseInvoiceItem !== 'undefined' && purchaseInvoiceItem) {
                ...
                if (balance) {
                    ...
                    selectedRows.forEach(function(row) {
                        sumQty += getVal(row, 'quantity');
                        sumWt += getVal(row, 'gross-wt');
                    });
            ...
            function addNextProduct() {
                if (index >= selectedRows.length) {
                    // All products added; keep modal open and focus Product Selection Gross Wt
                    if (errorCount > 0) {
                        alert(`Added ${selectedRows.length - errorCount} products. ${errorCount} products failed to add.`);
                    }
                    updateSummaryPanel();
                    // Focus first Gross Wt in Product Selection table (not Product List)
                    setTimeout(function() {
                        const selectionTbody = document.getElementById('productListBody');
                        if (selectionTbody) {
                            const firstGrossWt = selectionTbody.querySelector('[data-column="gross-wt"] input');
                            if (firstGrossWt) firstGrossWt.focus();
                        }
                    }, 150);
                    return;
                }
                ...
                console.log(`Adding product ${index + 1} of ${selectedRows.length}`);
                try {
                ...
                // Add next product after a short delay to allow processing to complete
    ...
    function addProductRowToSelectionTable(item, product) {
        // Prefer modal's tbody when modal exists so edit mode and Add Product show rows in the modal
        const tbody = document.querySelector('#productSelectionModal #productListBody') || document.getElementById('productListBody');
        if (!tbody) {
    ...
    function saveStockJournal() {
        ...
        .then(data => {
            ...
            if (data.status === 'success' || data.status === true) {
                alert(data.message || (isEditMode ? 'Stock updated successfully' : 'Stock Journal saved successfully!'));
                if (!isEditMode) {
    ...
    <?php if ($edit_item_id > 0 && $edit_mode && !$add_mode): ?>
    $(document).ready(function() {
        ...
        // Function to load items into Product Selection modal table
        function loadStockJournalItemsIntoModal() {
            ...
            if (!stockJournalItems || stockJournalItems.length === 0) {
                productListBody.innerHTML = '<tr><td colspan="70" class="text-center text-muted py-4">No stock journal items found</td></tr>';
                return;
            }
            ...
            stockJournalItems.forEach(function(sjItem) {
                ...
                if (typeof addRowFromStockJournal === 'function') {
                    ...
                    row.setAttribute('data-barcode', sjItem.barcode || '');
                    row.setAttribute('data-stock-id', sjItem.stock_id || '');
                    
                    const barcode = sjItem.barcode || '';
                    const productName = (sjItem.product_name || '') + (sjItem.metal_name ? ' - ' + sjItem.metal_name : '');
                    
                    // Create row HTML with all required columns
                    row.innerHTML = `
                        <td data-column="checkbox" style="text-align: center; position: sticky; left: 0; background: #fff; z-index: 1;">
                            <input type="checkbox" class="product-checkbox" data-product-id="${sjItem.product_id || ''}" data-characteristic-id="${sjItem.product_characteristic_id || ''}">
                        </td>
                        <td data-column="id">${sjItem.id || ''}</td>
                        <td data-column="rfid"><input type="text" class="form-control form-control-sm" value="" style="width: 100px; font-size: 0.7rem;"></td>
                        ...
                        <td data-column="barcode" style="position: relative;">
                            <div style="display: flex; align-items: center; gap: 5px;">
                                <input type="text" class="form-control form-control-sm barcode-input" value="${escapeHtml(barcode)}" style="width: 100px; font-size: 0.7rem;" readonly>
                                <i class="feather icon-printer barcode-print-icon" style="cursor: pointer; font-size: 0.9rem; color: #c5a864;" onclick="printBarcode('${escapeHtml(barcode)}', event)" title="Print Barcode"></i>
                            </div>
                        </td>
                        ...
                        <td data-column="purity"><input type="text" class="form-control form-control-sm" value="${sjItem.purity || 1}" step="0.01" style="width: 80px; font-size: 0.7rem;"></td>
                        <td data-column="purity-wt"><input type="text" class="form-control form-control-sm" value="${sjItem.purity_weight || sjItem.pure_weight || 0}" step="0.001" readonly style="width: 90px; font-size: 0.7rem;"></td>
                        <td data-column="wastage-per"><input type="text" class="form-control form-control-sm" value="0" step="0.01" style="width: 100px; font-size: 0.7rem;"></td>
                        <td data-column="wastage-wt"><input type="text" class="form-control form-control-sm" value="0" step="0.001" readonly style="width: 100px; font-size: 0.7rem;"></td>
                        <td data-column="final-wt"><input type="text" class="form-control form-control-sm" value="${sjItem.final_weight || sjItem.gross_weight || 0}" step="0.001" style="width: 80px; font-size: 0.7rem;"></td>
                        <td data-column="alloy-wt"><input type="text" class="form-control form-control-sm" value="0" step="0.001" style="width: 80px; font-size: 0.7rem;"></td>
                        ...
                        <td data-column="metal-value"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" readonly style="width: 100px; font-size: 0.7rem;"></td>
                        <td data-column="metal-cost"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" readonly style="width: 100px; font-size: 0.7rem; color: #7b1fa2;"></td>
                        <td data-column="amount"><input type="text" class="form-control form-control-sm" value="${sjItem.amount || 0}" step="0.01" readonly style="width: 100px; font-size: 0.7rem;"></td>
                        <td data-column="discount-type"><select class="form-control form-control-sm" style="width: 100px; font-size: 0.7rem;"><option value="On Amount" selected>On Amount</option><option value="On Percentage">On Percentage</option></select></td>
                        <td data-column="discount-per"><input type="text" class="form-control form-control-sm" value="0" step="0.01" style="width: 80px; font-size: 0.7rem;"></td>
                        <td data-column="discount-amount"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" readonly style="width: 100px; font-size: 0.7rem;"></td>
                        <td data-column="discount"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" readonly style="width: 100px; font-size: 0.7rem;"></td>
                        ...
                        <td data-column="discount-per2"><input type="text" class="form-control form-control-sm" value="0" step="0.01" style="width: 80px; font-size: 0.7rem;"></td>
                        <td data-column="discount-amount2"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" readonly style="width: 100px; font-size: 0.7rem;"></td>
                        <td data-column="discounted-amt"><input type="text" class="form-control form-control-sm" value="${sjItem.net_amount || 0}" step="0.01" readonly style="width: 120px; font-size: 0.7rem;"></td>
                        <td data-column="discounted-per"><input type="text" class="form-control form-control-sm" value="0" step="0.01" style="width: 120px; font-size: 0.7rem;"></td>
                        ...
                        <td data-column="making-rate"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" style="width: 100px; font-size: 0.7rem;"></td>
                        <td data-column="making-discount-amt"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" style="width: 130px; font-size: 0.7rem;"></td>
                        <td data-column="making-amount"><input type="text" class="form-control form-control-sm" value="${sjItem.making_amount || 0}" step="0.01" readonly style="width: 100px; font-size: 0.7rem;"></td>
                        <td data-column="making-actual-value"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" readonly style="width: 120px; font-size: 0.7rem;"></td>
                        <td data-column="making-cost"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" readonly style="width: 110px; font-size: 0.7rem;"></td>
                        <td data-column="min-price"><input type="text" class="form-control form-control-sm" value="0" step="0.01" style="width: 100px; font-size: 0.7rem;"></td>
                        <td data-column="minimum"><input type="text" class="form-control form-control-sm" value="0" step="0.01" style="width: 100px; font-size: 0.7rem;"></td>
                        ...
                        <td data-column="stone-cost"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" readonly style="width: 100px; font-size: 0.7rem;"></td>
                        <td data-column="diamond-amount"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" style="width: 120px; font-size: 0.7rem;"></td>
                        <td data-column="purchase-amount"><input type="text" class="form-control form-control-sm" value="${sjItem.net_amount || 0}" step="0.01" readonly style="width: 130px; font-size: 0.7rem;"></td>
                                    <td data-column="sale-percent"><input type="text" class="form-control form-control-sm" value="0" step="0.01" style="width: 80px; font-size: 0.7rem;" placeholder="%" title="Sale % on Purchase Amount"></td>            <td data-column="sale-amount"><input type="text" class="form-control form-control-sm" value="${sjItem.net_amount || 0}" step="0.01" readonly style="width: 110px; font-size: 0.7rem;"></td>
                        <td data-column="sale-amount-with"><input type="text" class="form-control form-control-sm" value="${sjItem.net_amt_with_tax || 0}" step="0.01" readonly style="width: 130px; font-size: 0.7rem;"></td>
                        <td data-column="net-amt"><input type="text" class="form-control form-control-sm" value="${sjItem.net_amount || 0}" step="0.01" readonly style="width: 100px; font-size: 0.7rem;"></td>
                        <td data-column="tax"><input type="text" class="form-control form-control-sm" value="${sjItem.tax_amount || 0}" step="0.01" style="width: 80px; font-size: 0.7rem;"></td>
                        <td data-column="other-charge-type"><select class="form-control form-control-sm" style="width: 100px; font-size: 0.7rem;"><option value="Fix" selected>Fix</option><option value="Percentage">Percentage</option></select></td>
                        <td data-column="other-weight"><input type="text" class="form-control form-control-sm" value="0" step="0.001" style="width: 110px; font-size: 0.7rem;"></td>
                        <td data-column="other-rate"><input type="text" class="form-control form-control-sm" value="0" step="0.01" style="width: 100px; font-size: 0.7rem;"></td>
                        <td data-column="other-info"><input type="text" class="form-control form-control-sm" value="" style="width: 100px; font-size: 0.7rem;"></td>
                        ...
                        <td data-column="hallmark-amount"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" style="width: 130px; font-size: 0.7rem;"></td>
                        <td data-column="hallmark-rate"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" style="width: 120px; font-size: 0.7rem;"></td>
                        <td data-column="net-amt-tax"><input type="text" class="form-control form-control-sm" value="${sjItem.net_amt_with_tax || 0}" step="0.01" readonly style="width: 120px; font-size: 0.7rem;"></td>
                        <td data-column="reverse"><input type="text" class="form-control form-control-sm" value="0" step="0.01" style="width: 80px; font-size: 0.7rem;"></td>
                        <td data-column="actions" style="text-align: center;">
                            <button type="button" class="btn btn-sm btn-primary" onclick="updateStockRow(this)" title="Update">Update</button>
                        </td>
                    `;
        ...
        setTimeout(function() {
            ...
            function loadStockJournalItemsIntoMainTable() {
            const tbody = document.getElementById('productTableBody');
            if (!tbody) return;
            const emptyRow = tbody.querySelector('.no-drag');
            if (emptyRow) emptyRow.remove();
            if (!stockJournalItems || stockJournalItems.length === 0) {
                tbody.innerHTML = '<tr class="no-drag"><td colspan="32" class="text-center text-muted py-4">No items</td></tr>';
                return;
            }
            stockJournalItems.forEach(function(sjItem, idx) {
                const row = document.createElement('tr');
                row.setAttribute('data-product-id', sjItem.product_id || '');
                row.setAttribute('data-barcode', sjItem.barcode || '');
                row.setAttribute('data-stock-id', sjItem.stock_id || '');
                row.setAttribute('data-characteristic-id', sjItem.product_characteristic_id || '');
                const qty = parseFloat(sjItem.quantity) || 1;
                const grossWt = parseFloat(sjItem.gross_weight) || 0;
                let purity = parseFloat(sjItem.purity) || 0;
                if (purity > 1) purity = purity / 100;
                const rate = parseFloat(sjItem.rate) || 0;
                const amount = parseFloat(sjItem.amount || sjItem.net_amount) || 0;
                const productName = (sjItem.product_name || '') + (sjItem.metal_name ? ' - ' + sjItem.metal_name : '');
                const netWt = grossWt;
                const pureWt = (grossWt * purity).toFixed(3);
                row.innerHTML = `
                <td data-column="print-barcode" style="text-align: center; width: 50px;"><i class="feather icon-printer" style="cursor: pointer; font-size: 0.9rem; color: #c5a864;" onclick="printBarcodeFromRow(this)" title="Print Barcode"></i></td>
                <td data-column="barcode" style="text-align: center; color: #11294b;">${typeof escapeHtml !== 'undefined' ? escapeHtml(sjItem.barcode || '') : (sjItem.barcode || '').replace(/&/g,'&amp;').replace(/</g,'&lt;')}</td>
                <td data-column="description" class="product-select-cell" style="cursor: pointer; color: #11294b;">${typeof escapeHtml !== 'undefined' ? escapeHtml(productName) : (productName || '').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}</td>
                <td data-column="quantity" style="text-align: right;"><input type="text" class="form-control form-control-sm editable-field" data-field="quantity" value="${qty.toFixed(2)}" step="0.01" style="text-align: right; border: none; background: transparent; padding: 0.25rem; color: #11294b; width: 80px;"></td>
                <td data-column="gross-wt" style="text-align: right;"><input type="text" class="form-control form-control-sm editable-field" data-field="gross_wt" value="${grossWt.toFixed(3)}" step="0.001" style="text-align: right; border: none; background: transparent; padding: 0.25rem; color: #11294b; width: 80px;"></td>
                <td data-column="less-wt" style="text-align: right;"><input type="text" class="form-control form-control-sm editable-field" data-field="less_wt" value="0" step="0.001" style="text-align: right; border: none; background: transparent; padding: 0.25rem; color: #11294b; width: 80px;"></td>
                <td data-column="purity" style="text-align: right;"><input type="text" class="form-control form-control-sm editable-field" data-field="purity" value="${purity.toFixed(2)}" step="0.01" style="text-align: right; border: none; background: transparent; padding: 0.25rem; color: #11294b; width: 80px;"></td>
                <td data-column="final-wt" style="text-align: right;"><input type="text" class="form-control form-control-sm editable-field" data-field="final_wt" value="${grossWt.toFixed(1)}" step="0.1" style="text-align: right; border: none; background: transparent; padding: 0.25rem; color: #11294b; width: 80px;"></td>
                <td data-column="net-wt" style="text-align: right; color: #11294b;">${netWt.toFixed(3)}</td>
                <td data-column="pure-wt" style="text-align: right; color: #11294b;">${pureWt}</td>
                <td data-column="making" style="text-align: right;"><input type="text" class="form-control form-control-sm editable-field" data-field="making" value="0" step="0.01" style="text-align: right; border: none; background: transparent; padding: 0.25rem; color: #11294b; width: 80px;"></td>
                <td data-column="design-no" style="text-align: right;"><input type="text" class="form-control form-control-sm editable-field" data-field="design_no" value="${typeof escapeHtml !== 'undefined' ? escapeHtml(sjItem.article || '') : (sjItem.article || '').replace(/</g,'&lt;')}" style="text-align: right; border: none; background: transparent; padding: 0.25rem; color: #11294b; width: 80px;"></td>
                <td data-column="stone-charges" style="text-align: right;"><input type="text" class="form-control form-control-sm editable-field" value="0" step="0.01" style="text-align: right; border: none; background: transparent; padding: 0.25rem; color: #11294b; width: 80px;"></td>
                <td data-column="other-charges" style="text-align: right;"><input type="text" class="form-control form-control-sm editable-field" value="0" step="0.01" style="text-align: right; border: none; background: transparent; padding: 0.25rem; color: #11294b; width: 80px;"></td>
                <td data-column="diamond-value" style="text-align: right;"><input type="text" class="form-control form-control-sm editable-field" value="0" step="0.01" style="text-align: right; border: none; background: transparent; padding: 0.25rem; color: #11294b; width: 80px;"></td>
                <td data-column="gemstone-value" style="text-align: right;"><input type="text" class="form-control form-control-sm editable-field" value="0" step="0.01" style="text-align: right; border: none; background: transparent; padding: 0.25rem; color: #11294b; width: 80px;"></td>
                <td data-column="rate" style="text-align: right;"><input type="text" class="form-control form-control-sm editable-field" data-field="rate" value="${rate.toFixed(2)}" step="0.01" style="text-align: right; border: none; background: transparent; padding: 0.25rem; color: #11294b; width: 80px;"></td>
                <td data-column="metal-value" style="text-align: right; color: #11294b;">${(grossWt * purity * rate).toFixed(2)}</td>
                <td data-column="discount" style="text-align: right; color: #11294b;">0.00</td>
                ...
                <td data-column="purchase-amount" style="text-align: right; color: #11294b;">${amount.toFixed(2)}</td>
                <td data-column="sale-amount" style="text-align: right; color: #11294b;">${amount.toFixed(2)}</td>
                <td data-column="sale-amount-with" style="text-align: right; color: #11294b;">${amount.toFixed(2)}</td>
                <td data-column="reverse" style="text-align: right; color: #11294b;">0.00</td>
                <td data-column="tax" style="text-align: right;"><input type="text" class="form-control form-control-sm editable-field" value="0" step="0.01" style="text-align: right; border: none; background: transparent; padding: 0.25rem; color: #11294b; width: 80px;"></td>
                <td data-column="amount" style="text-align: right; font-weight: 600; color: #11294b;"><input type="text" class="form-control form-control-sm editable-field" data-field="amount" value="${amount.toFixed(2)}" step="0.01" style="text-align: right; border: none; background: transparent; padding: 0.25rem; color: #11294b; width: 80px;"></td>
                <td data-column="net-amt" style="text-align: right; color: #11294b;">${amount.toFixed(2)}</td>
                <td data-column="net-amt-tax" style="text-align: right; color: #11294b;">${amount.toFixed(2)}</td>
                <td><div class="action-btns"><button type="button" class="btn btn-sm btn-primary" onclick="updateStockRow(this)" title="Update">Update</button></div></td>
                `;
                tbody.appendChild(row);
            });
            if (typeof updateSummaryRow === 'function') updateSummaryRow();
            if (typeof updateSummaryPanel === 'function') updateSummaryPanel();
            if (typeof updateBalance === 'function') setTimeout(updateBalance, 200);
            }
            
            window.updateStockRow = function(btn) {
                var row = btn.closest('tr');
                if (!row) return;
                var stockId = row.getAttribute('data-stock-id');
                var barcode = row.getAttribute('data-barcode');
                var productId = row.getAttribute('data-product-id');
                if (!stockId) { alert('Missing stock row'); return; }
                var qtyEl = row.querySelector('[data-column="quantity"] input');
                var grossEl = row.querySelector('[data-column="gross-wt"] input');
                var purityEl = row.querySelector('[data-column="purity"] input');
                var rateEl = row.querySelector('[data-column="rate"] input') || row.querySelector('[data-column="rate"]');
                var amountEl = row.querySelector('[data-column="amount"] input') || row.querySelector('[data-column="amount"]');
                var quantity = qtyEl ? parseFloat(qtyEl.value) || 0 : 0;
                var gross_weight = grossEl ? parseFloat(grossEl.value) || 0 : 0;
                var purity = purityEl ? parseFloat(purityEl.value) || 0 : 0;
                var rate = rateEl ? (rateEl.value !== undefined ? parseFloat(rateEl.value) : parseFloat(rateEl.textContent)) || 0 : 0;
                var value = amountEl ? (amountEl.value !== undefined ? parseFloat(amountEl.value) : parseFloat(amountEl.textContent)) || 0 : 0;
                var fd = new FormData();
                fd.append('update_row', '1');
                fd.append('id', stockId);
                fd.append('barcode', barcode || '');
                fd.append('product_id', productId || '');
                fd.append('quantity', quantity);
                fd.append('gross_weight', gross_weight);
                fd.append('purity', purity);
                fd.append('rate', rate);
                fd.append('value', value);
                var xhr = new XMLHttpRequest();
                xhr.open('POST', 'ajax/update-stock-journal-row.php');
                xhr.onload = function() {
                    var res = {};
                    try { res = JSON.parse(xhr.responseText); } catch (e) { res = { status: 'error', message: xhr.responseText }; }
                    if (res.status === 'success') {
                        if (typeof updateBalance === 'function') updateBalance();
                        if (typeof loadStockJournalItemsIntoModal === 'function') loadStockJournalItemsIntoModal();
                        if (typeof loadStockJournalItemsIntoMainTable === 'function') loadStockJournalItemsIntoMainTable();
                    } else {
            ...
            loadStockJournalItemsIntoMainTable();
            ...
            // Also load items when Product Selection modal is opened (in case modal wasn't ready when page loaded)
            $('#productSelectionModal').on('shown.bs.modal', function() {
                // Reload saved column preferences (apply runs in load success)
                loadProductModalColumnPreferences();
                const productListBody = document.getElementById('productListBody');
                if (productListBody) {
                    const existingRows = productListBody.querySelectorAll('tr.product-row');
                    if (existingRows.length === 0 && stockJournalItems && stockJournalItems.length > 0) {
                        loadStockJournalItemsIntoModal();
                        // Update balance after loading
                        setTimeout(function() {
                            updateBalance();
                        }, 500);
                    }
