...
<?php auragold_echo_party_select2_scripts(); ?>
    ...
    function saveReceiptPayment(type) {
        ...
        try {
            ...
            console.error('saveReceiptPayment:', err);
            alert('Could not add payment line. Please check the form and try again.');
        } finally {
            ...
            try {
                ...
                console.warn('clearPaymentModal:', clearErr);
            }
        }
    }
    ...
    function clearPaymentModal(type) {
        if (type === 'cash') {
            ...
            var meProduct = document.getElementById('metalExchangeProduct');
            if (meMetal) meMetal.value = '';
            if (meProduct) {
                meProduct.value = '';
                if (typeof jQuery !== 'undefined' && jQuery(meProduct).data('select2')) {
                    jQuery(meProduct).val('').trigger('change');
                }
            }
            ...
            var meFileInput = document.getElementById('metalExchangeFileInput');
            if (meFileInput) meFileInput.value = '';
            if (typeof metalExchangeFiles !== 'undefined') metalExchangeFiles = [];
            var fileList = document.getElementById('metalExchangeFileList');
            if (fileList) {
                fileList.innerHTML = '';
            }
        } else if (type === 'scrap') {
...
// Collect items by data-column (order-independent)
    ...
    $(document).ready(function() {
        var editData = <?php echo json_encode([
            ...
            'exchange_rate' => $edit_voucher['exchange_rate'] ?? ($edit_voucher['currency_rate'] ?? ''),
            'currency_rate' => $edit_voucher['exchange_rate'] ?? ($edit_voucher['currency_rate'] ?? ''),
        ...
        if (typeof setAuragoldPartyValue === 'function') {
            setAuragoldPartyValue(editData.customer_id || '', editData.customer_name || '');
        } else {
        ...
        if (typeof window.auragoldRestoreVoucherCurrencyRate === 'function') {
            window.auragoldRestoreVoucherCurrencyRate(editData);
        } else if ($('#currencyRate').length) {
            var _er = (editData.exchange_rate != null && editData.exchange_rate !== '')
                ? editData.exchange_rate
                : (editData.currency_rate != null && editData.currency_rate !== '' ? editData.currency_rate : '');
            if (_er !== '' && _er != null) {
                $('#currencyRate').val(_er);
    ...
    $(document).ready(function() {
        if (typeof initPaymentListPagination === 'function') {
            paymentListPager = initPaymentListPagination({
                getUrl: 'ajax/get-payment-vouchers.php',
                pageSize: 10,
        ...
        if (typeof initVoucherListFilterExport === 'function') {
            initVoucherListFilterExport({
                ...
                exportExcelUrl: 'ajax/export-voucher-list-excel.php?kind=payment',
                exportPdfUrl: 'ajax/export-voucher-list-pdf.php?kind=payment'
            });
        }
    });
    </script>
