...
$print_settings_early = function_exists('getInvoicePrintSettingsForDocument') ? getInvoicePrintSettingsForDocument('purchase_order') : [];
...
if (isset($_GET['lang'])) {
    ...
    $allowed_langs = function_exists('getInvoicePrintAllowedLanguages') ? getInvoicePrintAllowedLanguages('purchase_order') : ['en'];
...
$items = getList("
    SELECT poi.*, COALESCE(p.name, poi.product_name) as product_name, c.name as category_name
    FROM tbl_purchase_order_items poi
    LEFT JOIN tbl_products p ON poi.product_id = p.id
    LEFT JOIN tbl_categories c ON p.category_id = c.id
    WHERE poi.order_id = $order_id ORDER BY poi.id ASC
");
...
$print_settings = function_exists('getInvoicePrintSettingsForDocument') ? getInvoicePrintSettingsForDocument('purchase_order') : [];
...
if (empty($customer_ref) && !empty($order['customer_id'])) {
    ...
    if ($tbl_sup && mysqli_num_rows($tbl_sup) > 0) {
        mysqli_free_result($tbl_sup);
        $sup = @getRecord("SELECT ref_no, mobile_no FROM tbl_suppliers WHERE id = " . (int)$order['customer_id'] . " LIMIT 1");
    } elseif ($tbl_sup) {
        mysqli_free_result($tbl_sup);
    }
    if ($sup) {
        $customer_ref = $sup['ref_no'] ?? $sup['mobile_no'] ?? '';
    }
}

$L = [
    'en' => [
        'purchase_order' => 'PURCHASE ORDER', 'order_no' => 'Order No.', 'date' => 'Date', 'salesman' => 'Salesman', 'purchase_person' => 'Purchase person',
        'mr_ms' => 'Supplier', 'customer_no' => 'Ref. No.', 'passport_id' => 'Passport / ID No.', 'phone_no' => 'Phone No.',
    ...
    'hi' => [
        'purchase_order' => 'खरीद ऑर्डर', 'order_no' => 'ऑर्डर सं.', 'date' => 'तारीख', 'salesman' => 'विक्रेता', 'purchase_person' => 'खरीद व्यक्ति',
        'mr_ms' => 'आपूर्तिकर्ता', 'customer_no' => 'संदर्भ सं.', 'passport_id' => 'पासपोर्ट / आईडी सं.', 'phone_no' => 'फोन सं.',
    ...
    'mr' => [
        'purchase_order' => 'खरेदी ऑर्डर', 'order_no' => 'ऑर्डर क्र.', 'date' => 'तारीख', 'salesman' => 'विक्रेता', 'purchase_person' => 'खरेदी व्यक्ती',
        'mr_ms' => 'पुरवठादार', 'customer_no' => 'संदर्भ क्र.', 'passport_id' => 'पासपोर्ट / आयडी क्र.', 'phone_no' => 'फोन क्र.',
    ...
    'ar' => [
        'purchase_order' => 'أمر الشراء', 'order_no' => 'رقم الأمر', 'date' => 'التاريخ', 'salesman' => 'البائع', 'purchase_person' => 'مسؤول الشراء',
        'mr_ms' => 'المورد', 'customer_no' => 'المرجع', 'passport_id' => 'رقم الجواز', 'phone_no' => 'رقم التلفون',
...
$person_label = $t['purchase_person'] ?? $t['salesman'];
$person_value = $order['sales_person'] ?? '';
$doc_comment = $order['comment'] ?? '';
$doc_title = !empty($print_settings['invoice_title']) ? $print_settings['invoice_title'] : $t['purchase_order'];
$back_url = 'purchase-order.php';
$document = $order;

$template = isset($_GET['template_preview']) ? trim((string)$_GET['template_preview']) : (function_exists('getInvoiceTemplateForDocument') ? getInvoiceTemplateForDocument('purchase_order') : 'template_classic');
...
<head>
  ...
  <title><?php echo $t['purchase_order']; ?> - <?php echo htmlspecialchars($order['order_no']); ?></title>
  <style>
