...
$auragold_voucher_runtime_client = auragold_voucher_runtime_bootstrap($conn, $metals, 'Credit Note');
...
// Credit Note: use tbl_credit_notes (run admin/sql/create_credit_note_tables.sql if not exists)
$credit_note_table_exists = false;
$t_cn = @mysqli_query($conn, "SHOW TABLES LIKE 'tbl_credit_notes'");
if ($t_cn && mysqli_num_rows($t_cn) > 0) {
    $credit_note_table_exists = true;
}

$next_order_no = 'CN-1';
if ($credit_note_table_exists) {
    $last_cn = getRecord("SELECT credit_note_no FROM tbl_credit_notes ORDER BY id DESC LIMIT 1");
    if ($last_cn && $last_cn['credit_note_no']) {
        $last_num = (int)preg_replace('/[^0-9]/', '', $last_cn['credit_note_no']);
        $next_order_no = 'CN-' . ($last_num + 1);
    }
}

// Load credit note for editing if ID provided
$edit_order_id = isset($_GET['id']) ? (int)$_GET['id'] : 0;
$edit_order = null;
$edit_items = [];
$edit_payments = [];

if ($edit_order_id > 0 && $credit_note_table_exists) {
    $edit_order = getRecord("SELECT * FROM tbl_credit_notes WHERE id = $edit_order_id");
    if ($edit_order) {
        $edit_items = getList("SELECT * FROM tbl_credit_note_items WHERE credit_note_id = $edit_order_id");
        $edit_payments = getList("SELECT * FROM tbl_credit_note_payments WHERE credit_note_id = $edit_order_id");
        $next_order_no = $edit_order['credit_note_no'];
        $edit_order['order_no'] = $edit_order['credit_note_no'];
        $edit_order['order_date'] = $edit_order['credit_note_date'];
        $edit_order['order_id'] = $edit_order['id'];
        $edit_order['invoice_no'] = $edit_order['credit_note_no'];
        $edit_order['invoice_date'] = $edit_order['credit_note_date'];
    }
}

$saved_orders = [];
if ($credit_note_table_exists) {
    $saved_orders = getList("SELECT id, credit_note_no as order_no, customer_name, credit_note_date as order_date, grand_total FROM tbl_credit_notes ORDER BY id DESC LIMIT 50");
}
?>
<!DOCTYPE html>
                            ...
                            <div class="summary-panel" >
                                ...
                                <div class="invoice-header" style="margin: -1rem -1rem 1rem -1rem; border-radius: 10px 10px 0 0; display: flex; align-items: center; justify-content: space-between;">
                                    <div style="display: flex; align-items: center; gap: 0.5rem; flex: 1;">
                                        ...
                                        <h5 class="mb-0" style="font-size: 0.9rem;">Credit Note No: <span id="currentOrderNo"><?php echo htmlspecialchars($next_order_no); ?></span></h5>
                                    </div>
...
<?php if (function_exists('auragold_echo_product_modal_spec_masters_js')) { auragold_echo_product_modal_spec_masters_js(); } ?>
    ...
    const PRODUCT_MODAL_COLUMNS_PAGE = 'credit-note-product-modal';
    ...
    $(document).ready(function() {
        ...
        saleInvoiceSearchInput.on('input', function() {
            ...
            saleInvoiceSearchTimeout = setTimeout(function() {
                $.ajax({
                    url: 'ajax/search-credit-notes.php',
                    type: 'GET',
                    data: { q: searchTerm },
                    dataType: 'json',
                    success: function(response) {
                        if (response.status === 'success' && response.invoices && response.invoices.length > 0) {
                            ...
                            response.invoices.forEach(function(invoice) {
                                const cnNo = invoice.credit_note_no || invoice.invoice_no;
                                html += `
                            ...
                            saleInvoiceSuggestions.html('<div style="padding: 0.75rem; color: #94a3b8; text-align: center;">No credit notes found</div>').show();
                        }
                    },
                    error: function() {
                        saleInvoiceSuggestions.hide();
...
<td data-column="sale-amount-with"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" readonly style="width: 130px; font-size: 0.7rem;"></td>
    ...
    function saveOrder() {
        ...
        if (typeof jQuery !== 'undefined' && jQuery.ajax) {
            jQuery.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        // Redirect to credit-note.php for fresh entry after save
                        window.location.href = 'credit-note.php';
                    } else {
                        alert('Error: ' + (response.message || 'Failed to save credit note'));
                    }
                },
            ...
            fetch('ajax/save-credit-note.php', {
                method: 'POST',
                body: formData
            })
            .then(response => {
            ...
            .then(data => {
                ...
                if (data.status === 'success') {
                    ...
                    alert('Error: ' + (data.message || 'Failed to save credit note'));
                }
            })
            .catch(error => {
                if (saveBtn) {
    ...
    function clearInvoiceForm() {
        ...
        if (window.history && window.history.replaceState) {
            window.history.replaceState({}, document.title, 'credit-note.php');
        }
    ...
    function resetOrder() {
        if (confirm('Are you sure you want to create a new credit note? All unsaved data will be lost.')) {
            window.location.href = 'credit-note.php';
        }
    }
            ...
            jQuery.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        window.history.pushState({}, '', 'credit-note.php?id=' + orderId);
                    } else {
                        alert('Error loading credit note: ' + (response.message || 'Unknown error'));
                    }
                },
                error: function(xhr, status, error) {
                    const msg = document.getElementById('orderLoadingMsg');
                    ...
                    console.error('Load credit note error:', xhr.responseText);
                }
            });
        } else {
            fetch('ajax/get-credit-note.php?order_id=' + orderId)
                .then(response => response.json())
                .then(data => {
                    const msg = document.getElementById('orderLoadingMsg');
                    if (msg) document.body.removeChild(msg);
                    if (data.status === 'success') {
                        ...
                        alert('Error loading credit note: ' + (data.message || 'Unknown error'));
                    }
                })
                .catch(error => {
                    const msg = document.getElementById('orderLoadingMsg');
    ...
    function showEditRowModal(rowData) {
        ...
        const fieldLabels = {
            ...
            'discount': 'Discount': 'Discount Type 2',
