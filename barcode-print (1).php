...
$overrides = [];
$override_keys = ['gross_wt', 'less_wt', 'purity', 'final_wt', 'net_wt', 'pure_wt', 'product_name', 'design_no', 'amount', 'net_amt', 'rate', 'making_amount', 'sale_amount', 'quantity', 'short_code', 'karat', 'carat', 'carat_id', 'carat_name'];
...
foreach ($barcodes as $i => $barcode) {
    ...
    if ($is_82x38_2box && $print_code_kind === 'barcode' && !empty($conn) && $conn instanceof mysqli) {
        if (!function_exists('auragold_get_or_assign_physical_scan_code')) {
            require_once __DIR__ . '/includes/auragold_barcode_scan_map.php';
        }
        $assigned_scan = auragold_get_or_assign_physical_scan_code($conn, $barcode);
        if ($assigned_scan !== null) {
            $row_data['PhysicalScanCode'] = $assigned_scan;
            $row_data['physical_scan_code'] = $assigned_scan;
        }
    }
...
if ($is_82x38_2box && $print_code_kind === 'barcode') {
    if (!function_exists('auragold_resolve_physical_scan_code_for_print')) {
        require_once __DIR__ . '/includes/auragold_barcode_scan_map.php';
    }
    foreach ($barcodes as $bc_raw) {
        $bc_raw = trim((string) $bc_raw);
        if ($bc_raw === '') {
            continue;
        }
        foreach ($barcode_items as $bi) {
            if (($bi['barcode'] ?? '') === $bc_raw && !empty($bi['row']['PhysicalScanCode'])) {
                $existing = trim((string) $bi['row']['PhysicalScanCode']);
                if (auragold_is_valid_physical_scan_code($existing)) {
                    $auragold_physical_scan_codes[$bc_raw] = $existing;
                    continue 2;
                }
            }
        }
        $sc = auragold_resolve_physical_scan_code_for_print(
            (!empty($conn) && $conn instanceof mysqli) ? $conn : null,
            $bc_raw
        );
        if ($sc !== null) {
            $auragold_physical_scan_codes[$bc_raw] = $sc;
        }
    }
}
...
$render_settings = [
    ...
    'physical_scan_codes' => $auragold_physical_scan_codes,
];
if ($is_82x38_2box) {
    $label_width_mm = 82;
    $label_height_mm = 38;
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* Fill mm box; 82×38 uses canvas at exact 203-DPI mm size — not included here */
        ...
        .barcode-print--82x38-2box .barcode-box-inner .design-field.design-white-strip,
        .barcode-print--82x38-2box .barcode-box-inner .design-field.design-strip-line,
        .barcode-print--82x38-2box .barcode-box-inner .design-field.design-field--half-strip {
            background: transparent !important;
            background-color: transparent !important;
        }
        .barcode-print--82x38-2box .barcode-box-inner .design-field.design-field--company-name {
        ...
        .barcode-print--82x38-2box img.barcode-canvas-img--82x38 {
            display: none !important;
        }
        ...
        /* Screen: 1 canvas pixel = 1 printer dot preview; print uses mm via CSS vars */
        .barcode-print--82x38-2box canvas.barcode-canvas--82x38,
        .barcode-print--82x38-2box .barcode-canvas--82x38.barcode-canvas--82x38-sized {
            display: block !important;
            flex: none !important;
...
<body class="barcode-print <?php echo $use_design ? 'barcode-print--design' : 'barcode-print--simple'; ?><?php echo $is_120x50_double_barcode ? ' barcode-print--120x50-double' : ''; ?><?php echo $is_82x38_2box ? ' barcode-print--82x38-2box' : ''; ?><?php echo $item_count === 1 ? ' barcode-print--single-page' : ''; ?>">
    ...
    <script>
        ...
        var PRINTER_DPI = 203;
        var DOT_MM = 25.4 / PRINTER_DPI;
        var auragold82x38MaxPhysicalWidthMm = 20;
        ...
        /** 82×38 CODE128C — 2 dots/module; exact mm at 203 DPI for screen + print. */
        function auragoldRender82x38ScanCodeCanvas(el) {
            if (typeof JsBarcode === 'undefined') {
                console.error('82x38 JsBarcode library not loaded');
                return;
            }
            var graphic = el.closest('.barcode-82x38-graphic');
            if (!graphic) return;
            if (String(el.tagName || '').toLowerCase() !== 'canvas') return;
            var actualBarcode = String(el.getAttribute('data-barcode') || '').trim();
            var scanCode = String(el.getAttribute('data-scan-code') || '').trim();
            if (!/^\d{4}$/.test(scanCode) && actualBarcode && auragold82x38ScanMapByBarcode && auragold82x38ScanMapByBarcode[actualBarcode]) {
                scanCode = String(auragold82x38ScanMapByBarcode[actualBarcode]).trim();
            }
            if (!/^\d{4}$/.test(scanCode) && actualBarcode) {
                scanCode = auragoldDeriveScanCodeFromBarcode(actualBarcode);
            }
            if (!/^\d{4}$/.test(scanCode)) {
                console.error('82x38 invalid scan code — cannot print physical barcode', {
                    actualBarcode: actualBarcode,
                    scanCode: scanCode,
                    scanMap: auragold82x38ScanMapByBarcode
                });
                return;
            }
            el.setAttribute('data-scan-code', scanCode);
            var oldImg = el.nextElementSibling;
            if (oldImg && oldImg.classList && oldImg.classList.contains('barcode-canvas-img--82x38')) {
                oldImg.parentNode.removeChild(oldImg);
            }
            var graphicHMm = parseFloat(el.getAttribute('data-barcode-graphic-height-mm') || '0');
            if (!(graphicHMm > 0)) {
                graphicHMm = auragoldMmFromStyle(graphic, 'height');
            }
            try {
                JsBarcode(el, scanCode, {
                    format: 'CODE128',
                    width: 2,
                    height: 55,
                    displayValue: false,
                ...
                console.error('82x38 JsBarcode failed', actualBarcode, scanCode, e);
                return;
            }
            var dotW = el.width;
            var dotH = el.height;
            if (!(dotW > 0) || !(dotH > 0)) {
                console.error('82x38 barcode canvas has zero size', actualBarcode, scanCode);
                return;
            }
            if (dotW > 220) {
                console.warn(
                    '82x38 barcode symbol too wide — check scan code payload (expect 4-digit CODE128C, not full barcode)',
                    actualBarcode,
                    scanCode,
                    dotW
                );
            }
            var physicalWidthMm = dotW * DOT_MM;
            var physicalHeightMm = dotH * DOT_MM;
            console.log({
                actualBarcode: actualBarcode,
                scanCode: scanCode,
                canvasWidthDots: dotW,
                canvasHeightDots: dotH,
                physicalWidthMm: physicalWidthMm,
                physicalHeightMm: physicalHeightMm,
                moduleDots: 2,
                dpi: PRINTER_DPI
            });
            if (physicalWidthMm > auragold82x38MaxPhysicalWidthMm + 0.05) {
                console.warn(
                    'CODE128C scan barcode exceeds 82x38 physical width — not squeezing',
                    actualBarcode,
                    scanCode,
                    physicalWidthMm
                );
            }
            el.setAttribute('width', String(dotW));
            el.setAttribute('height', String(dotH));
            el.classList.add('barcode-canvas--82x38-sized');
            el.style.setProperty('--auragold-bpw-mm', physicalWidthMm + 'mm');
            el.style.setProperty('--auragold-bph-mm', physicalHeightMm + 'mm');
            el.style.setProperty('display', 'block', 'important');
            el.style.setProperty('width', physicalWidthMm + 'mm', 'important');
            el.style.setProperty('height', physicalHeightMm + 'mm', 'important');
            ...
            if (graphicHMm > 0) {
                graphic.style.setProperty('height', graphicHMm + 'mm', 'important');
                graphic.style.setProperty('min-height', graphicHMm + 'mm', 'important');
                graphic.style.setProperty('max-height', graphicHMm + 'mm', 'important');
            }
            graphic.style.setProperty('overflow', 'visible', 'important');
        ...
        function auragoldFitBoxBarcodeSvgToGraphic(el, graphicSelector) {
            ...
            if (!code) return;
            var baseModW = Math.max(2, auragoldPrintBarW);
            var targetBarH = Math.max(24, Math.round(boxH * 2.5));
            var natW = 0;
            var natH = 0;
            var modW = baseModW;
            ...
            function widthFitLoop() {
                ...
                for (var pass = 0; pass < 14 && natW > 0 && natW < boxW * 0.96; pass++) {
                    modW = Math.min(12, Math.max(1, modW * (boxW / natW) * 1.006));
                    draw(modW, targetBarH);
                    nat = readNatSize();
                    natW = nat.w;
                    natH = nat.h;
                ...
                if (natW > boxW * 1.015) {
                    modW = Math.max(1, Math.min(12, modW * (boxW / natW) * 0.998));
                    draw(modW, targetBarH);
                    nat = readNatSize();
                    natW = nat.w;
                    natH = nat.h;
            ...
            for (var attempt = 0; attempt < 8; attempt++) {
                ...
                if (scaledH <= boxH || targetBarH <= 10) {
                    break;
                }
                targetBarH = Math.max(10, Math.floor(targetBarH * (boxH / scaledH) * 0.9));
            }
        ...
        function auragoldFitDesignWrapBarcode(el) {
            if (el.classList.contains('barcode-canvas--82x38')) {
                auragoldRender82x38ScanCodeCanvas(el);
                return;
            }
            auragoldFitBoxBarcodeSvgToGraphic(el, '.barcode-print-wrap');
        }
        function auragoldRender82x38Barcode(el) {
            auragoldRender82x38ScanCodeCanvas(el);
        }
        function auragoldRenderBarcodeSvg(el, barWidth, barHeight, stretchToBox) {
            ...
            try {
                ...
                if (stretchToBox && el.classList.contains('barcode-canvas--82x38')) {
                    auragoldRender82x38Barcode(el);
                    return;
                }
                JsBarcode(el, code, {
                    format: "CODE128",
        ...
        function auragoldRenderAll82x38DesignBarcodes() {
            ...
            document.querySelectorAll('.barcode-canvas--82x38').forEach(function(el) {
                auragoldRender82x38ScanCodeCanvas(el);
            });
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.barcode-svg:not(.barcode-svg--120x50):not(.barcode-svg-box1):not(.barcode-svg-box2)').forEach(function(el) {
