...
// Build WHERE clause
// Current Stock tab: available_qty = SUM(COALESCE(current_qty, opening_qty)) over grouped stock rows. No production_qty in display qty.
...
// Shared inner query: group by product + branch + metal (used by Current Stock and Stock Details tabs).
...
//   SELECT ... FROM tbl_stock s LEFT JOIN ... WHERE ... GROUP BY s.product_id, s.branch_id, s.metal_id
$stock_inner_from = "
    FROM tbl_stock s
...
$stock_inner_select = "
    SELECT 
        ...
        SUM(
            COALESCE(s.current_qty, s.opening_qty, 0)
        ) as available_qty,
...
// Get branches for filters: login main branch + its sub-branches only
...
$metals = getList("SELECT id, display_name as name FROM tbl_metal WHERE status = 1 ORDER BY display_name ASC");
                                ...
                                <div class="card-body" style="padding: 0; display: flex; flex-direction: column; overflow: hidden;">
                                    ...
                                    <div class="stock-analysis-wrapper">
                                        ...
                                        <div class="page-header-bar">
                                            <div>Gold / Silver Analysis / Current Stock</div>
                                                    ...
                                                    <tbody>
                                                        ...
                                                        if (!empty($stock_data)) {
                                                            foreach($stock_data as $stock) {
                                                                ...
                                                                $qty_for_display = (float)($stock['available_qty'] ?? 0);
