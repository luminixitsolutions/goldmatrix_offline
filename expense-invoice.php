...
$saved_expenses = [];
if ($expense_table_exists) {
    $saved_expenses = getList("SELECT id, expense_no, ledger_name, expense_date, ref_no, grand_total, sales_person FROM tbl_expenses ORDER BY id DESC LIMIT 50");
}

$ei_sp_selected = '';
if (!empty($edit_expense) && is_array($edit_expense)) {
    $ei_sp_selected = trim((string)($edit_expense['sales_person'] ?? ''));
} else {
...
$ei_with_tax = 1;
if (!empty($edit_expense) && is_array($edit_expense)) {
    if (!empty($edit_expense['expense_date'])) {
        $ei_expense_date = substr($edit_expense['expense_date'], 0, 10);
    }
    if (!empty($edit_expense['due_date'])) {
        $ei_due_date = substr($edit_expense['due_date'], 0, 10);
    }
    $ei_currency = trim((string)($edit_expense['currency'] ?? 'AED')) ?: 'AED';
    $ei_exchange_rate = (string)($edit_expense['exchange_rate'] ?? '1');
    $ei_fixing_type = trim((string)($edit_expense['fixing_type'] ?? 'Standard'));
    $ei_against_of = trim((string)($edit_expense['against_of'] ?? ''));
    $ei_with_tax = !empty($edit_expense['with_tax']) ? 1 : 0;
}
...
<head>
    <title>Expense Invoice - <?php echo htmlspecialchars(auragold_app_name(), ENT_QUOTES, 'UTF-8'); ?> Software</title>
    <meta charset="utf-8">
...
<style>
    ...
    #expenseItemsTable thead th {
        background: #11294b;
        color: #fff;
    }

    /* Billing card — match payment-voucher layout */
    .expense-invoice-page .ei-billing-card.card {
    ...
    .expense-invoice-page .ei-billing-card .card-body {
        padding: 1rem 1.25rem;
    }
    .expense-invoice-page .billing-form .row {
        margin-left: -8px;
        margin-right: -8px;
    }
    .expense-invoice-page .billing-form .row > [class*="col-"] {
        padding-left: 8px;
        padding-right: 8px;
        margin-bottom: 8px;
    }
    .expense-invoice-page .billing-form .form-group {
        margin-bottom: 0;
        display: block;
    }
    .expense-invoice-page .billing-form label {
        display: block;
        font-weight: 600;
        font-size: 11px;
        margin-bottom: 4px;
    ...
    .expense-invoice-page .billing-form .form-control,
    .expense-invoice-page .billing-form .form-control-sm {
        display: block;
        width: 100%;
        font-size: 0.8rem;
    ...
    .expense-invoice-page .billing-form .form-group > div:not(.input-group) {
        width: 100%;
        min-width: 0;
    }
    .expense-invoice-page .billing-form .with-tax-group {
        display: flex;
        align-items: center;
    ...
    .expense-invoice-page .billing-form .with-tax-group .form-check-input {
    ...
    .expense-invoice-page .billing-form .with-tax-group .form-check-label {
    ...
    .expense-invoice-page .billing-form .ei-with-tax-col {
        min-width: 110px;
    }
    .expense-invoice-page .billing-form .ei-name-col {
        min-width: 0;
        overflow: hidden;
    }
    .expense-invoice-page .billing-form .ei-name-wrap {
        display: flex;
        align-items: stretch;
        gap: 0;
        width: 100%;
    ...
    .expense-invoice-page .billing-form .ei-name-wrap .auragold-party-select2-wrap {
        flex: 1;
        min-width: 0;
    }
    .expense-invoice-page .billing-form .ei-name-wrap .auragold-party-select2-wrap .select2-container--default .select2-selection--single {
    ...
    .expense-invoice-page .billing-form .ei-name-wrap .auragold-party-select2-wrap .select2-container--default .select2-selection--single .select2-selection__rendered {
        line-height: calc(1.5em + 0.7rem);
    }
    .expense-invoice-page .billing-form .ei-name-wrap .auragold-party-select2-wrap .select2-container--default .select2-selection--single .select2-selection__arrow {
        height: calc(1.5em + 0.7rem + 2px);
    }
    .expense-invoice-page .billing-form .ei-name-wrap .btn {
        flex-shrink: 0;
        width: 34px;
        min-width: 34px;
        align-self: stretch;
    ...
    .expense-invoice-page .billing-form .ei-name-wrap .btn:hover {
        background: #eef2f7;
        color: #11294b;
        border-color: #ced4da;
    }
    .expense-invoice-page .billing-form .auragold-party-select2-wrap .select2-container {
        width: 100% !important;
        max-width: 100%;
    }
    .expense-invoice-page .section-card .card-header-bar {
        display: flex;
        justify-content: space-between;
        align-items: center;
        margin-bottom: 8px;
    }
    .expense-invoice-page .section-card .card-header-bar h6 {
        margin: 0;
        font-weight: 600;
        font-size: 0.85rem;
        color: #11294b;
    }
    .expense-invoice-page #receiptTable thead th {
        background: #11294b;
        color: #fff;
    }
    .expense-invoice-page .payment-icons {
        display: flex;
        gap: 8px;
        flex-wrap: wrap;
        margin-bottom: 10px;
    }
    .expense-invoice-page .payment-icon {
        cursor: pointer;
    }
    .expense-invoice-page .payment-icon img {
        width: 42px;
        height: 42px;
        border-radius: 50%;
        border: 1px solid #e2e8f0;
    }
    .expense-invoice-page .sidebar-panel h6 {
        font-size: 0.8rem;
        font-weight: 600;
        color: #1e293b;
    ...
    .expense-invoice-page .totals-row {
        display: flex;
        justify-content: space-between;
        align-items: center;
        margin-bottom: 6px;
    ...
    .expense-invoice-page .totals-row.grand {
        font-weight: 600;
        font-size: 0.85rem;
        padding-top: 8px;
        border-top: 1px solid #e2e8f0;
    }
    .expense-invoice-page .pi-currency-rate-combo {
        display: flex;
        gap: 6px;
        width: 100%;
        min-width: 0;
    }
    .expense-invoice-page .pi-currency-rate-combo select {
        flex: 1;
        min-width: 0;
        width: auto;
    }
    .expense-invoice-page .pi-currency-rate-combo .auragold-currency-rate {
        width: 72px;
        flex-shrink: 0;
    }
    .expense-invoice-page #expenseListTable thead th {
        background: #11294b;
        color: #fff;
    }
...
<body>
        ...
        <div class="layout-inner">
            ...
            <div class="layout-container">
                ...
                <div class="layout-content">
                    <!-- [ content ] Start -->
                    <div class="container-fluid flex-grow-1 expense-invoice-page" style="padding-top: 0; padding-bottom: 0;">
                        <?php include 'sidebar.php';?>

                        <form id="expenseInvoiceForm" autocomplete="off" onsubmit="return false;">
                                    ...
                                    <div class="card-body" style="padding: 6px 12px;">
                                        <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 8px;">
                                            <div style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
                                                <h5 style="margin: 0; font-weight: 600; font-size: 0.9rem;">Expense Invoice No: <span id="expenseNoDisplay"><?php echo htmlspecialchars($next_expense_no); ?></span></h5>
                                            </div>
                                    ...
                                    <div class="card-body billing-form">
                                        <div class="row">
                                            <div class="col-md-3 col-lg-3">
                                                <div class="form-group">
                                                    ...
                                                    <div style="position: relative;">
                                                        <input type="text" id="searchExpenseInput" class="form-control form-control-sm" placeholder="Expense no or vendor..." autocomplete="off">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div class="ei-name-wrap">
                                                        <div class="auragold-party-select2-wrap">
                                                            ...
                                                            <input type="hidden" id="customerName" name="customer_name" value="<?php echo ($edit_expense && !empty($edit_expense['ledger_name'])) ? htmlspecialchars($edit_expense['ledger_name']) : ''; ?>">
                                                        </div>
                                                        <button type="button" class="btn btn-sm btn-outline-secondary" id="addCustomerBtn" title="Add vendor"><i class="feather icon-plus"></i></button>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="col-md-2 col-lg-2">
                                                <div class="form-group">
                                                    <label>Ref No.</label>
                                                    <input type="text" class="form-control form-control-sm" id="refNo" placeholder="Ref No" value="<?php echo ($edit_expense && isset($edit_expense['ref_no'])) ? htmlspecialchars($edit_expense['ref_no']) : ''; ?>">
                                                </div>
                                            </div>
                                            <div class="col-md-3 col-lg-3">
                                                <div class="form-group">
                                            ...
                                            <div class="col-md-3 col-lg-3">
                                                <div class="form-group">
                                                    <label>Date</label>
                                                    <input type="date" class="form-control form-control-sm" id="voucherDate" value="<?php echo htmlspecialchars($ei_expense_date); ?>">
                                                </div>
                                            </div>
                                            <div class="col-md-3 col-lg-3">
                                                <div class="form-group">
                                ...
                                <div class="card mb-2 section-card" style="margin-left: 0; margin-right: 0;">
                                    <div class="card-body" style="padding: 8px 12px;">
                                        <div class="card-header-bar">
                                            <h6>Expense Items</h6>
                                            <button type="button" class="btn btn-sm btn-outline-primary" onclick="addExpenseItemRow()" style="font-size: 0.75rem;"><i class="feather icon-plus"></i> Add Row</button>
                                        </div>
                                        <div class="table-responsive">
                                            <table class="table table-bordered table-sm" id="expenseItemsTable" style="margin-bottom: 0;">
                                                <thead>
                                                    <tr>
                                    ...
                                    <div class="card-body" style="padding: 8px 12px;">
                                        ...
                                        <div id="paymentTab">
                                                ...
                                                <div style="margin-top: 8px;">
                                                    ...
                                                    <textarea class="form-control form-control-sm" id="comment" rows="2" placeholder="Enter comment here..." style="font-size: 0.75rem; padding: 0.4rem 0.6rem; min-height: 45px;"><?php echo ($edit_expense && !empty($edit_expense['comment'])) ? htmlspecialchars($edit_expense['comment']) : ''; ?></textarea>
                                                </div>
                                        </div>
                                    </div>
                                </div>
                                                ...
                                                <tbody id="expenseListTableBody">
                                                    ...
                                                    <tr data-expense-id="<?php echo $ex_id; ?>" style="cursor: pointer;">
                                                        ...
                                                        <td class="text-center">
                                                            <a href="expense-invoice.php?id=<?php echo $ex_id; ?>" class="btn btn-sm btn-outline-primary" title="Edit expense" onclick="event.stopPropagation();"><i class="feather icon-edit-2"></i></a>
                                    ...
                                    <div class="card-body" style="padding: 8px 12px;">
                                        ...
                                        <div class="form-group mb-2">
                                            ...
                                            <div class="d-flex" style="gap: 6px;">
                                                <input type="number" class="form-control form-control-sm" id="discountPercent" value="<?php echo ($edit_expense && isset($edit_expense['discount_percent'])) ? htmlspecialchars($edit_expense['discount_percent']) : '0.000'; ?>" step="0.001" min="0">
                                                <input type="number" class="form-control form-control-sm" id="discountAmount" value="<?php echo ($edit_expense && isset($edit_expense['discount_amt'])) ? number_format((float)$edit_expense['discount_amt'], 2, '.', '') : '0.00'; ?>" step="0.01" min="0">
                                            </div>
                                        </div>
    ...
    <div class="modal fade" id="expenseTaxModal" tabindex="-1" role="dialog" aria-labelledby="expenseTaxModalLabel" data-backdrop="true" data-keyboard="true">
        <div class="modal-dialog modal-dialog-centered" role="document" style="max-width: 420px;">
            <div class="modal-content" style="border-radius: 8px; box-shadow: 0 10px 40px rgba(0,0,0,0.15);">
                ...
                <div class="modal-body p-0">
                    ...
                    <div id="expenseTaxModalBody" style="max-height: 260px; overflow-y: auto;"></div>
                </div>
...
<?php auragold_echo_party_select2_scripts([
    ...
    'value_id' => (!empty($edit_expense) && !empty($edit_expense['ledger_id'])) ? (int)$edit_expense['ledger_id'] : '',
    'value_name' => (!empty($edit_expense) && !empty($edit_expense['ledger_name'])) ? (string)$edit_expense['ledger_name'] : '',
]); ?>
...
<script>window.AURAGOLD_WORKING_BRANCH_ID = <?php echo (int) $auragold_working_branch_id; ?>;</script>
    ...
    function loadExpenseCategories(searchTerm) {
        ...
        const q = searchTerm || '';
        $.when(
            $.get('ajax/get-expense-ledger-accounts.php', { q: q, limit: 80 }),
            $.get('ajax/get-expense-categories.php', { q: q, limit: 80 })
    ...
    function openExpenseTaxModal(row) {
        ...
        expenseTaxSelectedRate = parseFloat(row.querySelector('.expense-item-tax').value) || 0;
        const body = document.getElementById('expenseTaxModalBody');
        if (!body) return;
        body.innerHTML = '<div class="text-center text-muted py-3">Loading...</div>';
    ...
    function applyExpenseTaxSelection() {
        if (!expenseTaxTargetRow) {
            $('#expenseTaxModal').modal('hide');
            return;
        }
        const picked = document.querySelector('input[name="expenseTaxPick"]:checked');
        const rate = picked ? parseFloat(picked.value) || 0 : 0;
        const taxInp = expenseTaxTargetRow.querySelector('.expense-item-tax');
        const amountInp = expenseTaxTargetRow.querySelector('.expense-item-amount');
        const taxWithInp = expenseTaxTargetRow.querySelector('.expense-item-tax-with-amount');
    ...
    function updateExpenseSummary() {
        ...
        document.querySelectorAll('#expenseItemsTableBody tr:not(.no-expense-rows)').forEach(function(tr) {
            subtotal += parseFloat(tr.querySelector('.expense-item-tax-with-amount') && tr.querySelector('.expense-item-tax-with-amount').value) || 0;
        });
        ...
        $('#balanceAfterExpense').text((prevBal - grandTotal + paidAmt).toFixed(2));
    }
    ...
    function addExpenseItemRow() {
        ...
        tr.innerHTML = '<td><input type="text" class="form-control form-control-sm expense-item-category" placeholder="Category *" readonly style="background:#fff;cursor:pointer;"></td>' +
            ...
            '<td><input type="text" class="form-control form-control-sm expense-item-tax" value="0" readonly style="background:#fff;cursor:pointer;" placeholder="Tax"></td>' +
        ...
        tr.querySelector('.expense-item-tax').addEventListener('click', function() { openExpenseTaxModal(tr); });
    ...
    function loadCustomerBalance() {
        ...
        if (!customerId && !customerName) {
            ...
            $('#noHistoryMessage').show().text('Select a vendor to view history');
    ...
    function resetExpense() {
        if (confirm('Are you sure you want to create a new expense? All unsaved data will be lost.')) {
            window.location.href = 'expense-invoice.php';
        }
    }
