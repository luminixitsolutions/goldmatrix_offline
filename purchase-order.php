...
// Get next order number for purchase orders (PO-1, PO-2, ...)
...
if ($edit_order_id > 0) {
    $edit_order = getRecord("SELECT * FROM tbl_purchase_orders WHERE id = $edit_order_id");
    if ($edit_order) {
        $edit_items = getList("SELECT * FROM tbl_purchase_order_items WHERE order_id = $edit_order_id");
        $edit_payments = getList("SELECT * FROM tbl_purchase_order_payments WHERE order_id = $edit_order_id");
        $next_order_no = $edit_order['order_no'];
    }
}

// Saved orders are loaded via search (ajax/search-purchase-orders.php)
?>
<!DOCTYPE html>
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    ...
                                                    <div style="position: relative;">
                                                        <input type="text" class="form-control form-control-sm" id="searchSaleOrder" placeholder="Type purchase order no or customer name..." autocomplete="off" style="padding-right: 35px;">
                            ...
                            <div class="summary-panel" >
                                ...
                                <div class="invoice-header" style="margin: -1rem -1rem 1rem -1rem; border-radius: 10px 10px 0 0; display: flex; align-items: center; justify-content: space-between;">
                                    <div style="display: flex; align-items: center; gap: 0.5rem; flex: 1;">
                                        ...
                                        <h5 class="mb-0" style="font-size: 0.9rem;">Purchase Order No: <span id="currentOrderNo"><?php echo htmlspecialchars($next_order_no); ?></span></h5>
                                    </div>
...
<script>
    ...
    const PRODUCT_MODAL_COLUMNS_PAGE = 'purchase-order-product-modal';
...
<td data-column="sale-amount-with"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" readonly style="width: 130px; font-size: 0.7rem;"></td>
    ...
    function fetchProductByBarcodeAndAdd(barcode) {
        ...
        fetch('ajax/get-product-by-barcode.php?barcode=' + encodeURIComponent(barcode.trim()) + '&allow_non_stock_barcode=1')
    ...
    $(document).ready(function() {
        ...
        // ================== SALE ORDER SEARCH FUNCTIONALITY ==================
        ...
        searchSaleOrderInput.on('input', function() {
            ...
            saleOrderSearchTimeout = setTimeout(function() {
                $.ajax({
                    url: 'ajax/search-purchase-orders.php',
                    type: 'GET',
                    data: { q: searchTerm },
                    dataType: 'json',
                    success: function(response) {
                        if (response.status === 'success' && response.orders && response.orders.length > 0) {
                            ...
                            saleOrderSuggestions.html('<div style="padding: 0.75rem; color: #94a3b8; text-align: center;">No purchase orders found</div>').show();
                        }
                    },
                    error: function() {
                        saleOrderSuggestions.hide();
...
<td data-column="sale-amount-with"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" readonly style="width: 130px; font-size: 0.7rem;"></td>
    ...
    function saveOrder() {
        ...
        if (typeof jQuery !== 'undefined' && jQuery.ajax) {
            jQuery.ajax({
                url: 'ajax/save-purchase-order.php',
                type: 'POST',
                data: postData,
                dataType: 'json',
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        if (invoiceId) {
                            ...
                            window.pendingRedirectUrl = 'purchase-order.php';
                            // Show print confirmation modal
                            setTimeout(function() {
                                showPrintOrderModal(invoiceId);
                            }, 100);
                        } else {
                            alert('Purchase Order saved successfully! Order No: ' + (response.invoice_no || response.order_no || 'N/A'));
                            // Redirect to fresh page
                            window.location.href = 'purchase-order.php';
                        }
                    } else {
                        alert('Error: ' + (response.message || 'Failed to save purchase order'));
                    }
                },
            ...
            fetch('ajax/save-purchase-order.php', {
                method: 'POST',
                body: formData
            })
            .then(response => response.json())
            .then(data => {
                ...
                if (data.status === 'success') {
                    ...
                    if (invoiceId) {
                        ...
                        alert('Purchase Order saved successfully! Order No: ' + (data.invoice_no || data.order_no || 'N/A'));
                        // Redirect to fresh page
                        window.location.href = 'purchase-order.php';
                    }
                } else {
    ...
    function printSaleOrder(invoiceId) {
        ...
        window.open('purchase-order-print.php?id=' + invoiceId, '_blank', 'width=1200,height=800');
    }
    ...
    function updatePrintIcon(invoiceId) {
        ...
        if (printIcon) {
            if (invoiceId && invoiceId > 0) {
                ...
                printIcon.setAttribute('title', 'Print Purchase Order');
            } else {
    ...
    function loadOrder(orderId) {
        ...
        if (typeof jQuery !== 'undefined' && jQuery.ajax) {
            jQuery.ajax({
                url: 'ajax/get-purchase-order.php?order_id=' + orderId,
                type: 'GET',
                dataType: 'json',
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        window.history.pushState({}, '', 'purchase-order.php?id=' + orderId);
            ...
            fetch('ajax/get-purchase-order.php?order_id=' + orderId)
                .then(response => response.json())
                .then(data => {
                    const msg = document.getElementById('orderLoadingMsg');
                    if (msg) document.body.removeChild(msg);
    ...
    function populateOrderForm(order, items, payments) {
        ...
        if (items && items.length > 0) {
            ...
            try {
                const modalRowsData = items.map(function(item) { return savedItemToModalRowData(item); });
...
<script>
    ...
    function showPrintOrderModal(invoiceId) {
        ...
        setTimeout(function() {
            ...
            if (modal) {
                ...
                if (confirm('Purchase Order saved successfully! Do you want to print the order?')) {
                    window.open('purchase-order-print.php?id=' + invoiceId, '_blank', 'width=1200,height=800');
                }
    ...
    function confirmPrintInvoice() {
        if (savedInvoiceId) {
            window.open('purchase-order-print.php?id=' + savedInvoiceId, '_blank', 'width=1200,height=800');
        }
