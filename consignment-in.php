...
$auragold_voucher_runtime_client = auragold_voucher_runtime_bootstrap($conn, $metals, 'Consignment IN');
...
// Next consignment no: Bill Series for voucher "Consignment In" / "Consignment IN" (tbl_bill_series + tbl_voucher_types); else legacy CI-1, CI-2
$next_order_no = function_exists('getNextConsignmentInNo') ? getNextConsignmentInNo($conn) : 'CI-1';

// Sales person dropdown — branch-scoped (stored in tbl_consignment_in.sales_person)
$sales_person_users = auragold_sales_person_user_display_names($conn_master);

// Load consignment for editing (tbl_consignment_in / tbl_consignment_in_items)
...
if (!empty($edit_order_id)) {
    $edit_order = getRecord("SELECT * FROM tbl_consignment_in WHERE id = " . intval($edit_order_id));
    if ($edit_order) {
        $edit_items = getList("
            ...
            FROM tbl_consignment_in_items i
                ...
                <div class="container-fluid flex-grow-1" style="padding-top: 0; padding-bottom: 0;">
                    ...
                    <div class="row invoice-content-row">
                        ...
                        <div class="col-lg-8" >
                                        ...
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label>Search Consignment IN</label>
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control form-control-sm" id="againstOf">
                                                    <option value="Consignment Out" selected>Consignment Out</option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="col-md-3">
                            ...
                            <!-- Payment UI hidden on Consignment IN; comments below stay visible -->
                            <div class="card mb-4">
                                <div class="card-body">
                                    <div id="consignmentInPaymentBlock" class="consignment-in-payment-hidden" style="display: none !important;" aria-hidden="true">
                            ...
                            <div class="summary-panel" >
                                ...
                                <div class="invoice-header" style="margin: -1rem -1rem 1rem -1rem; border-radius: 10px 10px 0 0; display: flex; align-items: center; justify-content: space-between;">
                                    <div style="display: flex; align-items: center; gap: 0.5rem; flex: 1;">
                                        <i class="feather icon-printer" id="printInvoiceIcon" style="font-size: 12px; color: #94a3b8; opacity: 0.5; cursor: not-allowed; pointer-events: none;" title="Save Consignment IN first to print"></i>
                                        <h5 class="mb-0" style="font-size: 0.9rem;">Consignment IN No: <span id="currentOrderNo"><?php echo htmlspecialchars($next_order_no); ?></span></h5>
                                    </div>
    ...
    $(document).ready(function() {
        ...
        saleInvoiceSearchInput.on('input', function() {
            ...
            saleInvoiceSearchTimeout = setTimeout(function() {
                $.ajax({
                    url: 'ajax/search-consignment-in.php',
                    type: 'GET',
                    data: { q: searchTerm },
                    dataType: 'json',
                    success: function(response) {
                        if (response.status === 'success' && response.invoices && response.invoices.length > 0) {
                            ...
                            saleInvoiceSuggestions.html('<div style="padding: 0.75rem; color: #94a3b8; text-align: center;">No Consignment IN documents found</div>').show();
                        }
                    },
                    error: function() {
                        saleInvoiceSuggestions.hide();
        ...
        $(document).on('click', '#saleInvoiceSuggestions .invoice-suggestion-item', function() {
            ...
            saleInvoiceSuggestions.hide();
            if (invoiceId > 0) {
                window.location.href = 'consignment-in.php?id=' + invoiceId;
            }
        });
...
<td data-column="sale-amount-with"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" readonly style="width: 130px; font-size: 0.7rem;"></td>
    ...
    function saveOrder() {
        ...
        const currentOrderNoText = document.getElementById('currentOrderNo')?.textContent || <?php echo json_encode(isset($next_order_no) ? $next_order_no : 'CI-1'); ?>;
        ...
        // Consignment IN master fields (maps to save-consignment-in.php / tbl_consignment_in)
        ...
        if (typeof jQuery !== 'undefined' && jQuery.ajax) {
            jQuery.ajax({
                url: 'ajax/save-consignment-in.php',
                type: 'POST',
                data: postData,
                dataType: 'json',
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        if (savedId) {
                            window.pendingRedirectUrl = 'consignment-in.php';
                            setTimeout(function() {
                                showPrintInvoiceModal(savedId);
                            }, 100);
                        } else {
                            alert('Consignment IN saved successfully! No.: ' + (response.consignment_no || response.order_no || 'N/A'));
                            window.location.href = 'consignment-in.php';
                        }
                    } else {
                        alert('Error: ' + (response.message || 'Failed to save Consignment IN'));
                    }
                },
            ...
            fetch('ajax/save-consignment-in.php', {
                method: 'POST',
                body: formData
            })
            .then(response => response.json())
            .then(data => {
                ...
                if (data.status === 'success') {
                    ...
                    if (sid) {
                        ...
                        alert('Consignment IN saved successfully! No.: ' + (data.consignment_no || data.order_no || 'N/A'));
                        window.location.href = 'consignment-in.php';
                    }
                } else {
                    alert('Error: ' + (data.message || 'Failed to save Consignment IN'));
                }
            })
            .catch(error => {
                // Reset saving flag
    ...
    // Reset order form
    function resetOrder() {
        if (confirm('Are you sure you want to create a new Consignment IN? All unsaved data will be lost.')) {
            window.location.href = 'consignment-in.php';
        }
    }
    ...
    function printPurchaseInvoice(invoiceId) {
        ...
        if (!invoiceId || invoiceId <= 0) {
            alert('Please save the Consignment IN first before printing.');
            return;
        }
        
        // Open print page in new window
        window.open('consignment-in-print.php?id=' + invoiceId, '_blank', 'width=1200,height=800');
    }
    ...
    function updatePrintIcon(invoiceId) {
        ...
        if (printIcon) {
            if (invoiceId && invoiceId > 0) {
                ...
                printIcon.setAttribute('title', 'Print Consignment IN');
            } else {
                ...
                printIcon.setAttribute('title', 'Save Consignment IN first to print');
            }
        }
    }
    ...
    function loadOrder(orderId) {
        ...
        var getUrl = 'ajax/get-consignment-in.php?id=' + orderId + '&_=' + (Date.now ? Date.now() : '');
        if (typeof jQuery !== 'undefined' && jQuery.ajax) {
            jQuery.ajax({
                url: getUrl,
                type: 'GET',
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        window.history.pushState({}, '', 'consignment-in.php?id=' + orderId);
    ...
    function populateOrderForm(order, items, loadedPayments) {
        ...
        if (document.getElementById('againstOf')) {
            document.getElementById('againstOf').value = 'Consignment Out';
        }
        ...
        var _pfClearCi = document.getElementById('paymentTableFooter');
        if (_pfClearCi) _pfClearCi.style.display = 'none';
        ...
        if (loadedPayments && loadedPayments.length > 0) {
            loadedPayments.forEach(function(paymentRow) {
                ...
                var _prevBalLoadCi = parseFloat(payment.previous_balance_amount || 0) || 0;
                var _dbTotalLoadCi = parseFloat(payment.amount || 0) || 0;
                var _dbCurLoadCi = payment.current_order_amount != null && payment.current_order_amount !== '' ? parseFloat(payment.current_order_amount) : NaN;
                var _curOrderLoadCi = !isNaN(_dbCurLoadCi) ? _dbCurLoadCi : Math.max(0, _dbTotalLoadCi - _prevBalLoadCi);
                const paymentData = {
                    ...
                    amount: _curOrderLoadCi,
                    previous_balance_amount: _prevBalLoadCi,
                ...
                if (paymentType === 'metal-exchange') {
                    ...
                    var meGrossCi = payment.metal_exchange_gross_wt != null && payment.metal_exchange_gross_wt !== '' ? String(payment.metal_exchange_gross_wt) : (payment.gross_weight != null && payment.gross_weight !== '' ? String(payment.gross_weight) : '');
                    if (!meGrossCi || meGrossCi === '0') {
                        var qMeCi = parseFloat(payment.quantity || 0);
                        if (qMeCi > 0) meGrossCi = String(qMeCi);
                    }
                    paymentData.metal_exchange_gross_wt = meGrossCi || '0';
                    ...
                    var mePureCi = payment.metal_exchange_purity_wt != null && payment.metal_exchange_purity_wt !== '' ? String(payment.metal_exchange_purity_wt) : (payment.purity_weight != null && payment.purity_weight !== '' ? String(payment.purity_weight) : '');
                    if (!mePureCi || mePureCi === '0') {
                        var gNumCi = parseFloat(paymentData.metal_exchange_gross_wt) || 0;
                        var pCaratCi = parseFloat(payment.purity_carat || payment.purity || 0) || 0;
                        if (gNumCi > 0 && pCaratCi > 0) {
                            if (pCaratCi <= 1) mePureCi = String(gNumCi * pCaratCi);
                            else if (pCaratCi <= 100) mePureCi = String(gNumCi * (pCaratCi / 100));
                            else mePureCi = String(gNumCi * (pCaratCi / 1000));
                        }
                    }
                    paymentData.metal_exchange_purity_wt = mePureCi || '0';
                }
            ...
            var _pfEmptyCi = document.getElementById('paymentTableFooter');
            if (_pfEmptyCi) _pfEmptyCi.style.display = 'none';
        }
    ...
    (function() {
        ...
        function doPopulateFromEmbed() {
            ...
            try {
                ...
                if (window.history && window.history.replaceState) window.history.replaceState({}, '', 'consignment-in.php?id=' + editId);
                window.isPurchaseInvoiceEditMode = true;
                return true;
            } catch (e) {
...
<!-- Print Consignment IN confirmation modal -->
<div id="printInvoiceModal" class="print-invoice-modal" style="display: none;">
    <div class="print-invoice-modal-content">
        ...
        <h3 class="print-invoice-modal-title">Print Consignment IN</h3>
        <p class="print-invoice-modal-message">Do you want to print this Consignment IN document?</p>
...
<script>
    ...
    function showPrintInvoiceModal(invoiceId) {
        ...
        setTimeout(function() {
            ...
            if (modal) {
                ...
                if (confirm('Consignment IN saved successfully! Do you want to print?')) {
                    window.open('consignment-in-print.php?id=' + invoiceId, '_blank', 'width=1200,height=800');
                }
    ...
    function confirmPrintInvoice() {
        if (savedInvoiceId) {
            window.open('consignment-in-print.php?id=' + savedInvoiceId, '_blank', 'width=1200,height=800');
        }
