<?php
/**
 * Diamond & Stone — serial stock list with Jewellery / Diamonds / GemStones tabs.
 * Scoped to tbl_metal "Diamond & Stones" and pc.diamond_category per tab.
 */
session_start();
require_once __DIR__ . '/config.php';

require_once __DIR__ . '/includes/diamond_and_stones_stock_columns.php';
require_once __DIR__ . '/includes/diamond_and_stones_stock_row_map.php';
...
require_once __DIR__ . '/includes/diamond_and_stones_stock_list_sql_include.php';

$dass_tab = isset($dass_tab) ? $dass_tab : dass_stock_normalize_tab($_GET['tab'] ?? 'jewellery');
$sql = $dass_stock_list_sql . ' LIMIT 5000';

if (!isset($dass_adv_branch)) {
    $dass_adv_branch = 0;
}
if (!isset($dass_adv_category)) {
    $dass_adv_category = 0;
}
if (!isset($dass_adv_barcode)) {
    $dass_adv_barcode = '';
}
if (!isset($dass_adv_rfid)) {
    $dass_adv_rfid = '';
}
if (!isset($dass_adv_product)) {
    $dass_adv_product = '';
}
if (!isset($dass_adv_filter_count)) {
    $dass_adv_filter_count = 0;
}
if (!isset($dass_filter_branches) || !is_array($dass_filter_branches)) {
    $dass_filter_branches = [];
}
if (!isset($dass_filter_categories) || !is_array($dass_filter_categories)) {
    $dass_filter_categories = [];
}

$q_base = ['tab' => $dass_tab];
if ($dass_adv_branch > 0) {
    $q_base['adv_branch'] = $dass_adv_branch;
}
if ($dass_adv_category > 0) {
    $q_base['adv_category'] = $dass_adv_category;
}
if ($dass_adv_barcode !== '') {
    $q_base['adv_barcode'] = $dass_adv_barcode;
}
if ($dass_adv_rfid !== '') {
    $q_base['adv_rfid'] = $dass_adv_rfid;
}
if ($dass_adv_product !== '') {
    $q_base['adv_product'] = $dass_adv_product;
}
$href_jewellery = 'diamond-and-stones.php?' . http_build_query(array_merge($q_base, ['tab' => 'jewellery']));
$href_diamond = 'diamond-and-stones.php?' . http_build_query(array_merge($q_base, ['tab' => 'diamond']));
$href_gemstone = 'diamond-and-stones.php?' . http_build_query(array_merge($q_base, ['tab' => 'gemstone']));
$href_clear_filter = 'diamond-and-stones.php?' . http_build_query(['tab' => $dass_tab]);
...
$gas_columns = dass_stock_columns_for_tab($dass_tab);
$gas_column_group_defs = dass_stock_column_group_defs($gas_columns);
...
$gas_width_map = [];
if ($gas_user_id > 0) {
    $gas_tab_esc = mysqli_real_escape_string($conn, $dass_tab);
    ...
    $gas_pref_rows = getList(
        "SELECT column_key, column_order, is_visible$wsel
         ...
         WHERE user_id = $gas_user_id AND page_name = 'diamond-and-stones' AND tab_key = '$gas_tab_esc'
...
$gas_grand = array_fill_keys(dass_stock_numeric_total_keys(), 0.0);

$dass_can_sale_invoice = !function_exists('auragold_nav_show_php_href') || auragold_nav_show_php_href('sale-invoice.php');
$dass_can_sale_enquiry = !function_exists('auragold_nav_show_php_href') || auragold_nav_show_php_href('sale-quotations.php');
$dass_can_sale_order = !function_exists('auragold_nav_show_php_href') || auragold_nav_show_php_href('sale-order.php');

?>
<!DOCTYPE html>
<html lang="en" class="default-style">
<head>
    <title>Diamond &amp; Stone - <?php echo htmlspecialchars(auragold_app_name(), ENT_QUOTES, 'UTF-8'); ?></title>
    <meta charset="utf-8">
...
<style>
    ...
    .gas-create-btn {
        ...
        background: linear-gradient(180deg, #11294b 0%, #0d2039 100%);
        color: #fff;
        font-size: 0.85rem;
        font-weight: 600;
        cursor: pointer;
    }
    .gas-create-btn:hover,
    .gas-create-btn:focus {
        color: #fff;
        background: linear-gradient(180deg, #1a3a5c 0%, #11294b 100%);
    }
    .gas-create-btn .badge {
        background: #c9a962;
        color: #11294b;
        font-size: 0.7rem;
        font-weight: 700;
    ...
    .gas-create-menu .dropdown-item {
        font-size: 0.85rem;
        color: #11294b;
    }
    .gas-create-menu .dropdown-item.disabled {
        opacity: 0.45;
        pointer-events: none;
    }
    .gas-th-select,
    ...
    .gas-barcode-search-icon i {
        width: 15px;
        height: 15px;
    }
    .gas-barcode-search input {
        width: 100%;
        height: 34px;
    ...
    .gas-barcode-search input::placeholder {
        color: #a89a7c;
    }
    .gas-barcode-search input:focus {
        border-color: #c9a962;
        box-shadow: 0 0 0 2px rgba(201, 169, 98, 0.22);
    }
    .gas-table tbody tr.gas-barcode-filter-hide {
    ...
    .gas-adv-filter-modal .modal-content {
        border: none;
        border-radius: 12px;
        overflow: hidden;
        box-shadow: 0 12px 40px rgba(17, 41, 75, 0.2);
    }
    .gas-adv-filter-modal .modal-header {
        background: linear-gradient(180deg, #fdf8f0 0%, #f0e6d4 100%);
        border-bottom: 2px solid #c9a962;
        padding: 14px 20px;
    }
    .gas-adv-filter-modal .modal-title {
        width: 100%;
        text-align: center;
        font-weight: 700;
        font-size: 1rem;
    ...
    .gas-adv-filter-modal .close:hover { opacity: 1; }
    .gas-adv-filter-modal .modal-body { padding: 20px 24px 24px; }
    .gas-adv-filter-modal .form-group label {
        font-weight: 600;
        font-size: 0.8rem;
        color: #11294b;
        margin-bottom: 6px;
    }
    .gas-adv-filter-modal .form-control {
        border-radius: 8px;
        border-color: #cfd8e3;
        font-size: 0.875rem;
    }
    .gas-adv-filter-modal .form-control:focus {
        border-color: #c9a962;
        box-shadow: 0 0 0 0.15rem rgba(201, 169, 98, 0.25);
    }
    .gas-adv-filter-modal .modal-footer-adv {
        display: flex;
        justify-content: center;
        gap: 12px;
        padding: 0 24px 22px;
    ...
    .gas-adv-filter-modal .btn-adv-apply {
        min-width: 120px;
        border-radius: 8px;
        font-weight: 600;
        background: #11294b;
    ...
    .gas-adv-filter-modal .btn-adv-apply:hover {
        background: #0d2039;
        border-color: #0d2039;
        color: #fff;
    }
    .gas-adv-filter-modal .btn-adv-clear {
        min-width: 120px;
        border-radius: 8px;
        font-weight: 600;
        background: #fff;
    ...
    .gas-adv-filter-modal .btn-adv-clear:hover {
        background: #f8fafc;
        border-color: #c9a962;
        color: #11294b;
        text-decoration: none;
    ...
    .dass-row-view-btn { padding: 2px 8px; line-height: 1; }
    .dass-row-view-btn .feather { font-size: 0.95rem; vertical-align: middle; }
    .dass-row-view-btn:hover { background: #1a2b4b !important; color: #fff !important; border-color: #1a2b4b !important; }
    #dassDetailModal { display: none; position: fixed; inset: 0; z-index: 10060; background: rgba(15, 23, 42, 0.55); align-items: center; justify-content: center; padding: 16px; }
    #dassDetailModal.dass-detail-show { display: flex; }
    #dassDetailModal .dass-detail-box { background: #fff; border-radius: 10px; box-shadow: 0 20px 50px rgba(0,0,0,0.28); width: min(920px, 96vw); max-height: 90vh; display: flex; flex-direction: column; overflow: hidden; }
    #dassDetailModal .dass-detail-header { background: linear-gradient(135deg, #1a2b4b 0%, #243656 100%); color: #fff; padding: 14px 18px; display: flex; align-items: flex-start; justify-content: space-between; gap: 12px; }
    #dassDetailModal .dass-detail-header h5 { margin: 0 0 4px; font-size: 1.05rem; font-weight: 600; }
    #dassDetailModal .dass-detail-sub { font-size: 0.78rem; opacity: 0.88; line-height: 1.45; }
    #dassDetailModal .dass-detail-close { background: transparent; border: none; color: #fff; font-size: 1.6rem; line-height: 1; cursor: pointer; padding: 0 4px; opacity: 0.9; }
    #dassDetailModal .dass-detail-close:hover { opacity: 1; }
    #dassDetailModal .dass-detail-body { flex: 1; overflow: auto; padding: 16px 18px; background: #f8fafc; }
    #dassDetailModal .dass-detail-photo { text-align: center; margin-bottom: 14px; }
    #dassDetailModal .dass-detail-photo img { max-width: 140px; max-height: 140px; object-fit: contain; border-radius: 8px; border: 1px solid #e2e8f0; background: #fff; padding: 4px; }
    #dassDetailModal .dass-detail-section { background: #fff; border: 1px solid #e2e8f0; border-radius: 8px; margin-bottom: 12px; overflow: hidden; }
    #dassDetailModal .dass-detail-section-title { margin: 0; padding: 10px 14px; background: #f1f5f9; border-bottom: 1px solid #e2e8f0; font-size: 0.82rem; font-weight: 700; color: #1a2b4b; text-transform: uppercase; letter-spacing: 0.03em; }
    #dassDetailModal .dass-detail-line { padding: 12px 14px; border-top: 1px dashed #e2e8f0; }
    #dassDetailModal .dass-detail-line:first-of-type { border-top: none; }
    #dassDetailModal .dass-detail-line-label { font-size: 0.72rem; font-weight: 600; color: #64748b; margin-bottom: 8px; }
    #dassDetailModal .dass-detail-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 8px 14px; }
    #dassDetailModal .dass-detail-field label { display: block; font-size: 0.68rem; font-weight: 600; color: #64748b; margin-bottom: 2px; }
    #dassDetailModal .dass-detail-field span { display: block; font-size: 0.8rem; color: #0f172a; word-break: break-word; }
    #dassDetailModal .dass-detail-empty { text-align: center; color: #64748b; padding: 28px 12px; font-size: 0.88rem; }
    #dassDetailModal .dass-detail-loading { text-align: center; color: #64748b; padding: 28px 12px; font-size: 0.88rem; }
    #dassDetailModal .dass-detail-footer { padding: 12px 18px; border-top: 1px solid #e2e8f0; background: #fff; display: flex; justify-content: flex-end; }
    #dassDetailModal .dass-detail-footer button { padding: 8px 18px; border-radius: 6px; border: none; background: #c4b896; color: #fff; font-size: 0.82rem; font-weight: 600; cursor: pointer; }
    #dassDetailModal .dass-detail-footer button:hover { background: #b3a385; }
...
<div class="layout-wrapper layout-2">
        ...
        <div class="layout-container">
            ...
            <div class="layout-content">
                <div class="container-fluid flex-grow-1" style="padding-top: 0;">
                    ...
                    <div class="gas-wrap">
                        <div class="gas-head">
                            <div class="gas-head-main">
                                <div>
                                    <h1 class="gas-title">Diamond &amp; Stone</h1>
                                    <nav class="gas-tabs mt-2" aria-label="Stock category">
                                        <a href="<?php echo htmlspecialchars($href_jewellery, ENT_QUOTES, 'UTF-8'); ?>" class="<?php echo $dass_tab === 'jewellery' ? 'active' : ''; ?>">Jewellery</a>
                                        <a href="<?php echo htmlspecialchars($href_diamond, ENT_QUOTES, 'UTF-8'); ?>" class="<?php echo $dass_tab === 'diamond' ? 'active' : ''; ?>">Diamonds</a>
                                        <a href="<?php echo htmlspecialchars($href_gemstone, ENT_QUOTES, 'UTF-8'); ?>" class="<?php echo $dass_tab === 'gemstone' ? 'active' : ''; ?>">GemStones</a>
                                    </nav>
                                </div>
                                <div class="gas-create-wrap mt-2" id="gasCreateWrap" aria-hidden="true">
                                    <div class="dropdown">
                                        <button type="button" class="gas-create-btn dropdown-toggle" id="gasCreateBtn" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                            Create <span class="badge" id="gasCreateCount">0</span>
                                        </button>
                                        <div class="dropdown-menu gas-create-menu" aria-labelledby="gasCreateBtn">
                                            <a class="dropdown-item<?php echo $dass_can_sale_invoice ? '' : ' disabled'; ?>" href="#" id="gasCreateSaleInvoice" role="button" <?php echo $dass_can_sale_invoice ? '' : ' aria-disabled="true" tabindex="-1"'; ?>>
                                                <i class="feather icon-file-text mr-1"></i> Create Sale Invoice
                                            </a>
                                            <a class="dropdown-item<?php echo $dass_can_sale_enquiry ? '' : ' disabled'; ?>" href="#" id="gasCreateSaleEnquiry" role="button" <?php echo $dass_can_sale_enquiry ? '' : ' aria-disabled="true" tabindex="-1"'; ?>>
                                                <i class="feather icon-file mr-1"></i> Create Sale Enquiry
                                            </a>
                                            <a class="dropdown-item<?php echo $dass_can_sale_order ? '' : ' disabled'; ?>" href="#" id="gasCreateSaleOrder" role="button" <?php echo $dass_can_sale_order ? '' : ' aria-disabled="true" tabindex="-1"'; ?>>
                                                <i class="feather icon-shopping-bag mr-1"></i> Create Sale Order
                                            </a>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="gas-table-card">
                            <div class="gas-table-card-header">
                                <h2>Stock list</h2>
                                <div class="gas-barcode-search">
                                    <span class="gas-barcode-search-icon" aria-hidden="true"><i class="feather icon-search"></i></span>
                                    <input type="text" id="gasBarcodeSearch" placeholder="Search barcode…" autocomplete="off" aria-label="Search barcode">
                                </div>
                                <div class="gas-table-card-actions">
                                    <button type="button" class="gas-filter-btn" title="Advance Filter" data-toggle="modal" data-target="#gasAdvFilterModal">
                                        <i class="feather icon-filter" style="width:20px;height:20px;"></i>
                                        <?php if ((int) $dass_adv_filter_count > 0) { ?>
                                        <span class="gas-filter-badge"><?php echo (int) $dass_adv_filter_count; ?></span>
                                        <?php } ?>
                                    </button>
    ...
    ?>
                            ...
                            <div class="gas-table-scroll">
                                <table class="table table-sm table-bordered gas-table" id="gasStockTable" data-gas-visible-count="<?php echo (int) max(1, $gas_visible_count + 1); ?>">
                                    <thead>
                                        <tr class="gas-thead-group">
                                            <th class="gas-th-group gas-th-select" scope="colgroup" colspan="1"></th>
<?php
...
?>
                                        ...
                                        <tr class="gas-thead-cols">
                                            <th class="gas-th-select" scope="col">
                                                <input type="checkbox" id="gasSelectAll" title="Select all" aria-label="Select all products">
                                            </th>
<?php
foreach ($gas_columns as $ck => $clab) {
    $gm = $gas_col_meta[$ck];
...
foreach ($rows as $r) {
    ...
    dass_accumulate_grand($gas_grand, $r);

    $gas_row_cells = dass_build_row_cells($r, $gas_columns, [
        'img_cell' => $img_cell,
        'barcoded' => (string) $barcoded,
    ]);
    $gas_row_product_id = (int) ($r['product_id'] ?? 0);
    $gas_row_stock_id = (int) ($r['stock_id'] ?? 0);
    $gas_row_qty = isset($r['current_qty']) ? (float) $r['current_qty'] : 0.0;
    $gas_row_wt = isset($r['current_weight']) ? (float) $r['current_weight'] : 0.0;
    $gas_row_pname = trim((string) ($r['product_name'] ?? ''));
    echo '<tr data-gas-barcode="' . htmlspecialchars($gas_row_barcode, ENT_QUOTES, 'UTF-8') . '"'
        . ' data-gas-product-id="' . $gas_row_product_id . '"'
        . ' data-gas-stock-id="' . $gas_row_stock_id . '"'
        . ' data-gas-qty="' . htmlspecialchars((string) $gas_row_qty, ENT_QUOTES, 'UTF-8') . '"'
        . ' data-gas-weight="' . htmlspecialchars((string) $gas_row_wt, ENT_QUOTES, 'UTF-8') . '"'
        . ' data-gas-product-name="' . htmlspecialchars($gas_row_pname, ENT_QUOTES, 'UTF-8') . '">';
    echo '<td class="gas-td-select">';
    if ($gas_row_barcode !== '') {
        echo '<input type="checkbox" class="gas-row-check" value="' . htmlspecialchars($gas_row_barcode, ENT_QUOTES, 'UTF-8') . '" aria-label="Select ' . htmlspecialchars($gas_row_barcode, ENT_QUOTES, 'UTF-8') . '">';
    }
    echo '</td>';
...
if (count($rows) === 0 && $load_error === '') {
    echo '<tr><td colspan="' . (int) max(2, $gas_visible_count + 1) . '" class="text-center text-muted py-4 gas-empty-row" data-gas-empty="1">No rows to show.</td></tr>';
}
echo '<tr id="gasBarcodeEmptyRow" hidden><td colspan="' . (int) max(2, $gas_visible_count + 1) . '" class="text-center text-muted py-4 gas-empty-row" data-gas-empty="barcode">No barcode matches.</td></tr>';

?>
                                    </tbody>
                                    <tfoot class="gas-tfoot-totals">
                                        <tr>
                                            <td class="gas-tfoot-select"></td>
<?php
foreach ($gas_columns as $ck => $clab) {
    $foot = dass_footer_cell($ck, $gas_grand, $gas_columns);
    $cls = 'gas-tfoot-cell ' . $foot['cls'];
    $inner = $foot['inner'];
    $gm = $gas_col_meta[$ck];
    $hcls = !empty($gm['hidden']) ? ' gas-col-hidden' : '';
    $wsty = ($gm['width'] !== null) ? ' style="min-width:' . (int) $gm['width'] . 'px;width:' . (int) $gm['width'] . 'px;max-width:560px;"' : '';
    if ($ck === 'action' || $ck === 'imageUrls') {
        echo '<td data-gas-col="' . htmlspecialchars($ck, ENT_QUOTES, 'UTF-8') . '" class="' . htmlspecialchars(trim($cls . $hcls), ENT_QUOTES, 'UTF-8') . '"' . $wsty . '>' . ($ck === 'imageUrls' ? htmlspecialchars($inner, ENT_QUOTES, 'UTF-8') : '') . '</td>';
    } else {
        echo '<td data-gas-col="' . htmlspecialchars($ck, ENT_QUOTES, 'UTF-8') . '" class="' . htmlspecialchars(trim($cls . $hcls), ENT_QUOTES, 'UTF-8') . '"' . $wsty . '>' . htmlspecialchars($inner, ENT_QUOTES, 'UTF-8') . '</td>';
    }
}
?>
                                        </tr>
...
<div class="modal fade gas-adv-filter-modal" id="gasAdvFilterModal" tabindex="-1" role="dialog" aria-labelledby="gasAdvFilterModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document" style="max-width: 420px;">
        <div class="modal-content">
            <div class="modal-header position-relative">
                <h5 class="modal-title" id="gasAdvFilterModalLabel">Advance Filter</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form method="get" action="diamond-and-stones.php" id="gasAdvFilterForm">
                <input type="hidden" name="tab" value="<?php echo htmlspecialchars($dass_tab, ENT_QUOTES, 'UTF-8'); ?>">
                <div class="modal-body">
                    <div class="form-group">
                        <label for="gas_adv_branch">Branch</label>
                        <select class="form-control" id="gas_adv_branch" name="adv_branch">
                            <option value="0" <?php echo ((int) $dass_adv_branch <= 0) ? 'selected' : ''; ?>>All Branches</option>
                            <?php foreach ($dass_filter_branches as $fb) { ?>
                                <option value="<?php echo (int) $fb['id']; ?>" <?php echo ((int) $dass_adv_branch === (int) $fb['id']) ? 'selected' : ''; ?>><?php echo htmlspecialchars((string) ($fb['name'] ?? ''), ENT_QUOTES, 'UTF-8'); ?></option>
                            <?php } ?>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="gas_adv_category">Category</label>
                        <select class="form-control" id="gas_adv_category" name="adv_category">
                            <option value="0" <?php echo ((int) $dass_adv_category <= 0) ? 'selected' : ''; ?>>Select Category</option>
                            <?php foreach ($dass_filter_categories as $fc) { ?>
                                <option value="<?php echo (int) $fc['id']; ?>" <?php echo ((int) $dass_adv_category === (int) $fc['id']) ? 'selected' : ''; ?>><?php echo htmlspecialchars((string) ($fc['name'] ?? ''), ENT_QUOTES, 'UTF-8'); ?></option>
                            <?php } ?>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="gas_adv_product">Product / Article / SKU</label>
                        <input type="text" class="form-control" id="gas_adv_product" name="adv_product" value="<?php echo htmlspecialchars($dass_adv_product, ENT_QUOTES, 'UTF-8'); ?>" placeholder="Search product…" autocomplete="off">
                    </div>
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            <label for="gas_adv_barcode">Barcode No.</label>
                            <input type="text" class="form-control" id="gas_adv_barcode" name="adv_barcode" value="<?php echo htmlspecialchars($dass_adv_barcode, ENT_QUOTES, 'UTF-8'); ?>" placeholder="" autocomplete="off">
                        </div>
                        <div class="form-group col-md-6">
                            <label for="gas_adv_rfid">RFID Code</label>
                            <input type="text" class="form-control" id="gas_adv_rfid" name="adv_rfid" value="<?php echo htmlspecialchars($dass_adv_rfid, ENT_QUOTES, 'UTF-8'); ?>" placeholder="" autocomplete="off">
                        </div>
                    </div>
                </div>
                <div class="modal-footer-adv">
                    <button type="submit" class="btn btn-adv-apply">Apply Filter</button>
                    <a href="<?php echo htmlspecialchars($href_clear_filter, ENT_QUOTES, 'UTF-8'); ?>" class="btn btn-adv-clear">Clear Filter</a>
                </div>
            </form>
        </div>
    </div>
...
<script>
(function () {
    var STORAGE_KEY = 'auragold_dass_stock_items';
    var selectAll = document.getElementById('gasSelectAll');
    var createWrap = document.getElementById('gasCreateWrap');
    var createCount = document.getElementById('gasCreateCount');
    var tbl = document.getElementById('gasStockTable');

    function rowChecks() {
        return tbl ? Array.prototype.slice.call(tbl.querySelectorAll('tbody tr:not(.gas-barcode-filter-hide) .gas-row-check')) : [];
    }
    ...
    function syncCreateButton() {
        ...
        if (createCount) createCount.textContent = String(n);
        if (createWrap) {
            if (n > 0) {
                createWrap.classList.add('is-visible');
                createWrap.setAttribute('aria-hidden', 'false');
            } else {
                createWrap.classList.remove('is-visible');
    ...
    function buildPayload() {
        return selectedRows().map(function (cb) {
            var tr = cb.closest('tr');
            return {
                barcode: (cb.value || (tr && tr.getAttribute('data-gas-barcode')) || '').trim(),
                product_id: tr ? (parseInt(tr.getAttribute('data-gas-product-id'), 10) || 0) : 0,
                stock_id: tr ? (parseInt(tr.getAttribute('data-gas-stock-id'), 10) || 0) : 0,
                current_qty: tr ? (parseFloat(tr.getAttribute('data-gas-qty')) || 0) : 0,
                current_weight: tr ? (parseFloat(tr.getAttribute('data-gas-weight')) || 0) : 0,
                product_name: tr ? (tr.getAttribute('data-gas-product-name') || '') : ''
            };
        }).filter(function (it) { return it.barcode !== ''; });
    }

    function saveAndGo(url, canAccess) {
        if (!canAccess) return;
    ...
    if (tbl) {
        tbl.addEventListener('change', function (e) {
            var t = e.target;
            if (t && t.classList && t.classList.contains('gas-row-check')) {
                syncCreateButton();
            }
        });
    }

    var barcodeSearch = document.getElementById('gasBarcodeSearch');
    var barcodeEmptyRow = document.getElementById('gasBarcodeEmptyRow');
    function applyBarcodeFilter() {
        if (!tbl) return;
        var q = barcodeSearch ? String(barcodeSearch.value || '').trim().toLowerCase() : '';
        var rows = tbl.querySelectorAll('tbody tr[data-gas-barcode]');
        var shown = 0;
        for (var i = 0; i < rows.length; i++) {
            var bc = String(rows[i].getAttribute('data-gas-barcode') || '').toLowerCase();
            var match = q === '' || bc.indexOf(q) !== -1;
            rows[i].classList.toggle('gas-barcode-filter-hide', !match);
            if (match) shown++;
        }
        if (barcodeEmptyRow) {
            barcodeEmptyRow.hidden = shown > 0 || rows.length === 0;
        }
        syncCreateButton();
    }
    if (barcodeSearch) {
        barcodeSearch.addEventListener('input', applyBarcodeFilter);
    ...
    var btnInvoice = document.getElementById('gasCreateSaleInvoice');
    var btnEnquiry = document.getElementById('gasCreateSaleEnquiry');
    var btnOrder = document.getElementById('gasCreateSaleOrder');
    if (btnInvoice) {
        btnInvoice.addEventListener('click', function (e) {
            e.preventDefault();
            saveAndGo('sale-invoice.php?from_diamond_stones=1', <?php echo $dass_can_sale_invoice ? 'true' : 'false'; ?>);
        });
    }
    if (btnEnquiry) {
        btnEnquiry.addEventListener('click', function (e) {
            e.preventDefault();
            saveAndGo('sale-quotations.php?from_diamond_stones=1', <?php echo $dass_can_sale_enquiry ? 'true' : 'false'; ?>);
        });
    }
    if (btnOrder) {
        btnOrder.addEventListener('click', function (e) {
            e.preventDefault();
            saveAndGo('sale-order.php?from_diamond_stones=1', <?php echo $dass_can_sale_order ? 'true' : 'false'; ?>);
        });
    }
    syncCreateButton();
})();
...
(function () {
    ...
    btn.addEventListener('click', function () {
        window.location.href = 'ajax/export-diamond-stock-excel.php?tab=' + encodeURIComponent(<?php echo json_encode($dass_tab, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT); ?>);
    });
})();
</script>
<script>
...
(function () {
    ...
    function gasVisibleColCount() {
        return 1 + tbl.querySelectorAll('thead th[data-gas-col]:not(.gas-col-hidden)').length;
    }

    function gasSyncEmptyColspan() {
        var n = Math.max(1, gasVisibleColCount());
        tbl.querySelectorAll('td[data-gas-empty]').forEach(function (empty) {
            empty.colSpan = n;
        });
        tbl.setAttribute('data-gas-visible-count', String(n));
    }
    ...
    function gasSaveState() {
        ...
        var fd = new FormData();
        fd.append('page_name', prefs.pageName || 'diamond-and-stones');
    ...
    function gasReflowGroupRow() {
        ...
        sel.className = 'gas-th-group gas-th-select';
        sel.setAttribute('scope', 'colgroup');
        sel.setAttribute('colspan', '1');
...
<?php endif; ?>
<div id="dassDetailModal" aria-hidden="true">
    <div class="dass-detail-box" role="dialog" aria-labelledby="dassDetailTitle" aria-modal="true">
        <div class="dass-detail-header">
            <div>
                <h5 id="dassDetailTitle">Item Details</h5>
                <div class="dass-detail-sub" id="dassDetailSubtitle"></div>
            </div>
            <button type="button" class="dass-detail-close" id="dassDetailClose" title="Close">&times;</button>
        </div>
        <div class="dass-detail-body" id="dassDetailBody">
            <div class="dass-detail-loading">Loading…</div>
        </div>
        <div class="dass-detail-footer">
            <button type="button" id="dassDetailCloseBtn">Close</button>
        </div>
    </div>
</div>
<script>
window.DASS_DETAIL_CFG = <?php echo json_encode([
    'url' => 'ajax/dass-barcode-composite-details.php',
    'noImageSrc' => $gas_no_image_src,
], JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT); ?>;
</script>
<script>
(function () {
    var modal = document.getElementById('dassDetailModal');
    var bodyEl = document.getElementById('dassDetailBody');
    var titleEl = document.getElementById('dassDetailTitle');
    var subEl = document.getElementById('dassDetailSubtitle');
    var cfg = window.DASS_DETAIL_CFG || {};
    ...
    function hideModal() {
        modal.classList.remove('dass-detail-show');
        modal.setAttribute('aria-hidden', 'true');
    }

    function showModal() {
        modal.classList.add('dass-detail-show');
        modal.setAttribute('aria-hidden', 'false');
    }

    function renderFields(fields) {
        if (!fields || !fields.length) return '';
        var html = '<div class="dass-detail-grid">';
        fields.forEach(function (f) {
            html += '<div class="dass-detail-field"><label>' + esc(f.label || f.key || '') + '</label><span>' + esc(f.value || '') + '</span></div>';
        });
        html += '</div>';
        return html;
    }
    ...
    function renderPayload(data) {
        ...
        if (data.image_urls && data.image_urls.length) {
            html += '<div class="dass-detail-photo"><img src="' + esc(data.image_urls[0]) + '" alt="" onerror="this.onerror=null;this.src=' + JSON.stringify(cfg.noImageSrc || 'no_image.jpg') + ';"></div>';
        }
        var sections = data.sections || [];
        if (!sections.length) {
            html += '<div class="dass-detail-empty">No details found for this barcode.</div>';
            return html;
        }
        sections.forEach(function (sec) {
            var cat = sec.category || 'Details';
            html += '<div class="dass-detail-section">';
            html += '<h6 class="dass-detail-section-title">' + esc(cat) + '</h6>';
            var lines = sec.lines || [];
            if (!lines.length) {
                html += '<div class="dass-detail-empty">No lines in this section.</div>';
            } else {
                lines.forEach(function (line, idx) {
                    html += '<div class="dass-detail-line">';
                    if (lines.length > 1) {
                        html += '<div class="dass-detail-line-label">Line ' + (line.line_index || (idx + 1)) + '</div>';
                    }
                    html += renderFields(line.fields || []);
                    html += '</div>';
                });
            }
            html += '</div>';
    ...
    function openDetail(barcode) {
        ...
        if (!barcode) return;
        titleEl.textContent = 'Item Details — ' + barcode;
        subEl.textContent = '';
        bodyEl.innerHTML = '<div class="dass-detail-loading">Loading…</div>';
        showModal();

        var url = (cfg.url || 'ajax/dass-barcode-composite-details.php') + '?barcode=' + encodeURIComponent(barcode);
        fetch(url, { credentials: 'same-origin' })
            .then(function (r) { return r.json(); })
            .then(function (data) {
                if (!data || data.status !== 'success') {
                    bodyEl.innerHTML = '<div class="dass-detail-empty">' + esc((data && data.message) ? data.message : 'Could not load details.') + '</div>';
                    return;
                }
                var bits = [];
                if (data.invoice_no) bits.push('Invoice: ' + data.invoice_no);
                if (data.invoice_date) bits.push('Date: ' + String(data.invoice_date).substring(0, 10));
                if (data.supplier_name) bits.push('Supplier: ' + data.supplier_name);
                subEl.textContent = bits.join(' · ');
                bodyEl.innerHTML = renderPayload(data);
            })
            .catch(function () {
                bodyEl.innerHTML = '<div class="dass-detail-empty">Could not load details (network error).</div>';
            });
    }

    document.addEventListener('click', function (e) {
        var btn = e.target.closest('.dass-row-view-btn');
        if (!btn) return;
        e.preventDefault();
        e.stopPropagation();
        var bc = btn.getAttribute('data-barcode') || '';
        if (!bc) {
            var tr = btn.closest('tr[data-gas-barcode]');
            if (tr) bc = tr.getAttribute('data-gas-barcode') || '';
        }
        openDetail(bc);
    });

    var closeBtn = document.getElementById('dassDetailClose');
    var closeBtn2 = document.getElementById('dassDetailCloseBtn');
    ...
    document.addEventListener('keydown', function (e) {
        if (e.key === 'Escape' && modal.classList.contains('dass-detail-show')) hideModal();
    });
})();
</script>
</body>
