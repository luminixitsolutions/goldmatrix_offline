...
$auragold_voucher_runtime_client = auragold_voucher_runtime_bootstrap($conn, $metals, 'Jobwork Invoice');
...
// Jobwork Invoice: document number from Bill Series (voucher "Jobwork Invoice"); finalized below after JWO load
$is_jobwork_invoice_page = true;
$next_order_no = function_exists('getNextJobworkInvoiceNo') ? getNextJobworkInvoiceNo($conn) : 'JWI-1';

// Load order for editing if ID provided (?id= or ?Jobwork_order_id= from manufacturing "Stock In")
$edit_order_id = isset($_GET['id']) ? (int)$_GET['id'] : 0;
if (isset($_GET['Jobwork_order_id']) && (int)$_GET['Jobwork_order_id'] > 0) {
    $edit_order_id = (int)$_GET['Jobwork_order_id'];
}
// True when ?id= refers to tbl_jobwork_orders (do not treat as sale order id in JS history / loadOrder)
...
$repair_jobwork_order_id_param = isset($_GET['repair_jobwork_order_id']) ? (int)$_GET['repair_jobwork_order_id'] : 0;

if ($repair_jobwork_order_id_param > 0 && $conn) {
    $t_rjwo = @mysqli_query($conn, "SHOW TABLES LIKE 'tbl_repair_jobwork_orders'");
    $has_rjwo_tbl = ($t_rjwo && mysqli_num_rows($t_rjwo) > 0);
    if ($t_rjwo) {
        mysqli_free_result($t_rjwo);
    }
    if ($has_rjwo_tbl) {
        $rjwo_row = getRecord('SELECT * FROM tbl_repair_jobwork_orders WHERE id = ' . (int) $repair_jobwork_order_id_param . ' LIMIT 1');
        if ($rjwo_row && !empty($rjwo_row['id'])) {
            $is_jobwork_from_repair_jwo = true;
            $current_rjwo_id = (int) $rjwo_row['id'];
            $jwi_repair_order_id = (int) ($rjwo_row['repair_order_id'] ?? 0);
            $ro_row = ($jwi_repair_order_id > 0) ? getRecord('SELECT * FROM tbl_repair_orders WHERE id = ' . (int) $jwi_repair_order_id . ' LIMIT 1') : null;
            $edit_order = [
                'id' => $current_rjwo_id,
                'order_no' => $rjwo_row['jobwork_no'] ?? '',
                'sale_order_no' => $rjwo_row['repair_order_no'] ?? '',
                'customer_name' => $rjwo_row['customer_name'] ?? ($ro_row ? ($ro_row['customer_name'] ?? '') : ''),
                'order_date' => $rjwo_row['order_date'] ?? '',
                'due_date' => $rjwo_row['due_date'] ?? '',
                'grand_total' => $rjwo_row['grand_total'] ?? 0,
                'status' => $rjwo_row['status'] ?? 'draft',
                'ref_no' => $rjwo_row['repair_order_no'] ?? '',
                'department_id' => (isset($rjwo_row['department_id']) && $rjwo_row['department_id'] !== '' && $rjwo_row['department_id'] !== null) ? (int) $rjwo_row['department_id'] : null,
                'department_user_id' => (isset($rjwo_row['department_user_id']) && $rjwo_row['department_user_id'] !== '' && $rjwo_row['department_user_id'] !== null) ? (int) $rjwo_row['department_user_id'] : null,
                'priority' => $rjwo_row['priority'] ?? 'Medium',
                'jwo_status' => $rjwo_row['status'] ?? 'draft',
            ];
            if ($ro_row) {
                $edit_order['sales_person'] = $ro_row['sales_person'] ?? '';
                $edit_order['purchase_person'] = $ro_row['sales_person'] ?? '';
                if (array_key_exists('against_of', $ro_row)) {
                    $edit_order['against_of'] = $ro_row['against_of'] ?? '';
                }
                $edit_order['against_of_ref'] = $ro_row['ref_no'] ?? '';
                if (array_key_exists('currency', $ro_row) && ($ro_row['currency'] ?? '') !== '') {
                    $edit_order['currency'] = $ro_row['currency'];
                }
                if (isset($ro_row['customer_id']) && (int) $ro_row['customer_id'] > 0) {
                    $edit_order['customer_id'] = (int) $ro_row['customer_id'];
                }
            }
            $edit_items = [];
            $rj_items = getList('SELECT * FROM tbl_repair_jobwork_order_items WHERE repair_jobwork_order_id = ' . (int) $current_rjwo_id);
            foreach ($rj_items as $it) {
                if (isset($it['product_characteristic_id']) && !isset($it['characteristic_id'])) {
                    $it['characteristic_id'] = $it['product_characteristic_id'];
                }
                $it['order_id'] = $it['repair_jobwork_order_id'];
                $edit_items[] = $it;
            }
            $edit_payments = [];
            $is_jobwork_from_sale_order = true;
            $jwo_sale_order_id_param = 0;
            $current_jwo_id = 0;
            $edit_url_id_is_jobwork_order = false;
        }
    }
...
if (!$is_jobwork_from_repair_jwo && $edit_order_id > 0) {
    // First check if this id is a Job Work Order (edit JWO mode)
    $jwo_row = getRecord("SELECT * FROM tbl_jobwork_orders WHERE id = $edit_order_id");
    if ($jwo_row) {
        $edit_url_id_is_jobwork_order = true;
        $current_jwo_id = $edit_order_id;
        $edit_order = [
            'id' => $jwo_row['id'],
            'order_no' => $jwo_row['jobwork_no'],
        ...
        $jwo_items = getList("SELECT * FROM tbl_jobwork_order_items WHERE jobwork_order_id = $edit_order_id");
        foreach ($jwo_items as $it) {
            $it['order_id'] = $it['jobwork_order_id'];
            $edit_items[] = $it;
        }
        $is_jobwork_from_sale_order = true;
...
// Job Work Order from Sale Order: same design, pre-fill sale order data
$is_jobwork_from_sale_order = isset($is_jobwork_from_sale_order) ? $is_jobwork_from_sale_order : false;
$jwi_oj_refine_return_url = '';
if (!empty($_GET['from_oj_refine']) && (int) $_GET['from_oj_refine'] === 1) {
    $qs_oj = ['tab' => 'received'];
    $fd = isset($_GET['oj_from_date']) ? trim((string) $_GET['oj_from_date']) : '';
    $td = isset($_GET['oj_to_date']) ? trim((string) $_GET['oj_to_date']) : '';
    if ($fd !== '') {
        $qs_oj['from_date'] = $fd;
    }
    if ($td !== '') {
    ...
    $jwi_oj_refine_return_url = 'old-jewellery.php?' . http_build_query($qs_oj);
}
...
// When sale_order_id in URL (and not already in JWO edit mode): load sale order for creating new JWO
if ($jwo_sale_order_id_param > 0 && $current_jwo_id === 0 && !$is_jobwork_from_repair_jwo) {
    ...
    if ($jwo_so) {
        ...
        // Preview: Jobwork Invoice number from Bill Series (not Jobwork Order number)
        $next_order_no = function_exists('getNextJobworkInvoiceNo') ? getNextJobworkInvoiceNo($conn) : 'JWI-1';
        // Always load existing JWO for this sale order (do not gate on SHOW TABLES — that could skip department/items)
        $existing_jwo = getRecord("SELECT * FROM tbl_jobwork_orders WHERE sale_order_id = " . (int)$jwo_sale_order_id_param . " ORDER BY id DESC LIMIT 1");
        if ($existing_jwo && !empty($existing_jwo['id'])) {
            ...
            $jwo_items_existing = getList("SELECT * FROM tbl_jobwork_order_items WHERE jobwork_order_id = " . $current_jwo_id);
            $edit_items = [];
...
// If JWO saved a department worker but department_id is empty, resolve department from tbl_department_user_map
// so Department + Name match what was chosen on Job Work Order (e.g. jobwork-invoice.php?sale_order_id=N).
if (!empty($edit_order) && is_array($edit_order) && !empty($is_jobwork_from_sale_order) && $is_jobwork_from_sale_order) {
    $map_chk_jwi = @mysqli_query($conn, "SHOW TABLES LIKE 'tbl_department_user_map'");
    $has_map_jwi = ($map_chk_jwi && mysqli_num_rows($map_chk_jwi) > 0);
    if ($map_chk_jwi) {
        mysqli_free_result($map_chk_jwi);
    }
    if ($has_map_jwi) {
        $uid = isset($edit_order['department_user_id']) ? (int) $edit_order['department_user_id'] : 0;
        $did = isset($edit_order['department_id']) ? (int) $edit_order['department_id'] : 0;
        if ($uid > 0 && $did <= 0) {
            $rowMap = getRecord('SELECT department_id FROM tbl_department_user_map WHERE user_id = ' . $uid . ' AND status = 1 ORDER BY id ASC LIMIT 1');
            if ($rowMap && isset($rowMap['department_id']) && (int) $rowMap['department_id'] > 0) {
                $edit_order['department_id'] = (int) $rowMap['department_id'];
            }
        }
    }
}

// Jobwork Invoice No: use saved row for this job work order if present, else next from bill series
if (!empty($is_jobwork_invoice_page)) {
    $invNo = function_exists('getNextJobworkInvoiceNo') ? getNextJobworkInvoiceNo($conn) : 'JWI-1';
    if ($current_jwo_id > 0) {
    ...
    } elseif ($is_jobwork_from_repair_jwo && $current_rjwo_id > 0) {
        $tc = @mysqli_query($conn, "SHOW TABLES LIKE 'tbl_jobwork_invoices'");
        if ($tc && mysqli_num_rows($tc) > 0) {
            mysqli_free_result($tc);
...
// Default department for Job Work Order when none saved on sale order / JWO: Main Office (skip if a department worker is already saved — avoids wrong dept for Name)
if (!empty($is_jobwork_from_sale_order) && $is_jobwork_from_sale_order && !$is_jobwork_from_repair_jwo && $ensure_jwo_dept_id <= 0) {
    $jwo_has_saved_worker = !empty($edit_order) && is_array($edit_order) && isset($edit_order['department_user_id']) && (int) $edit_order['department_user_id'] > 0;
    if (!$jwo_has_saved_worker) {
        $main_office = getRecord("SELECT id, dept_name AS name FROM tbl_departments WHERE status = 1 AND LOWER(TRIM(dept_name)) = 'main office' LIMIT 1");
        if (!$main_office) {
            $main_office = getRecord("SELECT id, dept_name AS name FROM tbl_departments WHERE status = 1 AND LOWER(TRIM(dept_name)) LIKE '%main office%' ORDER BY id ASC LIMIT 1");
        }
        if ($main_office && !empty($main_office['id'])) {
            $ensure_jwo_dept_id = (int) $main_office['id'];
            $have_mo = false;
            foreach ($jwo_departments as $d) {
                if ((int) ($d['id'] ?? 0) === $ensure_jwo_dept_id) {
                    $have_mo = true;
...
if ($jwo_has_department_user_tables && $ensure_jwo_dept_id > 0) {
    ...
    if ($ensure_jwo_dept_user_id > 0) {
        ...
        if (!$have_u) {
            ...
            if (!$extra_u) {
                $extra_u = getRecord('SELECT id, name AS user_name FROM tbl_customers WHERE id = ' . (int) $ensure_jwo_dept_user_id . ' AND status = 1 LIMIT 1');
            }
            if ($extra_u) {
...
$jwo_initial_user_display = '';
if ($ensure_jwo_dept_user_id > 0) {
    if (!empty($jwo_department_users_initial) && is_array($jwo_department_users_initial)) {
        foreach ($jwo_department_users_initial as $u) {
            if ((int) ($u['id'] ?? 0) === $ensure_jwo_dept_user_id) {
                $jwo_initial_user_display = (string) ($u['user_name'] ?? '');
                break;
            }
        }
    }
    if ($jwo_initial_user_display === '' && function_exists('getRecord')) {
        $nr = getRecord('SELECT name FROM tbl_customers WHERE id = ' . (int) $ensure_jwo_dept_user_id . ' LIMIT 1');
        if ($nr && !empty($nr['name'])) {
            $jwo_initial_user_display = (string) $nr['name'];
        }
    }
}
...
<head>
<title>Jobwork Invoice - Gold Matrix</title>
...
text-transform: uppercase;
    ...
    /* Jobwork header: equal-width Department / Name / Search; searchable Name list */
    .jwo-header-field .form-group { min-width: 0; }
    /* Stack header fields above top-nav dropdowns (z-index 1000). col-lg-8 overflow:hidden
       was trapping z-index so inputs stayed under the ORDERS menu; see .invoice-content-row > .col-lg-8 */
    .jwo-header-field { position: relative; z-index: 1080; }
    .jwo-header-field #jwoDepartmentUserWrap { position: relative; }
    .jwo-header-field #jwoAddDepartmentUserBtn { position: relative; z-index: 1081; }
    .jwo-user-suggestions {
        position: absolute; left: 0; right: 0; top: 100%; z-index: 1082;
        background: #fff; border: 1px solid #e2e8f0; border-radius: 4px;
        max-height: 260px; overflow-y: auto; margin-top: 2px;
        box-shadow: 0 8px 20px rgba(15, 23, 42, 0.12);
    }
    .jwo-user-suggestions .jwo-user-suggestion-item {
        padding: 8px 10px; font-size: 12px; cursor: pointer; color: #334155;
        border-bottom: 1px solid #f1f5f9;
    }
    .jwo-user-suggestions .jwo-user-suggestion-item:last-child { border-bottom: none; }
    .jwo-user-suggestions .jwo-user-suggestion-item:hover { background: #f8fafc; }
    /* Prevent Product List table from disturbing layout - scroll inside left column, sidebar stays fixed.
       Do not use overflow:hidden on this column: it creates a stacking context so jobwork header
       fields cannot layer above .top-navbar dropdowns (z-index 1000). Table overflow is on .product-list-card* */
...
<body class="jobwork-invoice-page">
<!-- [ Preloader ] Start -->
                ...
                <div class="container-fluid flex-grow-1" style="padding-top: 0; padding-bottom: 0;">
                    ...
                    <div class="row invoice-content-row">
                        ...
                        <div class="col-lg-8" >
                            ...
                            <div class="card mb-4">
                                <div class="card-body billing-form">
                                        <div class="row">
                                            ...
                                            <div class="col-md-4 jwo-header-field">
                                                <div class="form-group mb-2">
                                                    <label>Department<?php if (!empty($is_jobwork_from_repair_jwo)): ?> <span class="text-muted" style="font-weight: normal; font-size: 0.75rem;"></span><?php endif; ?></label>
                                            ...
                                            <div class="col-md-4 jwo-header-field">
                                                <div class="form-group mb-2">
                                                    <label>Name <?php if (empty($is_jobwork_from_repair_jwo)): ?><span class="text-danger">*</span><?php else: ?><span class="text-muted" style="font-weight: normal; font-size: 0.75rem;"></span><?php endif; ?></label>
                                                    <div class="d-flex align-items-stretch" style="gap: 4px;">
                                                        <div class="position-relative flex-grow-1" id="jwoDepartmentUserWrap" style="min-width: 0;">
                                                            <input type="hidden" id="jwoDepartmentUser" name="department_user_id" value="<?php echo (int) $ensure_jwo_dept_user_id; ?>">
                                                            <input type="text" class="form-control form-control-sm" id="jwoDepartmentUserSearch" autocomplete="off" placeholder="Type to search name..." value="<?php echo htmlspecialchars($jwo_initial_user_display, ENT_QUOTES, 'UTF-8'); ?>" <?php echo $jwo_has_department_user_tables ? '' : 'disabled'; ?> style="width: 100%;">
                                                            <div id="jwoDepartmentUserSuggestions" class="jwo-user-suggestions" style="display: none;"></div>
                                                        </div>
                                                        <button type="button" class="btn btn-sm btn-outline-secondary p-0" id="jwoAddDepartmentUserBtn" title="Add Job Worker (Ledger Details)" style="width: 32px; min-width: 32px; line-height: 1;" <?php echo !$jwo_has_department_user_tables ? 'disabled' : ''; ?>><i class="feather icon-plus"></i></button>
                                                    </div>
                                                </div>
                                            </div>
                                            <?php endif; ?>
                                            <div class="col-md-4 jwo-header-field">
                                                <div class="form-group mb-2">
                                                    <label><?php echo !empty($is_jobwork_invoice_page) ? 'Search Job Work Invoice' : 'Search Job Work Order'; ?></label>
                                                    <div style="position: relative;">
                                                        <input type="text" class="form-control form-control-sm" id="searchSaleOrder" placeholder="<?php echo !empty($is_jobwork_invoice_page) ? 'Type invoice no, JWO no or customer...' : 'Type JWO no or customer name...'; ?>" autocomplete="off" style="padding-right: 35px; width: 100%;">
                                            ...
                                            <div class="col-md-6" style="display:none;" aria-hidden="true">
                                                <input type="hidden" id="customerName" name="customer_name" value="<?php echo (!empty($edit_order) && is_array($edit_order) && !empty($edit_order['customer_name'])) ? htmlspecialchars((string)$edit_order['customer_name'], ENT_QUOTES, 'UTF-8') : ''; ?>">
                                                <input type="hidden" id="customerId" name="customer_id" value="<?php echo (!empty($edit_order) && is_array($edit_order) && !empty($edit_order['customer_id'])) ? (int)$edit_order['customer_id'] : ''; ?>">
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    ...
                                                    if (!empty($edit_order) && is_array($edit_order)) {
                                                        $jwi_party_id = (int) ($edit_order['customer_id'] ?? 0);
                                                        $jwi_party_name = trim((string) ($edit_order['customer_name'] ?? ''));
                                                    }
                                                    auragold_party_select2_field([
                                                        'value_id' => $jwi_party_id,
                                                        'value_name' => $jwi_party_name,
                                                    ]);
                                                    ?>
                                                </div>
                                            </div>
                                        ...
                                        <?php if ($is_jobwork_from_sale_order && empty($is_jobwork_invoice_page)): ?>
                                        <?php
                            ...
                            <?php
                            $product_list_prefs_page = 'jobwork-invoice-product-table';
                            ...
                            <div class="summary-panel" >
                                ...
                                <div class="invoice-header" style="margin: -1rem -1rem 1rem -1rem; border-radius: 10px 10px 0 0; display: flex; align-items: center; justify-content: space-between;">
                                    ...
                                    <div class="invoice-header-actions">
                                        <?php if (!empty($is_jobwork_invoice_page)): ?>
                                        <button type="button" class="btn-new-invoice btn-sm" onclick="window.location.href='jobwork-invoice.php'">New</button>
                                        <span id="siSaveInvoiceWrap" class="si-save-invoice-wrap<?php echo !empty($si_save_blocked_by_purchase_fixing) ? ' si-save-wrap-blocked' : ''; ?>" title="<?php echo !empty($si_save_blocked_by_purchase_fixing) ? htmlspecialchars($si_pfd_save_blocked_tip, ENT_QUOTES, 'UTF-8') : htmlspecialchars('Save', ENT_QUOTES, 'UTF-8'); ?>">
                                            <button type="button" class="btn-save-invoice btn-sm" id="jwiSaveBtn" onclick="saveJobworkInvoice()" <?php echo !empty($si_save_blocked_by_purchase_fixing) ? 'disabled' : ''; ?>>Save</button>
                                        </span>
                                        <?php else: ?>
                                        <button class="btn-new-invoice btn-sm" onclick="resetOrder()">New +</button>
                                        <span id="siSaveInvoiceWrap" class="si-save-invoice-wrap<?php echo !empty($si_save_blocked_by_purchase_fixing) ? ' si-save-wrap-blocked' : ''; ?>" title="<?php echo !empty($si_save_blocked_by_purchase_fixing) ? htmlspecialchars($si_pfd_save_blocked_tip, ENT_QUOTES, 'UTF-8') : htmlspecialchars('Save', ENT_QUOTES, 'UTF-8'); ?>">
                                            <button type="button" class="btn-save-invoice btn-sm" onclick="<?php echo (!empty($is_jobwork_from_sale_order) && $is_jobwork_from_sale_order) ? 'saveJobWorkOrder()' : 'saveOrder()'; ?>" <?php echo !empty($si_save_blocked_by_purchase_fixing) ? 'disabled' : ''; ?>>Save</button>
                                        </span>
                                        <?php endif; ?>
                                    </div>
                                </div>
...
<div id="printJWOModal" class="print-invoice-modal" style="display: none;">
    <div class="print-invoice-modal-content">
        ...
        <p class="print-invoice-modal-message">Do you want to print the job work invoice?</p>
...
<script>
    ...
    window.isJobworkFromRepairJwo = <?php echo !empty($is_jobwork_from_repair_jwo) ? 'true' : 'false'; ?>;
    window.jwiRepairOrderId = <?php echo (int)($jwi_repair_order_id ?? 0); ?>;
    window.jwiRjwoId = <?php echo (int)($current_rjwo_id ?? 0); ?>;
    window.isJobworkInvoicePage = <?php echo !empty($is_jobwork_invoice_page) ? 'true' : 'false'; ?>;
    ...
    window.jwoInitialDepartmentUserId = <?php echo (int) ($ensure_jwo_dept_user_id ?? 0); ?>;
    ...
    window.jwiOjRefineReturnUrl = <?php echo json_encode(isset($jwi_oj_refine_return_url) ? $jwi_oj_refine_return_url : '', JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT); ?>;
</script>
<script>
...
<script>
    ...
    $embed_items = [];
    $embed_payments = [];
    $embed_order_ready = !empty($edit_order) && is_array($edit_order) && (
        (!empty($edit_order_id) && $edit_order_id > 0)
        || (!empty($is_jobwork_from_repair_jwo) && !empty($current_rjwo_id))
    );
    if ($embed_order_ready) {
        $embed_order = [
            ...
            'department_id' => isset($edit_order['department_id']) && $edit_order['department_id'] !== '' && $edit_order['department_id'] !== null ? (int) $edit_order['department_id'] : null,
            'department_user_id' => isset($edit_order['department_user_id']) && $edit_order['department_user_id'] !== '' && $edit_order['department_user_id'] !== null ? (int) $edit_order['department_user_id'] : null,
            'jwo_status' => $edit_order['jwo_status'] ?? $edit_order['status'] ?? '',
            'priority' => $edit_order['priority'] ?? 'Medium',
        ];
        ...
        unset($emb_it);
        if (is_array($embed_payments) && $embed_payments !== []) {
            $pd_merge_path = __DIR__ . '/includes/auragold_payment_details_merge.php';
            if (is_file($pd_merge_path)) {
                require_once $pd_merge_path;
        ...
        if (!empty($is_jobwork_from_sale_order) && is_array($embed_payments) && $embed_payments !== []) {
        ...
        // Jobwork invoice: metal exchange belongs on sale order / job work order — not on this screen.
        if (!empty($is_jobwork_invoice_page) && is_array($embed_payments) && $embed_payments !== []) {
            if (!function_exists('auragold_payment_is_metal_exchange_inward')) {
                require_once __DIR__ . '/includes/auragold_metal_exchange_stock.php';
            }
            $embed_payments = array_values(array_filter($embed_payments, function ($ep) use ($conn) {
                if (!is_array($ep)) {
                    return false;
                }
                $merged = function_exists('auragold_payment_merge_stored_details')
                    ? auragold_payment_merge_stored_details($ep)
                    : $ep;
                if (function_exists('auragold_payment_is_metal_exchange_inward')
                    && auragold_payment_is_metal_exchange_inward($conn, $merged)) {
                    return false;
                }
                $pt = strtolower(trim((string) ($merged['payment_type'] ?? $merged['type'] ?? '')));
                $dep = strtolower(trim((string) ($merged['deposit_into'] ?? '')));

                return !(strpos($pt, 'exch') !== false || strpos($pt, 'metal') !== false || $dep === 'metal exchange');
            }));
        }
    ...
    /** Open job work order print when tbl_jobwork_orders row exists; else sale order print (same sale order id). */
    window.jobworkOrderPrintOpenUrl = function(saleOrderIdFallback) {
        if (window.isJobworkFromRepairJwo && window.jwiRjwoId > 0) {
            return 'jobwork-order-print.php?rjwo_id=' + window.jwiRjwoId;
        }
        var jwoEl = document.getElementById('jwoCurrentId');
        var jid = jwoEl ? parseInt(jwoEl.getAttribute('data-jwo-id') || '0', 10) : 0;
        if (jid > 0) {
            return 'jobwork-order-print.php?id=' + jid;
        }
        if (typeof window.jwoCurrentRecordId === 'number' && window.jwoCurrentRecordId > 0) {
            return 'jobwork-order-print.php?id=' + window.jwoCurrentRecordId;
        }
    ...
    window.jobworkOrderScreenUrl = function(saleOrderId) {
        if (!saleOrderId) return 'jobwork-invoice.php';
        return 'jobwork-invoice.php?sale_order_id=' + encodeURIComponent(saleOrderId);
    };
    ...
    const PRODUCT_MODAL_COLUMNS_PAGE = 'jobwork-invoice-product-modal';
    ...
    $(document).ready(function() {
        ...
        saleReturnSearchInput.on('input', function() {
            ...
            saleReturnSearchTimeout = setTimeout(function() {
                var searchUrl = <?php echo !empty($is_jobwork_invoice_page) ? "'ajax/search-jobwork-invoices.php'" : "'ajax/search-jobwork-orders.php'"; ?>;
                $.ajax({
                    url: searchUrl,
                    type: 'GET',
                    data: { q: searchTerm },
                    ...
                    success: function(response) {
                        if (response.status === 'success' && response.orders && response.orders.length > 0) {
                            let html = '';
                            var isJwi = <?php echo !empty($is_jobwork_invoice_page) ? 'true' : 'false'; ?>;
                            response.orders.forEach(function(order) {
                                var line1 = isJwi && order.invoice_no ? order.invoice_no : (order.order_no || '');
                                var line2 = isJwi
                                    ? ((order.jobwork_no ? '<span style="color:#64748b;">' + order.jobwork_no + '</span> · ' : '') + (order.customer_name || ''))
                                    : (order.customer_name || '');
                                html += `
                                    <div class="sale-order-suggestion-item" 
                                         data-order-id="${order.id}" 
                                         data-order-no="${order.order_no || order.invoice_no || ''}"
                                         style="padding: 0.75rem; cursor: pointer; border-bottom: 1px solid #f1f5f9; transition: background 0.2s;"
                                         onmouseover="this.style.background='#f8fafc'" 
                                         onmouseout="this.style.background='#fff'">
                                        <div style="font-weight: 600; color: #11294b; font-size: 0.9rem;">${line1}</div>
                                        <div style="font-size: 0.85rem; color: #64748b; margin-top: 0.25rem;">${line2}</div>
                                        ...
                                        ${order.grand_total ? '<div style="font-size: 0.75rem; color: #c9a24a; margin-top: 0.15rem; font-weight: 500;">' + (order.currency || 'AED') + ' ' + parseFloat(order.grand_total).toFixed(2) + '</div>' : ''}
                                    </div>
                                `;
                            });
                            saleReturnSuggestions.html(html).show();
                        } else {
                            var emptyMsg = <?php echo !empty($is_jobwork_invoice_page) ? "'No job work invoices found'" : "'No job work orders found'"; ?>;
                            saleReturnSuggestions.html('<div style="padding: 0.75rem; color: #94a3b8; text-align: center;">' + emptyMsg + '</div>').show();
                        }
                    },
                    error: function() {
                        saleReturnSuggestions.hide();
        ...
        $(document).on('click', '.sale-order-suggestion-item', function() {
            const jwoId = $(this).data('order-id');
            saleReturnSearchInput.val('');
            saleReturnSuggestions.hide();
            if (jwoId) {
                <?php if (!empty($is_jobwork_invoice_page)): ?>
                window.location.href = 'jobwork-invoice.php?Jobwork_order_id=' + jwoId;
                <?php else: ?>
                window.location.href = 'jobwork-invoice.php?id=' + jwoId;
                <?php endif; ?>
            }
        });
    ...
    function openJwoJobWorkerLedgerModal() {
        ...
        var deptNum = (d && parseInt(d, 10) > 0) ? parseInt(d, 10) : 0;
        window._jwoPendingDepartmentIdForNewJobWorker = deptNum > 0 ? deptNum : null;
        clearCustomerForm();
        setCustomerModalMode('add');
        applyDefaultLedgerCustomerType('Job Worker');
        var modalEl = document.getElementById('customerCreationModal');
        if (typeof $ !== 'undefined' && modalEl && $.fn && $.fn.modal) {
            $(modalEl).modal('show');
            return;
        }
    ...
    function initCategoryTabs() {
        ...
        if (window.preferredProductModalMetalId && typeof switchToMetalTab === 'function') {
            switchToMetalTab(String(window.preferredProductModalMetalId));
        }
...
<td data-column="sale-amount-with"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" readonly style="width: 130px; font-size: 0.7rem;"></td>
    ...
    function populateRowWithProduct(row, product) {
        ...
        // Metal qty / weight from merged stock journal (or tbl_stock)
    ...
    function addProductToTableFromModalRow(modalRowData, metalIdForImage) {
        ...
        if (modalRowData.metal_id != null && modalRowData.metal_id !== '') {
            row.setAttribute('data-metal-id', String(modalRowData.metal_id));
        }
        if (modalRowData.diamond_category || modalRowData.category) {
            row.setAttribute('data-diamond-category', String(modalRowData.diamond_category || modalRowData.category).trim());
        }
        var savedLineTotal = parseFloat(modalRowData.net_amt_tax || modalRowData.net_amt || modalRowData.amount || 0) || 0;
        if (savedLineTotal > 0) {
    ...
    function addMergedProductsToTable(modalRowsData, metalIdForImage) {
        ...
        var firstRowGiJwInv = '';
        for (var _agiJwInv = 0; _agiJwInv < modalRowsData.length; _agiJwInv++) {
            var rgi = modalRowsData[_agiJwInv].group_image;
            if (rgi && String(rgi).trim()) { firstRowGiJwInv = rgi; break; }
        }
        var groupImagePayload = tabPayloadMerge || ((typeof window.auragoldCoParseGroupImageAttr === 'function') ? window.auragoldCoParseGroupImageAttr(firstRowGiJwInv) : '') || '';
        var groupImageAttr = (typeof groupImagePayload === 'object' && groupImagePayload != null) ? JSON.stringify(groupImagePayload) : (groupImagePayload || '');
        row.setAttribute('data-group-image', groupImageAttr);
        var primaryUrl = typeof getGroupImagePrimary === 'function' ? getGroupImagePrimary(groupImagePayload) : (typeof groupImagePayload === 'string' ? groupImagePayload : '');
        var actionCell = '<td><div class="action-btns"><button type="button" class="btn-edit" onclick="editProductRow(\'' + rowId + '\')" title="Edit"><i class="feather icon-edit"></i></button><button type="button" class="btn-delete" onclick="deleteProductRow(\'' + rowId + '\')" title="Delete"><i class="feather icon-trash-2"></i></button></div></td>';
    ...
    function updateMergedRowFromModalRows(rowId, modalRowsData) {
        ...
        (function() {
            function setAmtCell(sel, val, dec) {
                var c = row.querySelector(sel);
                if (!c) return;
                var s = (parseFloat(val) || 0).toFixed(dec);
                var inp = c.querySelector('input');
                if (inp) inp.value = s; else c.textContent = s;
            }
            setAmtCell('[data-column="net-wt"]', merged.net_wt || 0, 3);
            setAmtCell('[data-column="pure-wt"]', merged.pure_wt || 0, 3);
            setAmtCell('[data-column="amount"]', merged.amount || 0, 2);
            setAmtCell('[data-column="net-amt"]', merged.net_amt || 0, 2);
            setAmtCell('[data-column="net-amt-tax"]', merged.net_amt_tax || 0, 2);
        })();
    ...
    /** Read numeric cell: main grid uses inputs inside td[data-column], not text nodes. */
    function getJobworkCellNumber(row, dataColumn) {
        var cell = row.querySelector('[data-column="' + dataColumn + '"]');
        if (!cell) return 0;
        var ctl = cell.querySelector('input, select');
        if (ctl && ctl.value !== undefined && String(ctl.value).trim() !== '') {
            var v = parseFloat(String(ctl.value).replace(/,/g, ''));
            return isNaN(v) ? 0 : v;
        }
        var t = parseFloat(String(cell.textContent || '').replace(/,/g, '').trim());
        return isNaN(t) ? 0 : t;
    }
    ...
    function updateSummaryPanel() {
        ...
        rows.forEach(function(row) {
            const qty = parseFloat(row.querySelector('[data-field="quantity"]')?.value || '') || getJobworkCellNumber(row, 'quantity');
            const grossWt = parseFloat(row.querySelector('[data-field="gross_wt"]')?.value || '') || getJobworkCellNumber(row, 'gross-wt');
            const lessWt = parseFloat(row.querySelector('[data-field="less_wt"]')?.value || '') || getJobworkCellNumber(row, 'less-wt');
            const purity = parseFloat(row.querySelector('[data-field="purity"]')?.value || '') || getJobworkCellNumber(row, 'purity');
            const finalWt = parseFloat(row.querySelector('[data-field="final_wt"]')?.value || '') || getJobworkCellNumber(row, 'final-wt');
            const netWt = getJobworkCellNumber(row, 'net-wt');
            const pureWt = getJobworkCellNumber(row, 'pure-wt');
            const making = parseFloat(row.querySelector('[data-field="making"]')?.value || '') || getJobworkCellNumber(row, 'making-amount');
            const tax = parseFloat(row.querySelector('[data-field="tax"]')?.value || '') || getJobworkCellNumber(row, 'tax');
            const amount = getJobworkCellNumber(row, 'amount');
            const netAmt = getJobworkCellNumber(row, 'net-amt');
            const netAmtTax = getJobworkCellNumber(row, 'net-amt-tax');
            const stoneCharges = parseFloat(row.querySelector('[data-field="stone_charges"]')?.value || '') || getJobworkCellNumber(row, 'stone-charges') || getJobworkCellNumber(row, 'stone-amount');
            const otherCharges = parseFloat(row.querySelector('[data-field="other_charges"]')?.value || '') || getJobworkCellNumber(row, 'other-charges') || getJobworkCellNumber(row, 'other-amount');
            const diamondValue = parseFloat(row.querySelector('[data-field="diamond_value"]')?.value || '') || getJobworkCellNumber(row, 'diamond-value');
            const gemstoneValue = parseFloat(row.querySelector('[data-field="gemstone_value"]')?.value || '') || getJobworkCellNumber(row, 'gemstone-value');
            const rate = getJobworkCellNumber(row, 'rate');
            const metalValue = getJobworkCellNumber(row, 'metal-value');
            const discount = getJobworkCellNumber(row, 'discount');
            const makingAmount = getJobworkCellNumber(row, 'making-amount');
            const stoneAmount = getJobworkCellNumber(row, 'stone-amount');
            const otherAmount = getJobworkCellNumber(row, 'other-amount');
            const diamondAmount = getJobworkCellNumber(row, 'diamond-amount');
            const purchaseAmount = getJobworkCellNumber(row, 'purchase-amount');
            const saleAmount = getJobworkCellNumber(row, 'sale-amount');
            const saleAmountWith = getJobworkCellNumber(row, 'sale-amount-with');
            const reverse = getJobworkCellNumber(row, 'reverse');
    ...
    function editProductRow(rowId) {
        ...
        if (groupItemsJson) {
            try {
                ...
                if (groupItems && groupItems.length > 0) {
                    ...
                    var bestMetalId = (typeof resolveBestMetalTabIdFromItems === 'function')
                        ? resolveBestMetalTabIdFromItems(groupItems, typeof metals !== 'undefined' ? metals : [])
                        : null;
                    if (bestMetalId && typeof applyPreferredProductModalMetalTab === 'function') {
                        applyPreferredProductModalMetalTab(bestMetalId);
                    }
                    openProductModal();
                    setTimeout(function() {
        ...
        const rowData = {
            ...
            amount: getJobworkCellNumber(row, 'amount'),
            net_amt: getJobworkCellNumber(row, 'net-amt'),
            net_amt_tax: getJobworkCellNumber(row, 'net-amt-tax'),
        ...
        fetch(url)
            ...
            .then(data => {
                ...
                if (data.success && data.product) {
                    ...
                    setTimeout(function() {
                        var editMid = row.getAttribute('data-metal-id') || data.product.metal_id || '';
                        var editCat = row.getAttribute('data-diamond-category') || '';
                        if (typeof resolveBestMetalTabIdFromItems === 'function') {
                            var probe = [{ metal_id: editMid, diamond_category: editCat, category: editCat }];
                            var resolvedMid = resolveBestMetalTabIdFromItems(probe, typeof metals !== 'undefined' ? metals : []);
                            if (resolvedMid) editMid = resolvedMid;
                        }
                        if (editMid && typeof switchToMetalTab === 'function') {
                            switchToMetalTab(String(editMid));
                        }
                        ...
                        const item = {
                            ...
                            metal_id: editMid || data.product.metal_id || '',
                            diamond_category: editCat,
                            category: editCat,
    ...
    (function() {
        var PAGE_MODAL_COLUMN_PREFS = 'jobwork-invoice-product-modal';
        var TAB_MAIN = 'main';
    ...
    function applyJwoDepartmentUserCache(users, selectUserId, callback) {
        var hid = document.getElementById('jwoDepartmentUser');
        var inp = document.getElementById('jwoDepartmentUserSearch');
        var sug = document.getElementById('jwoDepartmentUserSuggestions');
        if (!hid || !inp) {
            if (callback) callback();
            return;
        }
        window._jwoDepartmentUserList = Array.isArray(users) ? users : [];
        var uid = (selectUserId !== undefined && selectUserId !== null && selectUserId !== '') ? String(parseInt(selectUserId, 10)) : '';
        if (uid === 'NaN' || uid === '0') uid = '';
        var label = '';
        if (uid) {
            for (var i = 0; i < window._jwoDepartmentUserList.length; i++) {
                if (String(window._jwoDepartmentUserList[i].id) === uid) {
                    label = window._jwoDepartmentUserList[i].user_name || '';
                    break;
                }
            }
            if (!label) {
    ...
    function refreshJwoDepartmentUsers(deptId, selectUserId, callback) {
        ...
        if (btn) {
            btn.disabled = false;
            btn.style.opacity = okDept ? '1' : '0.65';
        }
        if (!hid || !inp || !window.jwoHasDepartmentUserTables) {
            if (callback) callback();
            return;
        }

        if (!okDept) {
            fetch('ajax/get-department-users.php?all_ledgers=1', { credentials: 'same-origin' })
                .then(function(r) { return r.json(); })
                .then(function(data) {
                    var users = (data && data.status === 'success' && Array.isArray(data.users)) ? data.users : [];
                    applyJwoDepartmentUserCache(users, selectUserId, callback);
                })
                .catch(function() {
                    if (callback) callback();
                });
        ...
        fetch('ajax/get-department-users.php?department_id=' + encodeURIComponent(d), { credentials: 'same-origin' })
            .then(function(r) { return r.json(); })
            .then(function(data) {
                var users = (data && data.status === 'success' && Array.isArray(data.users)) ? data.users : [];
                applyJwoDepartmentUserCache(users, selectUserId, callback);
    ...
    function renderJwoUserSuggestions(filterText) {
        var box = document.getElementById('jwoDepartmentUserSuggestions');
        var inp = document.getElementById('jwoDepartmentUserSearch');
        if (!box || !inp) {
            return;
        }
        var list = window._jwoDepartmentUserList;
        if (!list || !list.length) {
            box.style.display = 'none';
            box.innerHTML = '';
            return;
        }
        var q = (filterText == null ? inp.value : filterText);
        q = String(q || '').toLowerCase().trim();
        var maxN = 100;
        var out = [];
        for (var i = 0; i < list.length && out.length < maxN; i++) {
            var u = list[i];
            var name = String(u.user_name || '');
        ...
        out.forEach(function(u) {
            ...
            div.className = 'jwo-user-suggestion-item';
            div.textContent = u.user_name || '';
            div.setAttribute('data-id', String(u.id));
            div.addEventListener('mousedown', function(e) {
                e.preventDefault();
                var hid = document.getElementById('jwoDepartmentUser');
                if (hid) hid.value = String(u.id);
                inp.value = u.user_name || '';
                box.style.display = 'none';
            });
            box.appendChild(div);
        });
    ...
    window.renderJwoUserSuggestions = renderJwoUserSuggestions;

    function initJwoDepartmentUserCombobox() {
        if (window._jwoUserComboBound) {
            return;
        }
        var inp = document.getElementById('jwoDepartmentUserSearch');
        var box = document.getElementById('jwoDepartmentUserSuggestions');
        var wrap = document.getElementById('jwoDepartmentUserWrap');
        if (!inp || !box) {
            return;
        }
        window._jwoUserComboBound = true;
        var deb;
        inp.addEventListener('input', function() {
            ...
            deb = setTimeout(function() {
                renderJwoUserSuggestions(inp.value);
            }, 120);
        });
        ...
        inp.addEventListener('keydown', function(e) {
            ...
            e.preventDefault();
            var firstItem = box && box.querySelector('.jwo-user-suggestion-item');
            if (box && box.style.display !== 'none' && firstItem) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (deptEl) {
                deptEl.addEventListener('change', function() {
                    var uEl = document.getElementById('jwoDepartmentUser');
                    var keep = uEl && uEl.value ? uEl.value : '';
                    refreshJwoDepartmentUsers(deptEl.value, keep);
                });
            }
            ...
            var uHid = document.getElementById('jwoDepartmentUser');
            var fromPhp = (typeof window.jwoInitialDepartmentUserId !== 'undefined' && window.jwoInitialDepartmentUserId) ? String(window.jwoInitialDepartmentUserId) : '';
            var preUser = fromPhp || (uHid && uHid.value ? String(uHid.value).trim() : '');
            if (typeof refreshJwoDepartmentUsers === 'function') {
                if (deptEl && deptEl.value) {
                    refreshJwoDepartmentUsers(deptEl.value, preUser);
                } else {
                    refreshJwoDepartmentUsers('', preUser);
                }
            }
            if (typeof initJwoDepartmentUserCombobox === 'function') {
                initJwoDepartmentUserCombobox();
            }
        });
    }
    ...
    function saveJobWorkOrder() {
        ...
        if (!saleOrderId) {
            alert('Sale order reference is missing. Open this screen from a sale order or use Search Job Work Order.');
            return;
        }
        var deptEl = document.getElementById('jwoDepartment');
        if (deptEl && deptEl.required && (!deptEl.value || deptEl.value.trim() === '')) {
            alert('Please select Department.');
            return;
        }
        var userEl = document.getElementById('jwoDepartmentUser');
        if (window.isJobworkFromSaleOrder && window.jwoHasDepartmentUserTables && deptEl && deptEl.value && userEl && (!userEl.value || userEl.value.trim() === '')) {
            alert('Please select Name (department user).');
            return;
        }
        if (isSaving) return;
        isSaving = true;
        ...
        productRows.forEach(function(row) {
            ...
            if (productId) {
                ...
                var netWeight = getJobworkCellNumber(row, 'net-wt');
                var pureWeight = getJobworkCellNumber(row, 'pure-wt') || getJobworkCellNumber(row, 'purity-wt');
                var purity = parseFloat(row.getAttribute('data-purity')) || parseFloat(row.querySelector('[data-field="purity"]')?.value || '') || getJobworkCellNumber(row, 'purity');
                var rate = parseFloat(row.getAttribute('data-rate')) || getJobworkCellNumber(row, 'rate');
                var makingAmount = parseFloat(row.querySelector('[data-field="making"]')?.value || '') || getJobworkCellNumber(row, 'making-amount');
                var designNo = (row.querySelector('[data-field="design_no"]')?.value || row.querySelector('[data-column="design-no"]')?.textContent || '').trim();
                var tax = parseFloat(row.querySelector('[data-field="tax"]')?.value || '') || getJobworkCellNumber(row, 'tax');
                var amount = getJobworkCellNumber(row, 'amount');
                var netAmount = getJobworkCellNumber(row, 'net-amt');
                var netAmtWithTax = getJobworkCellNumber(row, 'net-amt-tax');
                    ...
                    if (msg.indexOf('already exists') !== -1 && typeof swal === 'function') {
                        swal({
                            ...
                            if (isConfirm && data.jwo_id) {
                                if (saleOrderId > 0) {
                                    window.location.href = 'jobwork-invoice.php?sale_order_id=' + saleOrderId;
                                } else {
                                    window.location.href = 'jobwork-invoice.php?id=' + data.jwo_id;
                                }
                            }
                        });
                    } else {
    ...
    /** Repair JWO: save RJWO lines then jobwork invoice header (repair_jobwork_order_id). */
    function saveRepairJobworkInvoiceFlow() {
        var repairOrderId = (typeof window.jwiRepairOrderId === 'number' && window.jwiRepairOrderId > 0) ? window.jwiRepairOrderId : 0;
        var rjwoId = (typeof window.jwiRjwoId === 'number' && window.jwiRjwoId > 0) ? window.jwiRjwoId : 0;
        var elRef = document.getElementById('jwoSaleOrderId');
        if (elRef) {
            repairOrderId = parseInt(elRef.getAttribute('data-repair-order-id') || repairOrderId, 10) || repairOrderId;
            rjwoId = parseInt(elRef.getAttribute('data-rjwo-id') || rjwoId, 10) || rjwoId;
        }
        if (!repairOrderId || !rjwoId) {
            alert('Repair job work reference is missing. Open this screen from Manufacturing (RJWO).');
            return;
        }
        if (isSaving) return;
        isSaving = true;
        var btn = document.getElementById('jwiSaveBtn') || document.querySelector('.btn-save-invoice[onclick*="saveJobworkInvoice"]');
        if (btn) { btn.disabled = true; btn.textContent = 'Saving...'; }
        var items = [];
        var productRows = document.querySelectorAll('#productTableBody tr:not(.no-drag)');
        productRows.forEach(function(row) {
        ...
        var fd = new FormData();
        fd.append('repair_order_id', String(repairOrderId));
        fd.append('rjwo_id', String(rjwoId));
        fd.append('items', JSON.stringify(items));
        var deptElR = document.getElementById('jwoDepartment');
        if (deptElR && deptElR.value && deptElR.value.trim() !== '') {
            fd.append('department_id', deptElR.value);
        }
        var userElR = document.getElementById('jwoDepartmentUser');
        if (userElR && userElR.value && String(userElR.value).trim() !== '') {
            fd.append('department_user_id', String(userElR.value).trim());
        }
        var statusElR = document.getElementById('jwoStatus');
        if (statusElR && statusElR.value) {
            fd.append('jwo_status', statusElR.value);
        }
        var priorityElR = document.getElementById('jwoPriority');
        if (priorityElR && priorityElR.value) {
            fd.append('priority', priorityElR.value);
        }
        fetch('ajax/save-repair-jobwork-order.php', { method: 'POST', body: fd, credentials: 'same-origin' })
            .then(function(r) { return r.json(); })
            .then(function(data) {
                if (data.status !== 'success') {
                    isSaving = false;
                ...
                var invNo = (document.getElementById('currentOrderNo') && document.getElementById('currentOrderNo').textContent) ? document.getElementById('currentOrderNo').textContent.trim() : '';
                var gtEl = document.getElementById('summaryGrandTotal');
                var grandTotal = gtEl ? parseFloat(String(gtEl.textContent || '0').replace(/,/g, '')) || 0 : 0;
                var custEl = document.getElementById('customerName');
                var custName = custEl ? String(custEl.value || '').trim() : '';
                var jwiFd = new FormData();
                jwiFd.append('repair_jobwork_order_id', String(rjwoId));
                ...
                if (typeof jwiCollectPaymentsForSave === 'function') {
                    try {
                        jwiFd.append('payments', JSON.stringify(jwiCollectPaymentsForSave()));
                ...
                return fetch('ajax/save-jobwork-invoice.php', { method: 'POST', body: jwiFd, credentials: 'same-origin' })
                    ...
                    .then(function(jwi) {
                        isSaving = false;
                        if (btn) { btn.disabled = false; btn.textContent = 'Save'; }
                        if (jwi.status === 'success' && jwi.invoice_no) {
                            var cEl = document.getElementById('currentOrderNo');
                            if (cEl) cEl.textContent = jwi.invoice_no;
                    ...
                    .catch(function() {
                        isSaving = false;
                        if (btn) { btn.disabled = false; btn.textContent = 'Save'; }
                        alert('Network error saving jobwork invoice');
                    });
            })
            .catch(function() {
                isSaving = false;
    ...
    /** Jobwork Invoice: save JWO lines then persist invoice header (tbl_jobwork_invoices) for bill series / re-open. */
    function saveJobworkInvoice() {
        if (window.isJobworkFromRepairJwo) {
            saveRepairJobworkInvoiceFlow();
            return;
        }
        var el = document.getElementById('jwoSaleOrderId');
        var saleOrderId = el ? parseInt(el.getAttribute('data-sale-order-id') || 0, 10) : 0;
        if (!saleOrderId) {
            alert('Sale order reference is missing. Open this screen from a sale order or use Search Job Work Invoice.');
            return;
        }
        var deptEl = document.getElementById('jwoDepartment');
        if (deptEl && deptEl.required && (!deptEl.value || deptEl.value.trim() === '')) {
        ...
        fd.append('skip_metal_exchange_auto_issue', '1');
        fetch('ajax/save-jobwork-order.php', { method: 'POST', body: fd, credentials: 'same-origin' })
            .then(function(r) { return r.json(); })
            .then(function(data) {
                if (data.status !== 'success') {
                ...
                var newJwoId = data.jwo_id || data.jobwork_id || currentJwoId;
                if (newJwoId && document.getElementById('jwoCurrentId')) {
                    document.getElementById('jwoCurrentId').setAttribute('data-jwo-id', String(newJwoId));
                }
                var invNo = (document.getElementById('currentOrderNo') && document.getElementById('currentOrderNo').textContent) ? document.getElementById('currentOrderNo').textContent.trim() : '';
                ...
                var custName = custEl ? String(custEl.value || '').trim() : '';
                var jwiFd = new FormData();
                jwiFd.append('jobwork_order_id', String(newJwoId));
                jwiFd.append('invoice_no', invNo);
                jwiFd.append('grand_total', String(grandTotal));
                jwiFd.append('customer_name', custName);
    ...
    function closePrintJWOModal() {
        ...
        savedJwoId = null;
        var ojRet = window.jwiOjRefineReturnUrl;
        if (ojRet && String(ojRet).trim() !== '') {
            window.jwiOjRefineReturnUrl = '';
            window.location.href = ojRet;
            savedSaleOrderId = null;
            return;
        }
        var mpDeptId = window._jwoRedirectMpDeptId || '';
        var mpUserId = window._jwoRedirectMpUserId || '';
        window._jwoRedirectMpDeptId = null;
        window._jwoRedirectMpUserId = null;
        var soId = savedSaleOrderId ? parseInt(savedSaleOrderId, 10) : 0;
        savedSaleOrderId = null;
        if (window.isJobworkInvoicePage && soId > 0) {
            window.location.href = typeof window.jobworkOrderScreenUrl === 'function'
                ? window.jobworkOrderScreenUrl(soId)
                : ('jobwork-invoice.php?sale_order_id=' + encodeURIComponent(soId));
            return;
        }
        if (window.isJobworkInvoicePage) {
            window.location.href = 'jobwork-invoice.php';
            return;
        }
        var mpQs = [];
        if (mpDeptId) {
            mpQs.push('department_id=' + encodeURIComponent(mpDeptId));
        }
        if (mpUserId) {
            mpQs.push('user_id=' + encodeURIComponent(mpUserId));
        }
    ...
    function confirmPrintJWO() {
        if (savedJwoId) {
            if (window.isJobworkFromRepairJwo) {
                window.location.href = 'jobwork-order-print.php?rjwo_id=' + savedJwoId;
            } else {
                window.location.href = 'jobwork-order-print.php?id=' + savedJwoId;
            }
        } else {
            closePrintJWOModal();
        }
    ...
    function saleReturnAfterSavePrompts(returnId, returnNo, newBarcodes) {
        ...
        function doRedirect(options) {
            ...
            if (returnId) {
                window.location.href = typeof window.jobworkOrderScreenUrl === 'function'
                    ...
                    : ('jobwork-invoice.php?sale_order_id=' + encodeURIComponent(returnId));
            } else {
                var msg = 'Job Work Order saved successfully! Order No: ' + (returnNo || 'N/A');
        ...
        function openReturnPrintModalOnly() {
            ...
            if (printU && confirm('Do you want to print the Job Work Order?')) {
    ...
    function saveOrder() {
        ...
        productRows.forEach(function(row) {
            ...
            if (productId) {
                ...
                const netWeight = getJobworkCellNumber(row, 'net-wt');
                const pureWeight = getJobworkCellNumber(row, 'purity-wt') || getJobworkCellNumber(row, 'pure-wt');
                const purity = parseFloat(row.getAttribute('data-purity')) || parseFloat(row.querySelector('[data-field="purity"]')?.value || row.querySelector('[data-column="purity"] input')?.value || '') || getJobworkCellNumber(row, 'purity');
                const rate = parseFloat(row.getAttribute('data-rate')) || getJobworkCellNumber(row, 'rate');
                ...
                const tax = parseFloat(row.querySelector('[data-field="tax"]')?.value || '') || getJobworkCellNumber(row, 'tax');
                const amount = getJobworkCellNumber(row, 'amount');
                const netAmount = getJobworkCellNumber(row, 'net-amt');
                const netAmtWithTax = getJobworkCellNumber(row, 'net-amt-tax');
                const stoneCharges = parseFloat(row.querySelector('[data-field="stone_charges"]')?.value || '') || getJobworkCellNumber(row, 'stone-charges') || getJobworkCellNumber(row, 'stone-amount');
                const otherCharges = parseFloat(row.querySelector('[data-field="other_charges"]')?.value || '') || getJobworkCellNumber(row, 'other-charges') || getJobworkCellNumber(row, 'other-amount');
                const diamondValue = parseFloat(row.querySelector('[data-field="diamond_value"]')?.value || '') || getJobworkCellNumber(row, 'diamond-value');
                const gemstoneValue = parseFloat(row.querySelector('[data-field="gemstone_value"]')?.value || '') || getJobworkCellNumber(row, 'gemstone-value');
                ...
                const discount = getJobworkCellNumber(row, 'discount');
                const makingAmount = parseFloat(row.querySelector('[data-column="making-amount"] input')?.value || row.querySelector('[data-field="making"]')?.value || '') || getJobworkCellNumber(row, 'making-amount');
                const stoneAmount = getJobworkCellNumber(row, 'stone-amount');
                const otherAmount = getJobworkCellNumber(row, 'other-amount');
                const diamondAmount = getJobworkCellNumber(row, 'diamond-amount');
                const purchaseAmount = getJobworkCellNumber(row, 'purchase-amount');
    ...
    function resetOrder() {
        ...
        } else if (confirm('Are you sure you want to create a new job work invoice? All unsaved data will be lost.')) {
            window.location.href = 'jobwork-invoice.php';
        }
    }
    ...
    if (saveInvoiceBtn) {
        ...
        saveInvoiceBtn.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
            if (window.isJobworkInvoicePage && window.isJobworkFromSaleOrder && typeof saveJobworkInvoice === 'function') {
                saveJobworkInvoice();
            } else if (window.isJobworkFromSaleOrder && typeof saveJobWorkOrder === 'function') {
                saveJobWorkOrder();
            } else {
                saveOrder();
            }
    ...
    function updatePrintIcon(invoiceId) {
        ...
        if (printIcon) {
            if (invoiceId && invoiceId > 0) {
                ...
                printIcon.setAttribute('title', 'Print Job Work Order');
            } else {
            ...
            jQuery.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        var histPathLoad = 'jobwork-invoice.php?sale_order_id=' + orderId;
            ...
            fetch(getUrl, { cache: 'no-store' })
                ...
                .then(data => {
                    ...
                    if (data.status === 'success') {
                        ...
                        var histPathLoadF = 'jobwork-invoice.php?sale_order_id=' + orderId;
                        if (window.isJobworkFromSaleOrder && typeof jobworkOrderCanonicalPath === 'function') {
    ...
    function addProductRowToSelectionTable(item, product) {
        ...
        row.setAttribute('data-metal-id', (item.metal_id != null && item.metal_id !== '') ? String(item.metal_id) : (currentMetalId || ''));
        ...
        if (categorySelectEl) {
            var isDiamondCat = (item.diamond_category || item.category || '').toString().trim();
            useDiamondOptions = (typeof isKnownDiamondCategoryValue === 'function' && isKnownDiamondCategoryValue(isDiamondCat))
                || isDiamondCat === 'Jewellery' || isDiamondCat === 'Diamonds' || isDiamondCat === 'GemStones'
                || isDiamondCat === 'Diamond' || isDiamondCat === 'Stone';
        ...
        var savedModalNetTax = parseFloat(item.net_amount_tax || item.net_amt_tax || item.net_amt_with_tax || 0) || 0;
        if (savedModalNetTax <= 0 && typeof calculateModalRowNetWeight === 'function') {
            calculateModalRowNetWeight(row);
        } else if (typeof updateJewelleryNetAmountAndFinal === 'function') {
    ...
    function populateOrderForm(order, items, loadedPayments) {
        ...
        if (window.isJobworkFromSaleOrder) {
            var jwoDeptEl = document.getElementById('jwoDepartment');
            var ddRaw = order.department_id;
            var ddId = (ddRaw !== undefined && ddRaw !== null && ddRaw !== '') ? parseInt(ddRaw, 10) : 0;
            var duRaw = order.department_user_id;
            var duId = (duRaw !== undefined && duRaw !== null && duRaw !== '') ? parseInt(duRaw, 10) : 0;
            if (jwoDeptEl && ddId > 0 && !isNaN(ddId)) {
                jwoDeptEl.value = String(ddId);
            }
            if (document.getElementById('jwoStatus')) {
                var jwoSt = order.jwo_status || order.status || '';
                if (jwoSt) {
                    document.getElementById('jwoStatus').value = jwoSt;
                }
            }
            if (document.getElementById('jwoPriority') && order.priority) {
                document.getElementById('jwoPriority').value = order.priority;
            }
            var deptValNow = jwoDeptEl && jwoDeptEl.value ? jwoDeptEl.value : '';
            if (typeof refreshJwoDepartmentUsers === 'function') {
                refreshJwoDepartmentUsers(deptValNow, duId > 0 && !isNaN(duId) ? String(duId) : '', function() {
                    if (typeof initJwoDepartmentUserCombobox === 'function') {
                        initJwoDepartmentUserCombobox();
                    }
                });
        ...
        if (itemList.length > 0) {
            ...
            try {
                ...
                var tabProbeItems = itemList.slice();
                if (order && order.diamond_issues && order.diamond_issues.length) {
                    order.diamond_issues.forEach(function(iss) {
                        if (!iss) return;
                        tabProbeItems.push({
                            metal_id: iss.metal_id || '',
                            diamond_category: iss.diamond_category || 'Diamonds',
                            category: iss.diamond_category || 'Diamonds'
                        });
                    });
                }
                if (order && order.stone_issues && order.stone_issues.length) {
                    order.stone_issues.forEach(function(iss) {
                        if (!iss) return;
                        tabProbeItems.push({
                            metal_id: iss.metal_id || '',
                            diamond_category: iss.stone_category || 'GemStones',
                            category: iss.stone_category || 'GemStones'
                        });
                    });
                }
                var preferredTabId = (typeof resolveBestMetalTabIdFromItems === 'function')
                    ? resolveBestMetalTabIdFromItems(tabProbeItems, typeof metals !== 'undefined' ? metals : [])
                    : null;
                if (preferredTabId && typeof applyPreferredProductModalMetalTab === 'function') {
                    applyPreferredProductModalMetalTab(preferredTabId);
                }
            } catch (e) {
            ...
            var preferredTabIdEmpty = (typeof resolveBestMetalTabIdFromItems === 'function')
                ? resolveBestMetalTabIdFromItems(tabProbeEmpty, typeof metals !== 'undefined' ? metals : [])
                : null;
            if (preferredTabIdEmpty && typeof applyPreferredProductModalMetalTab === 'function') {
                applyPreferredProductModalMetalTab(preferredTabIdEmpty);
            }
        }
        ...
        if (loadedPayments && loadedPayments.length > 0) {
            loadedPayments.forEach(function(payment) {
                if (payment.payment_details && typeof payment.payment_details === 'string' && payment.payment_details.trim() !== '') {
                    try {
                        var pdMerged = JSON.parse(payment.payment_details);
                        if (pdMerged && typeof pdMerged === 'object') {
                            payment = Object.assign({}, payment, pdMerged);
                        }
                    } catch (ePd) { /* ignore */ }
                }
                ...
                if (window.isJobworkInvoicePage && paymentType === 'metal-exchange') {
                    return;
                }
                ...
                if (paymentType === 'metal-exchange') {
                    ...
                    if (window.isJobworkFromSaleOrder && (payment.readonly_from_sale_order || payment.sale_order_metal_exchange)) {
                        paymentData.readonly_from_sale_order = true;
                    }
                    if (payment.mi_stock_id != null && payment.mi_stock_id !== '') {
                        paymentData.mi_stock_id = String(payment.mi_stock_id);
                    }
                }
        ...
        if (typeof jwiPurgeMetalExchangePaymentsFromUi === 'function') {
            jwiPurgeMetalExchangePaymentsFromUi();
        }
    ...
    function jobworkOrderCanonicalPath(fallbackEditId) {
        if (window.isJobworkFromRepairJwo && window.jwiRjwoId > 0) {
            return 'jobwork-invoice.php?repair_jobwork_order_id=' + window.jwiRjwoId;
        }
        if (!window.isJobworkFromSaleOrder) return null;
        if (window.jwoSaleOrderIdParam > 0) {
            return 'jobwork-invoice.php?sale_order_id=' + window.jwoSaleOrderIdParam;
        }
        if (window.jwoCurrentRecordId > 0) {
            return 'jobwork-invoice.php?id=' + window.jwoCurrentRecordId;
        }
        if (fallbackEditId) {
            return 'jobwork-invoice.php?sale_order_id=' + fallbackEditId;
        }
        return 'jobwork-invoice.php';
    }
    
    // Load order on page load when ?id= is in URL (edit mode) - run after window.load so DOM (product table, summary) is ready
    <?php if ($edit_order_id > 0 || (!empty($is_jobwork_from_repair_jwo) && !empty($current_rjwo_id))): ?>
    (function() {
        var editId = <?php echo (int)($edit_order_id ?? 0); ?>;
        var rjwoLoadId = <?php echo (int)($current_rjwo_id ?? 0); ?>;
        function doPopulateFromEmbed() {
            ...
            try {
                ...
                if (window.history && window.history.replaceState) {
                    ...
                    if (window.isJobworkFromRepairJwo && rjwoLoadId > 0) {
                        jwoPath = 'jobwork-invoice.php?repair_jobwork_order_id=' + rjwoLoadId;
                    }
                    if (jwoPath) {
                        window.history.replaceState({}, '', jwoPath);
                    } else if (!window.isJobworkFromRepairJwo) {
                        window.history.replaceState({}, '', 'jobwork-invoice.php?sale_order_id=' + editId);
                    }
                }
                window.isPurchaseInvoiceEditMode = true;
                return true;
    ...
    /** Jobwork invoice: do not re-post sale-order metal exchange (already in stock). */
    function jwiCollectPaymentsForSave() {
        ...
        return raw.filter(function (p) {
            if (!p) return false;
            if (p.readonly_from_sale_order) return false;
            if ((p.type || '') === 'metal-exchange') return false;
            return true;
        });
    }
    window.jwiCollectPaymentsForSave = jwiCollectPaymentsForSave;

    /** Remove metal exchange cards — not used on jobwork invoice (handled on sale order / JWO). */
    function jwiPurgeMetalExchangePaymentsFromUi() {
        if (!window.isJobworkInvoicePage) return;
        if (typeof payments !== 'undefined' && Array.isArray(payments)) {
            payments = payments.filter(function (p) { return (p.type || '') !== 'metal-exchange'; });
        }
        var tbody = document.getElementById('paymentTableBody');
        if (!tbody) return;
        tbody.querySelectorAll('.pos-payment-card[data-payment-type="metal-exchange"]').forEach(function (el) { el.remove(); });
        var rows = tbody.querySelectorAll('.pos-payment-card');
        if (rows.length === 0 && !tbody.querySelector('.no-payment-row')) {
            tbody.innerHTML = '<div class="no-payment-row pos-payment-empty w-100 text-center text-muted py-3">No payment entries</div>';
            var footer = document.getElementById('paymentTableFooter');
            if (footer) footer.style.display = 'none';
        }
        if (typeof updatePaymentTotals === 'function') updatePaymentTotals();
    }
    window.jwiPurgeMetalExchangePaymentsFromUi = jwiPurgeMetalExchangePaymentsFromUi;
    ...
    function initPaymentIconHandlers() {
        document.querySelectorAll('.payment-icon').forEach(function(icon) {
            ...
            icon.addEventListener('click', function() {
                ...
                if (window.isJobworkInvoicePage && this.classList.contains('payment-exchange')) {
                    return;
                }
    ...
    function deletePayment(paymentId) {
        ...
        if (payDel && payDel.readonly_from_sale_order) {
            alert('This metal exchange is from the sale order. Change it on the sale order screen.');
            return;
        }
    ...
    function editPayment(paymentId) {
        ...
        if (payment && payment.readonly_from_sale_order && String(payment.type || '') === 'metal-exchange') {
            alert('Metal exchange from sale order: ' + (payment.metal_exchange_product_name || payment.metal_exchange_item_code || 'view on sale order'));
            return;
        }
        if (!payment) {
            const row = document.getElementById(paymentId);
...
<div id="printInvoiceModal" class="print-invoice-modal" style="display: none;">
    <div class="print-invoice-modal-content">
        ...
        <p class="print-invoice-modal-message" id="printInvoiceModalMessage">Do you want to print the job work invoice?</p>
        <div class="print-invoice-modal-buttons">
...
body.jobwork-invoice-page.stop-scrolling .sweet-overlay {
    background: rgba(15, 23, 42, 0.45);
}
body.jobwork-invoice-page .sweet-alert.sale-return-swal {
    border-radius: 20px;
    padding: 32px 28px 28px;
    box-shadow: 0 10px 40px rgba(0, 0, 0, 0.18);
    font-family: inherit;
}
body.jobwork-invoice-page .sweet-alert.sale-return-swal h2 {
    font-size: 1.35rem;
    font-weight: 700;
    color: #1e3a5f;
    margin: 0 0 12px;
...
body.jobwork-invoice-page .sweet-alert.sale-return-swal p.lead {
    color: #6b7280;
    font-size: 0.95rem;
    line-height: 1.5;
    margin: 0 0 8px;
}
body.jobwork-invoice-page .sweet-alert.sale-return-swal .sa-button-container {
    margin-top: 22px;
    gap: 14px;
    display: flex;
    flex-wrap: wrap;
...
body.jobwork-invoice-page .sweet-alert.sale-return-swal button.confirm.btn-sale-return-sw-confirm {
...
body.jobwork-invoice-page .sweet-alert.sale-return-swal button.confirm.btn-sale-return-sw-confirm:hover {
    filter: brightness(1.06);
}
body.jobwork-invoice-page .sweet-alert.sale-return-swal button.cancel.btn-sale-return-sw-cancel {
...
body.jobwork-invoice-page .sweet-alert.sale-return-swal button.cancel.btn-sale-return-sw-cancel:hover {
        ...
        setTimeout(function () {
            if (!srOpenPrintBillModal({
                ...
                var pu = (typeof window.jobworkOrderPrintOpenUrl === 'function' && window.jobworkOrderPrintOpenUrl(invoiceId))
                    || ('sale-order-print.php?id=' + encodeURIComponent(invoiceId));
                if (confirm('Job Work Order saved successfully! Do you want to print?')) {
        ...
        if (!afterPrint && flow && flow.phase === 'barcode') {
            ...
            setTimeout(function () {
                if (typeof window.srOpenPrintBillModal === 'function' && document.getElementById('printInvoiceModal')) {
                    ...
                    if (confirm('Do you want to print the Job Work Order?')) {
