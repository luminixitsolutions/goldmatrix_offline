 * @param string|null $metalType  Display name e.g. Gold, Silver — loads that metal's row when set.
 ...
 */
function getBarcodeSettings($metalType = null) {
    global $conn;
    ...
    if ($metalFilter !== '') {
        ...
        if (!$row) {
            $legacyWhere = ($branchSql !== '' ? $branchSql . ' AND' : ' WHERE') . " (metal_type IS NULL OR metal_type = '')";
