...
$allowed_langs = function_exists('getInvoicePrintAllowedLanguages') ? getInvoicePrintAllowedLanguages('sale_return') : ['en'];
...
if (!$return) die('Sale return not found');

$items = getList("SELECT sri.*, COALESCE(p.name, sri.product_name) as product_name FROM tbl_sale_return_items sri LEFT JOIN tbl_products p ON sri.product_id = p.id WHERE sri.return_id = $return_id ORDER BY sri.id ASC");

// Print settings for this document type (fallback to default)
$print_settings = function_exists('getInvoicePrintSettingsForDocument') ? getInvoicePrintSettingsForDocument('sale_return') : [];
...
$doc_title = !empty($print_settings['invoice_title']) ? $print_settings['invoice_title'] : 'SALE RETURN';
...
$t = ['invoice_no' => 'Return No.', 'date' => 'Date', 'salesman' => 'Sales Person', 'mr_ms' => 'Customer', 'customer_no' => 'Ref', 'passport_id' => 'ID', 'sr' => '#', 'description' => 'Description', 'weight' => 'Weight', 'stone_weight' => 'Stone', 'net_wt' => 'Net Wt', 'net_amt' => 'Net Amt', 'vat_5' => 'VAT', 'total_amount' => 'Amount', 'total' => 'Total', 'advance_amount' => 'Advance', 'total_before_vat' => 'Before VAT', 'vat_5_label' => 'VAT 5%', 'total_including_vat' => 'Incl. VAT', 'less_scrap' => 'Less Scrap', 'balance_amount' => 'Balance', 'amount_in_words' => 'Amount in Words', 'card' => 'Card', 'comment' => 'Comment', 'tax_invoice' => 'SALE RETURN'];
$t2 = [];
...
$party_name = $return['customer_name'] ?? '';
$party_ref = '';
$person_label = $t['salesman'];
$person_value = $return['sales_person'] ?? '';
$doc_comment = $return['comment'] ?? '';
$back_url = 'sale-return.php';
$document = $return;
$template = isset($_GET['template_preview']) ? trim((string)$_GET['template_preview']) : (function_exists('getInvoiceTemplateForDocument') ? getInvoiceTemplateForDocument('sale_return') : 'template_classic');
...
<head>
  ...
  <title>Sale Return - <?php echo htmlspecialchars($return_no); ?></title>
  <style>
