...
$print_settings_early = function_exists('getInvoicePrintSettingsForDocument') ? getInvoicePrintSettingsForDocument('purchase_quotation') : [];
...
if (isset($_GET['lang'])) {
    ...
    $allowed_langs = function_exists('getInvoicePrintAllowedLanguages') ? getInvoicePrintAllowedLanguages('purchase_quotation') : ['en'];
...
$items = getList("
    SELECT pqi.*, COALESCE(p.name, pqi.product_name) as product_name, c.name as category_name
    FROM tbl_purchase_quotation_items pqi
    LEFT JOIN tbl_products p ON pqi.product_id = p.id
    LEFT JOIN tbl_categories c ON p.category_id = c.id
    WHERE pqi.quotation_id = $quotation_id ORDER BY pqi.id ASC
");
...
$currency = !empty($quotation['currency']) ? trim((string) $quotation['currency']) : 'AED';
$currency_symbol = invoice_print_currency_symbol(isset($conn) ? $conn : null, $currency);

$quotation_date_raw = !empty($quotation['quotation_date']) ? strtotime($quotation['quotation_date']) : time();
$quotation_date = date('d/m/Y', $quotation_date_raw);
$quotation_time = date('h:i A', $quotation_date_raw);
...
$print_settings = function_exists('getInvoicePrintSettingsForDocument') ? getInvoicePrintSettingsForDocument('purchase_quotation') : [];
...
$discount_amt = (float)($quotation['discount_amt'] ?? 0);
$additional_amt = (float)($quotation['additional_amt'] ?? 0);
$tax_amount = 0;
foreach ($items as $item) $tax_amount += (float)($item['tax_amount'] ?? 0);
$grand_total = (float)($quotation['grand_total'] ?? 0);
$paid_amt = (float)($quotation['paid_amt'] ?? 0);
$balance_amt = (float)($quotation['balance_amt'] ?? 0);
$advance_amt = (float)($quotation['advance_payment'] ?? 0);
...
foreach ($items as $item) {
    ...
    $stone_wt = (float)($item['diamond_weight'] ?? $item['gemstone_weight'] ?? $item['stone_weight'] ?? $item['carat'] ?? 0);
    $total_stone_weight += $stone_wt;
    $total_net_wt += (float)($item['net_weight'] ?? $item['final_weight'] ?? 0);
}

$customer_ref = $quotation['ref_no'] ?? ($quotation['supplier_id'] ?? '');
if (empty($customer_ref) && !empty($quotation['supplier_id'])) {
    $supp = @getRecord("SELECT ref_no, mobile_no FROM tbl_customers WHERE id = " . (int)$quotation['supplier_id'] . " LIMIT 1");
    if ($supp) $customer_ref = $supp['ref_no'] ?? $supp['mobile_no'] ?? $quotation['supplier_id'];
    if (empty($customer_ref)) $customer_ref = $quotation['supplier_id'];
}

$L = [
    'en' => [
        'tax_invoice' => 'PURCHASE QUOTATION', 'invoice_no' => 'Quotation No.', 'date' => 'Date', 'salesman' => 'Purchase Person',
    ...
    'hi' => [
        'tax_invoice' => 'खरीद उद्धरण', 'invoice_no' => 'उद्धरण सं.', 'date' => 'तारीख', 'salesman' => 'खरीद व्यक्ति',
    ...
    'mr' => [
        'tax_invoice' => 'खरेदी अंदाज', 'invoice_no' => 'अंदाज क्र.', 'date' => 'तारीख', 'salesman' => 'खरेदी व्यक्ती',
    ...
    'ar' => [
        'tax_invoice' => 'عرض سعر الشراء', 'invoice_no' => 'رقم العرض', 'date' => 'التاريخ', 'salesman' => 'مسؤول الشراء',
        'mr_ms' => 'السيد / السيدة', 'customer_no' => 'رقم المورد', 'passport_id' => 'رقم الجواز', 'phone_no' => 'رقم التلفون',
...
function numberToWordsQuotation($num, $currency = 'AED') {
...
$amount_words = numberToWordsQuotation($grand_total, $currency);
...
if (!isset($t['invoice_no'])) $t['invoice_no'] = 'Quotation No.';
...
$doc_time = $quotation_time;
$party_name = $quotation['supplier_name'] ?? '';
$party_ref = $customer_ref;
$person_label = $t['salesman'];
$person_value = $quotation['purchase_person'] ?? '';
$doc_comment = $quotation['comment'] ?? '';
$doc_title = !empty($print_settings['invoice_title']) ? $print_settings['invoice_title'] : $t['tax_invoice'];
$back_url = 'purchase-quotation.php';
$document = $quotation;

$template = isset($_GET['template_preview']) ? trim((string)$_GET['template_preview']) : (function_exists('getInvoiceTemplateForDocument') ? getInvoiceTemplateForDocument('purchase_quotation') : 'template_classic');
...
<head>
  ...
  <title><?php echo $t['tax_invoice']; ?> - <?php echo htmlspecialchars($quotation['quotation_no']); ?></title>
  <style>
