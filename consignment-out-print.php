...
$c = getRecord("SELECT * FROM tbl_consignment_out WHERE id = $consignment_id");
if (!$c) {
    die('Consignment Out not found');
}

$items = getList("SELECT * FROM tbl_consignment_out_items WHERE consignment_id = $consignment_id ORDER BY id ASC");
$currency = !empty($c['currency']) ? htmlspecialchars($c['currency']) : 'AED';
$cdate = !empty($c['consignment_date']) ? $c['consignment_date'] : '';
$title = defined('COMPANY_NAME') ? COMPANY_NAME : (isset($Proj_Title) ? $Proj_Title : 'Consignment Out');
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
    <title><?php echo htmlspecialchars($title); ?> — Consignment Out <?php echo htmlspecialchars($c['consignment_no'] ?? ''); ?></title>
    <style>
