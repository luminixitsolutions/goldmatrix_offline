...
$print_settings_early = function_exists('getInvoicePrintSettingsForDocument') ? getInvoicePrintSettingsForDocument('sale_quotation') : [];
...
if (isset($_GET['lang'])) {
    ...
    $allowed_langs = function_exists('getInvoicePrintAllowedLanguages') ? getInvoicePrintAllowedLanguages('sale_quotation') : ['en'];
...
$items = getList("
    SELECT sqi.*, COALESCE(p.name, sqi.product_name) as product_name, c.name as category_name
    FROM tbl_sale_quotation_items sqi
    LEFT JOIN tbl_products p ON sqi.product_id = p.id
    LEFT JOIN tbl_categories c ON p.category_id = c.id
    WHERE sqi.quotation_id = $quotation_id ORDER BY sqi.id ASC
");
...
$print_settings = function_exists('getInvoicePrintSettingsForDocument') ? getInvoicePrintSettingsForDocument('sale_quotation') : [];
...
$customer_ref = $quotation['ref_no'] ?? ($quotation['customer_id'] ?? '');
if (empty($customer_ref) && !empty($quotation['customer_id'])) {
    $cust = getRecord("SELECT ref_no, mobile_no FROM tbl_customers WHERE id = " . (int)$quotation['customer_id'] . " LIMIT 1");
    $customer_ref = $cust['ref_no'] ?? $cust['mobile_no'] ?? $quotation['customer_id'];
}

$L = [
    'en' => [
        'tax_invoice' => 'SALE QUOTATION', 'invoice_no' => 'Quotation No.', 'date' => 'Date', 'salesman' => 'Salesman',
    ...
    'hi' => [
        'tax_invoice' => 'बिक्री उद्धरण', 'invoice_no' => 'उद्धरण सं.', 'date' => 'तारीख', 'salesman' => 'विक्रेता',
    ...
    'mr' => [
        'tax_invoice' => 'विक्री अंदाज', 'invoice_no' => 'अंदाज क्र.', 'date' => 'तारीख', 'salesman' => 'विक्रेता',
    ...
    'ar' => [
        'tax_invoice' => 'عرض سعر البيع', 'invoice_no' => 'رقم العرض', 'date' => 'التاريخ', 'salesman' => 'البائع',
...
if (!isset($t['invoice_no'])) $t['invoice_no'] = isset($t['quotation_no']) ? $t['quotation_no'] : 'No.';
...
$person_value = $quotation['sales_person'] ?? '';
$doc_comment = $quotation['comment'] ?? '';
$doc_title = !empty($print_settings['invoice_title']) ? $print_settings['invoice_title'] : $t['tax_invoice'];
$back_url = 'sale-quotation.php';
$document = $quotation;

$template = isset($_GET['template_preview']) ? trim((string)$_GET['template_preview']) : (function_exists('getInvoiceTemplateForDocument') ? getInvoiceTemplateForDocument('sale_quotation') : 'template_classic');
