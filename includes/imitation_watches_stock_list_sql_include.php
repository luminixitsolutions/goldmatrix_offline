...
/**
 * Imitation / Watches stock list — shared helpers + list SQL (no LIMIT).
...
$scope_metals = getList("SELECT id, display_name AS name FROM tbl_metal WHERE status = 1 AND display_name = 'Imitation Or Watches' ORDER BY display_name ASC, id ASC");
...
if (!$has_stock_journal) {
    $imitation_watches_stock_list_sql = "
    SELECT
        s.id AS stock_id,
        s.barcode,
        bal.bal_qty AS current_qty,
