...
/**
 * Diamond stock — Excel export row shaping + fetch (same display rules as diamond-stock.php).
 */
require_once __DIR__ . '/platinum_stock_gas_url_helpers.php';

/** @return list<string> */
function auragold_diamond_stock_export_column_keys(): array
{
    return [
        'imageUrls',
        'info',
...
function auragold_diamond_stock_export_column_label_map(): array
{
    return [
...
/** @return list<string> */
function auragold_diamond_stock_export_header_titles(): array
{
    $map = auragold_diamond_stock_export_column_label_map();
    $titles = [];
    foreach (auragold_diamond_stock_export_column_keys() as $k) {
        $titles[] = $map[$k] ?? $k;
    }
...
function auragold_diamond_stock_export_first_image_url(array $r, $siteUrl): string
{
 ...
 */
function auragold_diamond_stock_export_row_scalars(array $r, $siteUrl): array
{
    ...
    return [
        'imageUrls' => auragold_diamond_stock_export_first_image_url($r, $siteUrl),
        'info' => $info,
...
function auragold_diamond_stock_export_grand_initial(): array
{
    return [
        'weight' => 0.0,
        'gross_wt' => 0.0,
 ...
 */
function auragold_diamond_stock_export_accumulate_grand(array $grand, float $wps, int $wpc, array $scalars): array
{
    $g = $grand;
...
function auragold_diamond_stock_export_fetch_rows(mysqli $conn, int $limit = 15000): array
{
    require_once __DIR__ . '/diamond_stock_list_sql_include.php';
    if (!$has_stock) {
        return ['rows' => [], 'error' => 'Stock table not found.'];
    }
    $limit = max(1, min(50000, $limit));
    $sql = $diamond_stock_list_sql . ' LIMIT ' . $limit;
    $rows = [];
...
/** @return list<string> */
function auragold_diamond_stock_export_numeric_keys(): array
{
    return [
