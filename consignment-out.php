...
$auragold_voucher_runtime_client = auragold_voucher_runtime_bootstrap($conn, $metals, 'Consignment Out');
...
// Next consignment no: Bill Series for voucher "Consignment Out" (tbl_bill_series + tbl_voucher_types); else legacy CO-1
$next_order_no = function_exists('getNextConsignmentOutNo') ? getNextConsignmentOutNo($conn) : 'CO-1';

// Sales person dropdown — branch-scoped (stored in tbl_consignment_out.sales_person)
$sales_person_users = auragold_sales_person_user_display_names($conn_master);

// Load consignment for editing (tbl_consignment_out / tbl_consignment_out_items)
...
if (!empty($edit_order_id)) {
    $edit_order = getRecord("SELECT * FROM tbl_consignment_out WHERE id = " . intval($edit_order_id));
    if ($edit_order) {
        $edit_items = getList("
            ...
            FROM tbl_consignment_out_items i
            LEFT JOIN tbl_product_characteristics pc ON pc.id = i.product_characteristic_id AND pc.product_id = i.product_id
            WHERE i.consignment_id = " . intval($edit_order_id) . " ORDER BY i.id ASC");
        $edit_payments = [];
        $next_order_no = $edit_order['consignment_no'] ?? '';
        // Map to supplier-style keys used by populateOrderForm / previous-balance
        $edit_order['invoice_no'] = $edit_order['consignment_no'] ?? '';
        $edit_order['order_no'] = $edit_order['consignment_no'] ?? '';
        $edit_order['invoice_date'] = $edit_order['consignment_date'] ?? '';
        $edit_order['supplier_name'] = $edit_order['customer_name'] ?? '';
        $edit_order['customer_name'] = $edit_order['customer_name'] ?? '';
        $edit_order['supplier_id'] = (int)($edit_order['customer_id'] ?? 0);
        $edit_order['customer_id'] = (int)($edit_order['customer_id'] ?? 0);
        ...
        $edit_order['subtotal'] = isset($edit_order['gross_total']) ? (float)$edit_order['gross_total'] : 0;
        $edit_order['net_total'] = isset($edit_order['gross_total']) ? (float)$edit_order['gross_total'] : 0;
        $edit_order['discount_amt'] = isset($edit_order['discount_amount']) ? (float)$edit_order['discount_amount'] : 0;
        $edit_order['paid_amt'] = 0;
        $edit_order['balance_amt'] = 0;
        $edit_order['metal_amt'] = 0;
        $edit_order['round_off'] = 0;
        $edit_order['previous_diamond'] = 0;
        $edit_order['previous_gemstone'] = 0;
        $edit_order['layaways_id'] = '';
        $edit_order['due_date'] = '';
        $edit_order['payment_comments'] = '[]';
    }
}

$pi_save_blocked_by_sale_fixing = false;

// Allowed print languages (English + optional one from Invoice Print Settings)
                ...
                <div class="container-fluid flex-grow-1" style="padding-top: 0; padding-bottom: 0;">
                    ...
                    <div class="row invoice-content-row">
                        ...
                        <div class="col-lg-8" >
                                        ...
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label>Search Consignment Out</label>
                                                    <div style="position: relative;">
                                                        <input type="text" class="form-control form-control-sm" id="searchSaleInvoice" placeholder="Search by party name or consignment number..." autocomplete="off" style="padding-right: 35px;">
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control form-control-sm" id="againstOf">
                                                    ...
                                                    <option value="Consignment In">Consignment In</option>
                                                </select>
                                                <input type="hidden" id="againstCiId" value="">
                                            </div>
                                        </div>
                                        <div class="col-md-3">
                                            <div class="form-group">
                            ...
                            <!-- Payment UI hidden on Consignment Out; comments below stay visible -->
                            <div class="card mb-4">
                                <div class="card-body">
                                    <div id="consignmentOutPaymentBlock" class="consignment-out-payment-hidden" style="display: none !important;" aria-hidden="true">
                            ...
                            <div class="summary-panel" >
                                ...
                                <div class="invoice-header" style="margin: -1rem -1rem 1rem -1rem; border-radius: 10px 10px 0 0; display: flex; align-items: center; justify-content: space-between;">
                                    <div style="display: flex; align-items: center; gap: 0.5rem; flex: 1;">
                                        <i class="feather icon-printer" id="printInvoiceIcon" style="font-size: 12px; color: #94a3b8; opacity: 0.5; cursor: not-allowed; pointer-events: none;" title="Save Consignment Out first to print"></i>
                                        <h5 class="mb-0" style="font-size: 0.9rem;">Consignment Out No: <span id="currentOrderNo"><?php echo htmlspecialchars($next_order_no); ?></span></h5>
                                    </div>
...
<!-- Against Of: pick Consignment In document for selected customer -->
<div class="modal fade" id="coAgainstCiModal" tabindex="-1" role="dialog" aria-labelledby="coAgainstCiModalTitle" aria-hidden="true" data-backdrop="static">
    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            <div class="modal-header" style="background: #f8fafc; border-bottom: 1px solid #e2e8f0;">
                <h5 class="modal-title" id="coAgainstCiModalTitle">Consignment In</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <p class="text-muted small mb-2" id="coAgainstCiModalHint">Showing Consignment In documents for the selected customer only.</p>
                <div class="form-group mb-3">
                    <input type="text" class="form-control" id="coAgainstCiSearch" placeholder="Search by consignment number..." autocomplete="off">
                </div>
                <div class="table-responsive" id="coAgainstCiModalTableWrap" style="max-height: min(65vh, 560px); overflow-y: auto;">
                    <table class="table table-sm table-hover table-bordered mb-0">
                        <thead class="thead-light" style="position: sticky; top: 0; z-index: 1; background-color: #1e3a5f;">
                            <tr>
                                ...
                                <th style="color: #fff;">Invoice Date</th>
                        ...
                        <tbody id="coAgainstCiModalTbody">
                ...
                <div class="mt-3 text-right">
                    ...
                    <button type="button" class="btn btn-primary" id="coAgainstCiDoneBtn">Done</button>
                </div>
            </div>
        </div>
    </div>
...
<script src="assets/js/product-list-table-shared.js?v=<?php echo @filemtime(__DIR__ . '/assets/js/product-list-table-shared.js') ?: '1'; ?>"></script>
...
<script>
    ...
    $(document).ready(function() {
        ...
        // ================== AGAINST OF: Consignment In (modal picker) ==================
        var coAgainstCiSearchTimeout = null;
        function coClearAgainstCiId() {
            var el = document.getElementById('againstCiId');
            if (el) el.value = '';
        }
        function coLoadConsignmentInListForModal() {
            var tbody = document.getElementById('coAgainstCiModalTbody');
            if (!tbody) return;
            var cid = document.getElementById('customerId') ? String(document.getElementById('customerId').value || '').trim() : '';
            var q = document.getElementById('coAgainstCiSearch') ? document.getElementById('coAgainstCiSearch').value.trim() : '';
            if (!cid) {
            ...
            var url = 'ajax/list-quotation-against-documents.php?type=' + encodeURIComponent('Consignment In') + '&customer_id=' + encodeURIComponent(cid) + (q ? '&q=' + encodeURIComponent(q) : '');
            fetch(url)
                ...
                .then(function(data) {
                    ...
                    if (orders.length === 0) {
                        tbody.innerHTML = '<tr><td colspan="10" class="text-center text-muted py-4">No Consignment In records for this customer.</td></tr>';
                        return;
                    }
                    var html = '';
                    orders.forEach(function(o) {
                        ...
                        html += '<tr class="co-against-ci-row">' +
                            '<td style="text-align: center;"><input type="radio" name="coAgainstCiRadio" value="' + (o.id) + '" data-doc-no="' + String(invNo).replace(/&/g, '&amp;').replace(/"/g, '&quot;').replace(/</g, '&lt;') + '"></td>' +
                            '<td>' + invNo + '</td>' +
                            '<td>' + item + '</td>' +
                            '<td>' + invDate + '</td>' +
                            '<td>' + dueDate + '</td>' +
        ...
        $('#coAgainstCiModal').on('hidden.bs.modal', function() {
            var aid = document.getElementById('againstCiId');
            if (aid && !String(aid.value || '').trim() && $('#againstOf').length) {
                $('#againstOf').val('');
            }
        });
        $('#coAgainstCiModal').on('show.bs.modal', function() {
            var el = document.getElementById('coAgainstCiSearch');
            if (el) el.value = '';
            coLoadConsignmentInListForModal();
        });
        $(document).on('input', '#coAgainstCiSearch', function() {
            clearTimeout(coAgainstCiSearchTimeout);
            coAgainstCiSearchTimeout = setTimeout(coLoadConsignmentInListForModal, 300);
        });
        $('#coAgainstCiDoneBtn').on('click', function() {
            var radio = document.querySelector('input[name="coAgainstCiRadio"]:checked');
            if (!radio || !radio.value) {
                alert('Please select a Consignment In document.');
                return;
            }
            var selectedId = radio.value;
            var docNo = radio.getAttribute('data-doc-no') || '';
            var url = 'ajax/get-order-items-by-type.php?order_id=' + encodeURIComponent(selectedId) + '&type=' + encodeURIComponent('Consignment In');
            fetch(url)
                .then(function(r) { return r.json(); })
                .then(function(data) {
                    if (data.status === 'error') {
                    ...
                    if (!productTableBody) { $('#coAgainstCiModal').modal('hide'); return; }
                    ...
                    if (items.length > 0 && typeof savedItemToModalRowData === 'function') {
                        ...
                        var rowGroups = typeof partitionPurchaseInvoiceItemsForProductRows === 'function'
                            ? partitionPurchaseInvoiceItemsForProductRows(itemList)
                        ...
                        var added = productTableBody.querySelectorAll('tr:not(.no-drag)').length > 0;
                    ...
                    var againstCiEl = document.getElementById('againstCiId');
                    if (againstCiEl) againstCiEl.value = selectedId;
        ...
        $('#againstOf').on('change', function() {
            ...
            var titleEl = document.getElementById('coAgainstCiModalTitle');
            if (titleEl) titleEl.textContent = 'Consignment In';
            $('#coAgainstCiModal').modal('show');
        });
        
        // ================== CONSIGNMENT OUT SEARCH ==================
        ...
        saleInvoiceSearchInput.on('input', function() {
            ...
            saleInvoiceSearchTimeout = setTimeout(function() {
                $.ajax({
                    url: 'ajax/search-consignment-out.php',
                    type: 'GET',
                    data: { q: searchTerm },
                    dataType: 'json',
                    success: function(response) {
                        if (response.status === 'success' && response.invoices && response.invoices.length > 0) {
                            ...
                            saleInvoiceSuggestions.html('<div style="padding: 0.75rem; color: #94a3b8; text-align: center;">No Consignment Out documents found</div>').show();
                        }
                    },
                    error: function() {
                        saleInvoiceSuggestions.hide();
        ...
        $(document).on('click', '#saleInvoiceSuggestions .invoice-suggestion-item', function() {
            ...
            saleInvoiceSuggestions.hide();
            if (invoiceId > 0) {
                window.location.href = 'consignment-out.php?id=' + invoiceId;
            }
        });
...
<td data-column="sale-amount-with"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" readonly style="width: 130px; font-size: 0.7rem;"></td>
    ...
    function selectProduct(row, closeModal = false) {
        ...
        console.log('Extracted modal row data:', modalRowData);
            ...
            addProductToTableFromModalRow(modalRowData, (typeof currentMetalId !== 'undefined' && currentMetalId !== null && String(currentMetalId).trim() !== '') ? currentMetalId : null);
                        if (closeModal) {
                            hideProductModal();
                        }
                    }
    ...
    function addProductToTableFromModalRow(modalRowData, metalIdForImage) {
        ...
        try {
            ...
            if (groupImageAttr) row.setAttribute('data-group-image', groupImageAttr);
            else row.removeAttribute('data-group-image');
    ...
    function addMergedProductsToTable(modalRowsData, metalIdForImage) {
        ...
        if (merged.barcode) row.setAttribute('data-barcode', String(merged.barcode).trim());
        ...
        var firstRowGi = '';
        for (var _coMix = 0; _coMix < modalRowsData.length; _coMix++) {
            var rgi = modalRowsData[_coMix].group_image;
            if (rgi && String(rgi).trim()) {
                firstRowGi = rgi;
                break;
            }
        }
    ...
    function updateMergedRowFromModalRows(rowId, modalRowsData) {
        ...
        if (!groupImagePayload) {
            for (var _coE = 0; _coE < modalRowsData.length; _coE++) {
                var rawGi = modalRowsData[_coE].group_image;
                if (rawGi && String(rawGi).trim()) {
                    groupImagePayload = (typeof window.auragoldCoParseGroupImageAttr === 'function') ? window.auragoldCoParseGroupImageAttr(rawGi) : '';
                    break;
                }
            }
        }
        var bootPh = window.PRODUCT_LIST_TABLE_BOOT || {};
        var noImgPh = bootPh.noImageSrc || 'no_image.jpg';
        if (groupImagePayload) {
            row.setAttribute('data-group-image', (typeof groupImagePayload === 'object' && groupImagePayload != null) ? JSON.stringify(groupImagePayload) : String(groupImagePayload));
        } else {
        ...
        if (photoCell) {
            ...
            var primaryUrl = typeof getGroupImagePrimary === 'function' ? getGroupImagePrimary(groupImagePayload) : (typeof groupImagePayload === 'string' ? groupImagePayload : '');
            if (!primaryUrl) primaryUrl = noImgPh;
            if (img) {
                img.src = primaryUrl;
                img.style.display = '';
                if (primaryUrl !== noImgPh) img.onerror = function() { this.onerror = null; this.src = noImgPh; };
    ...
    function updateProductListRowFromModalRow(productListRowId, modalRow) {
        ...
        (function syncPhotoFromModalToProductListCo() {
            var boot = window.PRODUCT_LIST_TABLE_BOOT || {};
            var noImg = boot.noImageSrc || 'no_image.jpg';
            var mPhoto = modalRow.querySelector('[data-column="photo"]');
            var pPhoto = productListRow.querySelector('[data-column="photo"]');
            if (!pPhoto) return;
            var gi = modalRow.getAttribute('data-group-image') || '';
            if (gi) productListRow.setAttribute('data-group-image', gi);
            else productListRow.removeAttribute('data-group-image');
            var mImg = mPhoto ? mPhoto.querySelector('.product-photo-thumb') : null;
            var src = '';
            if (mImg && mImg.style.display !== 'none' && mImg.src) src = mImg.src;
            if (!src && gi) {
                var parsed = (typeof window.auragoldCoParseGroupImageAttr === 'function') ? window.auragoldCoParseGroupImageAttr(gi) : '';
                src = (typeof getGroupImagePrimary === 'function') ? getGroupImagePrimary(parsed) : (typeof parsed === 'string' ? parsed : '');
            }
            if (!src) src = noImg;
            var pImg = pPhoto.querySelector('img');
            var pPh = pPhoto.querySelector('.text-muted');
            if (pImg) {
                pImg.src = src;
                pImg.style.display = '';
                if (src !== noImg) pImg.onerror = function() { this.onerror = null; this.src = noImg; };
                else pImg.onerror = null;
            }
            if (pPh) pPh.style.display = 'none';
        })();
        
        var modalBcInpUp = modalRow.querySelector('[data-column="barcode"] input');
        var modalBcUp = modalBcInpUp ? String(modalBcInpUp.value || '').trim() : '';
        if (modalBcUp) {
            productListRow.setAttribute('data-barcode', modalBcUp);
            var bcSpanUp = productListRow.querySelector('[data-column="barcode"] span');
            if (bcSpanUp) bcSpanUp.textContent = modalBcUp;
        }
        if (typeof window.auragoldRefreshProductTableRowPhotoFromJournal === 'function') window.auragoldRefreshProductTableRowPhotoFromJournal(productListRow);
    ...
    function applyProductModalGroupImageToPhotoColumns(dataUrl, tabKey) {
        ...
        photoCells.forEach(function(cell) {
            ...
            if (row && row.getAttribute('data-journal-photo') === '1') return;
    ...
    function saveOrder() {
        ...
        const currentOrderNoText = document.getElementById('currentOrderNo')?.textContent || <?php echo json_encode(isset($next_order_no) ? $next_order_no : 'CO-1'); ?>;
        ...
        orderData.payments = (typeof collectPosPaymentsForSave === 'function') ? collectPosPaymentsForSave() : [];

        // Consignment Out master fields (maps to save-consignment-out.php / tbl_consignment_out)
        orderData.consignment_id = orderData.order_id;
        orderData.consignment_no = orderData.order_no;
        orderData.consignment_date = orderData.order_date;
        orderData.gross_total = orderData.subtotal;
        orderData.discount_amount = orderData.discount_amt;
        var __lineTaxSum = 0;
        if (orderData.items && orderData.items.length) {
            orderData.items.forEach(function (it) {
                __lineTaxSum += parseFloat(it.tax) || 0;
            });
        }
        orderData.tax_amount = __lineTaxSum;
        ...
        if (typeof jQuery !== 'undefined' && jQuery.ajax) {
            jQuery.ajax({
                url: 'ajax/save-consignment-out.php',
                type: 'POST',
                data: postData,
                dataType: 'json',
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        var savedId = response.consignment_id || response.invoice_id || response.order_id;
                        if (savedId) {
                            window.pendingRedirectUrl = 'consignment-out.php';
                            setTimeout(function() {
                                showPrintInvoiceModal(savedId);
                            }, 100);
                        } else {
                            alert('Consignment Out saved successfully! No.: ' + (response.consignment_no || response.order_no || 'N/A'));
                            window.location.href = 'consignment-out.php';
                        }
                    } else {
                        alert('Error: ' + (response.message || 'Failed to save Consignment Out'));
                    }
                },
            ...
            fetch('ajax/save-consignment-out.php', {
                method: 'POST',
                body: formData
            })
            .then(response => response.json())
            .then(data => {
                ...
                if (data.status === 'success') {
                    var sid = data.consignment_id || data.invoice_id || data.order_id;
                    if (sid) {
                        window.pendingRedirectUrl = 'consignment-out.php';
                        setTimeout(function() {
                            showPrintInvoiceModal(sid);
                        }, 100);
                    } else {
                        alert('Consignment Out saved successfully! No.: ' + (data.consignment_no || data.order_no || 'N/A'));
                        window.location.href = 'consignment-out.php';
                    }
                } else {
                    alert('Error: ' + (data.message || 'Failed to save Consignment Out'));
                }
            })
            .catch(error => {
                // Reset saving flag
    ...
    // Reset order form
    function resetOrder() {
        if (confirm('Are you sure you want to create a new Consignment Out? All unsaved data will be lost.')) {
            window.location.href = 'consignment-out.php';
        }
    }
    ...
    function printPurchaseInvoice(invoiceId) {
        ...
        if (!invoiceId || invoiceId <= 0) {
            alert('Please save the Consignment Out first before printing.');
            return;
        }
        
        // Open print page in new window
        window.open('consignment-out-print.php?id=' + invoiceId, '_blank', 'width=1200,height=800');
    }
    ...
    function updatePrintIcon(invoiceId) {
        ...
        if (printIcon) {
            if (invoiceId && invoiceId > 0) {
                ...
                printIcon.setAttribute('title', 'Print Consignment Out');
            } else {
                ...
                printIcon.setAttribute('title', 'Save Consignment Out first to print');
            }
        }
    }
    ...
    function loadOrder(orderId) {
        ...
        var getUrl = 'ajax/get-consignment-out.php?id=' + orderId + '&_=' + (Date.now ? Date.now() : '');
        if (typeof jQuery !== 'undefined' && jQuery.ajax) {
            jQuery.ajax({
                url: getUrl,
                type: 'GET',
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        window.history.pushState({}, '', 'consignment-out.php?id=' + orderId);
    ...
    function populateOrderForm(order, items, loadedPayments) {
        ...
        if (document.getElementById('againstOf')) {
            ...
            var hasOption = false;
            for (var ai = 0; ai < sel.options.length; ai++) {
                if (sel.options[ai].value === againstVal) { hasOption = true; break; }
            }
        ...
        var againstCiElPop = document.getElementById('againstCiId');
        if (againstCiElPop) againstCiElPop.value = '';
        ...
        if (itemList.length > 0) {
            ...
            try {
                ...
                // Restore group_image on first row when saved; otherwise show placeholder image
                var firstRow = productTableBody && productTableBody.querySelector('tr:not(.no-drag)');
                var firstItem = itemList[0];
                var bootL = window.PRODUCT_LIST_TABLE_BOOT || {};
                var noImgL = bootL.noImageSrc || 'no_image.jpg';
                if (firstRow) {
                    var photoCell0 = firstRow.querySelector('[data-column="photo"]');
                    if (photoCell0) {
                        var img0 = photoCell0.querySelector('img');
                        var ph0 = photoCell0.querySelector('.product-photo-placeholder') || photoCell0.querySelector('.text-muted');
                        var primary0 = '';
                        if (firstItem && firstItem.group_image) {
                            firstRow.setAttribute('data-group-image', firstItem.group_image);
                            var payload0 = firstItem.group_image;
                            if (typeof payload0 === 'string' && payload0.trim().startsWith('{')) { try { payload0 = JSON.parse(payload0); } catch (e0) {} }
                            primary0 = typeof getGroupImagePrimary === 'function' ? getGroupImagePrimary(payload0) : (typeof payload0 === 'string' ? payload0 : '');
                        } else {
                            firstRow.removeAttribute('data-group-image');
                        }
                        if (!primary0) primary0 = noImgL;
                        if (img0) {
                            img0.src = primary0;
                            img0.style.display = '';
                            if (primary0 !== noImgL) img0.onerror = function() { this.onerror = null; this.src = noImgL; };
                            else img0.onerror = null;
                        }
        ...
        var _pfClearCo = document.getElementById('paymentTableFooter');
        if (_pfClearCo) _pfClearCo.style.display = 'none';
        ...
        if (loadedPayments && loadedPayments.length > 0) {
            loadedPayments.forEach(function(paymentRow) {
                ...
                var _prevBalLoadCo = parseFloat(payment.previous_balance_amount || 0) || 0;
                var _dbTotalLoadCo = parseFloat(payment.amount || 0) || 0;
                var _dbCurLoadCo = payment.current_order_amount != null && payment.current_order_amount !== '' ? parseFloat(payment.current_order_amount) : NaN;
                var _curOrderLoadCo = !isNaN(_dbCurLoadCo) ? _dbCurLoadCo : Math.max(0, _dbTotalLoadCo - _prevBalLoadCo);
                const paymentData = {
                    ...
                    amount: _curOrderLoadCo,
                    previous_balance_amount: _prevBalLoadCo,
                ...
                if (paymentType === 'metal-exchange') {
                    ...
                    var meGrossCo = payment.metal_exchange_gross_wt != null && payment.metal_exchange_gross_wt !== '' ? String(payment.metal_exchange_gross_wt) : (payment.gross_weight != null && payment.gross_weight !== '' ? String(payment.gross_weight) : '');
                    if (!meGrossCo || meGrossCo === '0') {
                        var qMeCo = parseFloat(payment.quantity || 0);
                        if (qMeCo > 0) meGrossCo = String(qMeCo);
                    }
                    paymentData.metal_exchange_gross_wt = meGrossCo || '0';
                    ...
                    var mePureCo = payment.metal_exchange_purity_wt != null && payment.metal_exchange_purity_wt !== '' ? String(payment.metal_exchange_purity_wt) : (payment.purity_weight != null && payment.purity_weight !== '' ? String(payment.purity_weight) : '');
                    if (!mePureCo || mePureCo === '0') {
                        var gNumCo = parseFloat(paymentData.metal_exchange_gross_wt) || 0;
                        var pCaratCo = parseFloat(payment.purity_carat || payment.purity || 0) || 0;
                        if (gNumCo > 0 && pCaratCo > 0) {
                            if (pCaratCo <= 1) mePureCo = String(gNumCo * pCaratCo);
                            else if (pCaratCo <= 100) mePureCo = String(gNumCo * (pCaratCo / 100));
                            else mePureCo = String(gNumCo * (pCaratCo / 1000));
                        }
                    }
                    paymentData.metal_exchange_purity_wt = mePureCo || '0';
                }
            ...
            var _pfEmptyCo = document.getElementById('paymentTableFooter');
            if (_pfEmptyCo) _pfEmptyCo.style.display = 'none';
        }
    ...
    (function() {
        ...
        function doPopulateFromEmbed() {
            ...
            try {
                ...
                if (window.history && window.history.replaceState) window.history.replaceState({}, '', 'consignment-out.php?id=' + editId);
                window.isPurchaseInvoiceEditMode = true;
                return true;
            } catch (e) {
...
<!-- Print Consignment Out confirmation modal -->
<div id="printInvoiceModal" class="print-invoice-modal" style="display: none;">
    <div class="print-invoice-modal-content">
        <button class="print-invoice-modal-close" onclick="closePrintInvoiceModal()" type="button" aria-label="Close">&times;</button>
        ...
        <h3 class="print-invoice-modal-title">Print Consignment Out</h3>
        <p class="print-invoice-modal-message">Do you want to print this Consignment Out document?</p>
...
<script>
    ...
    function showPrintInvoiceModal(invoiceId) {
        ...
        setTimeout(function() {
            ...
            if (modal) {
                ...
                if (confirm('Consignment Out saved successfully! Do you want to print?')) {
                    window.open('consignment-out-print.php?id=' + invoiceId, '_blank', 'width=1200,height=800');
                }
    ...
    function confirmPrintInvoice() {
        if (savedInvoiceId) {
            window.open('consignment-out-print.php?id=' + savedInvoiceId, '_blank', 'width=1200,height=800');
        }
