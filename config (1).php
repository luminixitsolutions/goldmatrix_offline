<?php
$project = "prod"; // for production set to "prod"
...
$auragold_clone_source_mysql_user = 'goldmatrix_gm';
$auragold_clone_source_mysql_pass = 'VC10obmN5vZx6jQBGVaK';
...
$__boot_user   = getenv('AURAGOLD_BOOTSTRAP_USER') ?: 'goldmatrix_gm';
$__boot_pass   = getenv('AURAGOLD_BOOTSTRAP_PASS') ?: 'VC10obmN5vZx6jQBGVaK';
...
$SiteUrl = "https://main.goldmatrixsoft.com/";
 ...
 * @return array{company_name:string,company_address:string,company_trn:string,company_phone:string,company_email:string}
 */
function auragold_resolve_invoice_print_company_header(array $print_settings, int $branch_id = 0, array $defaults = []): array {
    ...
    if ($branch_id > 0 && !empty($GLOBALS['conn_master']) && function_exists('getRecordMaster')) {
        $branch = getRecordMaster(
            'SELECT name, address, phone, email, gst_no, location_area FROM tbl_branches WHERE id = '
    ...
    if ($fromSettings('company_phone') !== '') {
        ...
        $out['company_phone'] = trim((string) $branch['phone']);
    }
...
function auragold_barcode_print_field_aliases(): array {
    return [
        ...
        'StoneWeight' => ['StoneWeight', 'stone_weight'],
        'TotalDiamond' => ['TotalDiamond', 'total_diamond', 'diamond_wt', 'diamond_ct', 'diamond_carat'],
...
function auragold_barcode_default_decimal_places(string $field): ?int {
    ...
    foreach (['GrossWt', 'LessWt', 'NetWt', 'FinalWt', 'PureWt', 'PurityWt', 'PktWt', 'PktLessWt', 'StoneWeight', 'MetalWeight', 'Purity', 'TotalDiamond', 'TotalStones'] as $wtField) {
...
function renderBarcodeLayout($productData, $settings) {
    ...
    /* Per-box 82×38 coords are already mm inside the 20×25 canvas — global label pad must not shift them. */
    if (!empty($settings['sticker_82x38_2box']) || (isset($settings['layout_type']) && $settings['layout_type'] === '82x38_2box')) {
    ...
    foreach ($layout as $el) {
        ...
        if ($type === 'barcode_image' || $type === 'qr_image') {
            ...
            if ($use_qr_graphics) {
                ...
                if ($hasDesignBarcodeText) {
                    $text_area_mm = 0.0;
                    $graphic_h_mm = max(auragold_82x38_min_scan_graphic_mm(), $container_h_mm);
                } else {
                ...
                $scanAttr = ($is_82x38_box && function_exists('auragold_is_valid_physical_scan_code') && auragold_is_valid_physical_scan_code($physical_scan_code))
                    ...
                    : '';
                $html .= '<svg class="' . htmlspecialchars($svgClass, ENT_QUOTES, 'UTF-8') . '" data-barcode="' . htmlspecialchars($barcode, ENT_QUOTES, 'UTF-8') . '"'
                    . $scanAttr
        ...
        if ($type === 'text') {
            ...
            $fieldZ = (!empty($settings['sticker_82x38_2box']) || (isset($settings['layout_type']) && $settings['layout_type'] === '82x38_2box')) ? 3 : 0;
            $is82x38Box = !empty($settings['sticker_82x38_2box'])
                || (isset($settings['layout_type']) && $settings['layout_type'] === '82x38_2box');
            $fieldClass = 'design-field';
            $fieldExtraStyle = '';
            if ($is82x38Box && strcasecmp($field, 'CompanyName') === 0) {
...
function auragold_82x38_2box_layout(array $snapshot = []): array {
    ...
    /* Legacy designer forced box2_top_mm=0, which clips the right QR on physical die-cut tags. */
    if ($b2T <= 0.0) {
...
function auragold_82x38_box_barcode_item(array $snapshot, int $boxNum, array $options = []): ?array {
    ...
    if (isset($snapshot[$key]) && is_array($snapshot[$key]) && isset($snapshot[$key]['left_mm'])) {
        ...
        /* Right tag sits near the die-cut top — keep a tiny vertical inset. */
...
function render82x38DesignStickerLabel(array $print_item, array $settings, array $decoded_snapshot = [], int $page_index = 0): string {
    ...
    /* Use the taller saved barcode height on both boxes so left/right scan parity. */
    ...
    /* Use nearly the full 20 mm tag width; quiet zones are part of the generated CODE128. */
    ...
    if (!isset($settings['render_code_as']) || $settings['render_code_as'] !== 'qr') {
        ...
        $probe1 = auragold_82x38_box_barcode_item($printSnapshot, 1, $probeOpts);
        $probe2 = auragold_82x38_box_barcode_item($printSnapshot, 2, $probeOpts);
        $topCandidates = [];
        if (is_array($probe1)) {
            $topCandidates[] = (float) ($probe1['top'] ?? 0);
        }
        if (is_array($probe2)) {
            $topCandidates[] = (float) ($probe2['top'] ?? 0);
        }
        if ($topCandidates !== []) {
            $sharedBarcodeTopMm = max(0.5, min($topCandidates));
        }
    }
    /* Keep per-box left offsets — do not force both to min(left) (clips left tag quiet zone). */
    foreach ($pairs as $num => $pair) {
        ...
        if ($barcodeItem !== null) {
            if ($sharedBarcodeTopMm !== null && empty($barcodeOpts['as_qr'])) {
                $barcodeItem['top'] = $sharedBarcodeTopMm;
            }
            $extra = [];
            ...
            /* Same flush-top QR repair when falling back to default items. */
            if ((isset($settings['render_code_as']) && $settings['render_code_as'] === 'qr')
            ...
            /* Design items may still carry barcode_image at top=0 when barcode1/2 keys missing. */
            if (isset($settings['render_code_as']) && $settings['render_code_as'] === 'qr') {
        ...
        $boxStyle = 'position:absolute;left:' . $pair['pos']['left'] . 'mm;top:' . $pair['pos']['top'] . 'mm;'
