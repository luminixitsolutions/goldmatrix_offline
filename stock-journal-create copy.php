                                ...
                                <div class="card-body product-entry" style="padding: 1.5rem;">
                                    ...
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        ...
                                        <div class="d-flex align-items-center" style="gap: 0.5rem;">
                                            ...
                                            <button type="button" class="btn btn-sm" id="sjExcelImportBtn" title="Upload .xlsx: stock saves under your current branch (working branch or login branch). Put picture URL, path, or base64 in the Photo column, or paste/embed the image in the sheet; Metal Qty, Weight, Purity, Loss, Design No, HUID, RFID, Barcode optional." style="background: #0f766e; color: #fff; border: none; padding: 0.45rem 0.9rem; border-radius: 6px; font-size: 0.8rem; font-weight: 600;">
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-bordered product-table">
                                            ...
                                            <tbody id="productTableBody">
                                                <tr class="no-drag">
                                                    <td colspan="38" class="text-center text-muted py-4" id="emptyRowCell">No Rows To Show</td>
                                                </tr>
                                            </tbody>
...
<script>
    ...
    (function initSjExcelImport() {
        ...
        inp.addEventListener('change', function() {
            ...
            btn.innerHTML = '<i class="feather icon-loader"></i> Importing...';
            fetch('ajax/import-stock-journal-excel.php', { method: 'POST', body: fd, credentials: 'same-origin' })
                ...
                .then(function(data) {
                    ...
                    if (data.status === 'success' || data.status === true) {
                        var n = data.imported_rows != null ? data.imported_rows : (data.saved_count || '');
                        alert('Imported ' + (n !== '' ? n + ' row(s). ' : '') + (data.message || 'Done'));
    ...
    function applyProductModalColumnVisibilityForTab(tabKey) {
        ...
        var by = window.productModalColumnVisibilityByTab;
        var prefs = by && (by[tk] || (tk === 'main' ? by[''] : null));
    ...
    async function addProductToTableFromModalRow(modalRowData) {
        ...
        var skipBarcodeMerge = typeof sjProductOpeningLockProductField === 'function' && sjProductOpeningLockProductField();
        ...
        row.setAttribute('data-making-rate', modalRowData.making_rate || '0');
        row.setAttribute('data-metal-id', modalRowData.metal_id || '');
        row.setAttribute('data-metal-name', (modalRowData.metal_name || '').trim());
        try {
            // Product opening (single SKU): always allocate a new serial per line. Else reuse modal barcode when set, or fetch next.
            let barcode = '';
            var forceNewBarcodeSerial = typeof sjProductOpeningLockProductField === 'function' && sjProductOpeningLockProductField();
            if (!forceNewBarcodeSerial && modalRowData.barcode && modalRowData.barcode.trim() !== '') {
                barcode = modalRowData.barcode.trim();
            } else {
                const prefix = modalRowData.barcode_prefix || 'RN';
    ...
    async function updateProductListRowFromModalRow(productListRowId, modalRow) {
        ...
        if (!barcode || barcode === '') {
            const prefix = modalRow.getAttribute('data-barcode-prefix') || '';
            const digit = parseInt(modalRow.getAttribute('data-barcode-digits'), 10) || 0;
            try {
                barcode = await getNextBarcodeFromServer(prefix, digit);
            } catch (err) {
                console.error('getNextBarcodeFromServer failed:', err);
    ...
    function calculateModalRowNetWeight(row) {
        ...
        // Parse basic values (UI "Weight" column is metal-weight; use it when gross-wt is empty)
        ...
        let isDiamondOrStone = false;
        if (categoryId && typeof categories !== 'undefined' && categories && Array.isArray(categories)) {
    ...
    async function handleStockJournalModalAddClick(ev) {
            ...
            if (currentEditingRowId) {
                ...
                if (selectedRow) {
                    try {
                        await updateProductListRowFromModalRow(currentEditingRowId, selectedRow);
    ...
    function addProductRowToSelectionTable(item, product) {
        ...
        const tbody = document.querySelector('#productSelectionModal #productListBody') || document.getElementById('productListBodyPage');
        if (!tbody) {
            ...
            productRows.forEach(row => {
                ...
                if (files && files.length) {
                    files.forEach((file, j) => {
                        formData.append('images_' + idx + '_' + j, file);
                    });
                }
                idx++;
            });
