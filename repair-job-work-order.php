...
// Sundry Debtors/Creditors options (Sundry Debtors first = default for new customers)
$sundry_options = [
    ['id' => 1, 'name' => 'Sundry Debtors'],
    ['id' => 2, 'name' => 'Sundry Creditors'],
];

// Repair Job Work Order: get repair_order_id from URL
$repair_order_id_param = isset($_GET['repair_order_id']) ? (int)$_GET['repair_order_id'] : 0;
$edit_order_id = 0;
$edit_order = null;
$edit_items = [];
$edit_payments = [];
$current_rjwo_id = 0;
$next_order_no = 'RJWO-1';
$repair_order_exists = false;
$duplicate_rjwo_message = '';

// Next RJWO number (from tbl_repair_jobwork_orders)
$rjwo_tbl = @mysqli_query($conn, "SHOW TABLES LIKE 'tbl_repair_jobwork_orders'");
if ($rjwo_tbl && mysqli_num_rows($rjwo_tbl) > 0) {
    mysqli_free_result($rjwo_tbl);
    $max_row = getRecord("SELECT MAX(id) AS max_id FROM tbl_repair_jobwork_orders");
    if ($max_row && isset($max_row['max_id']) && (int)$max_row['max_id'] >= 1) {
        $next_order_no = 'RJWO-' . ((int)$max_row['max_id'] + 1);
    }
}

// Edit mode: ?id=X where X is repair jobwork order id
$edit_rjwo_id = isset($_GET['id']) ? (int)$_GET['id'] : 0;
if ($edit_rjwo_id > 0) {
    $rjwo_row = getRecord("SELECT * FROM tbl_repair_jobwork_orders WHERE id = $edit_rjwo_id");
    if ($rjwo_row) {
        $current_rjwo_id = $edit_rjwo_id;
        $repair_order_id_param = (int)$rjwo_row['repair_order_id'];
        $edit_order = [
            'id' => $rjwo_row['id'],
            'order_no' => $rjwo_row['jobwork_no'],
            'repair_order_no' => $rjwo_row['repair_order_no'] ?? '',
            'customer_name' => $rjwo_row['customer_name'] ?? '',
        ...
        $rjwo_items = getList("SELECT * FROM tbl_repair_jobwork_order_items WHERE repair_jobwork_order_id = $edit_rjwo_id");
        foreach ($rjwo_items as $it) {
            $it['order_id'] = $it['repair_jobwork_order_id'];
            $edit_items[] = $it;
        }
        $edit_order_id = $edit_rjwo_id;
        $next_order_no = $rjwo_row['jobwork_no'];
    }
}

// Create mode: repair_order_id in URL - load repair order and items
if ($repair_order_id_param > 0 && $current_rjwo_id === 0) {
    $repair_order = getRecord("SELECT * FROM tbl_repair_orders WHERE id = $repair_order_id_param");
    if ($repair_order) {
        $repair_order_exists = true;
        $edit_order_id = $repair_order_id_param;
        $edit_order = $repair_order;
        $edit_order['ref_no'] = $repair_order['order_no'] ?? '';
        $edit_order['repair_order_no'] = $repair_order['order_no'] ?? '';
        $edit_items = getList("SELECT * FROM tbl_repair_order_items WHERE order_id = $repair_order_id_param ORDER BY id ASC");
        $edit_payments = [];
        // Prevent duplicate: check if repair jobwork already exists for this repair order
        $existing_rjwo = getRecord("SELECT id, jobwork_no FROM tbl_repair_jobwork_orders WHERE repair_order_id = $repair_order_id_param");
        if ($existing_rjwo) {
            $duplicate_rjwo_message = 'A Repair Job Work Order already exists for this repair order ('. ($existing_rjwo['jobwork_no'] ?? '') .').';
        }
    }
}

$is_jobwork_from_repair_order = ($repair_order_exists || $current_rjwo_id > 0);

// Departments + job worker (Name): same master list as jobwork-invoice.php
$jwo_departments = getList("SELECT id, dept_name AS name FROM tbl_departments WHERE status = 1 ORDER BY dept_name ASC");
if (!is_array($jwo_departments)) {
    $jwo_departments = [];
}
...
if ($ensure_jwo_dept_id > 0) {
    ...
    if (!$have_dept) {
        $extra_dept = getRecord('SELECT id, dept_name AS name FROM tbl_departments WHERE id = ' . (int) $ensure_jwo_dept_id . ' LIMIT 1');
        if ($extra_dept) {
...
$map_chk_rjwo = @mysqli_query($conn, "SHOW TABLES LIKE 'tbl_department_user_map'");
$cust_chk_rjwo = @mysqli_query($conn, "SHOW TABLES LIKE 'tbl_customers'");
if ($map_chk_rjwo && mysqli_num_rows($map_chk_rjwo) > 0 && $cust_chk_rjwo && mysqli_num_rows($cust_chk_rjwo) > 0) {
    $jwo_has_department_user_tables = true;
}
if ($map_chk_rjwo) {
    mysqli_free_result($map_chk_rjwo);
}
if ($cust_chk_rjwo) {
    mysqli_free_result($cust_chk_rjwo);
}
$jwo_job_worker_type_id = 0;
$jw_t_rjwo = @mysqli_query($conn, "SELECT id FROM tbl_customer_types WHERE LOWER(name) = 'job worker' AND status = 1 LIMIT 1");
if ($jw_t_rjwo && mysqli_num_rows($jw_t_rjwo) > 0) {
    $jw_tr_r = mysqli_fetch_assoc($jw_t_rjwo);
    $jwo_job_worker_type_id = (int) $jw_tr_r['id'];
    mysqli_free_result($jw_t_rjwo);
} elseif ($jw_t_rjwo) {
    mysqli_free_result($jw_t_rjwo);
}
...
if ($jwo_has_department_user_tables && $ensure_jwo_dept_id <= 0) {
    $jwo_department_users_initial = getList('SELECT c.id, c.name AS user_name FROM tbl_customers c WHERE c.status = 1 ORDER BY c.name ASC LIMIT 8000');
    if (!is_array($jwo_department_users_initial)) {
        $jwo_department_users_initial = [];
    }
}
...
// Repair Job Work Order: loaded via repair_order_id in URL (from repair-order-process.php)
?>
<!DOCTYPE html>
...
<style>
    ...
    #jwoDepartmentUserWrap { position: relative; z-index: 1080; }
    #jwoAddDepartmentUserBtn { position: relative; z-index: 1081; }
        ...
        <div class="layout-container">
            ...
            <div class="layout-content">
                ...
                <div class="container-fluid flex-grow-1" style="padding-top: 0; padding-bottom: 0;">
                    ...
                    <?php if ($repair_order_id_param > 0 && !$repair_order_exists && $current_rjwo_id === 0): ?>
                    <div class="alert alert-danger mx-3 mt-2" role="alert">Repair order not found. <a href="repair-order-process.php" class="alert-link">Return to Repair Order List</a></div>
                    <?php endif; ?>
                    <?php if (!empty($duplicate_rjwo_message)): ?>
                    <div class="alert alert-warning mx-3 mt-2" role="alert"><?php echo htmlspecialchars($duplicate_rjwo_message); ?> <a href="repair-order-process.php" class="alert-link">Return to Repair Order List</a></div>
                    <?php endif; ?>
                        ...
                        <div class="col-lg-8" >
                            ...
                            <div class="card mb-4">
                                <div class="card-body billing-form">
                                        <div class="row">
                                            <?php if ($is_jobwork_from_repair_order): ?>
                                            <div class="col-md-3">
                                                <div class="form-group">
                                                    <label>Department <span class="text-muted" style="font-weight: normal; font-size: 0.75rem;">(optional)</span></label>
                                            ...
                                            <div class="col-md-3">
                                                <div class="form-group">
                                                    <label>Name <span class="text-muted" style="font-weight: normal; font-size: 0.75rem;">(optional)</span></label>
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label>Search Repair Order</label>
                                                    <div style="position: relative;">
                                                        <input type="text" class="form-control form-control-sm" id="searchSaleOrder" placeholder="Type repair order no or customer name..." autocomplete="off" style="padding-right: 35px;">
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label><?php echo $is_jobwork_from_repair_order ? 'Customer Name' : 'Name *'; ?></label>
                                                    <div style="position: relative;">
                                                        <input type="text" class="form-control form-control-sm" id="customerName" placeholder="Enter customer name" <?php echo $is_jobwork_from_repair_order ? 'readonly' : ''; ?> required style="padding-right: 35px;" autocomplete="off">
                                        ...
                                        <div class="col-md-3">
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control form-control-sm" id="refNo" placeholder="Against reference" <?php echo $is_jobwork_from_repair_order ? 'readonly' : ''; ?>>
                                            </div>
                                        </div>
                                        <div class="col-md-3">
                                            <div class="form-group">
                                        ...
                                        <div class="table-settings-wrapper">
                                            ...
                                            <div class="table-settings-dropdown" id="tableSettingsDropdown">
                                                ...
                                                $productListColumns = [
                                                    ...
                                                    ['quantity','Quantity'],['carat','Carat'],['pkt-wt','Pkt. Wt.'],['pkt-less-wt','Pkt. Less Wt.'],['requested-purity','Requested Purity'],['requested','Requested'],['gross-wt','Gross Wt.'],['less-wt','Less Wt.'],['gold-loss1','Gold Loss 1'],['gold-loss2','Gold Loss 2'],['setting-charge','Setting Charge'],['net-wt','Net Wt.'],['purity','Purity'],['purity-wt','Purity Wt.'],['wastage-per','Wastage Per.'],['wastage-wt','Wastage Wt.'],['final-wt','Final Wt.'],['alloy-wt','Alloy Wt.'],
                            ...
                            <div class="summary-panel" >
                                ...
                                <div class="invoice-header" style="margin: -1rem -1rem 1rem -1rem; border-radius: 10px 10px 0 0; display: flex; align-items: center; justify-content: space-between;">
                                    <div style="display: flex; align-items: center; gap: 0.5rem; flex: 1;">
                                        ...
                                        <h5 class="mb-0" style="font-size: 0.9rem;"><?php echo $is_jobwork_from_repair_order ? 'Repair Job Work Order No:' : 'Repair Job Work Order No:'; ?> <span id="currentOrderNo"><?php echo htmlspecialchars($next_order_no); ?></span></h5>
                                    </div>
                                    <div class="invoice-header-actions">
                                        <?php if ($is_jobwork_from_repair_order): ?>
                                        <span id="jwoRepairOrderId" data-repair-order-id="<?php echo (int)$repair_order_id_param; ?>"></span>
                                        <span id="jwoCurrentId" data-jwo-id="<?php echo (int)$current_rjwo_id; ?>"></span>
                                        <button class="btn-save-invoice btn-sm" onclick="saveJobWorkOrder()">Save Job Work Order</button>
                                        <?php else: ?>
...
<!-- Add Image Modal (opens when clicking camera icon in Product Selection) -->
        ...
        <div class="modal-content">
            ...
            <div class="modal-body" style="padding: 1.25rem;">
                <div class="d-flex align-items-stretch" style="gap: 1rem;">
                    <div id="addImagePreviewWrap" style="flex: 1; min-height: 160px; border: 1px dashed #cbd5e1; border-radius: 8px; display: flex; align-items: center; justify-content: center; background: #f8fafc;">
                        ...
                        <img id="addImagePreviewImg" src="" alt="Preview" style="max-width: 100%; max-height: 200px; object-fit: contain; display: none; border-radius: 6px;">
                    </div>
                    <div id="addImageUploadZone" style="width: 100px; min-height: 160px; border: 2px dashed #94a3b8; border-radius: 8px; display: flex; align-items: center; justify-content: center; background: #f1f5f9; cursor: pointer; transition: background 0.2s;">
                        <input type="file" id="addImageModalFileInput" accept="image/*" style="display: none;">
                        <i class="feather icon-upload" style="font-size: 2rem; color: #64748b;"></i>
                    </div>
                </div>
                <p class="text-muted small mt-2 mb-0">Click the upload area or use the camera below to attach an image.</p>
            </div>
            <div class="modal-footer" style="border-top: 1px solid #e2e8f0; padding: 0.75rem 1.25rem;">
                <button type="button" class="btn btn-outline-secondary btn-sm" id="addImageModalCameraBtn" title="Select image">
...
<!-- Print Repair Job Work Order Confirmation Modal -->
...
<script>
    ...
    window.isSaleOrderEditMode = <?php echo ($edit_order_id > 0) ? 'true' : 'false'; ?>;
    // Repair Job Work Order opened from repair order (repair_order_id in URL): show repair order items and Ref No = repair order no
    window.isJobworkFromRepairOrder = <?php echo $is_jobwork_from_repair_order ? 'true' : 'false'; ?>;
    ...
    let cart = [];
    <?php if (!empty($edit_items)): foreach ($edit_items as $__it): ?>
    cart.push(<?php echo json_encode($__it); ?>);
    <?php endforeach; endif; ?>
    <?php if (!empty($edit_items)): ?>
    setTimeout(function() { if (cart.length > 0 && typeof renderCart === 'function') renderCart(); }, 0);
    <?php endif; ?>
    const PRODUCT_MODAL_COLUMNS_PAGE = 'repair-job-work-order-product-modal';
...
<td data-column="sale-amount-with"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" readonly style="width: 130px; font-size: 0.7rem;"></td>
    ...
    $(document).ready(function() {
        ...
        // ================== REPAIR ORDER SEARCH FUNCTIONALITY ==================
        let saleOrderSearchTimeout;
        const searchSaleOrderInput = $('#searchSaleOrder');
        const saleOrderSuggestions = $('#saleOrderSuggestions');
        
        searchSaleOrderInput.on('input', function() {
            const searchTerm = $(this).val().trim();
            clearTimeout(saleOrderSearchTimeout);
            
            if (searchTerm.length < 2) {
                saleOrderSuggestions.hide();
                return;
            }
            
            saleOrderSearchTimeout = setTimeout(function() {
                $.ajax({
                    url: 'ajax/search-repair-orders.php',
                    type: 'GET',
                    data: { q: searchTerm },
                    dataType: 'json',
                    success: function(response) {
                        if (response.status === 'success' && response.orders && response.orders.length > 0) {
                            ...
                            saleOrderSuggestions.html(html).show();
                        } else {
                            saleOrderSuggestions.html('<div style="padding: 0.75rem; color: #94a3b8; text-align: center;">No repair orders found</div>').show();
                        }
                    },
                    error: function() {
                        saleOrderSuggestions.hide();
        ...
        // Click on Search Repair Order suggestion: go to repair-job-work-order.php?repair_order_id=X
        $(document).on('click', '.sale-order-suggestion-item', function() {
            const repairOrderId = $(this).data('order-id');
            searchSaleOrderInput.val('');
            saleOrderSuggestions.hide();
            if (repairOrderId) {
                window.location.href = 'repair-job-work-order.php?repair_order_id=' + repairOrderId;
            }
        });
...
<td data-column="sale-amount-with"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" readonly style="width: 130px; font-size: 0.7rem;"></td>
        ...
        // Populate category dropdown (Select Category) when adding product row
        const categorySelect = row.querySelector('[data-column="category"] select');
        if (categorySelect && typeof categories !== 'undefined') {
            populateSelect(categorySelect, categories, 'id', 'name', 'Select Category');
            categorySelect.classList.add('category-select');
    ...
    function openProductSearchModal(row) {
        ...
        // When opening from Product Selection modal (row set), filter search by current tab; from main table show all
    ...
    (function() {
        ...
        checkboxes.forEach(function(checkbox) {
            checkbox.addEventListener('change', function() {
                ...
                // Update group header colspans after layout so groups stay aligned (no column shift)
                requestAnimationFrame(function() {
                    requestAnimationFrame(function() {
                        if (typeof updateGroupHeaderVisibility === 'function') updateGroupHeaderVisibility();
                    });
                });
                // Save for current tab so next time this tab shows same columns; persist per user
        ...
        if (productSelectionModal) {
            productSelectionModal.addEventListener('shown.bs.modal', function() {
                // Reload saved column preferences and apply after load so Gold tab (and others) show saved check/uncheck
    ...
    // Apply column visibility for a given tab (Gold, Silver, etc.) from saved state
    function applyProductModalColumnVisibilityForTab(tabKey) {
        ...
        const prefs = (typeof window.mergeProductModalMetalTabPrefs === 'function')
        ...
        checkboxes.forEach(function(checkbox) {
            ...
            const isVisible = (prefs && prefs.hasOwnProperty(columnName)) ? (prefs[columnName] === 1) : true;
    ...
    function addProductToTableFromModalRow(modalRowData, metalIdForImage) {
        ...
        try {
            var groupImage = (metalIdForImage != null && window.productModalGroupImageByTab && window.productModalGroupImageByTab[metalIdForImage]) ? window.productModalGroupImageByTab[metalIdForImage] : '';
            row.setAttribute('data-group-image', groupImage || '');
            ...
            row.innerHTML = (typeof getProductListRowCells === 'function' ? getProductListRowCells(modalData, { groupImage: groupImage }) : []).join('') + actionCell;
    ...
    // Render product list from cart array (used when loading sale order items for JWO)
    function renderCart() {
        var tbody = document.getElementById('productTableBody');
        if (!tbody) return;
        var emptyRow = tbody.querySelector('.no-drag');
        if (emptyRow) emptyRow.remove();
        if (!cart || cart.length === 0) {
            tbody.innerHTML = '<tr class="no-drag"><td colspan="65" class="text-center text-muted py-4" id="emptyRowCell">No Rows To Show</td></tr>';
            if (typeof updateSummaryPanel === 'function') updateSummaryPanel();
            return;
        }
        cart.forEach(function(item) {
            var modalRow = typeof savedItemToModalRowData === 'function' ? savedItemToModalRowData(item) : { product_id: item.product_id, product_name: item.product_name || '', quantity: item.quantity || item.qty || 1, rate: item.rate || 0, amount: item.amount || 0, gross_wt: item.gross_weight || 0, less_wt: item.less_weight || 0, purity: item.purity || 0, final_wt: item.final_weight || 0, net_wt: item.net_weight || 0, pure_wt: item.pure_weight || item.purity_weight || 0, barcode: item.barcode || '', characteristic_id: item.product_characteristic_id || item.characteristic_id || '', design_no: item.design_no || '', making_amount: item.making_amount || 0, tax: item.tax_amount || item.tax || 0, net_amt: item.net_amount || 0, net_amt_tax: item.net_amt_with_tax || 0 };
            if (typeof addProductToTableFromModalRow === 'function') addProductToTableFromModalRow(modalRow);
        });
        if (typeof updateSummaryPanel === 'function') updateSummaryPanel();
    }
    ...
    function addMergedProductsToTable(modalRowsData, metalIdForImage) {
        ...
        var actionCell = '<td><div class="action-btns"><button type="button" class="btn-edit" onclick="editProductRow(\'' + rowId + '\')" title="Edit"><i class="feather icon-edit"></i></button><button type="button" class="btn-delete" onclick="deleteProductRow(\'' + rowId + '\')" title="Delete"><i class="feather icon-trash-2"></i></button></div></td>';
        row.innerHTML = (typeof getProductListRowCells === 'function' ? getProductListRowCells(merged, { groupImage: groupImage }) : []).join('') + actionCell;
    ...
    function updateMergedRowFromModalRows(rowId, modalRowsData) {
        ...
        var tabKey = (typeof currentMetalId !== 'undefined' && currentMetalId !== null) ? String(currentMetalId) : '';
        var groupImage = window.productModalGroupImageByTab && window.productModalGroupImageByTab[tabKey];
        if (groupImage) row.setAttribute('data-group-image', groupImage);
        if (photoCell) {
    ...
    function addModalRowCalculationListeners(row) {
        ...
        // Quantity and Carat fields (needed for making calculations)
        ...
        // Quantity and Carat listeners (affect making calculations for Per Piece and M.KT types)
    ...
    function calculateModalRowNetWeight(row) {
        ...
        if (makingTypeSelect && makingRateInput) {
            ...
            switch(makingType) {
                ...
                case 'M.KT':
                    // M.KT: Making per Karat - Rate × Carat
    ...
    function updateSummaryPanel() {
        ...
        // For purchase invoices: if remaining is negative (supplier has credit), treat as 0 for "amount to pay"
        ...
        fetch(url)
            ...
            .then(data => {
                ...
                if (data.status === 'success' && data.balance) {
                    // Show same Previous Balance as Payment Voucher (raw amount including negative)
                    const rawAmount = parseFloat(data.balance.amount) || 0;
                    console.log('Raw balance amount (same as Payment Voucher):', rawAmount);
    ...
    $(document).ready(function() {
        ...
        setTimeout(function() {
            if (window.isSaleOrderEditMode) return;
            const customerNameEl = document.getElementById('customerName');
            if (customerNameEl && customerNameEl.value.trim()) {
    ...
    // Group image per tab (Gold, Silver, etc.) – single image for merging group
    ...
    function applyProductModalGroupImageToPhotoColumns(dataUrl, tabKey) {
        if (!dataUrl) return;
        const key = (tabKey === undefined || tabKey === null) ? (currentMetalId === undefined || currentMetalId === null ? '' : String(currentMetalId)) : String(tabKey);
        const photoCells = document.querySelectorAll('#productListTable td[data-column="photo"]');
        photoCells.forEach(function(cell) {
            const row = cell.closest('tr.product-row');
            if (row && key !== '' && (row.getAttribute('data-metal-id') || '') !== key) return;
            const img = cell.querySelector('.product-photo-thumb');
            const placeholder = cell.querySelector('.product-photo-placeholder');
            if (img && placeholder) {
                img.src = dataUrl;
                img.style.display = '';
                img.alt = 'Group photo';
    ...
    var addImageModalPendingDataUrl = null;
    var addImageModalTabKey = '';

    function openAddImageModal() {
        addImageModalPendingDataUrl = null;
        ...
        if (existing) {
            addImageModalPendingDataUrl = existing;
            if (placeholder) placeholder.style.display = 'none';
            if (previewImg) { previewImg.src = existing; previewImg.style.display = 'block'; }
        }
    ...
    if (productModalGroupImageInput) {
        productModalGroupImageInput.addEventListener('change', function() {
            ...
            reader.onload = function(e) {
                ...
                const tabKey = (currentMetalId === undefined || currentMetalId === null) ? '' : String(currentMetalId);
                window.productModalGroupImageByTab[tabKey] = dataUrl;
                applyProductModalGroupImageToPhotoColumns(dataUrl);
            };
    ...
    (function setupAddImageModal() {
        ...
        var addImagePreviewPlaceholder = document.getElementById('addImagePreviewPlaceholder');
        var addImagePreviewImg = document.getElementById('addImagePreviewImg');
        ...
        function showPreview(dataUrl) {
            addImageModalPendingDataUrl = dataUrl;
            if (addImagePreviewPlaceholder) addImagePreviewPlaceholder.style.display = 'none';
            if (addImagePreviewImg) { addImagePreviewImg.src = dataUrl; addImagePreviewImg.style.display = 'block'; }
        }
        ...
        if (addImageModalSaveBtn) {
            addImageModalSaveBtn.addEventListener('click', function() {
                if (addImageModalPendingDataUrl) {
                    window.productModalGroupImageByTab = window.productModalGroupImageByTab || {};
                    window.productModalGroupImageByTab[addImageModalTabKey] = addImageModalPendingDataUrl;
                    if (typeof applyProductModalGroupImageToPhotoColumns === 'function') {
                        applyProductModalGroupImageToPhotoColumns(addImageModalPendingDataUrl, addImageModalTabKey);
                    }
                }
                closeAddImageModal();
            });
    ...
    if (window.isJobworkFromRepairOrder) {
        document.addEventListener('DOMContentLoaded', function() {
    ...
    // Save Repair Job Work Order (when opened from repair_order_id) - collect items from Product List, then confirm print
    function saveJobWorkOrder() {
        var el = document.getElementById('jwoRepairOrderId');
        var repairOrderId = el ? parseInt(el.getAttribute('data-repair-order-id') || 0, 10) : 0;
        if (!repairOrderId) {
            alert('Repair order ID missing.');
            return;
        }
        if (isSaving) return;
        isSaving = true;
        var btn = document.querySelector('.btn-save-invoice[onclick*="saveJobWorkOrder"]');
        ...
        var fd = new FormData();
        fd.append('repair_order_id', repairOrderId);
        ...
        if (currentJwoId > 0) fd.append('rjwo_id', currentJwoId);
        ...
        if (userElJwo && userElJwo.value && String(userElJwo.value).trim() !== '') {
            fd.append('department_user_id', String(userElJwo.value).trim());
        }
        ...
        fetch('ajax/save-repair-jobwork-order.php', { method: 'POST', body: fd, credentials: 'same-origin' })
            ...
            .then(function(data) {
                isSaving = false;
                if (btn) { btn.disabled = false; btn.textContent = 'Save Job Work Order'; }
                if (data.status === 'success') {
                    showPrintJWOModal(data.repair_jwo_id || data.jwo_id || data.jobwork_id || '', repairOrderId);
                } else {
                    var msg = data.message || 'Error saving repair job work order.';
                    if (msg.indexOf('already exists') !== -1 && typeof swal === 'function') {
                        swal({
                            title: 'Repair Job Work Order',
                            text: 'Repair job work order already exists for this repair order. Do you want to open saved repair job work order for editing?',
                            type: 'warning',
                            ...
                            if (isConfirm && (data.repair_jwo_id || data.jwo_id)) {
                                window.location.href = 'repair-job-work-order.php?id=' + (data.repair_jwo_id || data.jwo_id);
                            }
                        });
                    } else {
                        alert(msg);
    ...
    // Print RJWO modal
    var savedJwoId = null;
    var savedRepairOrderId = null;
    function showPrintJWOModal(jwoId, repairOrderId) {
        savedJwoId = jwoId;
        savedRepairOrderId = repairOrderId || '';
    ...
    function closePrintJWOModal() {
        ...
        savedJwoId = null;
        if (savedRepairOrderId !== null) {
            window.location.href = 'repair-order-process.php?saved=1' + (savedJwoId ? '&jobwork_no=RJWO-' + savedJwoId : '');
        }
        savedRepairOrderId = null;
    }
    function confirmPrintJWO() {
        if (savedJwoId) {
            window.location.href = 'repair-order-process.php?saved=1&jobwork_no=RJWO-' + savedJwoId;
        } else {
            closePrintJWOModal();
        }
    }
    ...
    function saveOrder() {
        ...
        productRows.forEach(function(row) {
                    ...
                    if (groupItems && groupItems.length > 0) {
                        groupItems.forEach(function(d) {
                            items.push({
                                ...
                                pure_weight: parseFloat(d.pure_wt) || 0,
                                rate: parseFloat(d.rate) || 0,
                                making: parseFloat(d.making_amount) || 0,
                                making_amount: parseFloat(d.making_amount) || 0,
                                design_no: d.design_no || '',
                                tax: parseFloat(d.tax) || 0,
                                amount: parseFloat(d.amount) || 0,
            ...
            if (productId) {
                ...
                const metalValue = parseFloat(row.querySelector('[data-column="metal-value"]')?.textContent) || 0;
                const discount = parseFloat(row.querySelector('[data-column="discount"]')?.textContent) || 0;
                const makingAmount = parseFloat(row.querySelector('[data-column="making-amount"] input')?.value || row.querySelector('[data-column="making-amount"]')?.textContent) || 0;
                const stoneAmount = parseFloat(row.querySelector('[data-column="stone-amount"]')?.textContent) || 0;
                const otherAmount = parseFloat(row.querySelector('[data-column="other-amount"]')?.textContent) || 0;
                const diamondAmount = parseFloat(row.querySelector('[data-column="diamond-amount"]')?.textContent) || 0;
                const purchaseAmount = parseFloat(row.querySelector('[data-column="purchase-amount"]')?.textContent) || 0;
        ...
        // Hedging: save making amount against sale fixing; create purchase invoice without making amount
            ...
            jQuery.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        if (invoiceId) {
                            // Store redirect URL - redirect to fresh page after save
                            window.pendingRedirectUrl = 'sale-order.php';
                            // Show print confirmation modal
                            setTimeout(function() {
                                showPrintOrderModal(invoiceId);
                            }, 100);
                        } else {
                            alert('Repair Job Work Order saved successfully! Order No: ' + (response.invoice_no || response.order_no || 'N/A'));
            ...
            .then(data => {
                ...
                if (data.status === 'success') {
                    ...
                    if (invoiceId) {
                        ...
                        alert('Repair Job Work Order saved successfully! Order No: ' + (data.invoice_no || data.order_no || 'N/A'));
                        // Redirect to fresh page
                        window.location.href = 'sale-order.php';
                    }
                } else {
    ...
    if (saveInvoiceBtn) {
        // Job Work Order: keep saveJobWorkOrder(); do not override with saveOrder()
        if (!window.isJobworkFromRepairOrder) {
    ...
    // Also handle the other save button (btn-purple) - only when not JWO
    const saveBtnPurple = document.querySelector('.btn-purple[onclick*="saveOrder"]');
    if (saveBtnPurple && !window.isJobworkFromRepairOrder) {
    ...
    function printSaleOrder(invoiceId) {
        if (!invoiceId) {
            const urlParams = new URLSearchParams(window.location.search);
    ...
    function updatePrintIcon(invoiceId) {
        ...
        if (printIcon) {
            if (invoiceId && invoiceId > 0) {
                ...
                printIcon.setAttribute('onclick', 'printSaleOrder(' + invoiceId + ')');
                printIcon.setAttribute('title', 'Print Repair Job Work Order');
            } else {
    ...
    function loadOrder(orderId) {
        ...
        if (typeof jQuery !== 'undefined' && jQuery.ajax) {
            jQuery.ajax({
                url: 'ajax/get-sale-order.php?order_id=' + orderId,
                type: 'GET',
                dataType: 'json',
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        window.isSaleOrderEditMode = true;
                    } else {
    ...
    function populateOrderForm(order, items, payments) {
        ...
        // Update order number
        // Job Work Order: keep server-rendered JWO number (e.g. JWO-1); do not overwrite with sale order no
        if (document.getElementById('currentOrderNo') && !window.isJobworkFromRepairOrder) {
        ...
        // Clear search input when order is loaded
        ...
        if (document.getElementById('refNo')) {
            // When opened from repair order (RJWO), show repair order number in Ref No (Against Ref No). RJWO edit has ref_no/repair_order_no; RJWO create has order_no = repair order no.
            var refVal = (window.isJobworkFromRepairOrder && (order.ref_no || order.repair_order_no)) ? (order.ref_no || order.repair_order_no) : (window.isJobworkFromRepairOrder && order.order_no) ? order.order_no : (order.ref_no || '');
            document.getElementById('refNo').value = refVal;
        }
        ...
        if (document.getElementById('dueDate')) {
            ...
            if (rawDue && rawDue.length >= 10) rawDue = rawDue.substring(0, 10);
        ...
        // Hedging fields (optional columns)
        toggleHedgingSection();
        ...
        // Job Work Order: department, status, priority (when loading JWO edit mode)
        if (window.isJobworkFromRepairOrder) {
            var jwoDeptEl = document.getElementById('jwoDepartment');
            if (jwoDeptEl) {
                // department_id is INT in DB; match dropdown option value (e.g. "1", "2")
                var raw = order.department_id;
                var deptVal = (raw !== undefined && raw !== null && raw !== '') ? String(Number(raw)) : '';
                if (deptVal !== '' && isNaN(Number(deptVal))) deptVal = '';
                jwoDeptEl.value = deptVal;
            }
            if (document.getElementById('jwoStatus') && order.status) {
                document.getElementById('jwoStatus').value = order.status;
            }
        ...
        // Clear existing products from Product List table (skip when JWO uses cart + renderCart)
        const productTableBody = document.getElementById('productTableBody');
        if (productTableBody && !(window.isJobworkFromRepairOrder && cart && cart.length > 0)) {
        ...
        // Load items as one merged Product List row with data-group-items (so Edit shows all products). Skip when JWO uses cart + renderCart.
        if (!(window.isJobworkFromRepairOrder && cart && cart.length > 0) && items && items.length > 0) {
            console.log('Found ' + items.length + ' items to load');
            try {
                var validItems = items.filter(function(item) { return item && typeof item === 'object'; });
                const modalRowsData = validItems.map(function(item) { return savedItemToModalRowData(item); });
                if (typeof addMergedProductsToTable === 'function') {
                    addMergedProductsToTable(modalRowsData);
                    // Fallback: if no rows were added (e.g. table not ready), add each item as its own row
                    var dataRows = productTableBody ? productTableBody.querySelectorAll('tr:not(.no-drag)') : [];
                    if (dataRows.length === 0 && typeof addProductToTableFromModalRow === 'function') {
                        console.log('Product list still empty - adding items one by one');
                        modalRowsData.forEach(function(modalRow) {
                            addProductToTableFromModalRow(modalRow);
                        });
                    }
                } else {
            ...
            // Show empty message (skip product table when JWO uses cart + renderCart)
            if (productTableBody && !(window.isJobworkFromRepairOrder && cart && cart.length > 0)) {
                productTableBody.innerHTML = '<tr class="no-drag"><td colspan="32" class="text-center text-muted py-4" id="emptyRowCell">No Rows To Show</td></tr>';
            }
            if (productListBody) {
        ...
        // Recalculate footer from product rows, then restore saved summary totals so balance shows correctly (e.g. 875 not 500)
        updateSummaryPanel();
        updatePaymentTotals();
        // Restore saved grand total and balance so edit mode shows exactly what is in the database
        if (document.getElementById('summaryGrandTotal')) {
            document.getElementById('summaryGrandTotal').textContent = savedGrandTotal.toFixed(2);
        }
        if (document.getElementById('summaryBalanceAmt')) {
    ...
    // Load order on page load if edit_order_id is set (includes Repair Job Work Order with repair_order_id in URL or ?id=RJWO id)
    <?php if ($edit_order_id > 0 && $edit_order): ?>
    $(document).ready(function() {
        // Populate form with existing order data
        const order = <?php echo json_encode($edit_order); ?>;
        var items = <?php echo json_encode($edit_items); ?>;
        ...
        console.log('Edit mode - Items count:', items.length);
        
        function doPopulate() {
            populateOrderForm(order, items, payments);
            if (cart && cart.length > 0 && typeof renderCart === 'function') {
                renderCart();
            }
        }
        function maybeRetry(attempt) {
            var tbody = document.getElementById('productTableBody');
            var dataRows = tbody ? tbody.querySelectorAll('tr:not(.no-drag)') : [];
            if (items.length > 0 && dataRows.length === 0 && attempt < 4) {
                console.log('Product list empty after load - retry ' + (attempt + 1));
                doPopulate();
                var delays = [500, 1000, 2000];
                if (attempt < 3) {
                    setTimeout(function() { maybeRetry(attempt + 1); }, delays[attempt] || 2000);
                }
            }
        }
        
        // Run after full page load so product table and getProductListRowCells are ready
        if (document.readyState === 'complete') {
            setTimeout(function() { doPopulate(); setTimeout(function() { maybeRetry(0); }, 400); }, 150);
        } else {
            window.addEventListener('load', function() {
                setTimeout(function() { doPopulate(); setTimeout(function() { maybeRetry(0); }, 400); }, 150);
            });
        ...
        // Also load items when Product Selection modal is opened for edit (in case modal wasn't ready when page loaded)
        $('#productSelectionModal').on('shown.bs.modal', function() {
            ...
            if (productListBody) {
                ...
                // Only repopulate when editing an existing row (not when adding new - keep table clear)
                var isEditingRow = (typeof currentEditingRowId !== 'undefined' && currentEditingRowId);
                if (existingRows.length === 0 && items && items.length > 0 && isEditingRow) {
                    console.log('No items in table (edit mode), reloading from saved items');
                    // Clear empty message
                    productListBody.innerHTML = '';
                    ...
                    items.forEach(function(item, index) {
                        const product = {
                            ...
                            value: item.amount || item.value || 0,
                            article: item.design_no || item.article || '',
                            vat_value: item.vat_value != null ? item.vat_value : (item.tax_percent != null ? item.tax_percent : '')
                        };
                        if (typeof addProductRowToSelectionTable === 'function') {
                            addProductRowToSelectionTable(item, product);
                        }
...
<script>
    ...
    // Show print order confirmation modal
    function showPrintOrderModal(invoiceId) {
        console.log('showPrintOrderModal called with invoiceId:', invoiceId);
        ...
        setTimeout(function() {
            ...
            if (modal) {
                ...
                if (confirm('Repair Job Work Order saved successfully! Do you want to print the order?')) {
                    window.open('sale-order-print.php?id=' + invoiceId, '_blank', 'width=1200,height=800');
                }
    ...
    window.showPrintOrderModal = showPrintOrderModal;
