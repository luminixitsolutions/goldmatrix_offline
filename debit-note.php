...
$auragold_voucher_runtime_client = auragold_voucher_runtime_bootstrap($conn, $metals, 'Debit Note');
...
// Debit Note: use tbl_debit_notes (run admin/sql/create_debit_note_tables.sql if not exists)
$debit_note_table_exists = false;
$t_dn = @mysqli_query($conn, "SHOW TABLES LIKE 'tbl_debit_notes'");
if ($t_dn && mysqli_num_rows($t_dn) > 0) {
    $debit_note_table_exists = true;
}

$next_order_no = 'DN-1';
if ($debit_note_table_exists) {
    $last_dn = getRecord("SELECT debit_note_no FROM tbl_debit_notes ORDER BY id DESC LIMIT 1");
    if ($last_dn && $last_dn['debit_note_no']) {
        $last_num = (int)preg_replace('/[^0-9]/', '', $last_dn['debit_note_no']);
        $next_order_no = 'DN-' . ($last_num + 1);
    }
}

// Load debit note for editing if ID provided
$edit_order_id = isset($_GET['id']) ? (int)$_GET['id'] : 0;
$edit_order = null;
$edit_items = [];
$edit_payments = [];

if ($edit_order_id > 0 && $debit_note_table_exists) {
    $edit_order = getRecord("SELECT * FROM tbl_debit_notes WHERE id = $edit_order_id");
    if ($edit_order) {
        $edit_items = getList("SELECT * FROM tbl_debit_note_items WHERE debit_note_id = $edit_order_id");
        $edit_payments = getList("SELECT * FROM tbl_debit_note_payments WHERE debit_note_id = $edit_order_id");
        $next_order_no = $edit_order['debit_note_no'];
        $edit_order['order_no'] = $edit_order['debit_note_no'];
        $edit_order['order_date'] = $edit_order['debit_note_date'];
        $edit_order['order_id'] = $edit_order['id'];
        $edit_order['invoice_no'] = $edit_order['debit_note_no'];
        $edit_order['invoice_date'] = $edit_order['debit_note_date'];
    }
}

$saved_orders = [];
if ($debit_note_table_exists) {
    $saved_orders = getList("SELECT id, debit_note_no as order_no, customer_name, debit_note_date as order_date, grand_total FROM tbl_debit_notes ORDER BY id DESC LIMIT 50");
}
?>
<!DOCTYPE html>
...
text-transform: uppercase;
    ...
    /* Hidden columns: collapse in table layout so headers stay aligned with cell values */
    ...
    /* Right sticky: positions set by adjustProductModalStickyRightColumns (.sticky-right) — do not hardcode right offsets */
                            ...
                            <div class="summary-panel" >
                                ...
                                <div class="invoice-header" style="margin: -1rem -1rem 1rem -1rem; border-radius: 10px 10px 0 0; display: flex; align-items: center; justify-content: space-between;">
                                    <div style="display: flex; align-items: center; gap: 0.5rem; flex: 1;">
                                        ...
                                        <h5 class="mb-0" style="font-size: 0.9rem;">Debit Note No: <span id="currentOrderNo"><?php echo htmlspecialchars($next_order_no); ?></span></h5>
                                    </div>
...
<!-- categoryCreationModal: provided by includes/common-modal.php -->
...
<script>window.isVoucherEditMode = <?php echo (!empty($edit_order_id) && (int)$edit_order_id > 0) ? 'true' : 'false'; ?>;</script>
...
<?php if (function_exists('auragold_echo_product_modal_spec_masters_js')) { auragold_echo_product_modal_spec_masters_js(); } ?>
    ...
    const PRODUCT_MODAL_COLUMNS_PAGE = 'debit-note-product-modal';
    ...
    $(document).ready(function() {
        ...
        saleInvoiceSearchInput.on('input', function() {
            ...
            saleInvoiceSearchTimeout = setTimeout(function() {
                $.ajax({
                    url: 'ajax/search-debit-notes.php',
                    type: 'GET',
                    data: { q: searchTerm },
                    dataType: 'json',
                    success: function(response) {
                        if (response.status === 'success' && response.invoices && response.invoices.length > 0) {
                            ...
                            response.invoices.forEach(function(invoice) {
                                const dnNo = invoice.debit_note_no || invoice.invoice_no;
                                html += `
                                    <div class="invoice-suggestion-item" 
                                         data-invoice-id="${invoice.id}" 
                                         data-invoice-no="${dnNo}"
                                         style="padding: 0.75rem; cursor: pointer; border-bottom: 1px solid #f1f5f9; transition: background 0.2s;"
                                         onmouseover="this.style.background='#f8fafc'" 
                                         onmouseout="this.style.background='#fff'">
                                        <div style="font-weight: 600; color: #1e293b; font-size: 0.9rem;">${dnNo}</div>
                            ...
                            saleInvoiceSuggestions.html('<div style="padding: 0.75rem; color: #94a3b8; text-align: center;">No debit notes found</div>').show();
                        }
                    },
                    error: function() {
                        saleInvoiceSuggestions.hide();
...
<td data-column="sale-amount-with"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" readonly style="width: 130px; font-size: 0.7rem;"></td>
    ...
    function addRowFromStockJournal(sjData) {
        if (typeof addEmptyProductRow !== 'function') return;
        addEmptyProductRow();
        const tbody = document.getElementById('productListBody');
        if (!tbody) return;
        const row = tbody.querySelector('tr.product-row:last-child');
        if (!row) return;
        ...
        row.setAttribute('data-metal-id', sjData.metal_id || currentMetalId || '');
        ...
        function setCol(col, val, isSelect) {
            const el = row.querySelector('[data-column="' + col + '"] ' + (isSelect ? 'select' : 'input'));
            if (el) el.value = val;
        }
        const idCell = row.querySelector('[data-column="id"]');
        if (idCell) idCell.textContent = sjData.id || sjData.product_id || '';
        setCol('voucher-type', 'Stock Journal');
        setCol('barcode', barcode);
        setCol('design-no', designNo);
        setCol('product', fullProductName);
        setCol('quantity', quantity);
        setCol('gross-wt', grossWt);
        setCol('less-wt', lessWt);
        setCol('net-wt', netWt);
        setCol('purity', purity);
        setCol('purity-wt', purityWt);
        setCol('final-wt', finalWt);
        setCol('rate', rate);
        setCol('amount', amount);
        setCol('making-amount', makingAmount);
        setCol('tax', taxAmount);
        setCol('net-amt', netAmount);
        setCol('net-amt-tax', netAmtWithTax);
        setCol('calculation', 'Rate X Gross Wt', true);

        const checkbox = row.querySelector('.product-checkbox');
        if (checkbox) {
            checkbox.setAttribute('data-product-id', sjData.product_id || '');
            checkbox.setAttribute('data-characteristic-id', sjData.product_characteristic_id || '');
    ...
    function updateSummaryPanel() {
        ...
        const additionalAmt = parseFloat(document.getElementById('summaryAdditionalAmt')?.textContent || 0) || 0;
        const discountAmount = parseFloat(document.getElementById('discountAmount')?.value || 0) || 0;
        ...
        // Keep paid/balance math on FX grand total
        if (!isFinite(grandTotal)) grandTotal = grandTotalBase;
    ...
    function saveOrder() {
        ...
        if (typeof jQuery !== 'undefined' && jQuery.ajax) {
            jQuery.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        // Redirect to debit-note.php for fresh entry after save
                        window.location.href = 'debit-note.php';
                    } else {
                        alert('Error: ' + (response.message || 'Failed to save debit note'));
                    }
                },
            ...
            fetch('ajax/save-debit-note.php', {
                method: 'POST',
                body: formData
            })
            .then(response => {
            ...
            .then(data => {
                ...
                if (data.status === 'success') {
                    ...
                    alert('Error: ' + (data.message || 'Failed to save debit note'));
                }
            })
            .catch(error => {
                if (saveBtn) {
    ...
    function clearInvoiceForm() {
        ...
        if (window.history && window.history.replaceState) {
            window.history.replaceState({}, document.title, 'debit-note.php');
        }
    ...
    function resetOrder() {
        if (confirm('Are you sure you want to create a new debit note? All unsaved data will be lost.')) {
            window.location.href = 'debit-note.php';
        }
    }
            ...
            jQuery.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        window.history.pushState({}, '', 'debit-note.php?id=' + orderId);
                    } else {
                        alert('Error loading debit note: ' + (response.message || 'Unknown error'));
                    }
                },
                error: function(xhr, status, error) {
                    const msg = document.getElementById('orderLoadingMsg');
                    if (msg) document.body.removeChild(msg);
                    alert('Error loading debit note: ' + error);
                    console.error('Load debit note error:', xhr.responseText);
                }
            });
        } else {
            fetch('ajax/get-debit-note.php?order_id=' + orderId)
                .then(response => response.json())
                .then(data => {
                    const msg = document.getElementById('orderLoadingMsg');
                    if (msg) document.body.removeChild(msg);
                    if (data.status === 'success') {
                        ...
                        alert('Error loading debit note: ' + (data.message || 'Unknown error'));
                    }
                })
                .catch(error => {
                    const msg = document.getElementById('orderLoadingMsg');
                    if (msg) document.body.removeChild(msg);
                    alert('Error loading debit note: ' + error);
                    console.error('Load debit note error:', error);
                });
        }
    }
