...
// Next POS / invoice no: same bill series as Sales Invoice (tbl_bill_series voucher "Sales Invoice"); shared sequence with sale-invoice screen
$next_order_no = function_exists('getNextPosSaleInvoiceNo') ? getNextPosSaleInvoiceNo($conn) : 'SI-1';

// Sales Person dropdown (stored in tbl_pos_sale_invoices.sales_person) — branch-scoped users from registry
$sales_person_users = auragold_sales_person_user_display_names($conn_master);

// Load invoice for editing if ID provided (tbl_pos_sale_invoices)
...
if (!empty($edit_order_id)) {
    ...
    if ($edit_order) {
        ...
        $edit_items = getList("
            ...
            FROM tbl_pos_sale_invoice_items i
        ...
        $edit_payments = getList("SELECT * FROM tbl_pos_sale_invoice_payments WHERE invoice_id = " . intval($edit_order_id));
...
if (!empty($conn) && function_exists('getList')) {
    ...
    if ($has_tm_scope) {
        $tm_rows = @getList("SELECT name, default_value, IFNULL(gst_supply_scope, 'local_state') AS gst_supply_scope FROM tbl_tax_master WHERE status = 1 " . auragold_master_list_sql_suffix($conn, 'tbl_tax_master') . " ORDER BY sort_order ASC, id ASC");
    } else {
        $tm_rows = @getList("SELECT name, default_value FROM tbl_tax_master WHERE status = 1 " . auragold_master_list_sql_suffix($conn, 'tbl_tax_master') . " ORDER BY sort_order ASC, id ASC");
    }
    if (is_array($tm_rows)) {
...
text-transform: uppercase;
    ...
    .product-entry.pos-barcode-entry-card label {
        color: #11294b;
    }
    .pos-barcode-scan-row .input-group {
        max-width: 32rem;
    }
    .add-item-link {
        text-align: center;
    ...
    /* Left column: avoid overflow-x: hidden here — it clips absolutely positioned autocomplete dropdowns
       (#customerSuggestions, #saleInvoiceSuggestions). Horizontal overflow is contained in the product table wrapper. */
    .invoice-content-row > .col-lg-8 {
        min-width: 0;
        overflow-x: visible;
        overflow-y: visible;
        padding-bottom: 4rem; /* space above fixed footer so payment totals stay visible */
    }
    /* Billing header card: allow dropdowns to paint outside card bounds */
    .invoice-content-row > .col-lg-8 > .card.mb-4:first-of-type {
        overflow: visible !important;
    }
    .invoice-content-row > .col-lg-8 > .card.mb-4:first-of-type .card-body.billing-form {
        overflow: visible !important;
    }
    #customerSuggestions,
    #saleInvoiceSuggestions {
    ...
    .pos-payment-inline-rows {
        font-size: 0.85rem;
        border-top: 1px solid #e2e8f0;
        padding-top: 1rem;
        margin-top: 0.75rem;
    }
    .pos-payment-inline-rows .pos-pay-label {
        min-width: 3.25rem;
        font-weight: 600;
        color: #334155;
        flex-shrink: 0;
    ...
    .pos-payment-inline-rows .pos-pay-inline-add {
        background: #11294b;
        color: #fff;
        border: none;
        flex-shrink: 0;
    }
    .pos-payment-inline-rows .pos-pay-inline-add:hover {
        background: #0c1f38;
        color: #fff;
    }
    .product-table {
                        ...
                        <div class="col-lg-8" >
                            ...
                            <div class="card mb-4">
                                <div class="card-body billing-form">
                                        ...
                                        <input type="hidden" id="refNo" value="<?php echo htmlspecialchars((!empty($edit_order) && is_array($edit_order)) ? trim((string)($edit_order['ref_no'] ?? '')) : '', ENT_QUOTES, 'UTF-8'); ?>">
                                        <input type="hidden" id="customerGstin" name="customer_gstin" value="<?php echo htmlspecialchars($si_cust_gst, ENT_QUOTES, 'UTF-8'); ?>">
                                        <input type="hidden" id="fixingType" value="<?php echo htmlspecialchars($si_fix, ENT_QUOTES, 'UTF-8'); ?>">
                                        <!-- Against Of and Due Date must be in separate .col-md-* wrappers: auragold-voucher-runtime-apply.js hides the closest column per field; sharing one column hides both when e.g. due_date is off in voucher-type. -->
                                        <div class="col-md-6">
                                        ...
                                        <div class="col-md-3">
                                            <div class="form-group mb-0">
                                                <label class="mb-0 small font-weight-bold text-dark" for="dueDate">Due Date</label>
                                        ...
                                        <div class="col-md-3">
                                            <div class="form-group mb-0">
                                                <label class="mb-0 small font-weight-bold text-dark" for="orderDate">Date</label>
                            ...
                            <!-- Barcode scan + Add Item (same product modal / customer rules as Sale Invoice) -->
                            <div class="card mb-4">
                                <div class="card-body product-entry pos-barcode-entry-card py-2 px-3">
                                    <div class="form-group mb-0 pos-barcode-scan-row d-flex flex-wrap align-items-center" style="gap: 0.5rem 0.75rem;">
                                        <label for="posInvoiceBarcodeScan" class="mb-0 text-uppercase small font-weight-bold" style="letter-spacing: 0.04em; color: #6b5b95;">Barcode</label>
                                        <div class="input-group input-group-sm flex-grow-1">
                                            <input type="text" class="form-control" id="posInvoiceBarcodeScan" autocomplete="off" placeholder="Scan or type barcode, press Enter" data-require-customer="1" disabled aria-label="Barcode scan">
                                            <div class="input-group-append">
                                                <button type="button" class="btn btn-outline-secondary" id="posInvoiceBarcodeScanBtn" title="Add line from barcode (same as Enter)" data-require-customer="1" disabled aria-label="Submit barcode">
                                                    <i class="feather icon-check" aria-hidden="true"></i>
                                                </button>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="add-item-link add-item-link--disabled mt-2" id="addItemBtn" role="button" aria-disabled="true" title="Select a customer first" style="cursor: not-allowed;">
                            ...
                            <!-- Comments (above payment block) -->
                            <div class="card mb-4">
                                <div class="card-body py-3">
                                    <label class="mb-2 d-block" style="font-size: 0.85rem; color: #475569; font-weight: 600;">Comments</label>
                            ...
                            <div class="card mb-4 pos-payment-section-card">
                                <div class="card-body pb-4">
                                    <div class="row align-items-end">
                                        ...
                                        <div class="col-md-7">
                                    <!-- Payment method icons (cash / bank / UPI / card: inline rows below) -->
                                    ...
                                    <div class="pos-payment-inline-rows">
                                        <div class="d-flex flex-wrap align-items-center mb-2">
                                            <span class="pos-pay-label mr-2">Cash</span>
                                            <input type="text" class="form-control form-control-sm mr-2" id="posInlineCashAmount" placeholder="Amount" inputmode="decimal" autocomplete="off" style="max-width: 11rem;">
                                            <button type="button" class="btn btn-sm pos-pay-inline-add" onclick="addPosInlinePayment('cash')">Add</button>
                                        </div>
                                        <div class="d-flex flex-wrap align-items-center mb-2">
                                            <span class="pos-pay-label mr-2">Bank</span>
                                            <select class="form-control form-control-sm mr-2" id="posInlineBankDepositInto" style="min-width: 10rem; max-width: 16rem;">
                                                <option value="">Select bank</option>
                                                <?php foreach ($bank_accounts as $bank): ?>
                                                <option value="<?php echo htmlspecialchars($bank['name']); ?>"><?php echo htmlspecialchars($bank['name']); ?></option>
                                                <?php endforeach; ?>
                                            </select>
                                            <input type="text" class="form-control form-control-sm mr-2" id="posInlineBankAmount" placeholder="Amount" inputmode="decimal" autocomplete="off" style="max-width: 7rem;">
                                            <button type="button" class="btn btn-sm pos-pay-inline-add" onclick="addPosInlinePayment('bank')">Add</button>
                                        </div>
                                        <div class="d-flex flex-wrap align-items-center mb-2">
                                            <span class="pos-pay-label mr-2">Card</span>
                                            <select class="form-control form-control-sm mr-2" id="posInlineCardDepositInto" style="min-width: 9rem; max-width: 14rem;">
                                                <option value="">Card / account</option>
                                                <option value="Credit Card">Credit Card</option>
                                                <option value="Debit Card">Debit Card</option>
                                            </select>
                                            <input type="text" class="form-control form-control-sm mr-2" id="posInlineCardTransNo" placeholder="Trans No." autocomplete="off" style="max-width: 7rem;">
                                            <input type="text" class="form-control form-control-sm mr-2" id="posInlineCardNumber" placeholder="Card No." autocomplete="off" style="max-width: 8rem;">
                                            <input type="text" class="form-control form-control-sm mr-2" id="posInlineCardAmount" placeholder="Amount" inputmode="decimal" autocomplete="off" style="max-width: 7rem;">
                                            <button type="button" class="btn btn-sm pos-pay-inline-add" onclick="addPosInlinePayment('card')">Add</button>
                                        </div>
                                        <div class="d-flex flex-wrap align-items-center mb-0">
                                            <span class="pos-pay-label mr-2">UPI</span>
                                            <select class="form-control form-control-sm mr-2" id="posInlineUpiDepositInto" style="min-width: 10rem; max-width: 16rem;">
                                                <option value="">Select account</option>
                                                <?php foreach ($bank_accounts as $bank): ?>
                                                <option value="<?php echo htmlspecialchars($bank['name']); ?>"><?php echo htmlspecialchars($bank['name']); ?></option>
                                                <?php endforeach; ?>
                                            </select>
                                            <input type="text" class="form-control form-control-sm mr-2" id="posInlineUpiTransNo" placeholder="Trans No." autocomplete="off" style="max-width: 7rem;">
                                            <input type="text" class="form-control form-control-sm mr-2" id="posInlineUpiAmount" placeholder="Amount" inputmode="decimal" autocomplete="off" style="max-width: 7rem;">
                                            <button type="button" class="btn btn-sm pos-pay-inline-add" onclick="addPosInlinePayment('upi')">Add</button>
                                        </div>
                                    </div>
                            ...
                            <div class="summary-panel" >
                                ...
                                <div class="invoice-header" style="margin: -1rem -1rem 1rem -1rem; border-radius: 10px 10px 0 0; display: flex; align-items: center; justify-content: space-between;">
                                    <div style="display: flex; align-items: center; gap: 0.5rem; flex: 1;">
                                        ...
                                        <h5 class="mb-0" style="font-size: 0.9rem;">POS No: <span id="currentOrderNo"><?php echo htmlspecialchars($next_order_no); ?></span></h5>
                                    </div>
...
<?php auragold_echo_party_select2_scripts([], true); ?>
...
<script>
    ...
    function addProductToProductSelectionTable(product) {
        ...
        var pmDlg = document.getElementById('productSelectionModal');
        var modalOpen = pmDlg && (pmDlg.classList.contains('show') || String(pmDlg.getAttribute('aria-hidden') || '') === 'false');
        if (!modalOpen && typeof getModalRowDataFromRow === 'function' && typeof addProductToTableFromModalRow === 'function') {
            if (typeof window.calculateModalRowNetWeight === 'function') {
                window.calculateModalRowNetWeight(row);
            }
            var modalRowData = getModalRowDataFromRow(row, false);
            var metalForRow = product.metal_id != null && product.metal_id !== '' ? product.metal_id : '';
            row.remove();
            addProductToTableFromModalRow(modalRowData, metalForRow, null);
    ...
    /** Same dedupe rules as getDuplicateProductListRow, but for main invoice lines (#productTableBody). */
    function getDuplicateProductTableRow(barcode, characteristicId, purchaseInvoiceItemId, productMetalId) {
        var bKey = normalizeProductListBarcodeKey(barcode);
        var cid = characteristicId != null && characteristicId !== '' ? String(characteristicId).trim() : '';
        var pii = purchaseInvoiceItemId != null && purchaseInvoiceItemId !== '' ? String(purchaseInvoiceItemId).trim() : '';
        ...
        for (var i = 0; i < rows.length; i++) {
            ...
            var fromSpan = span ? String(span.textContent || '').trim().replace(/\s+/g, ' ') : '';
            if (fromSpan === '—' || fromSpan === '-') fromSpan = '';
            var fromAttr = String(row.getAttribute('data-barcode') || '').trim();
            var v = fromInput || fromSpan || fromAttr;
            if (normalizeProductListBarcodeKey(v) !== bKey) continue;
            var rowCid = String(row.getAttribute('data-characteristic-id') || '').trim();
            if (cid === rowCid) return row;
        }
    ...
    function getDuplicateProductLineAnywhere(barcode, characteristicId, purchaseInvoiceItemId, productMetalId) {
        return getDuplicateProductListRow(barcode, characteristicId, purchaseInvoiceItemId, productMetalId)
            || getDuplicateProductTableRow(barcode, characteristicId, purchaseInvoiceItemId, productMetalId);
    }
    ...
    function isProductLineInProductList(barcode, characteristicId, purchaseInvoiceItemId, productMetalId) {
        return getDuplicateProductLineAnywhere(barcode, characteristicId, purchaseInvoiceItemId, productMetalId) !== null;
    }
    ...
    /** When product modal is open, use its barcode field; otherwise POS line scanner. */
    function getInvoiceLineBarcodeInput() {
        var modalEl = document.getElementById('modalProductBarcode');
        var posEl = document.getElementById('posInvoiceBarcodeScan');
        var pm = document.getElementById('productSelectionModal');
        var modalOpen = pm && (pm.classList.contains('show') || String(pm.getAttribute('aria-hidden') || '') === 'false');
    ...
    function fetchProductByBarcodeAndAdd(barcode) {
        ...
        const barcodeInput = getInvoiceLineBarcodeInput();
        if (barcodeInput) {
        ...
        fetch('ajax/get-product-by-barcode.php?barcode=' + encodeURIComponent(lookupBarcode) + '&expand_invoice_siblings=1&include_purchase_invoice_sibling_barcodes=1' + gpbBranchQs + gpbMetalQs)
            ...
            .then(data => {
                ...
                if (data.success && data.product) {
                    ...
                    var dupRow = getDuplicateProductLineAnywhere(resolvedBc, resCid, data.product.purchase_invoice_item_id, data.product.metal_id);
                    if (dupRow) {
                        var dupWasHidden = dupRow.style.display === 'none';
                        revealDuplicateProductRowIfHidden(dupRow);
                        alert(dupWasHidden
                            ? 'This barcode is already on the invoice (it was on another metal tab — tab switched so you can see the row).'
                            : 'This barcode is already added in the list.');
                        if (barcodeInput) {
                    ...
                    function addSiblingSequential(idx, list) {
                        ...
                        fetch('ajax/get-product-by-barcode.php?barcode=' + encodeURIComponent(sb) + '&expand_invoice_siblings=1&include_purchase_invoice_sibling_barcodes=1' + gpbBranchQs)
                            .then(parseBarcodeAjaxJson)
                            .then(function(d2) {
    ...
    $(document).ready(function() {
        ...
        $(document).on('keydown', '#modalProductBarcode, #posInvoiceBarcodeScan', function(e) {
            if (e.key === 'Enter') {
        ...
        $(document).on('input', '#modalProductBarcode, #posInvoiceBarcodeScan', function() {
            modalProductBarcodeLastFetchDoneBarcode = '';
            modalProductBarcodeLastFetchDoneTime = 0;
        });
        
        $(document).on('blur', '#modalProductBarcode, #posInvoiceBarcodeScan', function() {
            if (modalProductBarcodeCheckTimer) {
                clearTimeout(modalProductBarcodeCheckTimer);
            }
            var self = this;
    ...
    function initProductModalColumnDropdown() {
        ...
        const columnDefinitions = [
            ...
            { key: 'purity-sale', label: 'Purity (Sale)', visible: true },
            { key: 'purity-purchase', label: 'Purity (Purchase)', visible: true },
            { key: 'wastage-sale', label: 'Wastage (Sale)', visible: true },
            { key: 'wastage-purchase', label: 'Wastage (Purchase)', visible: true },
            { key: 'wt-per-piece', label: 'Wt / Piece', visible: true },
    ...
    function clearCustomerForm() {
        ...
        shareHolderFiles = [];
        var shDocBody = document.getElementById('shareHolderDocumentsTableBody');
        if (shDocBody) {
            shDocBody.innerHTML = '';
        }
        var shEmpty = document.getElementById('shareHolderDocumentsEmptyHint');
        if (shEmpty) {
            shEmpty.style.display = 'block';
        }
    }
        ...
        .then(data => {
            if (data.status === 'success' || data.success === true) {
                ...
                if (data.gstin != null && String(data.gstin).trim() !== '') {
                    var cgSi = document.getElementById('customerGstin');
                    if (cgSi) cgSi.value = String(data.gstin).trim().toUpperCase();
                }
...
<td data-column="sale-amount-with"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" readonly style="width: 130px; font-size: 0.7rem;"></td>
    ...
    function addMergedProductsToTable(modalRowsData, metalIdForImage, loadOpts) {
        ...
        var firstStockJid = '';
        for (var _sjPl = 0; _sjPl < modalRowsData.length; _sjPl++) {
            var _sjid = modalRowsData[_sjPl].stock_journal_id;
            if (_sjid != null && String(_sjid).trim() !== '') {
                firstStockJid = String(_sjid).trim();
                break;
            }
        }
        if (firstStockJid) row.setAttribute('data-stock-journal-id', firstStockJid);
    ...
    function updateSummaryPanel() {
        ...
        // Calculate paid amounts from payment cards (same ids/attrs as former table rows)
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '#posInvoiceBarcodeScanBtn', function(e) {
            e.preventDefault();
            e.stopPropagation();
            var inp = document.getElementById('posInvoiceBarcodeScan');
            if (!inp) {
                return;
            }
            if (typeof window.saleInvoiceHasCustomerSelected === 'function' && !window.saleInvoiceHasCustomerSelected()) {
    ...
    // Keyboard shortcut for Add Item (same as Sale Invoice)
    ...
    function saveOrder() {
        ...
        const currentOrderNoText = document.getElementById('currentOrderNo')?.textContent || <?php echo json_encode(isset($next_order_no) ? $next_order_no : 'PSI-1'); ?>;
        ...
        /* POS: never enable e-Way on save or sandbox sample payload */
        orderData.enable_eway_bill = 0;
        ...
        orderData.eway_sandbox_force_sample_payload = '0';
        ...
        // Collect payments from in-memory list (cards mirror this; avoids parsing table cells)
        ...
        if (typeof jQuery !== 'undefined' && jQuery.ajax) {
            jQuery.ajax({
                url: 'ajax/save-pos-sale-invoice.php',
                type: 'POST',
                data: postData,
                dataType: 'json',
                success: function(response) {
            ...
            fetch('ajax/save-pos-sale-invoice.php', {
                method: 'POST',
                body: formData
            })
            .then(response => response.json())
    ...
    function printPurchaseInvoice(invoiceId) {
        ...
        window.open('sale-invoice-print.php?id=' + invoiceId + '&pos=1', '_blank', 'width=1200,height=800');
    }
    ...
    function loadOrder(orderId) {
        ...
        var getUrl = 'ajax/get-pos-sale-invoice.php?id=' + orderId + '&_=' + (Date.now ? Date.now() : '');
        if (typeof jQuery !== 'undefined' && jQuery.ajax) {
            jQuery.ajax({
                url: getUrl,
                type: 'GET',
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        window.history.pushState({}, '', 'pos-sale-invoice.php?id=' + orderId);
    ...
    function populateOrderForm(order, items, loadedPayments) {
        ...
        if (order && order.id > 0) {
            var pfdRefreshUrl = 'ajax/get-pos-sale-invoice.php?id=' + encodeURIComponent(order.id) + '&_=' + (Date.now ? Date.now() : 0);
        ...
        var _pfClear = document.getElementById('paymentTableFooter');
        if (_pfClear) _pfClear.style.display = 'none';
        ...
        if (loadedPayments && loadedPayments.length > 0) {
            ...
            var _pfEmpty = document.getElementById('paymentTableFooter');
            if (_pfEmpty) _pfEmpty.style.display = 'none';
        }
    ...
    (function() {
        ...
        function doPopulateFromEmbed() {
            ...
            try {
                ...
                if (window.history && window.history.replaceState) window.history.replaceState({}, '', 'pos-sale-invoice.php?id=' + editId);
                window.isPurchaseInvoiceEditMode = true;
                return true;
            } catch (e) {
    ...
    function syncPosInlineToModal(type) {
        if (type === 'cash') {
            var s = document.getElementById('posInlineCashAmount');
            var t = document.getElementById('cashAmount');
            if (t && s) t.value = (s.value || '').trim() || '0.00';
        } else if (type === 'bank') {
            var sb = document.getElementById('posInlineBankDepositInto');
            var tb = document.getElementById('bankDepositInto');
            if (tb && sb) tb.value = sb.value || '';
            var tbt = document.getElementById('bankTransNo');
            if (tbt) tbt.value = '';
            var sa = document.getElementById('posInlineBankAmount');
            var ta = document.getElementById('bankAmount');
            if (ta && sa) ta.value = (sa.value || '').trim() || '0.00';
        } else if (type === 'upi') {
            var su = document.getElementById('posInlineUpiDepositInto');
            var tu = document.getElementById('upiDepositInto');
            if (tu && su) tu.value = su.value || '';
            var sut = document.getElementById('posInlineUpiTransNo');
            var tut = document.getElementById('upiTransNo');
            if (tut && sut) tut.value = sut.value || '';
            var sua = document.getElementById('posInlineUpiAmount');
            var tua = document.getElementById('upiAmount');
            if (tua && sua) tua.value = (sua.value || '').trim() || '0.00';
        } else if (type === 'card') {
            var sc = document.getElementById('posInlineCardDepositInto');
            var tc = document.getElementById('cardDepositInto');
            if (tc && sc) tc.value = sc.value || '';
            var sct = document.getElementById('posInlineCardTransNo');
            var tct = document.getElementById('cardTransNo');
            if (tct && sct) tct.value = sct.value || '';
            var scn = document.getElementById('posInlineCardNumber');
            var tcn = document.getElementById('cardNumber');
            if (tcn && scn) tcn.value = scn.value || '';
            var sca = document.getElementById('posInlineCardAmount');
            var tca = document.getElementById('cardAmount');
            if (tca && sca) tca.value = (sca.value || '').trim() || '0.00';
        }
    }
    
    function syncModalToPosInline() {
        var el, m;
        el = document.getElementById('posInlineCashAmount');
        m = document.getElementById('cashAmount');
        if (el && m) el.value = m.value;
        el = document.getElementById('posInlineBankDepositInto');
        m = document.getElementById('bankDepositInto');
        if (el && m) el.value = m.value;
        el = document.getElementById('posInlineBankAmount');
        m = document.getElementById('bankAmount');
        if (el && m) el.value = m.value;
        el = document.getElementById('posInlineUpiDepositInto');
        m = document.getElementById('upiDepositInto');
        if (el && m) el.value = m.value;
        el = document.getElementById('posInlineUpiTransNo');
        m = document.getElementById('upiTransNo');
        if (el && m) el.value = m.value;
        el = document.getElementById('posInlineUpiAmount');
        m = document.getElementById('upiAmount');
        if (el && m) el.value = m.value;
        el = document.getElementById('posInlineCardDepositInto');
        m = document.getElementById('cardDepositInto');
        if (el && m) el.value = m.value;
        el = document.getElementById('posInlineCardTransNo');
        m = document.getElementById('cardTransNo');
        if (el && m) el.value = m.value;
        el = document.getElementById('posInlineCardNumber');
        m = document.getElementById('cardNumber');
        if (el && m) el.value = m.value;
        el = document.getElementById('posInlineCardAmount');
        m = document.getElementById('cardAmount');
        if (el && m) el.value = m.value;
    }
    
    function prefillPosInlineAmountIfEmpty(inputEl) {
        if (!inputEl || String(inputEl.value || '').trim() !== '') return;
        ...
        if (balanceAmt > 0) inputEl.value = balanceAmt.toFixed(2);
    }
    
    function addPosInlinePayment(type) {
        syncPosInlineToModal(type);
        savePayment(type);
    }
    window.addPosInlinePayment = addPosInlinePayment;
    
    (function initPosInlinePaymentPrefill() {
        function bind(id) {
            var el = document.getElementById(id);
            if (!el) return;
            el.addEventListener('focus', function() {
                prefillPosInlineAmountIfEmpty(el);
            });
        }
        function run() {
            bind('posInlineCashAmount');
            bind('posInlineBankAmount');
            bind('posInlineCardAmount');
            bind('posInlineUpiAmount');
        }
        if (document.readyState === 'loading') {
    ...
    function savePayment(type) {
        ...
        if (type === 'bank') {
            ...
            if (!bankDepEl || String(bankDepEl.value || '').trim() === '') {
                ...
                var inlineBankDep = document.getElementById('posInlineBankDepositInto');
                if (inlineBankDep) {
                    inlineBankDep.focus();
                    inlineBankDep.classList.add('is-invalid');
                } else if (bankDepEl) {
                    bankDepEl.focus();
    ...
    function addPaymentToTable(payment) {
        ...
        if (!tbody) {
            console.error('Payment list (#paymentTableBody) not found. Payment card not added.');
            return;
        }
    ...
    function editPayment(paymentId) {
        ...
        if (!payment) {
            // Fallback: read from payment card (legacy table rows no longer used)
            const row = document.getElementById(paymentId);
            if (row && row.classList && row.classList.contains('pos-payment-card')) {
                ...
                if (type === 'cash' || type === 'bank' || type === 'upi' || type === 'card') {
                    syncModalToPosInline();
                }
        ...
        if (payment) {
            ...
            if (payment.type === 'cash' || payment.type === 'bank' || payment.type === 'upi' || payment.type === 'card') {
                syncModalToPosInline();
            }
            window._editingPaymentId = paymentId;
        }
    ...
    function clearPaymentModal(type) {
        if (type === 'cash') {
            ...
            var pic = document.getElementById('posInlineCashAmount');
            if (pic) pic.value = '';
        } else if (type === 'bank') {
            ...
            var pib = document.getElementById('posInlineBankDepositInto');
            if (pib) {
                pib.value = '';
                pib.classList.remove('is-invalid');
            }
            var piba = document.getElementById('posInlineBankAmount');
            if (piba) piba.value = '';
        } else if (type === 'cheque') {
            ...
            var piud = document.getElementById('posInlineUpiDepositInto');
            if (piud) piud.value = '';
            var piut = document.getElementById('posInlineUpiTransNo');
            if (piut) piut.value = '';
            var piua = document.getElementById('posInlineUpiAmount');
            if (piua) piua.value = '';
        } else if (type === 'card') {
            ...
            var picd = document.getElementById('posInlineCardDepositInto');
            if (picd) picd.value = '';
            var pict = document.getElementById('posInlineCardTransNo');
            if (pict) pict.value = '';
            var picn = document.getElementById('posInlineCardNumber');
            if (picn) picn.value = '';
            var pica = document.getElementById('posInlineCardAmount');
            if (pica) pica.value = '';
        } else if (type === 'metal-exchange') {
...
<script>
    ...
    function confirmPrintInvoice() {
        if (savedInvoiceId) {
            window.open('sale-invoice-print.php?id=' + savedInvoiceId + '&pos=1', '_blank', 'width=1200,height=800');
        }
