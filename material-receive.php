...
$mr_list_scope = function_exists('auragold_effective_branch_list_scope_sql')
...
$next_order_no = function_exists('getNextMaterialReceiveNo') ? getNextMaterialReceiveNo($conn) : 'MR-1';

// Load order for editing if ID provided
$edit_order_id = isset($_GET['id']) ? (int)$_GET['id'] : 0;
// True when ?id= refers to tbl_material_receives (do not treat as sale order id in JS history / loadOrder)
...
if ($edit_order_id > 0) {
    ...
    $jwo_row = getRecord("SELECT * FROM tbl_material_receives WHERE id = $edit_order_id");
    $mr_access_denied = '';
    if ($jwo_row && function_exists('auragold_branch_require_document_access') && auragold_tbl_has_column($conn, 'tbl_material_receives', 'branch_id')) {
        try {
            auragold_branch_require_document_access($conn, 'tbl_material_receives', $edit_order_id);
        } catch (Exception $e) {
            $jwo_row = null;
            $mr_access_denied = $e->getMessage();
            $material_branch_page_notice = $mr_access_denied;
        }
    }
    if ($jwo_row) {
        ...
        $edit_order = [
            'id' => $jwo_row['id'],
            'order_no' => $jwo_row['material_receive_no'],
        ...
        $jwo_items = getList("SELECT * FROM tbl_material_receive_items WHERE material_receive_id = $edit_order_id");
        foreach ($jwo_items as $it) {
            $it['order_id'] = $it['material_receive_id'];
            $edit_items[] = $it;
        }
        $next_order_no = $jwo_row['material_receive_no'];
        $is_jobwork_from_sale_order = true;
        $jwo_sale_order_id_param = (int)$jwo_row['sale_order_id'];
    } elseif ($mr_access_denied === '') {
...
if ($jwo_sale_order_id_param > 0 && $current_jwo_id === 0) {
        ...
        if ($ok_ro) {
            ...
            if ($jwo_ro) {
                ...
                if (!$jwo_from_process_selection) {
                    $rj_chk = @mysqli_query($conn, "SHOW TABLES LIKE 'tbl_repair_material_receives'");
                    if ($rj_chk && mysqli_num_rows($rj_chk) > 0) {
                        mysqli_free_result($rj_chk);
                        $existing_rjwo = getRecord("SELECT * FROM tbl_repair_material_receives WHERE repair_order_id = " . (int)$jwo_sale_order_id_param . " ORDER BY id DESC LIMIT 1");
                    } elseif ($rj_chk) {
                        mysqli_free_result($rj_chk);
                    }
                }
                if ($existing_rjwo && !empty($existing_rjwo['id'])) {
                    ...
                    $jw_no_cmp = isset($existing_rjwo['material_receive_no']) ? trim((string)$existing_rjwo['material_receive_no']) : '';
                    if ($jw_no_cmp !== '') {
                        $next_order_no = $existing_rjwo['material_receive_no'];
                    }
                    $rj_items = getList("SELECT * FROM tbl_repair_material_receive_items WHERE repair_material_receive_id = " . $jwo_current_rjwo_id);
                    $edit_items = [];
                    foreach ($rj_items as $it) {
                        $it['order_id'] = $it['repair_material_receive_id'];
                        $edit_items[] = $it;
                    }
                }
            }
    ...
    if ($jwo_so) {
        ...
        $existing_jwo = null;
        if (!$jwo_from_process_selection) {
            $existing_jwo = getRecord("SELECT * FROM tbl_material_receives WHERE sale_order_id = " . (int)$jwo_sale_order_id_param . $mr_list_scope . " ORDER BY id DESC LIMIT 1");
        }
        if ($existing_jwo && !empty($existing_jwo['id'])) {
            ...
            $jw_no_cmp = isset($existing_jwo['material_receive_no']) ? trim((string)$existing_jwo['material_receive_no']) : '';
            ...
            if ($jw_no_cmp !== '' && $so_order_no_cmp !== '' && strcasecmp($jw_no_cmp, $so_order_no_cmp) === 0) {
                $cfg_jwo_fix = function_exists('getMaterialReceiveBillSeriesConfig')
                    ? getMaterialReceiveBillSeriesConfig($conn)
                    : ['prefix' => 'MR-', 'suffix' => '', 'start_count' => 1, 'from_series_table' => false];
                $fix_no = function_exists('getNextMaterialReceiveNo') ? getNextMaterialReceiveNo($conn) : 'MR-1';
                $fix_esc = mysqli_real_escape_string($conn, $fix_no);
                $existing_chk = getRecord("SELECT id FROM tbl_material_receives WHERE material_receive_no = '$fix_esc'" . $mr_list_scope);
                $guard_fix = 0;
                while ($existing_chk && $guard_fix < 5000) {
                    $fix_no = function_exists('bumpMaterialReceiveNo') ? bumpMaterialReceiveNo($conn, $fix_no, $cfg_jwo_fix) : ($fix_no . '-1');
                    $fix_esc = mysqli_real_escape_string($conn, $fix_no);
                    $existing_chk = getRecord("SELECT id FROM tbl_material_receives WHERE material_receive_no = '$fix_esc'" . $mr_list_scope);
                    $guard_fix++;
                }
                mysqli_query($conn, "UPDATE tbl_material_receives SET material_receive_no = '$fix_esc' WHERE id = $current_jwo_id");
                $existing_jwo['material_receive_no'] = $fix_no;
                $next_order_no = $fix_no;
            } elseif ($jw_no_cmp !== '') {
                $next_order_no = $existing_jwo['material_receive_no'];
            }
            ...
            $jwo_items_existing = getList("SELECT * FROM tbl_material_receive_items WHERE material_receive_id = " . $current_jwo_id);
            $edit_items = [];
...
if ($selected_item_ids_param !== '') {
    ...
    if (!empty($selected_item_ids)) {
        ...
        if (!empty($bulk_items)) {
            ...
            // All selected process-list lines appear on one JWO draft; save inserts one tbl_material_receives row with multiple tbl_material_receive_items.
...
$mr_issued_load_soid = (int) ($jwo_sale_order_id_param ?? 0);
if ($mr_issued_load_soid < 1 && !empty($current_jwo_id)) {
    $mr_soid_row = getRecord('SELECT sale_order_id FROM tbl_material_receives WHERE id = ' . (int) $current_jwo_id . ' LIMIT 1');
    if ($mr_soid_row) {
        $mr_issued_load_soid = (int) ($mr_soid_row['sale_order_id'] ?? 0);
    }
}
if ($mr_issued_load_soid > 0) {
    require_once __DIR__ . '/includes/auragold_material_issue_rows_for_sale_order.php';
    if (!empty($jwo_from_repair_param)) {
        $mr_issued_diamond_rows = auragold_material_issue_list_diamond_rows_for_repair_order($conn, $mr_issued_load_soid);
        $mr_issued_stone_rows = auragold_material_issue_list_stone_rows_for_repair_order($conn, $mr_issued_load_soid);
    } else {
        ...
        $mr_load_metal_rows = static function (mysqli $c, int $soid, string $scope) {
            try {
                return auragold_material_issue_list_metal_exchange_rows_for_sale_order($c, $soid, $scope, true);
            } catch (Throwable $e) {
                error_log('material-receive metal exchange list: ' . $e->getMessage());
                return [];
            }
        };
        $mr_issued_metal_exchange_rows = $mr_load_metal_rows($conn, $mr_issued_load_soid, $mi_list_scope);
        if ($mr_issued_metal_exchange_rows === [] && $mi_list_scope !== '') {
            $mr_issued_metal_exchange_rows = $mr_load_metal_rows($conn, $mr_issued_load_soid, '');
        }
        try {
            if (function_exists('auragold_material_issue_embed_payments_for_sale_order')) {
                $mi_for_embed = function_exists('getRecord')
                    ? getRecord(
                        'SELECT id FROM tbl_material_issues WHERE sale_order_id = ' . (int) $mr_issued_load_soid
                        . ($mi_list_scope !== '' ? preg_replace('/\bbranch_id\b/', 'branch_id', $mi_list_scope) : '')
                        . ' ORDER BY id DESC LIMIT 1'
                    )
                    : null;
                if (!$mi_for_embed && $mi_list_scope !== '') {
                    $mi_for_embed = getRecord(
                        'SELECT id FROM tbl_material_issues WHERE sale_order_id = ' . (int) $mr_issued_load_soid
                        . ' ORDER BY id DESC LIMIT 1'
                    );
                }
...
<head>
<title>Material Receive — Gold Matrix</title>
...
text-transform: uppercase;
    ...
    /* —— Material Receive: mobile / tablet (≤991.98px) —— */
...
<link rel="stylesheet" href="assets/css/material-receive-issued-panels.css?v=<?php echo @filemtime(__DIR__ . '/assets/css/material-receive-issued-panels.css'); ?>">
</head>

<body class="material-receive-page">
<!-- [ Preloader ] Start -->
                ...
                <div class="container-fluid flex-grow-1" style="padding-top: 0; padding-bottom: 0;">
                    ...
                    <div class="row invoice-content-row">
                        ...
                        <div class="col-lg-8" >
                            ...
                            <div class="card mb-4">
                                <div class="card-body billing-form">
                                        <div class="row auragold-mq-billing-grid">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div style="position: relative; display: flex; align-items: stretch; gap: 4px;">
                                                        <div style="position: relative; flex: 1; min-width: 0;">
                                                            ...
                                                            <input type="text" class="form-control form-control-sm" id="jwoDepartmentUserSearch" autocomplete="off" placeholder="Search worker or mobile..." <?php echo $jwo_has_department_user_tables ? '' : 'disabled'; ?> value="<?php echo $jwo_has_department_user_tables ? htmlspecialchars((string) $jwo_department_user_display, ENT_QUOTES, 'UTF-8') : ''; ?>">
                                            ...
                                            <div class="col-md-4">
                                                <div class="form-group">
                                                    <label>Search Material Receive</label>
                                                    <div style="position: relative;">
                                                        <input type="text" class="form-control form-control-sm" id="searchSaleOrder" placeholder="Type MR no or customer name..." autocomplete="off">
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control form-control-sm" id="againstOf">
                                                    ...
                                                    <option value="Material Receive">Material Receive</option>
                            ...
                            <?php
                            $product_list_prefs_page = 'material-receive-product-table';
...
<?php require __DIR__ . '/includes/voucher_material_receive_issued_embed.php'; ?>
                            ...
                            <div class="summary-panel" >
                                ...
                                <div class="invoice-header" style="margin: -1rem -1rem 1rem -1rem; border-radius: 10px 10px 0 0; display: flex; align-items: center; justify-content: space-between;">
                                    <div style="display: flex; align-items: center; gap: 0.5rem; flex: 1;">
                                        ...
                                        <h5 class="mb-0" style="font-size: 0.9rem;">Material Receive No: <span id="currentOrderNo"><?php echo htmlspecialchars($next_order_no); ?></span></h5>
                                    ...
                                    <div class="invoice-header-actions">
                                        ...
                                        <span id="siSaveInvoiceWrap" class="si-save-invoice-wrap<?php echo !empty($si_save_blocked_by_purchase_fixing) ? ' si-save-wrap-blocked' : ''; ?>" title="<?php echo !empty($si_save_blocked_by_purchase_fixing) ? htmlspecialchars($si_pfd_save_blocked_tip, ENT_QUOTES, 'UTF-8') : htmlspecialchars('Save', ENT_QUOTES, 'UTF-8'); ?>">
                                            <button type="button" class="btn-save-invoice btn-sm" onclick="<?php echo (!empty($is_jobwork_from_sale_order) && $is_jobwork_from_sale_order) ? 'saveMaterialReceive()' : 'saveOrder()'; ?>" <?php echo !empty($si_save_blocked_by_purchase_fixing) ? 'disabled' : ''; ?>>Save</button>
                                        </span>
                                    </div>
                                </div>
...
<?php require __DIR__ . '/includes/voucher_mr_metal_receive_target_modal.php'; ?>
<?php require __DIR__ . '/includes/voucher_mr_ds_receive_target_modal.php'; ?>
<?php
...
<script>
    ...
    /** Open job work order print when tbl_material_receives row exists; else sale order print (same sale order id). */
    window.jobworkOrderPrintOpenUrl = function(saleOrderIdFallback) {
        ...
        if (jid > 0) {
            return 'material-receive-print.php?id=' + jid;
        }
        if (typeof window.jwoCurrentRecordId === 'number' && window.jwoCurrentRecordId > 0) {
            return 'material-receive-print.php?id=' + window.jwoCurrentRecordId;
        }
    ...
    window.jobworkOrderScreenUrl = function(saleOrderId) {
        if (!saleOrderId) return 'material-receive.php';
        return 'material-receive.php?sale_order_id=' + encodeURIComponent(saleOrderId);
    };
    ...
    const PRODUCT_MODAL_COLUMNS_PAGE = 'material-receive-product-modal';
    ...
    $(document).ready(function() {
        ...
        saleReturnSearchInput.on('input', function() {
            ...
            saleReturnSearchTimeout = setTimeout(function() {
                $.ajax({
                    url: 'ajax/search-material-receives.php',
                    type: 'GET',
                    data: { q: searchTerm },
                    dataType: 'json',
                    success: function(response) {
                        if (response.status === 'success' && response.orders && response.orders.length > 0) {
                            ...
                            response.orders.forEach(function(order) {
                                html += `
                                    <div class="material-receive-suggestion-item" 
                            ...
                            saleReturnSuggestions.html('<div style="padding: 0.75rem; color: #94a3b8; text-align: center;">No material receives found</div>').show();
                        }
                    },
                    error: function() {
                        saleReturnSuggestions.hide();
        ...
        $(document).on('click', '.material-receive-suggestion-item', function() {
            const jwoId = $(this).data('order-id');
            const soId = parseInt($(this).attr('data-sale-order-id') || '0', 10);
            saleReturnSearchInput.val('');
            saleReturnSuggestions.hide();
            if (soId > 0) {
                window.location.href = 'material-receive.php?sale_order_id=' + soId;
            } else if (jwoId) {
                window.location.href = 'material-receive.php?id=' + jwoId;
            }
        });
...
<td data-column="sale-amount-with"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" readonly style="width: 130px; font-size: 0.7rem;"></td>
    ...
    function addProductToTableFromModalRow(modalRowData, metalIdForImage) {
        ...
        try {
            ...
            if (!groupImagePayload && typeof window.auragoldCoParseGroupImageAttr === 'function') {
                var rowGiMr = window.auragoldCoParseGroupImageAttr(modalRowData.group_image || '');
                if (rowGiMr) groupImagePayload = rowGiMr;
            }
    ...
    function addMergedProductsToTable(modalRowsData, metalIdForImage) {
        ...
        var firstRowGiMrM = '';
        for (var _agiMrM = 0; _agiMrM < modalRowsData.length; _agiMrM++) {
            var rgi = modalRowsData[_agiMrM].group_image;
            if (rgi && String(rgi).trim()) { firstRowGiMrM = rgi; break; }
        }
        var groupImagePayload = tabPayloadMerge || ((typeof window.auragoldCoParseGroupImageAttr === 'function') ? window.auragoldCoParseGroupImageAttr(firstRowGiMrM) : '') || '';
        var groupImageAttr = (typeof groupImagePayload === 'object' && groupImagePayload != null) ? JSON.stringify(groupImagePayload) : (groupImagePayload || '');
        row.setAttribute('data-group-image', groupImageAttr);
        var primaryUrl = typeof getGroupImagePrimary === 'function' ? getGroupImagePrimary(groupImagePayload) : (typeof groupImagePayload === 'string' ? groupImagePayload : '');
        var actionCell = '<td><div class="action-btns"><button type="button" class="btn-edit" onclick="editProductRow(\'' + rowId + '\')" title="Edit"><i class="feather icon-edit"></i></button><button type="button" class="btn-delete" onclick="deleteProductRow(\'' + rowId + '\')" title="Delete"><i class="feather icon-trash-2"></i></button></div></td>';
    ...
    (function() {
        var PAGE_MODAL_COLUMN_PREFS = 'material-receive-product-modal';
        var TAB_MAIN = 'main';
    ...
    function mrAppendPaymentsToFormData(fd) {
        ...
        // Material-receive metal panel pending lines always post on Save (Inward stock).
    ...
    function saveMaterialReceive() {
        ...
        if (!saleOrderId) {
            alert('Sale order reference is missing. Open this screen from a sale order or use Search Material Receive.');
            return;
        }
        var deptEl = document.getElementById('jwoDepartment');
        var userEl = document.getElementById('jwoDepartmentUser');
        var deptSelected = deptEl && deptEl.value && String(deptEl.value).trim() !== '';
        if (window.jwoHasDepartmentUserTables && deptSelected && userEl && (!userEl.value || userEl.value.trim() === '')) {
            alert('Please select Worker Name for the selected department.');
            return;
        }
        if (isSaving) return;
        isSaving = true;
        ...
        if (window.jwoFromRepair) {
            ...
            fetch('ajax/save-repair-material-receive.php', { method: 'POST', body: fdRep, credentials: 'same-origin' })
                .then(function(r) { return r.json(); })
                .then(function(data) {
                    isSaving = false;
                    if (btn) { btn.disabled = false; btn.textContent = 'Save'; }
            ...
            .then(function(data) {
                ...
                if (data.status === 'success') {
                    ...
                    if (rem.length > 0) {
                        ...
                        window.location.href = 'material-receive.php?' + nextQs;
                        return;
                    }
                    ...
                    if (msg.indexOf('already exists') !== -1 && typeof swal === 'function') {
                        swal({
                            ...
                            if (isConfirm && data.jwo_id) {
                                if (saleOrderId > 0) {
                                    window.location.href = 'material-receive.php?sale_order_id=' + saleOrderId;
                                } else {
                                    window.location.href = 'material-receive.php?id=' + data.jwo_id;
                                }
                            }
                        });
                    } else {
    ...
    function confirmPrintJWO() {
        if (savedJwoId) {
            window.location.href = 'material-receive-print.php?id=' + savedJwoId;
        } else {
            closePrintJWOModal();
        }
    }
    ...
    function saleReturnAfterSavePrompts(returnId, returnNo, newBarcodes) {
        ...
        function doRedirect(options) {
            ...
            if (returnId) {
                window.location.href = typeof window.jobworkOrderScreenUrl === 'function'
                    ...
                    : ('material-receive.php?sale_order_id=' + encodeURIComponent(returnId));
            } else {
                var msg = 'Material Receive saved successfully! Order No: ' + (returnNo || 'N/A');
        ...
        function openReturnPrintModalOnly() {
            ...
            if (printU && confirm('Do you want to print the Material Receive?')) {
    ...
    function saveOrder() {
        ...
        var mrDeptEl = document.getElementById('jwoDepartment');
        if (mrDeptEl && mrDeptEl.value && String(mrDeptEl.value).trim() !== '') {
            orderData.department_id = mrDeptEl.value;
        }
        var mrDeptUserEl = document.getElementById('jwoDepartmentUser');
        if (mrDeptUserEl && mrDeptUserEl.value && String(mrDeptUserEl.value).trim() !== '') {
            orderData.department_user_id = mrDeptUserEl.value;
        }
    ...
    if (saveInvoiceBtn) {
        ...
        saveInvoiceBtn.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
            if (window.isJobworkFromSaleOrder && typeof saveMaterialReceive === 'function') {
                saveMaterialReceive();
            } else {
                saveOrder();
            }
    ...
    function updatePrintIcon(invoiceId) {
        ...
        if (printIcon) {
            if (invoiceId && invoiceId > 0) {
                ...
                printIcon.setAttribute('title', 'Print Material Receive');
            } else {
            ...
            jQuery.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        var histPathLoad = 'material-receive.php?sale_order_id=' + orderId;
            ...
            fetch(getUrl, { cache: 'no-store' })
                ...
                .then(data => {
                    ...
                    if (data.status === 'success') {
                        ...
                        var histPathLoadF = 'material-receive.php?sale_order_id=' + orderId;
                        if (window.isJobworkFromSaleOrder && typeof jobworkOrderCanonicalPath === 'function') {
    ...
    function jobworkOrderCanonicalPath(fallbackEditId) {
        ...
        if (window.jwoSaleOrderIdParam > 0) {
            return 'material-receive.php?sale_order_id=' + window.jwoSaleOrderIdParam;
        }
        if (window.jwoCurrentRecordId > 0) {
            return 'material-receive.php?id=' + window.jwoCurrentRecordId;
        }
        if (fallbackEditId) {
            return 'material-receive.php?sale_order_id=' + fallbackEditId;
        }
        return 'material-receive.php';
    }
            ...
            try {
                ...
                if (window.history && window.history.replaceState) {
                    ...
                    if (jwoPath) {
                        ...
                        window.history.replaceState({}, '', 'material-receive.php?sale_order_id=' + editId);
                    }
                }
                window.isPurchaseInvoiceEditMode = true;
                return true;
...
body.material-receive-page.stop-scrolling .sweet-overlay {
    background: rgba(15, 23, 42, 0.45);
}
body.material-receive-page .sweet-alert.sale-return-swal {
    border-radius: 20px;
    padding: 32px 28px 28px;
    box-shadow: 0 10px 40px rgba(0, 0, 0, 0.18);
    font-family: inherit;
}
body.material-receive-page .sweet-alert.sale-return-swal h2 {
    font-size: 1.35rem;
    font-weight: 700;
    color: #1e3a5f;
    margin: 0 0 12px;
...
body.material-receive-page .sweet-alert.sale-return-swal p.lead {
    color: #6b7280;
    font-size: 0.95rem;
    line-height: 1.5;
    margin: 0 0 8px;
}
body.material-receive-page .sweet-alert.sale-return-swal .sa-button-container {
    margin-top: 22px;
    gap: 14px;
    display: flex;
    flex-wrap: wrap;
...
body.material-receive-page .sweet-alert.sale-return-swal button.confirm.btn-sale-return-sw-confirm {
...
body.material-receive-page .sweet-alert.sale-return-swal button.confirm.btn-sale-return-sw-confirm:hover {
    filter: brightness(1.06);
}
body.material-receive-page .sweet-alert.sale-return-swal button.cancel.btn-sale-return-sw-cancel {
...
body.material-receive-page .sweet-alert.sale-return-swal button.cancel.btn-sale-return-sw-cancel:hover {
        ...
        setTimeout(function () {
            if (!srOpenPrintBillModal({
                ...
                var pu = (typeof window.jobworkOrderPrintOpenUrl === 'function' && window.jobworkOrderPrintOpenUrl(invoiceId))
                    || ('sale-order-print.php?id=' + encodeURIComponent(invoiceId));
                if (confirm('Material Receive saved successfully! Do you want to print?')) {
        ...
        if (!afterPrint && flow && flow.phase === 'barcode') {
            ...
            setTimeout(function () {
                if (typeof window.srOpenPrintBillModal === 'function' && document.getElementById('printInvoiceModal')) {
                    ...
                    if (confirm('Do you want to print the Material Receive?')) {
