...
$auragold_voucher_runtime_client = auragold_voucher_runtime_bootstrap($conn, $metals, 'Repair Order');
...
// Next order no: Bill Series (bill-series.php / tbl_bill_series) for voucher type "Repair Order"; else legacy RO-1
$next_order_no = function_exists('getNextRepairOrderNo') ? getNextRepairOrderNo($conn) : 'RO-1';

// Sales Person dropdown (stored in tbl_repair_orders.sales_person)
$sales_person_users = [];
...
// Load repair order for editing if ID provided (tbl_repair_orders)
...
if (!empty($edit_order_id)) {
    $edit_order = getRecord("SELECT * FROM tbl_repair_orders WHERE id = " . intval($edit_order_id));
    if ($edit_order) {
        $edit_items = getList("
        ...
        $edit_payments = getList("SELECT * FROM tbl_repair_order_payments WHERE order_id = " . intval($edit_order_id));
...
// Allowed Repair Order print languages (English + optional one from Invoice Print Settings)
                            ...
                            <div class="summary-panel" >
                                ...
                                <div class="invoice-header" style="margin: -1rem -1rem 1rem -1rem; border-radius: 10px 10px 0 0; display: flex; align-items: center; justify-content: space-between;">
                                    <div style="display: flex; align-items: center; gap: 0.5rem; flex: 1;">
                                        ...
                                        <h5 class="mb-0" style="font-size: 0.9rem;">Repair Order No: <span id="currentOrderNo"><?php echo htmlspecialchars($next_order_no); ?></span></h5>
                                    </div>
...
<script>
    ...
    const PRODUCT_MODAL_COLUMNS_PAGE = 'repair-order-product-modal';
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.status === 'success' && list.length > 0) {
                            ...
                            saleReturnSuggestions.html('<div style="padding: 0.75rem; color: #94a3b8; text-align: center;">No repair orders found</div>').show();
                        }
                    },
                    error: function() {
                        saleReturnSuggestions.hide();
...
<td data-column="sale-amount-with"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" readonly style="width: 130px; font-size: 0.7rem;"></td>
    ...
    function loadCustomerBalance() {
        ...
        // ledger_cl_balance: Σ(debit−credit) like Account Ledger / purchase invoice previous balance (not last-row balance_amount).
        url += 'type=supplier&ledger_cl_balance=1';
    ...
    (function() {
        ...
        const PAGE_COLUMN_PREFS = 'repair-order-product-table';
        const TAB_MAIN = 'main';
    ...
    (function() {
        var PAGE_MODAL_COLUMN_PREFS = 'repair-order-product-modal';
        var TAB_MAIN = 'main';
    ...
    /**
     * After successful save: same HTML “Print bill” modal as Sale Invoice — barcode labels (if any), then print repair order, then redirect.
     */
    function saleReturnAfterSavePrompts(returnId, returnNo, newBarcodes) {
        ...
        function doRedirect(options) {
            ...
            if (blankNew) {
                window.location.href = 'repair-order.php';
                return;
            }
            if (returnId) {
                window.location.href = 'repair-order.php?id=' + encodeURIComponent(returnId);
            } else {
                var msg = 'Repair Order saved successfully! Order No: ' + (returnNo || 'N/A');
        ...
        function openReturnPrintModalOnly() {
            ...
            if (typeof window.srOpenPrintBillModal === 'function' && document.getElementById('printInvoiceModal')) {
                window.srOpenPrintBillModal({
                    title: 'Print bill',
                    message: 'Do you want to print the repair order?',
                    whiteSpace: 'normal'
                });
                return;
            }
            if (confirm('Do you want to print Repair Order?')) {
                window.open('repair-order-print.php?id=' + encodeURIComponent(returnId), '_blank', 'width=1200,height=800');
    ...
    function saveOrder() {
        ...
        if (typeof jQuery !== 'undefined' && jQuery.ajax) {
            jQuery.ajax({
                url: 'ajax/save-repair-order.php',
                type: 'POST',
                data: postData,
                dataType: 'json',
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        alert('Error: ' + (response.message || 'Failed to save repair order'));
                    }
            ...
            fetch('ajax/save-repair-order.php', {
                method: 'POST',
                body: formData
            })
            .then(response => response.json())
            .then(data => {
                ...
                if (data.status === 'success') {
                    ...
                    alert('Error: ' + (data.message || 'Failed to save repair order'));
                }
                if (saveBtn && typeof updateSiSaveBlockedByPfd === 'function') {
                    updateSiSaveBlockedByPfd(!!window.siSaveBlockedByPurchaseFixing);
                } else if (saveBtn) {
    ...
    function resetOrder() {
        ...
        } else if (confirm('Are you sure you want to create a new repair order? All unsaved data will be lost.')) {
            window.location.href = 'repair-order.php';
        }
    }
    ...
    // Function to print repair order
    function printPurchaseInvoice(invoiceId) {
        ...
        if (!invoiceId || invoiceId <= 0) {
            if (typeof swal === 'function') {
                swal({
                    title: 'Save first',
                    text: 'Please save the repair order before printing.',
                    type: 'warning',
                    confirmButtonText: 'OK',
                    confirmButtonClass: 'btn-sale-return-sw-confirm',
                    customClass: 'sale-return-swal'
                ...
                alert('Please save the repair order first before printing.');
            }
            return;
        }
        
        // Open print page in new window
        window.open('repair-order-print.php?id=' + invoiceId, '_blank', 'width=1200,height=800');
    }
    ...
    function updatePrintIcon(invoiceId) {
        ...
        if (printIcon) {
            if (invoiceId && invoiceId > 0) {
                ...
                printIcon.setAttribute('title', 'Print Repair Order');
            } else {
    ...
    function loadOrder(orderId) {
        ...
        var getUrl = 'ajax/get-repair-order.php?id=' + orderId + '&_=' + (Date.now ? Date.now() : '');
        if (typeof jQuery !== 'undefined' && jQuery.ajax) {
            jQuery.ajax({
                url: getUrl,
                type: 'GET',
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        window.history.pushState({}, '', 'repair-order.php?id=' + orderId);
    ...
    function populateOrderForm(order, items, loadedPayments) {
        ...
        if (order && order.id > 0) {
            var pfdRefreshUrl = 'ajax/get-repair-order.php?id=' + encodeURIComponent(order.id) + '&_=' + (Date.now ? Date.now() : 0);
        ...
        // Load items as separate Product List rows in edit mode
        // (never merge on edit; each saved item must keep its own barcode row).
    ...
    (function() {
        ...
        function doPopulateFromEmbed() {
            ...
            try {
                ...
                if (window.history && window.history.replaceState) window.history.replaceState({}, '', 'repair-order.php?id=' + editId);
                window.isPurchaseInvoiceEditMode = true;
                return true;
            } catch (e) {
...
<div id="printInvoiceModal" class="print-invoice-modal" style="display: none;">
    <div class="print-invoice-modal-content">
        ...
        <p class="print-invoice-modal-message" id="printInvoiceModalMessage">Do you want to print the repair order?</p>
...
/* SweetAlert — Repair Order “print bill” style (navy Print / pink No, rounded card) */
body.repair-order-page.stop-scrolling .sweet-overlay {
    background: rgba(15, 23, 42, 0.45);
}
body.repair-order-page .sweet-alert.sale-return-swal {
    border-radius: 20px;
    padding: 32px 28px 28px;
    box-shadow: 0 10px 40px rgba(0, 0, 0, 0.18);
    font-family: inherit;
}
body.repair-order-page .sweet-alert.sale-return-swal h2 {
    font-size: 1.35rem;
    font-weight: 700;
    color: #1e3a5f;
    margin: 0 0 12px;
...
body.repair-order-page .sweet-alert.sale-return-swal p.lead {
    color: #6b7280;
    font-size: 0.95rem;
    line-height: 1.5;
    margin: 0 0 8px;
}
body.repair-order-page .sweet-alert.sale-return-swal .sa-button-container {
    margin-top: 22px;
    gap: 14px;
    display: flex;
    flex-wrap: wrap;
...
body.repair-order-page .sweet-alert.sale-return-swal button.confirm.btn-sale-return-sw-confirm {
...
body.repair-order-page .sweet-alert.sale-return-swal button.confirm.btn-sale-return-sw-confirm:hover {
    filter: brightness(1.06);
}
body.repair-order-page .sweet-alert.sale-return-swal button.cancel.btn-sale-return-sw-cancel {
...
body.repair-order-page .sweet-alert.sale-return-swal button.cancel.btn-sale-return-sw-cancel:hover {
...
<script>
    ...
    function showPrintInvoiceModal(invoiceId) {
        ...
        setTimeout(function () {
            if (!srOpenPrintBillModal({
                ...
                if (confirm('Repair Order saved successfully! Do you want to print?')) {
                    window.open('repair-order-print.php?id=' + invoiceId, '_blank', 'width=1200,height=800');
        ...
        if (!afterPrint && flow && flow.phase === 'barcode') {
            ...
            setTimeout(function () {
                if (typeof window.srOpenPrintBillModal === 'function' && document.getElementById('printInvoiceModal')) {
                    ...
                    if (confirm('Do you want to print Repair Order?')) {
                        window.open('repair-order-print.php?id=' + encodeURIComponent(nextReturnId), '_blank', 'width=1200,height=800');
    ...
    function confirmPrintInvoice() {
        ...
        if (flow && flow.phase === 'return') {
            window.open('repair-order-print.php?id=' + encodeURIComponent(flow.returnId), '_blank', 'width=1200,height=800');
            window._srPostSaveFlow = null;
            if (window._srPendingPostSaveRedirect) {
                window._srPendingPostSaveRedirect({ blankNew: false });
                window._srPendingPostSaveRedirect = null;
        ...
        if (savedInvoiceId) {
            window.open('repair-order-print.php?id=' + savedInvoiceId, '_blank', 'width=1200,height=800');
        }
        closePrintInvoiceModal({ afterPrint: true });
    }
