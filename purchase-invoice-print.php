...
$print_settings_early = function_exists('getInvoicePrintSettingsForDocument') ? getInvoicePrintSettingsForDocument('purchase_invoice') : [];
...
if (isset($_GET['lang'])) {
    ...
    $allowed_langs = function_exists('getInvoicePrintAllowedLanguages') ? getInvoicePrintAllowedLanguages('purchase_invoice') : ['en'];
...
$items = getList("
    SELECT pii.*, COALESCE(p.name, pii.product_name) as product_name, c.name as category_name
    ...
    WHERE pii.invoice_id = $invoice_id ORDER BY pii.id ASC
");
...
$print_settings = function_exists('getInvoicePrintSettingsForDocument') ? getInvoicePrintSettingsForDocument('purchase_invoice') : [];
...
$supplier_ref = $invoice['ref_no'] ?? ($invoice['supplier_id'] ?? '');
if (empty($supplier_ref) && !empty($invoice['supplier_id'])) {
    $sup = null;
    $tbl_sup = @mysqli_query($conn, "SHOW TABLES LIKE 'tbl_suppliers'");
    if ($tbl_sup && mysqli_num_rows($tbl_sup) > 0) {
        $sup = @getRecord("SELECT ref_no, mobile_no FROM tbl_suppliers WHERE id = " . (int)$invoice['supplier_id'] . " LIMIT 1");
    }
    if ($sup) $supplier_ref = $sup['ref_no'] ?? $sup['mobile_no'] ?? $invoice['supplier_id'];
    else $supplier_ref = (string)$invoice['supplier_id'];
}

$L = [
    'en' => [
        'tax_invoice' => 'TAX INVOICE', 'purchase_invoice' => 'PURCHASE INVOICE', 'invoice_no' => 'Invoice No.', 'date' => 'Date', 'salesman' => 'Salesman', 'purchase_person' => 'Purchase Person',
        'mr_ms' => 'Mr./M/S', 'supplier' => 'Supplier', 'customer_no' => 'Customer No.', 'supplier_no' => 'Supplier No.', 'passport_id' => 'Passport / ID No.', 'phone_no' => 'Phone No.',
    ...
    'hi' => [
        'tax_invoice' => 'कर चालान', 'purchase_invoice' => 'खरीद चालान', 'invoice_no' => 'चालान सं.', 'date' => 'तारीख', 'salesman' => 'विक्रेता', 'purchase_person' => 'खरीद व्यक्ति',
        'mr_ms' => 'श्री/श्रीमती', 'supplier' => 'आपूर्तिकर्ता', 'customer_no' => 'ग्राहक सं.', 'supplier_no' => 'आपूर्तिकर्ता सं.', 'passport_id' => 'पासपोर्ट / आईडी सं.', 'phone_no' => 'फोन सं.',
    ...
    'mr' => [
        'tax_invoice' => 'कर चालान', 'purchase_invoice' => 'खरेदी चालान', 'invoice_no' => 'चालान क्र.', 'date' => 'तारीख', 'salesman' => 'विक्रेता', 'purchase_person' => 'खरेदी व्यक्ती',
        'mr_ms' => 'श्री/श्रीमती', 'supplier' => 'पुरवठादार', 'customer_no' => 'ग्राहक क्र.', 'supplier_no' => 'पुरवठादार क्र.', 'passport_id' => 'पासपोर्ट / आयडी क्र.', 'phone_no' => 'फोन क्र.',
    ...
    'ar' => [
        'tax_invoice' => 'فاتورة ضريبية', 'purchase_invoice' => 'فاتورة الشراء', 'invoice_no' => 'رقم الفاتورة', 'date' => 'التاريخ', 'salesman' => 'البائع', 'purchase_person' => 'شخص الشراء',
        'mr_ms' => 'السيد / السيدة', 'supplier' => 'المورد', 'customer_no' => 'رقم العميل', 'supplier_no' => 'رقم المورد', 'passport_id' => 'رقم الجواز', 'phone_no' => 'رقم التلفون',
...
$party_ref = $supplier_ref;
$person_label = $t['purchase_person'];
$person_value = $invoice['purchase_person'] ?? '';
$doc_comment = $invoice['comment'] ?? '';
$doc_title = !empty($print_settings['invoice_title']) ? $print_settings['invoice_title'] : $t['purchase_invoice'];
$back_url = 'purchase-invoice.php';
$document = $invoice;

$template = isset($_GET['template_preview']) ? trim((string)$_GET['template_preview']) : (function_exists('getInvoiceTemplateForDocument') ? getInvoiceTemplateForDocument('purchase_invoice') : 'template_classic');
...
<head>
  ...
  <title><?php echo $t['purchase_invoice']; ?> - <?php echo htmlspecialchars($invoice['invoice_no']); ?></title>
  <style>
