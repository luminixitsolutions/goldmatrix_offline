...
/**
 * Diamond & Stone Analysis — Current Stock / Stock Details styled .xlsx (financial banner; same filters as page via GET).
 */
...
require_once __DIR__ . '/../includes/diamond_stone_analysis_export_data.php';
...
$data = auragold_dsa_analysis_export_fetch();
if ($data['error'] !== '') {
    header('HTTP/1.1 500 Internal Server Error');
...
$totals = $data['totals'];
$tab = $data['tab'];
$titles = auragold_dsa_analysis_export_headers();
$nCols = count($titles);
...
$periodLine = $tab === 'stock-details'
    ? 'Diamond & Stone Analysis — Stock Details (up to 15,000 rows; same filters as screen)'
    : 'Diamond & Stone Analysis — Current Stock (up to 15,000 rows; same filters as screen)';
...
foreach ($rows as $row) {
    ...
    $sheet->getStyle('B' . $r . ':' . $lastCol . $r)->getFill()->applyFromArray($fillPeachMetric);
...
$sheet->setCellValueExplicit('B' . $footRow, (int) round($tg($totals, 'total_qty'), 0), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_NUMERIC);
$sheet->setCellValueExplicit('C' . $footRow, $tg($totals, 'total_gross_weight'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_NUMERIC);
$sheet->setCellValueExplicit('D' . $footRow, $tg($totals, 'total_carat'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_NUMERIC);
$sheet->setCellValue('E' . $footRow, '');
$sheet->setCellValue('F' . $footRow, '');
$sheet->setCellValueExplicit('G' . $footRow, $tg($totals, 'total_pure_weight'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_NUMERIC);
$sheet->setCellValueExplicit('H' . $footRow, $tg($totals, 'total_diamond_ct'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_NUMERIC);
$sheet->setCellValueExplicit('I' . $footRow, $tg($totals, 'total_stone_weight'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_NUMERIC);
$sheet->setCellValueExplicit('J' . $footRow, $tg($totals, 'total_stone_ct'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_NUMERIC);
$sheet->setCellValueExplicit('K' . $footRow, $tg($totals, 'total_net_weight'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_NUMERIC);
$sheet->setCellValueExplicit('L' . $footRow, $tg($totals, 'total_purchase_amount'), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_NUMERIC);
...
$sheet->getStyle('B' . $footRow . ':' . $lastCol . $footRow)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
...
$fname = 'Diamond_Stone_Analysis_' . $fnameSuffix . '_' . date('Y-m-d') . '.xlsx';
while (ob_get_level()) {
    ob_end_clean();
}
