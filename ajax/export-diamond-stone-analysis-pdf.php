...
/**
 * Diamond & Stone Analysis — Current Stock / Stock Details PDF (same data as Excel export).
 */
...
$periodLine = $tab === 'stock-details'
    ? 'Diamond & Stone Analysis — Stock Details (up to 15,000 rows)'
    : 'Diamond & Stone Analysis — Current Stock (up to 15,000 rows)';
...
$html .= '<td style="text-align:center">' . htmlspecialchars(number_format($tg($totals, 'total_carat'), 3, '.', ''), ENT_QUOTES, 'UTF-8') . '</td>';
$html .= '<td></td><td></td>';
$html .= '<td style="text-align:center">' . htmlspecialchars(number_format($tg($totals, 'total_pure_weight'), 3, '.', ''), ENT_QUOTES, 'UTF-8') . '</td>';
$html .= '<td style="text-align:center">' . htmlspecialchars(number_format($tg($totals, 'total_diamond_ct'), 3, '.', ''), ENT_QUOTES, 'UTF-8') . '</td>';
$html .= '<td style="text-align:center">' . htmlspecialchars(number_format($tg($totals, 'total_stone_weight'), 3, '.', ''), ENT_QUOTES, 'UTF-8') . '</td>';
$html .= '<td style="text-align:center">' . htmlspecialchars(number_format($tg($totals, 'total_stone_ct'), 3, '.', ''), ENT_QUOTES, 'UTF-8') . '</td>';
...
$fname = 'Diamond_Stone_Analysis_' . $fnameSuffix . '_' . date('Y-m-d') . '.pdf';
while (ob_get_level()) {
    ob_end_clean();
}
