...
if (!$tbl || mysqli_num_rows($tbl) == 0) {
    echo json_encode(['status' => 'error', 'message' => 'Debit note tables not found. Please run admin/sql/create_debit_note_tables.sql']);
    exit;
}
...
try {
    ...
    $debit_note_no = esc($_POST['order_no'] ?? '');
    $debit_note_id = isset($_POST['order_id']) ? (int)$_POST['order_id'] : 0;
    ...
    $__fx = function_exists('auragold_exchange_rate_sql_fragments') ? auragold_exchange_rate_sql_fragments($conn, 'tbl_debit_notes') : ['has'=>false,'insert_col'=>'','insert_val'=>'','update'=>'','rate'=>1];
    ...
    $debit_note_date = esc($_POST['order_date'] ?? date('Y-m-d'));
    ...
    if (empty($debit_note_no)) {
        $last = getRecord("SELECT debit_note_no FROM tbl_debit_notes ORDER BY id DESC LIMIT 1");
        if ($last && $last['debit_note_no']) {
            $num = (int)preg_replace('/[^0-9]/', '', $last['debit_note_no']);
            $debit_note_no = 'DN-' . ($num + 1);
        } else {
            $debit_note_no = 'DN-1';
        }
    }

    $is_update = ($debit_note_id > 0);

    if ($is_update) {
        $cur = getRecord("SELECT debit_note_no FROM tbl_debit_notes WHERE id = $debit_note_id");
        $cur_no = $cur ? $cur['debit_note_no'] : '';
        if ($debit_note_no !== $cur_no) {
            $ex = getRecord("SELECT id FROM tbl_debit_notes WHERE debit_note_no = '$debit_note_no' AND id != $debit_note_id");
            if ($ex) throw new Exception("Debit Note No '$debit_note_no' already exists.");
        }
        mysqli_query($conn, "
            UPDATE tbl_debit_notes SET
                ...
                updated_at = NOW()
            WHERE id = $debit_note_id
        ");
        if (mysqli_error($conn)) throw new Exception('Update failed: ' . mysqli_error($conn));
        mysqli_query($conn, "DELETE FROM tbl_debit_note_items WHERE debit_note_id = $debit_note_id");
        mysqli_query($conn, "DELETE FROM tbl_debit_note_payments WHERE debit_note_id = $debit_note_id");
    } else {
        mysqli_query($conn, "
            INSERT INTO tbl_debit_notes (
                debit_note_no, customer_id, customer_name, against_of, currency$fx_insert_col_sql, ref_no, sales_person,
                debit_note_date, due_date, layaways_id, fixing_type,
        ...
        $debit_note_id = mysqli_insert_id($conn);
    }
    ...
    if (!empty($items) && is_array($items)) {
        foreach ($items as $item) {
            ...
            $iq = "INSERT INTO tbl_debit_note_items (
                debit_note_id, product_id, product_characteristic_id, barcode, product_name, carat,
                ...
                $debit_note_id, $product_id, " . ($characteristic_id ? $characteristic_id : 'NULL') . ",
    ...
    if (!empty($payments) && is_array($payments)) {
        foreach ($payments as $p) {
            ...
            $pq = "INSERT INTO tbl_debit_note_payments (
                debit_note_id, payment_type, deposit_into, transaction_no, cheque_date, purity_carat,
                amount, previous_balance_amount, current_order_amount, diamond_category, quantity, status, created_at
            ) VALUES (
                $debit_note_id, '$payment_type',
    ...
    echo json_encode([
        ...
        'order_id' => $debit_note_id,
        'order_no' => $debit_note_no,
