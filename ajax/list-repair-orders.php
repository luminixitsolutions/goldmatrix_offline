...
$query = "
    SELECT 
        ...
        (SELECT roi.product_name FROM tbl_repair_order_items roi WHERE roi.order_id = o.id ORDER BY roi.id LIMIT 1) AS first_item,
        (SELECT COALESCE(SUM(roi.gross_weight), 0) FROM tbl_repair_order_items roi WHERE roi.order_id = o.id) AS gross_wt
