...
/**
 * Imitation / Watches — serial list export to styled .xlsx (financial banner; same scope as screen / session branch).
 */
...
require_once __DIR__ . '/../includes/imitation_watches_stock_export_data.php';
...
$data = auragold_imitation_watches_stock_export_fetch_rows($conn, 15000);
if ($data['error'] !== '') {
    header('HTTP/1.1 500 Internal Server Error');
...
$keys = auragold_imitation_watches_stock_export_column_keys();
$titles = auragold_imitation_watches_stock_export_header_titles();
$nCols = count($titles);
$lastCol = Coordinate::stringFromColumnIndex($nCols);
$numKeySet = array_flip(auragold_imitation_watches_stock_export_numeric_keys());

$grand = auragold_imitation_watches_stock_export_grand_initial();
$wps = 0.0;
$wpc = 0;
$rowScalars = [];
foreach ($rows as $r) {
    $s = auragold_imitation_watches_stock_export_row_scalars($r, $siteUrl);
    $rowScalars[] = $s;
    $acc = auragold_imitation_watches_stock_export_accumulate_grand($grand, $wps, $wpc, $s);
    $grand = $acc['grand'];
...
$periodLine = 'Imitation / Watches — serial list (up to 15,000 rows; same branch scope as screen)';
...
$fname = 'Imitation_Watches_' . date('Y-m-d') . '.xlsx';
while (ob_get_level()) {
    ob_end_clean();
}
