...
if ($order_id < 1) {
    echo json_encode(['ok' => false, 'message' => 'Save the sale order first, then allocate stones.']);
    exit;
}
...
if ($lines === []) {
    echo json_encode(['ok' => false, 'message' => 'Select at least one row with quantity or weight.']);
    exit;
}
...
$tx_err = '';
try {
    $stats = auragold_sale_order_apply_stone_allocations($conn, $order_id, $lines, $order_no, $order_date, $tx_ok, $tx_err);
    if (!$tx_ok) {
    ...
    echo json_encode([
        ...
        'message' => $stats['saved'] > 0
            ? ('Allocated ' . (int) $stats['saved'] . ' stone line(s). Stock updated.')
            : 'Nothing allocated.',
