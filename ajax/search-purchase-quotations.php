...
$scope = auragold_tbl_has_column($conn, 'tbl_purchase_quotations', 'branch_id')
    ? auragold_sql_and_branch_scope($conn, 'tbl_purchase_quotations', 'tbl_purchase_quotations')
    : '';
// Search purchase quotations by supplier name or quotation number
$query = "
    SELECT 
        id, 
        quotation_no,
...
foreach ($quotations as $quotation) {
    $results[] = [
        ...
        'customer_name' => $quotation['supplier_name'] ?? '',
        'supplier_id' => $quotation['supplier_id'] ?? 0,
        'quotation_date' => $quotation['quotation_date'] ?? '',
        'grand_total' => $quotation['grand_total'] ?? 0,
        'currency' => $quotation['currency'] ?? 'USD',
        'display_text' => $quotation['quotation_no'] . ' - ' . $quotation['supplier_name'] . 
