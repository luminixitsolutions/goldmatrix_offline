 ...
 * Returns JSON string for DB: {"primary":"uploads/sale-invoice/1/item_5_0.png","images":["..."]} or empty string.
 */
function save_sale_invoice_item_images($group_image, $invoice_id, $item_id) {
    ...
    $base_dir = dirname(__DIR__) . '/uploads/sale-invoice/' . (int)$invoice_id;
    if (!is_dir($base_dir)) {
    ...
    foreach ($data_urls as $i => $data_url) {
        ...
        $relative = 'uploads/sale-invoice/' . (int)$invoice_id . '/' . $filename;
        $paths[] = $relative;
...
/**
 * Merge JSON from tbl_sale_invoice_payments.payment_details into the row (edit/save round-trip).
 */
...
try {
    ...
    $save_as_draft = !empty($_POST['save_as_draft']) && (int) $_POST['save_as_draft'] === 1;
    $invoice_doc_status = $save_as_draft ? 'draft' : 'saved';
    ...
    $__fx = function_exists('auragold_exchange_rate_sql_fragments') ? auragold_exchange_rate_sql_fragments($conn, 'tbl_sale_invoices') : ['has'=>false,'insert_col'=>'','insert_val'=>'','update'=>'','rate'=>1];
    ...
    $ozCols = auragold_ensure_sale_invoice_ounce_rate_columns($conn);
    $ozSave = auragold_sale_invoice_ounce_rate_save_values($conn);
    ...
    if ($has_discount_percent_col === null) {
        $chk = mysqli_query($conn, "SHOW COLUMNS FROM tbl_sale_invoices LIKE 'discount_percent'");
        $has_discount_percent_col = ($chk && mysqli_num_rows($chk) > 0);
        if (!$has_discount_percent_col) {
            mysqli_query($conn, "ALTER TABLE tbl_sale_invoices ADD COLUMN discount_percent DECIMAL(10,2) DEFAULT 0 AFTER discount_amt");
            $has_discount_percent_col = true;
        }
        if ($chk) mysqli_free_result($chk);
    }

    // E-Way Bill: tbl_ewaybill_generate_logs + tbl_sale_invoices e-Way columns (see ewaybill_ensure_eway_bill_migrations)
    if (function_exists('ewaybill_ensure_eway_bill_migrations')) {
        ewaybill_ensure_eway_bill_migrations($conn);
    } elseif (function_exists('ewaybill_ensure_generate_log_table') && function_exists('ewaybill_ensure_sale_invoice_eway_extras')) {
        ...
        if ($has_eway_bootstrap === null) {
            ...
            if ($do_boot) {
                @mysqli_query($conn, 'ALTER TABLE tbl_sale_invoices ADD COLUMN customer_gstin VARCHAR(20) NULL DEFAULT NULL');
                @mysqli_query($conn, 'ALTER TABLE tbl_sale_invoices ADD COLUMN eway_vehicle_no VARCHAR(32) NULL DEFAULT NULL');
                @mysqli_query($conn, 'ALTER TABLE tbl_sale_invoices ADD COLUMN eway_distance_km DECIMAL(10,2) NULL DEFAULT NULL');
                @mysqli_query($conn, 'ALTER TABLE tbl_sale_invoices ADD COLUMN eway_bill_no VARCHAR(50) NULL DEFAULT NULL');
                @mysqli_query($conn, 'ALTER TABLE tbl_sale_invoices ADD COLUMN eway_bill_date VARCHAR(50) NULL DEFAULT NULL');
                @mysqli_query($conn, 'ALTER TABLE tbl_sale_invoices ADD COLUMN eway_status VARCHAR(50) NULL DEFAULT NULL');
                @mysqli_query($conn, 'ALTER TABLE tbl_sale_invoices ADD COLUMN eway_response LONGTEXT NULL');
            }
    ...
    if ($has_eway_columns === null) {
        $ew_chk2 = mysqli_query($conn, "SHOW COLUMNS FROM tbl_sale_invoices LIKE 'eway_bill_no'");
    ...
    $pdg = @mysqli_query($conn, "SHOW COLUMNS FROM tbl_sale_invoices LIKE 'previous_diamond'");
    if ($pdg && mysqli_num_rows($pdg) > 0) {
        $pdg2 = @mysqli_query($conn, "SHOW COLUMNS FROM tbl_sale_invoices LIKE 'previous_gemstone'");
    ...
    if (!$has_previous_diamond_gemstone) {
        @mysqli_query($conn, "ALTER TABLE tbl_sale_invoices ADD COLUMN previous_diamond DECIMAL(12,3) DEFAULT 0 AFTER previous_silver");
        @mysqli_query($conn, "ALTER TABLE tbl_sale_invoices ADD COLUMN previous_gemstone DECIMAL(12,3) DEFAULT 0 AFTER previous_diamond");
    ...
    $cols = mysqli_query($conn, "SHOW COLUMNS FROM tbl_sale_invoices LIKE 'use_previous_balance'");
    if ($cols && mysqli_num_rows($cols) > 0) {
        $cols2 = mysqli_query($conn, "SHOW COLUMNS FROM tbl_sale_invoices LIKE 'previous_balance_used_amt'");
    ...
    $prev_invoice_row = null;
    $was_draft_status = false;
    if ($is_update && $invoice_id > 0) {
        $prev_invoice_row = getRecord("SELECT invoice_no, status FROM tbl_sale_invoices WHERE id = " . (int) $invoice_id . " LIMIT 1");
        if ($prev_invoice_row) {
            $was_draft_status = auragold_sale_invoice_is_draft_status($prev_invoice_row['status'] ?? '');
        }
    }

    $posted_order_no = trim((string) ($_POST['order_no'] ?? ''));
    if ($save_as_draft) {
        if ($is_update && $invoice_id > 0) {
            $prev_no = trim((string) ($prev_invoice_row['invoice_no'] ?? ''));
            if (preg_match('/^DRAFT-/i', $prev_no)) {
                $invoice_no = esc($prev_no);
            } else {
                $invoice_no = esc(auragold_sale_invoice_draft_label($invoice_id));
            }
        } else {
            $invoice_no = esc('__DRAFT_PENDING__' . uniqid('', true));
        }
    } elseif ($was_draft_status || auragold_sale_invoice_is_draft_placeholder_no($posted_order_no) || auragold_sale_invoice_is_draft_placeholder_no($invoice_no)) {
        $invoice_no = esc(getNextSaleInvoiceNo($conn));
    } elseif (empty($invoice_no)) {
        $invoice_no = esc(getNextSaleInvoiceNo($conn));
    }

    $has_si_branch_id = auragold_ensure_sale_invoice_branch_id_column($conn);
    $eff_branch       = auragold_effective_branch_id();
    $sale_inv_branch_row_id = 0;
    if ($is_update && $has_si_branch_id) {
        $own_br = getRecord("SELECT branch_id FROM tbl_sale_invoices WHERE id = $invoice_id LIMIT 1");
    ...
    // Validation
    if (empty($customer_name) && !$save_as_draft) {
    ...
    if (empty($invoice_no) && !$save_as_draft) {
        // Bill Series (bill-series.php): voucher "Sales Invoice" → prefix + number + suffix (e.g. SP10); else legacy SI-1, SI-2
        $invoice_no = esc(getNextSaleInvoiceNo($conn));
    }

    // For new invoices, if invoice_no already exists on an *active* sale/POS row, bump until unique.
    // Soft-deleted rows must not block reuse (delete SI-81 → next save can use SI-81 again).
    if (!$save_as_draft) {
    if (!$is_update) {
        ...
        while ($existing_invoice && $guard < 5000) {
            $invoice_no = esc(bumpSaleInvoiceNo($conn, $invoice_no, $cfg));
            $existing_invoice = getRecord("SELECT id FROM tbl_sale_invoices WHERE invoice_no = '$invoice_no'" . $si_active);
            if (!$existing_invoice) {
                $existing_invoice = getRecord("SELECT id FROM tbl_pos_sale_invoices WHERE invoice_no = '$invoice_no'" . $pos_active);
            }
        ...
        // Soft-deleted rows still hold UNIQUE(invoice_no) — rename them so INSERT can reuse SI-113 etc.
    ...
    if ($is_update) {
        ...
        $current_invoice = getRecord("SELECT invoice_no FROM tbl_sale_invoices WHERE id = $invoice_id");
        ...
        // Check if invoice_no is being changed and if it conflicts with another active sale or POS invoice
        if ($invoice_no !== $current_invoice_no) {
            $si_active = auragold_doc_series_active_sql($conn, 'tbl_sale_invoices');
            $pos_active = auragold_doc_series_active_sql($conn, 'tbl_pos_sale_invoices');
            $existing_invoice = getRecord("SELECT id FROM tbl_sale_invoices WHERE invoice_no = '$invoice_no' AND id != $invoice_id" . $si_active);
            if (!$existing_invoice) {
                $existing_invoice = getRecord("SELECT id FROM tbl_pos_sale_invoices WHERE invoice_no = '$invoice_no'" . $pos_active);
            }
            if ($existing_invoice) {
        ...
        if (!empty($ozCols['enabled'])) {
            $update_fields[] = 'ounce_rate_enabled = ' . (int) $ozSave['enabled'];
        }
        if (!empty($ozCols['rate'])) {
            $update_fields[] = 'ounce_rate = ' . (float) $ozSave['rate'];
        }
        if (!empty($ozCols['json'])) {
            $update_fields[] = "ounce_rate_json = '" . $ozSave['json'] . "'";
        }
        ...
        $update_fields[] = "status = '" . mysqli_real_escape_string($conn, $invoice_doc_status) . "'";
        $update_fields[] = "updated_at = NOW()";
        
        $sql = "UPDATE tbl_sale_invoices SET " . implode(", ", $update_fields) . " WHERE id = $invoice_id";
        ...
        mysqli_query($conn, "DELETE FROM tbl_sale_invoice_items WHERE invoice_id = $invoice_id");
        mysqli_query($conn, "DELETE FROM tbl_sale_invoice_payments WHERE invoice_id = $invoice_id");
        
        // Reverse previous outward stock: restore qty/weight onto latest inward/balance row, then delete linked outward rows
        if (!$save_as_draft && $tbl_stock_has_reference) {
            $rev_rows = getList("SELECT barcode, opening_weight, opening_qty, product_id, product_characteristic_id, branch_id FROM tbl_stock WHERE stock_type = 'outward' AND reference_id = $invoice_id AND reference_type = 'sale_invoice'");
            ...
            $del_stock = mysqli_query($conn, "DELETE FROM tbl_stock WHERE stock_type = 'outward' AND reference_id = $invoice_id AND reference_type = 'sale_invoice'");
            if (!$del_stock) {
        ...
        if (!empty($ozCols['enabled'])) {
            $insert_fields[] = 'ounce_rate_enabled';
        }
        if (!empty($ozCols['rate'])) {
            $insert_fields[] = 'ounce_rate';
        }
        if (!empty($ozCols['json'])) {
            $insert_fields[] = 'ounce_rate_json';
        }
        ...
        if (!empty($ozCols['enabled'])) {
            $insert_values[] = (int) $ozSave['enabled'];
        }
        if (!empty($ozCols['rate'])) {
            $insert_values[] = (float) $ozSave['rate'];
        }
        if (!empty($ozCols['json'])) {
            $insert_values[] = "'" . $ozSave['json'] . "'";
        }
        $insert_values = array_merge($insert_values, [
            $previous_balance,
            $previous_gold,
        ...
        $insert_values[] = "'" . mysqli_real_escape_string($conn, $invoice_doc_status) . "'";
        ...
        $sql = "INSERT INTO tbl_sale_invoices (" . implode(", ", $insert_fields) . ") VALUES (" . implode(", ", $insert_values) . ")";
        
        if (!mysqli_query($conn, $sql)) {
            $error = mysqli_error($conn);
            error_log("Sale Invoice Insert SQL Error: " . $error);
            error_log("Sale Invoice Insert SQL: " . $sql);
            if (strpos($error, 'Duplicate entry') !== false && function_exists('auragold_release_soft_deleted_sale_invoice_no')) {
                auragold_release_soft_deleted_sale_invoice_no($conn, $invoice_no);
                if (!mysqli_query($conn, $sql)) {
        ...
        if ($save_as_draft && $invoice_id > 0) {
            $draft_no = esc(auragold_sale_invoice_draft_label($invoice_id));
            mysqli_query($conn, "UPDATE tbl_sale_invoices SET invoice_no = '$draft_no' WHERE id = " . (int) $invoice_id);
            $invoice_no = $draft_no;
        }
    }
    ...
    if (!empty($has_si_branch_id)) {
        $sib_row = getRecord('SELECT branch_id FROM tbl_sale_invoices WHERE id = ' . (int) $invoice_id . ' LIMIT 1');
    ...
    $tbl_sale_invoice_items_has_images = false;
    $col_images = @mysqli_query($conn, "SHOW COLUMNS FROM tbl_sale_invoice_items LIKE 'images'");
    if ($col_images && mysqli_num_rows($col_images) > 0) {
        $tbl_sale_invoice_items_has_images = true;
        mysqli_free_result($col_images);
    }
    $tbl_sale_invoice_items_has_diamond_category = false;
    $col_diamond_cat = @mysqli_query($conn, "SHOW COLUMNS FROM tbl_sale_invoice_items LIKE 'diamond_category'");
    if ($col_diamond_cat && mysqli_num_rows($col_diamond_cat) > 0) {
        $tbl_sale_invoice_items_has_diamond_category = true;
        mysqli_free_result($col_diamond_cat);
    }
    $tbl_sale_invoice_items_has_metal_rate = false;
    $col_metal_rate = @mysqli_query($conn, "SHOW COLUMNS FROM tbl_sale_invoice_items LIKE 'metal_rate'");
    if ($col_metal_rate && mysqli_num_rows($col_metal_rate) > 0) {
        $tbl_sale_invoice_items_has_metal_rate = true;
        mysqli_free_result($col_metal_rate);
    }
    $tbl_sale_invoice_items_has_calculation_type = false;
    $col_calc = @mysqli_query($conn, "SHOW COLUMNS FROM tbl_sale_invoice_items LIKE 'calculation_type'");
    if ($col_calc && mysqli_num_rows($col_calc) > 0) {
        $tbl_sale_invoice_items_has_calculation_type = true;
        mysqli_free_result($col_calc);
    }
    $tbl_sale_invoice_items_has_diamond_amount = false;
    $col_da = @mysqli_query($conn, "SHOW COLUMNS FROM tbl_sale_invoice_items LIKE 'diamond_amount'");
    if ($col_da && mysqli_num_rows($col_da) > 0) {
        $tbl_sale_invoice_items_has_diamond_amount = true;
        mysqli_free_result($col_da);
    }
    $tbl_sale_invoice_items_has_stone_amount = false;
    $col_sa = @mysqli_query($conn, "SHOW COLUMNS FROM tbl_sale_invoice_items LIKE 'stone_amount'");
    if ($col_sa && mysqli_num_rows($col_sa) > 0) {
        $tbl_sale_invoice_items_has_stone_amount = true;
        mysqli_free_result($col_sa);
    }
    $tbl_sale_invoice_items_has_stone_weight = false;
    $col_sw = @mysqli_query($conn, "SHOW COLUMNS FROM tbl_sale_invoice_items LIKE 'stone_weight'");
    if ($col_sw && mysqli_num_rows($col_sw) > 0) {
        $tbl_sale_invoice_items_has_stone_weight = true;
        mysqli_free_result($col_sw);
    }
    $tbl_sale_invoice_items_has_metal_value = false;
    $col_mv = @mysqli_query($conn, "SHOW COLUMNS FROM tbl_sale_invoice_items LIKE 'metal_value'");
    if ($col_mv && mysqli_num_rows($col_mv) > 0) {
        $tbl_sale_invoice_items_has_metal_value = true;
        mysqli_free_result($col_mv);
    }
    $tbl_sale_invoice_items_has_metal_qty = false;
    $col_mq = @mysqli_query($conn, "SHOW COLUMNS FROM tbl_sale_invoice_items LIKE 'metal_qty'");
    if ($col_mq && mysqli_num_rows($col_mq) > 0) {
        $tbl_sale_invoice_items_has_metal_qty = true;
        mysqli_free_result($col_mq);
    } else {
        if ($col_mq) mysqli_free_result($col_mq);
        @mysqli_query($conn, "ALTER TABLE tbl_sale_invoice_items ADD COLUMN metal_qty DECIMAL(12,2) DEFAULT 1.00 AFTER quantity");
        $tbl_sale_invoice_items_has_metal_qty = true;
    }
    $tbl_sale_invoice_items_has_metal_weight = false;
    $col_mw = @mysqli_query($conn, "SHOW COLUMNS FROM tbl_sale_invoice_items LIKE 'metal_weight'");
    if ($col_mw && mysqli_num_rows($col_mw) > 0) {
        $tbl_sale_invoice_items_has_metal_weight = true;
        mysqli_free_result($col_mw);
    } else {
        if ($col_mw) mysqli_free_result($col_mw);
        @mysqli_query($conn, "ALTER TABLE tbl_sale_invoice_items ADD COLUMN metal_weight DECIMAL(12,4) DEFAULT 0.0000 AFTER metal_qty");
        $tbl_sale_invoice_items_has_metal_weight = true;
    }
    $tbl_sale_invoice_items_has_sort_order = false;
    $col_sort = @mysqli_query($conn, "SHOW COLUMNS FROM tbl_sale_invoice_items LIKE 'sort_order'");
    if ($col_sort && mysqli_num_rows($col_sort) > 0) {
        $tbl_sale_invoice_items_has_sort_order = true;
        mysqli_free_result($col_sort);
    } else {
        if ($col_sort) mysqli_free_result($col_sort);
        @mysqli_query($conn, "ALTER TABLE tbl_sale_invoice_items ADD COLUMN sort_order INT(11) NOT NULL DEFAULT 0 COMMENT 'Display order (drag-and-drop)' AFTER invoice_id");
        $tbl_sale_invoice_items_has_sort_order = true;
    }

    $tbl_sale_invoice_items_has_merge_group_index = false;
    $col_mgix = @mysqli_query($conn, "SHOW COLUMNS FROM tbl_sale_invoice_items LIKE 'merge_group_index'");
    if ($col_mgix && mysqli_num_rows($col_mgix) > 0) {
        $tbl_sale_invoice_items_has_merge_group_index = true;
        mysqli_free_result($col_mgix);
    } else {
        if ($col_mgix) {
            mysqli_free_result($col_mgix);
        }
        @mysqli_query($conn, "ALTER TABLE tbl_sale_invoice_items ADD COLUMN merge_group_index INT UNSIGNED NULL DEFAULT NULL COMMENT 'Same value = same product list row (merged modal lines)' AFTER net_amt_with_tax");
        $col_mgix2 = @mysqli_query($conn, "SHOW COLUMNS FROM tbl_sale_invoice_items LIKE 'merge_group_index'");
    ...
    if (!empty($items) && is_array($items)) {
        ...
        foreach ($items as $item) {
            ...
            if ($product_id > 0) {
                ...
                $sort_order_val = $tbl_sale_invoice_items_has_sort_order ? "$sort_order, " : "";
                $diamond_cat_col = $tbl_sale_invoice_items_has_diamond_category ? ", diamond_category" : "";
                $diamond_cat_val = $tbl_sale_invoice_items_has_diamond_category ? ", " . ($diamond_category ? "'$diamond_category'" : "NULL") : "";
                $metal_rate_col = $tbl_sale_invoice_items_has_metal_rate ? ", metal_rate" : "";
                $metal_rate_val = $tbl_sale_invoice_items_has_metal_rate ? ", $metal_rate" : "";
                $calc_col = $tbl_sale_invoice_items_has_calculation_type ? ", calculation_type" : "";
                $calc_val = $tbl_sale_invoice_items_has_calculation_type ? ", " . ($calculation_type ? "'" . mysqli_real_escape_string($conn, $calculation_type) . "'" : "NULL") : "";
                $da_col = $tbl_sale_invoice_items_has_diamond_amount ? ", diamond_amount" : "";
                $da_val = $tbl_sale_invoice_items_has_diamond_amount ? ", $diamond_amount" : "";
                $sa_col = $tbl_sale_invoice_items_has_stone_amount ? ", stone_amount" : "";
                $sa_val = $tbl_sale_invoice_items_has_stone_amount ? ", $stone_amount" : "";
                $sw_col = $tbl_sale_invoice_items_has_stone_weight ? ", stone_weight" : "";
                $sw_val = $tbl_sale_invoice_items_has_stone_weight ? ", $stone_weight" : "";
                $mv_col = $tbl_sale_invoice_items_has_metal_value ? ", metal_value" : "";
                $mv_val = $tbl_sale_invoice_items_has_metal_value ? ", $metal_value" : "";
                $mq_col = $tbl_sale_invoice_items_has_metal_qty ? ", metal_qty" : "";
                $mq_val = $tbl_sale_invoice_items_has_metal_qty ? ", $metal_qty" : "";
                $mw_col = $tbl_sale_invoice_items_has_metal_weight ? ", metal_weight" : "";
                ...
                // Save item images to uploads/sale-invoice/{invoice_id}/ (and store paths in DB if column exists)
                $group_image_raw = $item['group_image'] ?? '';
                if ($group_image_raw !== '') {
                    $images_json = save_sale_invoice_item_images($group_image_raw, $invoice_id, $item_id);
                    if ($images_json !== '' && $tbl_sale_invoice_items_has_images) {
                        $images_esc = mysqli_real_escape_string($conn, $images_json);
                        mysqli_query($conn, "UPDATE tbl_sale_invoice_items SET images = '$images_esc' WHERE id = $item_id");
                    }
                }
                ...
                if (!$save_as_draft && $product_id > 0 && $has_weight) {
                    $branch_id = 0;
                    $metal_id = 0;
                    $stock_purity = $purity;
                    ...
                    if ($branch_id <= 0) {
                        ...
                        if ($invoice_id > 0 && !empty($has_si_branch_id)) {
                            $hr = getRecord('SELECT branch_id FROM tbl_sale_invoices WHERE id = ' . (int) $invoice_id . ' LIMIT 1');
                    ...
                    if (!empty($stock_row)) {
                        ...
                        if ($tbl_stock_has_reference) {
                            ...
                            $ow_vals = str_replace("CURDATE(), 1, NOW()", "CURDATE(), 1, $invoice_id, 'sale_invoice', NOW()", $ow_vals);
                        }
                        ...
                        if ($tbl_stock_has_reference) {
                            ...
                            $stock_vals = str_replace("'$invoice_date', NOW()", "'$invoice_date', $invoice_id, 'sale_invoice', NOW()", $stock_vals);
                        }
    ...
    if (!empty($payments) && is_array($payments)) {
        ...
        if ($_sipdc && mysqli_num_rows($_sipdc) > 0) {
            ...
            @mysqli_query($conn, "ALTER TABLE tbl_sale_invoice_payments ADD COLUMN payment_details TEXT NULL COMMENT 'JSON copy of payment row (scrap weights, qty, etc.)'");
            $_sipdc2 = @mysqli_query($conn, "SHOW COLUMNS FROM tbl_sale_invoice_payments LIKE 'payment_details'");
    ...
    if (!$save_as_draft && ($customer_id > 0 || !empty($customer_name))) {
        $has_against = @mysqli_query($conn, "SHOW COLUMNS FROM tbl_customer_ledger LIKE 'against_ledger'");
        ...
        if ($is_update) {
            ...
            mysqli_query($conn, "
                ...
                AND transaction_type IN ('sale_invoice', 'previous_balance_payment', 'payment', 'sale_revenue')
            ");
            mysqli_query($conn, "
                DELETE FROM tbl_customer_ledger 
                WHERE customer_name = 'Hedging Account' AND transaction_id = $invoice_id AND transaction_type = 'sale_invoice' AND status = 1
                AND transaction_no IN ($ledger_del_txn_no_in)
            ");
        }
        ...
        if ($full_from_previous && $previous_balance_used_amt > 0) {
            ...
            if ($is_hedging && $previous_balance_used_amt > 0.00001) {
                ...
                $last_party_row = getRecord("
                    ...
                    AND transaction_type = 'sale_invoice' AND transaction_no = '$inv_esc'
                    ORDER BY id DESC LIMIT 1
                ");
...
") : '';
    ...
    if (!$save_as_draft && !$is_hedging && $invoice_id > 0 && function_exists('auragold_delete_purchase_fixing_direct_for_sale_invoice')) {
    ...
    if ($invoice_id > 0) {
        @mysqli_query($conn, "DELETE FROM invoice_fixing_mapping WHERE source_type = 'sale_invoice' AND source_transaction_id = " . (int)$invoice_id);
    }
    ...
    if ($is_hedging) {
        ...
        @mysqli_query($conn, "
            INSERT INTO invoice_fixing_mapping (
                ...
                'sale_invoice', " . (int)$invoice_id . ", '$inv_esc_map',
                NULL, NULL, NULL,
    ...
    if ($invoice_id > 0) {
        ...
        $chk_gst = @mysqli_query($conn, "SHOW COLUMNS FROM tbl_sale_invoices LIKE 'gst_supply_mode'");
        if (!$chk_gst || mysqli_num_rows($chk_gst) === 0) {
            @mysqli_query($conn, 'ALTER TABLE tbl_sale_invoices ADD COLUMN gst_supply_mode VARCHAR(24) NULL DEFAULT NULL');
            @mysqli_query($conn, 'ALTER TABLE tbl_sale_invoices ADD COLUMN gst_cgst_amount DECIMAL(15,2) NOT NULL DEFAULT 0.00');
            @mysqli_query($conn, 'ALTER TABLE tbl_sale_invoices ADD COLUMN gst_sgst_amount DECIMAL(15,2) NOT NULL DEFAULT 0.00');
            @mysqli_query($conn, 'ALTER TABLE tbl_sale_invoices ADD COLUMN gst_igst_amount DECIMAL(15,2) NOT NULL DEFAULT 0.00');
        }
        if ($chk_gst) {
        ...
        mysqli_query(
            $conn,
            'UPDATE tbl_sale_invoices SET gst_supply_mode = \'' . $mode_esc . '\', gst_cgst_amount = ' . $cgst_amt . ', gst_sgst_amount = ' . $sgst_amt . ', gst_igst_amount = ' . $igst_amt . ' WHERE id = ' . (int) $invoice_id
        );
    }
    ...
    if (!$save_as_draft && (int) $invoice_id > 0) {
    ...
    if (!$save_as_draft && !AURAGOLD_EWAY_DISABLED && $enable_eway_bill && function_exists('ewaybill_generate_from_sale_invoice') && (int) $invoice_id > 0) {
        $existing_ew = @getRecord('SELECT eway_bill_no FROM tbl_sale_invoices WHERE id = ' . (int) $invoice_id . ' LIMIT 1');
    ...
    if (!AURAGOLD_EWAY_DISABLED && !empty($enable_eway_bill) && (int) $invoice_id > 0) {
        $ewRespRow = @getRecord('SELECT eway_response FROM tbl_sale_invoices WHERE id = ' . (int) $invoice_id . ' LIMIT 1');
    ...
    if (!$save_as_draft && is_file($sync_si_scrap)) {
    ...
    $success_payload = [
        'status'     => 'success',
        'message'    => $save_as_draft ? 'Sale invoice saved as draft' : 'Sale invoice saved successfully',
        'order_id'   => $invoice_id,
        'invoice_id' => (int) $invoice_id,
        'order_no'   => $invoice_no,
        'invoice_no' => $invoice_no,
        'invoice_status' => $invoice_doc_status,
        'is_draft'   => $save_as_draft ? 1 : 0,
        'next_invoice_no' => $save_as_draft ? null : $invoice_no,
        'display_invoice_no' => ($save_as_draft || auragold_sale_invoice_is_draft_placeholder_no($invoice_no))
            ? (function_exists('getNextSaleInvoiceNo') ? getNextSaleInvoiceNo($conn) : '')
            : $invoice_no,
