...
// Get invoice details from tbl_sale_invoices
$invoice = getRecord("SELECT * FROM tbl_sale_invoices WHERE id = $invoice_id");
...
// Get invoice items
$items = getList("SELECT * FROM tbl_sale_invoice_items WHERE invoice_id = $invoice_id ORDER BY id ASC");
...
$payments = getList("SELECT * FROM tbl_sale_invoice_payments WHERE invoice_id = $invoice_id ORDER BY id ASC");
...
if ($mf && mysqli_num_rows($mf) > 0) {
    ...
    $fixing_mapping = getRecord("SELECT * FROM invoice_fixing_mapping WHERE source_type = 'sale_invoice' AND source_transaction_id = $invoice_id AND status = 1 ORDER BY id DESC LIMIT 1");
} elseif ($mf) {
    mysqli_free_result($mf);
}
