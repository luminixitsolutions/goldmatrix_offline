...
$voucher = getRecord("SELECT id FROM tbl_contra_vouchers WHERE id = $voucher_id");
if (!$voucher) {
...
try {
    mysqli_query($conn, "
        ...
        WHERE transaction_type = 'contra_voucher' AND transaction_id = $voucher_id
    ");
    mysqli_query($conn, "DELETE FROM tbl_contra_voucher_items WHERE voucher_id = $voucher_id");
    if (!mysqli_query($conn, "DELETE FROM tbl_contra_vouchers WHERE id = $voucher_id")) {
