...
if (!auragold_session_is_admin_login_type()) {
    echo json_encode(['status' => 'error', 'message' => 'Only admin users can change sub-branch status.']);
    exit;
}
...
if ((int) $target['main_branch_id'] === 0) {
    echo json_encode(['status' => 'error', 'message' => 'Main branch status cannot be changed.']);
    exit;
}
...
$status_sql = $status === 1 ? 1 : 0;
$ok = mysqli_query($conn_master, 'UPDATE tbl_branches SET status = ' . $status_sql . ' WHERE id = ' . $id . ' LIMIT 1');
if (!$ok) {
