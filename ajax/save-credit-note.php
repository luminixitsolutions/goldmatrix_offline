...
// Check if credit note tables exist
$tbl = @mysqli_query($conn, "SHOW TABLES LIKE 'tbl_credit_notes'");
if (!$tbl || mysqli_num_rows($tbl) == 0) {
    echo json_encode(['status' => 'error', 'message' => 'Credit note tables not found. Please run admin/sql/create_credit_note_tables.sql']);
    exit;
}
...
try {
    ...
    $credit_note_no = esc($_POST['order_no'] ?? '');
    $credit_note_id = isset($_POST['order_id']) ? (int)$_POST['order_id'] : 0;
    ...
    $__fx = function_exists('auragold_exchange_rate_sql_fragments') ? auragold_exchange_rate_sql_fragments($conn, 'tbl_credit_notes') : ['has'=>false,'insert_col'=>'','insert_val'=>'','update'=>'','rate'=>1];
    ...
    $credit_note_date = esc($_POST['order_date'] ?? date('Y-m-d'));
    ...
    if (empty($credit_note_no)) {
        $last = getRecord("SELECT credit_note_no FROM tbl_credit_notes ORDER BY id DESC LIMIT 1");
        if ($last && $last['credit_note_no']) {
            $num = (int)preg_replace('/[^0-9]/', '', $last['credit_note_no']);
            $credit_note_no = 'CN-' . ($num + 1);
        } else {
            $credit_note_no = 'CN-1';
        }
    }

    $is_update = ($credit_note_id > 0);

    if ($is_update) {
        $cur = getRecord("SELECT credit_note_no FROM tbl_credit_notes WHERE id = $credit_note_id");
        $cur_no = $cur ? $cur['credit_note_no'] : '';
        if ($credit_note_no !== $cur_no) {
            $ex = getRecord("SELECT id FROM tbl_credit_notes WHERE credit_note_no = '$credit_note_no' AND id != $credit_note_id");
            if ($ex) throw new Exception("Credit Note No '$credit_note_no' already exists.");
        }
        mysqli_query($conn, "
            UPDATE tbl_credit_notes SET
        ...
        mysqli_query($conn, "DELETE FROM tbl_credit_note_items WHERE credit_note_id = $credit_note_id");
        mysqli_query($conn, "DELETE FROM tbl_credit_note_payments WHERE credit_note_id = $credit_note_id");
    } else {
        mysqli_query($conn, "
            INSERT INTO tbl_credit_notes (
                credit_note_no, customer_id, customer_name, against_of, currency$fx_insert_col_sql, ref_no, sales_person,
                credit_note_date, due_date, layaways_id, fixing_type,
        ...
        $credit_note_id = mysqli_insert_id($conn);
    }
    ...
    if (!empty($items) && is_array($items)) {
        foreach ($items as $item) {
            ...
            $iq = "INSERT INTO tbl_credit_note_items (
                credit_note_id, product_id, product_characteristic_id, barcode, product_name, carat,
                ...
                $credit_note_id, $product_id, " . ($characteristic_id ? $characteristic_id : 'NULL') . ",
    ...
    if (!empty($payments) && is_array($payments)) {
        foreach ($payments as $p) {
            ...
            $pq = "INSERT INTO tbl_credit_note_payments (
                credit_note_id, payment_type, deposit_into, transaction_no, cheque_date, purity_carat,
                amount, previous_balance_amount, current_order_amount, diamond_category, quantity, status, created_at
            ) VALUES (
                $credit_note_id, '$payment_type',
    ...
    echo json_encode([
        ...
        'order_id' => $credit_note_id,
        'order_no' => $credit_note_no,
