...
if (!auragold_voucher_diamond_kind_valid($voucher_kind)) {
    echo json_encode(['ok' => false, 'message' => 'Invalid voucher type for stone allocation.']);
    exit;
}

if ($voucher_id < 1) {
    echo json_encode(['ok' => false, 'message' => 'Save the document first, then allocate stones.']);
    exit;
}
...
$tx_err = '';
try {
    $stats = auragold_voucher_apply_stone_allocations($conn, $voucher_kind, $voucher_id, $lines, $doc_no, $doc_date, $tx_ok, $tx_err);
    if (!$tx_ok) {
