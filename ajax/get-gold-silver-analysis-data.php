...
require_once __DIR__ . '/../includes/gold_silver_analysis_load.php';
require_once __DIR__ . '/../includes/gold_silver_analysis_render.php';
...
try {
    if ($summary_only) {
        $fetch = auragold_gsa_fetch_table_data($conn, $active_tab, $page, $per_page, [
            'include_totals' => true,
            'skip_rows' => true,
        ]);
        echo json_encode([
            ...
            'tfoot_cells_html' => $active_tab === 'stock-details'
                ? gsa_render_stock_details_tfoot_cells($fetch['totals'])
                : '',
        ]);
        exit;
    }
    ...
    if ($active_tab === 'current-stock') {
        $tbody_html = gsa_render_current_stock_tbody($stock_data, $fetch['gsa_cs_attach_image_map']);
    } else {
        $tbody_html = gsa_render_stock_details_tbody($stock_data);
    }
