...
// Next voucher no.: Bill Series for Old Jewelry Scrap Invoice, else legacy OJB-1, OJB-2 (tbl_old_jewelry_scrap_invoices)
$next_order_no = function_exists('getNextOldJewelryScrapInvoiceNo') ? getNextOldJewelryScrapInvoiceNo($conn) : 'OJB-1';

// Sales Person dropdown — branch-scoped (stored on scrap invoice header)
$sales_person_users = auragold_sales_person_user_display_names($conn_master);

// Load scrap voucher for editing (?id= tbl_old_jewelry_scrap_invoices.id); optional ?item_id= line highlight
$edit_order_id = isset($_GET['id']) ? (int)$_GET['id'] : 0;
$focus_scrap_item_id = isset($_GET['item_id']) ? (int)$_GET['item_id'] : 0;
$edit_order = null;
$edit_items = [];
$edit_payments = [];

if ($edit_order_id > 0) {
    $edit_order = getRecord("SELECT * FROM tbl_old_jewelry_scrap_invoices WHERE id = " . intval($edit_order_id));
    if ($edit_order) {
        $edit_items = getList("SELECT * FROM tbl_old_jewelry_scrap_invoice_items WHERE invoice_id = " . intval($edit_order_id));
        $edit_payments = getList("SELECT * FROM tbl_old_jewelry_scrap_invoice_payments WHERE invoice_id = " . intval($edit_order_id));
        $next_order_no = $edit_order['invoice_no'] ?? $next_order_no;
        $edit_order['order_no'] = $edit_order['invoice_no'] ?? '';
        $edit_order['order_date'] = $edit_order['invoice_date'] ?? '';
        $edit_order['order_id'] = $edit_order['id'] ?? $edit_order_id;
        $edit_order['supplier_name'] = $edit_order['customer_name'] ?? '';
        $edit_order['supplier_id'] = isset($edit_order['customer_id']) ? (int)$edit_order['customer_id'] : 0;
        $edit_order['purchase_person'] = $edit_order['sales_person'] ?? '';
        if (!isset($edit_order['previous_balance'])) {
            $edit_order['previous_balance'] = $edit_order['previous_balance_amt'] ?? 0;
        }
        if (!isset($edit_order['previous_gold'])) {
            $edit_order['previous_gold'] = $edit_order['previous_balance_gold'] ?? 0;
        }
        if (!isset($edit_order['previous_silver'])) {
            $edit_order['previous_silver'] = $edit_order['previous_balance_silver'] ?? 0;
        }
        if (is_array($edit_items)) {
            foreach ($edit_items as &$__it) {
                if (!is_array($__it)) {
                    continue;
                }
                $__it['product_id'] = $__it['product_id'] ?? 0;
                $__it['product_name'] = $__it['product_name'] ?? ($__it['description'] ?? '');
                $__it['description'] = $__it['description'] ?? ($__it['product_name'] ?? '');
                $__it['metal_id'] = $__it['metal_id'] ?? 0;
            }
            unset($__it);
        }
    }
...
$saved_orders = getList("SELECT id, invoice_no as order_no, customer_name, invoice_date as order_date, grand_total FROM tbl_old_jewelry_scrap_invoices ORDER BY id DESC LIMIT 50");

// Hedging / purchase-fixing gate applies to sale invoice only, not this page
...
$oj_scrap_focus_item_id = (int) ($focus_scrap_item_id ?? 0);
if ($oj_scrap_focus_item_id <= 0 && $edit_order_id > 0 && is_array($edit_items) && count($edit_items) === 1 && !empty($edit_items[0]['id'])) {
    $oj_scrap_focus_item_id = (int) $edit_items[0]['id'];
}

$oj_scrap_stock_banner = null;
if ($oj_scrap_focus_item_id > 0) {
    $oj_bal_inc = __DIR__ . '/includes/old_jewelry_scrap_stock_balance.php';
    $t_stk = @mysqli_query($conn, "SHOW TABLES LIKE 'tbl_old_jewelry_stock'");
    if ($t_stk && mysqli_num_rows($t_stk) > 0 && is_file($oj_bal_inc)) {
        mysqli_free_result($t_stk);
        require_once $oj_bal_inc;
        $bi = getRecord('SELECT gross_wt, invoice_id FROM tbl_old_jewelry_scrap_invoice_items WHERE id = ' . (int) $oj_scrap_focus_item_id . ' AND status = 1 LIMIT 1');
        if ($bi) {
            $og = (float) ($bi['gross_wt'] ?? 0);
            $inv_for_stk = (int) ($bi['invoice_id'] ?? 0);
            $st = function_exists('auragold_oj_scrap_stocked_gross_sum_for_line_including_single_line_orphans')
                ? auragold_oj_scrap_stocked_gross_sum_for_line_including_single_line_orphans($conn, $inv_for_stk, $oj_scrap_focus_item_id)
                : auragold_oj_scrap_stocked_gross_sum($conn, $oj_scrap_focus_item_id);
            $oj_scrap_stock_banner = [
                'original' => $og,
                'stocked' => $st,
                'remaining' => max(0, $og - $st),
            ];
        }
    } elseif ($t_stk) {
        mysqli_free_result($t_stk);
...
<style>
    ...
    /* Row highlight when opening scrap stock-in with ?item_id= (tbl_old_jewelry_scrap_invoice_items.id) */
    #productTableBody tr.ojd-stock-in-highlight {
        outline: 2px solid #c5a864;
...
text-transform: uppercase;
    ...
    /* Scrap stock-in only: height follows row count (no forced min — avoids empty gap hiding horizontal scrollbar) */
                        ...
                        <div class="col-lg-8" >
                            ...
                            <div class="card mb-4">
                                <div class="card-body product-entry">
                                    <?php if (!empty($oj_scrap_stock_banner) && is_array($oj_scrap_stock_banner)) { ?>
                                    <div class="alert alert-secondary border-0 mb-3 py-2 rounded" style="font-size:13px;background:#e8eef5;color:#11294b;">
                                        <strong>Scrap gross (this line):</strong>
                                        Total <?php echo number_format((float) $oj_scrap_stock_banner['original'], 4); ?> g
                                        &nbsp;|&nbsp; Already stocked <?php echo number_format((float) $oj_scrap_stock_banner['stocked'], 4); ?> g
                                        &nbsp;|&nbsp; <strong>Balance to stock <?php echo number_format((float) $oj_scrap_stock_banner['remaining'], 4); ?> g</strong>
                                        (gross to enter on the line cannot exceed this balance).
                                    </div>
                                    <?php } ?>
                            ...
                            <!-- Payments: data-only (no on-page grid); IDs kept for save/summary/edit flows -->
                            <div id="invoicePaymentsHidden" class="d-none" aria-hidden="true">
                                <div class="pos-payment-cards-area">
                                    <div class="pos-payment-cards-scroll">
                                        <div id="paymentTableBody" class="pos-payment-cards d-flex flex-wrap"></div>
                                    </div>
                            ...
                            <div class="summary-panel" >
                                ...
                                <div class="invoice-header" style="margin: -1rem -1rem 1rem -1rem; border-radius: 10px 10px 0 0; display: flex; align-items: center; justify-content: space-between;">
                                    <div style="display: flex; align-items: center; gap: 0.5rem; flex: 1;">
                                        ...
                                        <h5 class="mb-0" style="font-size: 0.9rem;">Old Jewellery Scrap Stock In: <span id="currentOrderNo"><?php echo htmlspecialchars($next_order_no); ?></span></h5>
                                    </div>
                                    <div class="invoice-header-actions">
                                        <span id="siSaveInvoiceWrap" class="si-save-invoice-wrap<?php echo !empty($si_save_blocked_by_purchase_fixing) ? ' si-save-wrap-blocked' : ''; ?>" title="<?php echo !empty($si_save_blocked_by_purchase_fixing) ? htmlspecialchars($si_pfd_save_blocked_tip, ENT_QUOTES, 'UTF-8') : htmlspecialchars('Stock In', ENT_QUOTES, 'UTF-8'); ?>">
                                            <button type="button" class="btn-save-invoice btn-sm" onclick="saveOrder()" <?php echo !empty($si_save_blocked_by_purchase_fixing) ? 'disabled' : ''; ?>>Stock In</button>
                                        </span>
                                    </div>
                                </div>
                                <!-- Previous balance panel hidden: stubs keep element IDs for save/summary scripts -->
                                <div class="d-none" aria-hidden="true">
                                    <span id="previousBalanceAmount" data-original-balance="0.00">0.00</span>
                                    <span id="previousBalanceGold" data-original-gold="0.000">0.000</span>
                                    <span id="previousBalanceSilver" data-original-silver="0.000">0.000</span>
                                    <span id="previousBalanceDiamond" data-original-diamond="0.000">0.000</span>
                                    <span id="previousBalanceGemstone" data-original-gemstone="0.000">0.000</span>
                                    <input type="checkbox" id="usePreviousBalanceCheck" value="1" tabindex="-1">
                                    <div id="previousBalanceUseAmountRow" class="pb-use-amount-row"></div>
                                    <input type="text" id="previousBalanceUseAmount" value="0.00" step="0.01" tabindex="-1">
                                </div>
...
<style id="auragold-swal2-print-barcode-theme">
    .auragold-print-barcode-swal { border-radius: 22px !important; padding: 0.5rem 0.5rem 1.25rem !important; box-shadow: 0 25px 50px -12px rgba(15, 23, 42, 0.28) !important; }
    .auragold-print-barcode-swal .swal2-title { color: #1e3a5f !important; font-size: 1.35rem !important; font-weight: 700 !important; padding: 0.35rem 0 0 !important; margin-bottom: 0 !important; }
    .auragold-print-barcode-swal .swal2-html-container { margin: 0.5rem 0 0 !important; }
    .auragold-swal-btn-print { background: #1a2b4b !important; color: #fff !important; border: none !important; border-radius: 14px !important; padding: 0.65rem 1.9rem !important; font-weight: 600 !important; font-size: 0.95rem !important; }
    .auragold-swal-btn-no { background: #fce7f3 !important; color: #be185d !important; border: none !important; border-radius: 14px !important; padding: 0.65rem 1.9rem !important; font-weight: 600 !important; font-size: 0.95rem !important; }
    .auragold-print-barcode-swal .swal2-actions { gap: 12px !important; margin-top: 1.35rem !important; }
    .auragold-print-barcode-swal .swal2-close { color: #94a3b8 !important; outline: none !important; box-shadow: none !important; font-size: 1.75rem !important; }
    .auragold-print-barcode-swal .swal2-close:hover { color: #64748b !important; }
    .auragold-print-barcode-swal .swal2-close:focus { box-shadow: none !important; }
</style>
...
<?php
$__auragold_psm_js = __DIR__ . '/assets/js/product-search-modal-common.js';
?>
<script src="assets/js/product-search-modal-common.js?v=<?php echo is_file($__auragold_psm_js) ? (int) filemtime($__auragold_psm_js) : time(); ?>"></script>
...
<script>
    ...
    window.OJD_STOCK_IN_FOCUS_ITEM_ID = <?php echo (int)($oj_scrap_focus_item_id ?? 0); ?>;
    window.OJD_SCRAP_STOCK_MAX_GROSS = <?php echo (!empty($oj_scrap_stock_banner) && array_key_exists('remaining', $oj_scrap_stock_banner)) ? json_encode(round((float) $oj_scrap_stock_banner['remaining'], 4)) : 'null'; ?>;
...
<script>
    ...
    if (!empty($edit_order_id) && $edit_order_id > 0 && !empty($edit_order) && is_array($edit_order)) {
        $embed_order = [
            ...
            'customer_id' => (int)($edit_order['customer_id'] ?? 0),
            'supplier_name' => $edit_order['supplier_name'] ?? ($edit_order['customer_name'] ?? ''),
            ...
            'previous_balance' => (float)($edit_order['previous_balance'] ?? $edit_order['previous_balance_amt'] ?? 0),
            'previous_gold' => (float)($edit_order['previous_gold'] ?? $edit_order['previous_balance_gold'] ?? 0),
            'previous_silver' => (float)($edit_order['previous_silver'] ?? $edit_order['previous_balance_silver'] ?? 0),
        ...
        $oj_bal_embed = __DIR__ . '/includes/old_jewelry_scrap_stock_balance.php';
        $t_stk_emb = @mysqli_query($conn, "SHOW TABLES LIKE 'tbl_old_jewelry_stock'");
        $has_oj_stock_emb = ($t_stk_emb && mysqli_num_rows($t_stk_emb) > 0);
        if ($t_stk_emb) {
            mysqli_free_result($t_stk_emb);
        }
        if ($has_oj_stock_emb && is_file($oj_bal_embed)) {
            require_once $oj_bal_embed;
        }
        ...
        foreach ($embed_items as &$emb_it) {
            if (is_array($emb_it)) {
                $eid = (int) ($emb_it['id'] ?? 0);
                $einv = (int) ($emb_it['invoice_id'] ?? $edit_order_id);
                if ($eid > 0 && $has_oj_stock_emb && is_file($oj_bal_embed)) {
                    $orig_e = (float) ($emb_it['gross_wt'] ?? 0);
                    $st_e = function_exists('auragold_oj_scrap_stocked_gross_sum_for_line_including_single_line_orphans')
                        ? auragold_oj_scrap_stocked_gross_sum_for_line_including_single_line_orphans($conn, $einv, $eid)
                        : 0;
                    $emb_it['original_gross_wt'] = $orig_e;
                    $emb_it['stocked_gross_wt'] = $st_e;
                    $emb_it['remaining_gross_wt'] = max(0, $orig_e - $st_e);
                }
            }
    ...
    const PRODUCT_MODAL_COLUMNS_PAGE = 'old-jewellery-scrap-stock-in-product-modal';
    ...
    /**
     * Duplicate only when same barcode AND same characteristic (merged diamond lines share one barcode).
     * When purchase_invoice_item_id is set (same barcode, same product+characteristic, multiple PI rows), dedupe by that id only.
     * @param {string} barcode
     * @param {string|number|null|undefined} characteristicId
     * @param {string|number|null|undefined} purchaseInvoiceItemId
     */
    function isProductLineInProductList(barcode, characteristicId, purchaseInvoiceItemId, productMetalId) {
        ...
        // After syncDiamondTabSharedBarcodes(), every line shows the anchor barcode (e.g. DIA00004) but
        // sibling AJAX still returns physical barcodes (DIAA…). Match by characteristic first so we do not add 3+3 duplicate lines.
    ...
    function fetchProductByBarcodeAndAdd(barcode) {
        ...
        fetch('ajax/get-product-by-barcode.php?barcode=' + encodeURIComponent(trimmed) + '&expand_invoice_siblings=1')
            ...
            .then(data => {
                ...
                if (data.success && data.product) {
                    ...
                    if (isProductLineInProductList(resolvedBc, resCid, data.product.purchase_invoice_item_id, data.product.metal_id)) {
                    ...
                    function addSiblingSequential(idx, list) {
                        ...
                        fetch('ajax/get-product-by-barcode.php?barcode=' + encodeURIComponent(sb) + '&expand_invoice_siblings=1')
                            .then(parseBarcodeAjaxJson)
                            .then(function(d2) {
                ...
                .then(function(data) {
                    ...
                    if (items.length > 0 && typeof savedItemToModalRowData === 'function') {
                        ...
                        if (!added && typeof addProductToTableFromModalRow === 'function') {
                            itemList.forEach(function(item) { addProductToTableFromModalRow(savedItemToModalRowData(item)); });
                        }
                    }
...
<td data-column="sale-amount-with"><input type="text" class="form-control form-control-sm" value="0.00" step="0.01" readonly style="width: 130px; font-size: 0.7rem;"></td>
    ...
    /** Weight to validate for stock-in: max of gross / pkt / metal. Preferring gross-only broke edit flows where the user changes Weight (metal) but gross-wt inputs still hold an older value. */
    window.ojdEffectiveStockGrossFromModalData = function (d) {
        if (!d) return 0;
        var g = parseFloat(d.gross_wt) || 0;
        var p = parseFloat(d.pkt_wt) || 0;
        var m = parseFloat(d.metal_weight) || 0;
        var mx = Math.max(g, p, m);
        return mx > 0.00001 ? mx : 0;
    };
    window.ojdScrapStockMaxGrossRemaining = function () {
        var v = window.OJD_SCRAP_STOCK_MAX_GROSS;
        if (v === null || v === undefined || v === '') return null;
        var n = parseFloat(v);
        return isNaN(n) ? null : n;
    };
    window.ojdShouldValidateScrapStockGross = function () {
        return window.ojdScrapStockMaxGrossRemaining() !== null;
    };
    /** After loadOrder / populateOrderForm: set cap from API-shaped items (remaining_gross_wt). Single line only; multi-line needs per-line links. */
    window.ojdSyncScrapStockCapFromInvoiceItems = function (items) {
        if (!items || items.length === 0) {
            window.OJD_SCRAP_STOCK_MAX_GROSS = null;
            return;
        }
        if (items.length !== 1) {
            window.OJD_SCRAP_STOCK_MAX_GROSS = null;
        ...
        var it = items[0];
        var rem = parseFloat(it.remaining_gross_wt);
        if (isNaN(rem)) {
            var orig = parseFloat(it.original_gross_wt);
            var st = parseFloat(it.stocked_gross_wt);
            if (!isNaN(orig)) {
                rem = Math.max(0, orig - (isNaN(st) ? 0 : st));
            } else {
                rem = parseFloat(it.gross_weight) || parseFloat(it.gross_wt) || 0;
            }
        }
        window.OJD_SCRAP_STOCK_MAX_GROSS = Math.round(Math.max(0, rem) * 10000) / 10000;
        var sid = parseInt(it.id, 10);
        if (!isNaN(sid) && sid > 0) {
            window.OJD_STOCK_IN_FOCUS_ITEM_ID = sid;
        }
    };
    window.ojdValidateScrapStockModalRowsGross = function (modalRowsDataArr) {
        if (!window.ojdShouldValidateScrapStockGross()) return { ok: true };
        var max = window.ojdScrapStockMaxGrossRemaining();
        var sum = 0;
        (modalRowsDataArr || []).forEach(function (d) {
            sum += window.ojdEffectiveStockGrossFromModalData(d);
        });
        var sr = Math.round(sum * 10000) / 10000;
        var mr = Math.round(max * 10000) / 10000;
        if (sr > mr) {
            return {
                ok: false,
                message: 'Total weight in Product Selection (' + sr.toFixed(4) + ' g) exceeds balance to stock for this scrap line (' + mr.toFixed(4) + ' g). Reduce Weight, Gross Wt., or Pkt. Wt. before adding.'
            };
        }
        return { ok: true };
    };
    ...
    function selectProduct(row, closeModal = false) {
        ...
        var grossChk = window.ojdValidateScrapStockModalRowsGross([modalRowData]);
        if (!grossChk.ok) {
            alert(grossChk.message);
            return;
        }
    ...
    function addMergedProductsToTable(modalRowsData, metalIdForImage) {
        ...
        var firstRowGiOs = '';
        for (var _agiOs = 0; _agiOs < modalRowsData.length; _agiOs++) {
            var rgi = modalRowsData[_agiOs].group_image;
            if (rgi && String(rgi).trim()) { firstRowGiOs = rgi; break; }
        }
        var groupImagePayload = tabPayloadMerge || ((typeof window.auragoldCoParseGroupImageAttr === 'function') ? window.auragoldCoParseGroupImageAttr(firstRowGiOs) : '') || '';
        var groupImageAttr = (typeof groupImagePayload === 'object' && groupImagePayload != null) ? JSON.stringify(groupImagePayload) : (groupImagePayload || '');
        row.setAttribute('data-group-image', groupImageAttr);
        var primaryUrl = typeof getGroupImagePrimary === 'function' ? getGroupImagePrimary(groupImagePayload) : (typeof groupImagePayload === 'string' ? groupImagePayload : '');
        var actionCell = '<td><div class="action-btns"><button type="button" class="btn-edit" onclick="editProductRow(\'' + rowId + '\')" title="Edit"><i class="feather icon-edit"></i></button><button type="button" class="btn-delete" onclick="deleteProductRow(\'' + rowId + '\')" title="Delete"><i class="feather icon-trash-2"></i></button></div></td>';
    ...
    function updateMergedRowFromModalRows(rowId, modalRowsData) {
        ...
        var grossChkMerge = window.ojdValidateScrapStockModalRowsGross(modalRowsData);
        if (!grossChkMerge.ok) {
            alert(grossChkMerge.message);
            return;
        }
        var productNames = modalRowsData.map(function(d) { return (d.product_name || '').trim(); }).filter(Boolean);
        var barcodes = modalRowsData.map(function(d) { var b = (d.barcode || '').trim(); return b || '—'; });
        ...
        var merged = {
            ...
            purity: parseFloat(first.purity) || 0,
            quantity: 0, gross_wt: 0, less_wt: 0, net_wt: 0, pure_wt: 0, final_wt: 0,
            rate: 0, metal_qty: 0, metal_weight: 0, metal_value: 0,
            amount: 0, discount: 0, making_amount: 0, stone_amount: 0, other_amount: 0,
            diamond_amount: 0, tax: 0, net_amt: 0, net_amt_tax: 0,
            purchase_amount: 0, sale_amount: 0, sale_amount_with: 0, reverse: 0,
            design_no: first.design_no || '', calculation_type: first.calculation_type || 'Rate X Gross Wt'
        };
        modalRowsData.forEach(function(d) {
        ...
        var primaryUrl = typeof getGroupImagePrimary === 'function' ? getGroupImagePrimary(groupImagePayload) : '';
        var scrapItemId = row.getAttribute('data-scrap-item-id');
        var actionCell = '<td><div class="action-btns"><button type="button" class="btn-edit" onclick="editProductRow(\'' + rowId + '\')" title="Edit"><i class="feather icon-edit"></i></button><button type="button" class="btn-delete" onclick="deleteProductRow(\'' + rowId + '\')" title="Delete"><i class="feather icon-trash-2"></i></button></div></td>';
        row.innerHTML = (typeof getProductListRowCells === 'function' ? getProductListRowCells(merged, { groupImage: primaryUrl || '' }) : []).join('') + actionCell;
        if (scrapItemId) row.setAttribute('data-scrap-item-id', scrapItemId);
        if (typeof window.applyProductListColumnVisibilityToRow === 'function') window.applyProductListColumnVisibilityToRow(row);
        var productCell = row.querySelector('[data-column="product"]');
        if (productCell) {
            productCell.addEventListener('click', function(e) {
    ...
    // Update Product List table row with data from Product Selection modal row (rebuild cells from full modal payload)
    function updateProductListRowFromModalRow(productListRowId, modalRow) {
        ...
        if (typeof getModalRowDataFromRow !== 'function') {
            console.error('getModalRowDataFromRow not available');
            return;
        }
        var d = getModalRowDataFromRow(modalRow, true);
        var grossChkUp = window.ojdValidateScrapStockModalRowsGross([d]);
        if (!grossChkUp.ok) {
            alert(grossChkUp.message);
            return;
        }
        var listData = Object.assign({}, d);
        var tabKey = (d.metal_id != null && d.metal_id !== '') ? String(d.metal_id) : ((typeof currentMetalId !== 'undefined' && currentMetalId !== null) ? String(currentMetalId) : '');
        var groupImagePayload = window.productModalGroupImageByTab && tabKey !== '' ? window.productModalGroupImageByTab[tabKey] : null;
        var primaryUrl = typeof getGroupImagePrimary === 'function' ? getGroupImagePrimary(groupImagePayload) : '';
        var scrapItemId = productListRow.getAttribute('data-scrap-item-id');
        var actionCell = '<td><div class="action-btns"><button type="button" class="btn-edit" onclick="editProductRow(\'' + productListRowId + '\')" title="Edit"><i class="feather icon-edit"></i></button><button type="button" class="btn-delete" onclick="deleteProductRow(\'' + productListRowId + '\')" title="Delete"><i class="feather icon-trash-2"></i></button></div></td>';
        productListRow.innerHTML = (typeof getProductListRowCells === 'function' ? getProductListRowCells(listData, { groupImage: primaryUrl || '' }) : []).join('') + actionCell;
        productListRow.setAttribute('data-product-id', d.product_id || '');
        productListRow.setAttribute('data-characteristic-id', d.characteristic_id || '');
        productListRow.setAttribute('data-purity', parseFloat(d.purity) || 0);
        productListRow.setAttribute('data-rate', parseFloat(d.rate) || 0);
        productListRow.setAttribute('data-barcode', (d.barcode || '').trim());
        productListRow.setAttribute('data-calculation-type', d.calculation_type || 'Rate X Gross Wt');
        if (groupImagePayload) {
            productListRow.setAttribute('data-group-image', (typeof groupImagePayload === 'object' && groupImagePayload != null) ? JSON.stringify(groupImagePayload) : groupImagePayload);
        }
        if (scrapItemId) productListRow.setAttribute('data-scrap-item-id', scrapItemId);
        if (typeof window.applyProductListColumnVisibilityToRow === 'function') window.applyProductListColumnVisibilityToRow(productListRow);
        addRowCalculationListeners(productListRow);
        if (typeof calculateRowAmounts === 'function') calculateRowAmounts(productListRow);
        var productCell = productListRow.querySelector('[data-column="product"]');
        if (productCell) {
            productCell.addEventListener('click', function(e) {
                if (e.target.tagName !== 'A') editProductRow(productListRowId);
            });
        }
        updateSummaryRow();
        updateSummaryPanel();
        refreshProductListBarcodeGroups();
        if (typeof window.assignUniqueBarcodeToProductListRow === 'function') window.assignUniqueBarcodeToProductListRow(productListRow, listData);
    ...
    function editProductRow(rowId) {
        ...
        const grossWtEditRaw = parseFloat(row.querySelector('[data-field="gross_wt"]')?.value || row.querySelector('[data-column="gross-wt"] input')?.value || row.querySelector('[data-column="gross-wt"]')?.textContent?.trim() || 0);
        const pktWtEdit = parseFloat(row.querySelector('[data-column="pkt-wt"] input')?.value || row.querySelector('[data-column="pkt-wt"]')?.textContent?.trim() || 0);
        const rowData = {
            ...
            gross_wt: grossWtEditRaw > 0.00001 ? grossWtEditRaw : pktWtEdit,
        ...
        modalAddBtn.addEventListener('click', function() {
            ...
            if (currentEditingRowId) {
                ...
                if (typeof window.calculateModalRowNetWeight === 'function') {
                    allProductRows.forEach(function (r) { window.calculateModalRowNetWeight(r); });
                }
            ...
            try {
                ...
                var modalRowsDataAllForChk = [];
                productRows.forEach(function (r) {
                    modalRowsDataAllForChk.push(getModalRowDataFromRow(r, false));
                });
                var grossChkAddAll = window.ojdValidateScrapStockModalRowsGross(modalRowsDataAllForChk);
                if (!grossChkAddAll.ok) {
                    alert(grossChkAddAll.message);
                    return;
                }
                var byMetal = {};
    ...
    /** Scrap list URL for same calendar month as voucher date (or today). */
    function getOldJewelleryScrapListRedirectUrl() {
        var input = document.getElementById('orderDate');
        var d = null;
        if (input && String(input.value || '').trim()) {
            d = new Date(String(input.value).trim() + 'T12:00:00');
        }
        ...
        return 'old-jewellery.php?tab=scrap&from_date=' + encodeURIComponent(fmt(from)) + '&to_date=' + encodeURIComponent(fmt(to));
    }
    window.getOldJewelleryScrapListRedirectUrl = getOldJewelleryScrapListRedirectUrl;

    /** After Stock In: offer barcode label print (uses final barcodes from server). Then runs afterClosed() when the prompt is done (or immediately if no barcodes). */
    function auragoldEscapeHtmlScrap(str) {
        return String(str == null ? '' : str).replace(/[&<>"']/g, function(ch) {
            return ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' })[ch];
        });
    }
    function promptPrintScrapStockBarcodesAfterSave(barcodes, afterClosed) {
        var after = typeof afterClosed === 'function' ? afterClosed : function() {};
        var uniq = [];
        var seen = {};
        ...
        var list = uniq.join(', ');
        function openBarcodePrintWindow() {
            window.open('barcode-print.php?barcodes=' + encodeURIComponent(list), '_blank', 'width=900,height=700');
        }
        if (typeof Swal !== 'undefined' && typeof Swal.fire === 'function') {
            var illustration = '<div style="margin:4px auto 8px;width:128px;height:104px;">' +
                '<svg viewBox="0 0 120 100" xmlns="http://www.w3.org/2000/svg" width="128" height="104" aria-hidden="true">' +
                '<rect x="24" y="14" width="72" height="76" rx="9" fill="#e8ecff" stroke="#c7d2fe" stroke-width="2"/>' +
                '<line x1="36" y1="34" x2="84" y2="34" stroke="#a5b4fc" stroke-width="2" stroke-linecap="round"/>' +
                '<line x1="36" y1="48" x2="78" y2="48" stroke="#c7d2fe" stroke-width="1.5" stroke-linecap="round"/>' +
                '<line x1="36" y1="60" x2="82" y2="60" stroke="#c7d2fe" stroke-width="1.5" stroke-linecap="round"/>' +
                '<circle cx="36" cy="26" r="15" fill="#c5a864" stroke="#fff" stroke-width="2.5"/>' +
                '<text x="36" y="31" text-anchor="middle" fill="#fff" font-size="15" font-weight="700" font-family="system-ui,sans-serif">$</text>' +
                '<circle cx="86" cy="76" r="13" fill="#22c55e" stroke="#fff" stroke-width="2"/>' +
                '<path d="M80.5 76.5l4.2 4.2 9.5-11.2" fill="none" stroke="#fff" stroke-width="2.8" stroke-linecap="round" stroke-linejoin="round"/>' +
                '</svg></div>';
            var bodyHtml = illustration +
                '<p style="color:#64748b;font-size:15px;margin:0 0 14px;line-height:1.55;">Do you want to print barcode number(s)?</p>' +
                '<div style="background:#f8fafc;border:1px solid #e2e8f0;padding:14px 16px;border-radius:16px;font-size:15px;font-weight:600;color:#0f172a;text-align:center;word-break:break-word;">' +
                auragoldEscapeHtmlScrap(list) + '</div>';
            Swal.fire({
                title: 'Print barcode',
                html: bodyHtml,
                ...
                focusConfirm: false,
                customClass: {
                    popup: 'auragold-print-barcode-swal',
                    title: 'auragold-print-barcode-swal-title',
                    confirmButton: 'auragold-swal-btn-print',
                    cancelButton: 'auragold-swal-btn-no'
                }
            }).then(function(result) {
                if (result && result.isConfirmed) openBarcodePrintWindow();
                after();
            });
        } else {
            if (confirm('Do you want to print barcode number(s)?\n\n' + list)) {
                openBarcodePrintWindow();
            }
            after();
        }
    }
    window.promptPrintScrapStockBarcodesAfterSave = promptPrintScrapStockBarcodesAfterSave;
    ...
    function saveOrder() {
        ...
        const currentOrderNoText = document.getElementById('currentOrderNo')?.textContent || <?php echo json_encode(isset($next_order_no) ? $next_order_no : 'OJB-1'); ?>;
        ...
        productRows.forEach(function(row) {
            ...
            if (groupItemsJson) {
                try {
                    ...
                    if (groupItems && groupItems.length > 0) {
                        ...
                        var rowGrossEl = row.querySelector('[data-column="gross-wt"] input') || row.querySelector('[data-column="gross-wt"]');
                        var rowPktEl = row.querySelector('[data-column="pkt-wt"] input') || row.querySelector('[data-column="pkt-wt"]');
                        var rowGrossUiRaw = rowGrossEl ? (parseFloat(rowGrossEl.value) || parseFloat(rowGrossEl.textContent) || 0) : 0;
                        var rowPktUi = rowPktEl ? (parseFloat(rowPktEl.value) || parseFloat(rowPktEl.textContent) || 0) : 0;
                        var rowGrossUi = rowGrossUiRaw > 0.00001 ? rowGrossUiRaw : rowPktUi;
                        var giLen = groupItems.length;
                        var baseWeights = groupItems.map(function (dx) {
                            return Math.max(0, parseFloat(dx.gross_wt) || parseFloat(dx.metal_weight) || 0);
                        });
                        var bwSum = baseWeights.reduce(function (a, b) { return a + b; }, 0);
                        groupItems.forEach(function(d, idx) {
                            var metalWt = parseFloat(d.metal_weight) || 0;
                            var share = giLen <= 1 ? 1 : (bwSum > 0.00001 ? (baseWeights[idx] / bwSum) : (1 / giLen));
                            var grossWt = rowGrossUi > 0.00001 ? (rowGrossUi * share) : (parseFloat(d.gross_wt) || 0);
                            ...
                            items.push({
                                id: (d.id != null && d.id !== '') ? parseInt(d.id, 10) : 0,
            ...
            if (productId) {
                ...
                const grossWeightRaw = parseFloat(row.querySelector('[data-field="gross_wt"]')?.value || row.querySelector('[data-column="gross-wt"] input')?.value || row.querySelector('[data-column="gross-wt"]')?.textContent) || 0;
                const pktWtForGross = parseFloat(row.querySelector('[data-column="pkt-wt"] input')?.value || row.querySelector('[data-column="pkt-wt"]')?.textContent) || 0;
                const grossWeight = grossWeightRaw > 0.00001 ? grossWeightRaw : pktWtForGross;
                ...
                var scrapLineIdAttr = row.getAttribute('data-scrap-item-id');
                var scrapLineId = scrapLineIdAttr ? (parseInt(scrapLineIdAttr, 10) || 0) : 0;
                items.push({
                    id: scrapLineId,
                    product_id: productId,
                    characteristic_id: characteristicId,
        ...
        const originalText = saveBtn?.textContent || 'Stock In';
        if (saveBtn) {
            saveBtn.disabled = true;
        ...
        if (typeof jQuery !== 'undefined' && jQuery.ajax) {
            jQuery.ajax({
                url: 'ajax/save-old-jewelry-scrap-stock-in.php',
                type: 'POST',
                data: postData,
                dataType: 'json',
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        window.pendingRedirectUrl = getOldJewelleryScrapListRedirectUrl();
                        const invoiceId = response.invoice_id || response.order_id;
                        function openPrintInvoiceStep() {
                            if (invoiceId) {
                                setTimeout(function() {
                                    showPrintInvoiceModal(invoiceId);
                                }, 100);
                            } else {
                                alert('Scrap stock-in saved successfully! No: ' + (response.invoice_no || response.order_no || 'N/A'));
                                window.location.href = window.pendingRedirectUrl;
                            }
                        }
                        if (Array.isArray(response.barcodes) && response.barcodes.length > 0) {
                            promptPrintScrapStockBarcodesAfterSave(response.barcodes, openPrintInvoiceStep);
                        } else {
                            openPrintInvoiceStep();
                        }
                    } else {
                        alert('Error: ' + (response.message || 'Failed to save scrap stock-in'));
                    }
            ...
            fetch('ajax/save-old-jewelry-scrap-stock-in.php', {
                method: 'POST',
                body: formData
            })
            .then(response => response.json())
            .then(data => {
                ...
                if (data.status === 'success') {
                    ...
                    function openPrintInvoiceStepFetch() {
                        if (invoiceId) {
                            setTimeout(function() {
                                showPrintInvoiceModal(invoiceId);
                            }, 100);
                        } else {
                            alert('Scrap stock-in saved successfully! No: ' + (data.invoice_no || data.order_no || 'N/A'));
                            window.location.href = window.pendingRedirectUrl;
                        }
                    }
                    if (Array.isArray(data.barcodes) && data.barcodes.length > 0) {
                        promptPrintScrapStockBarcodesAfterSave(data.barcodes, openPrintInvoiceStepFetch);
                    } else {
                        openPrintInvoiceStepFetch();
                    }
                } else {
                    alert('Error: ' + (data.message || 'Failed to save scrap stock-in'));
                }
                if (saveBtn && typeof updateSiSaveBlockedByPfd === 'function') {
                    updateSiSaveBlockedByPfd(!!window.siSaveBlockedByPurchaseFixing);
                } else if (saveBtn) {
    ...
    function resetOrder() {
        if (confirm('Are you sure you want to create a new invoice? All unsaved data will be lost.')) {
            window.location.href = 'old-jewellery-scrap-stock-in.php';
        }
    }
    ...
    function printPurchaseInvoice(invoiceId) {
        ...
        window.open('old-jewelry-scrap-invoice-print.php?id=' + invoiceId, '_blank', 'width=1200,height=800');
    }
    ...
    function loadOrder(orderId) {
        ...
        var getUrl = 'ajax/get-old-jewelry-scrap-invoice.php?id=' + orderId + '&purpose=stock_remaining&_=' + (Date.now ? Date.now() : '');
        if (typeof jQuery !== 'undefined' && jQuery.ajax) {
            jQuery.ajax({
                url: getUrl,
                type: 'GET',
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        window.history.pushState({}, '', 'old-jewellery-scrap-stock-in.php?id=' + orderId);
    ...
    function updateSiSaveBlockedByPfd(blocked) {
        ...
        document.querySelectorAll('button.btn-purple').forEach(function(b) {
            if (b && b.textContent && /^(Save|Stock In)$/.test(b.textContent.trim())) {
                b.disabled = !!blocked;
                b.title = tip;
            }
        });
    ...
    /** Scroll/highlight main grid row when opened with ?item_id= */
    window.ojdFocusScrapItemRow = function () {
        var id = parseInt(window.OJD_STOCK_IN_FOCUS_ITEM_ID, 10) || 0;
        if (!id) return;
        var row = document.querySelector('#productTableBody tr[data-scrap-item-id="' + id + '"]');
        if (!row) return;
        row.classList.add('ojd-stock-in-highlight');
        try {
        ...
        setTimeout(function () {
            row.classList.remove('ojd-stock-in-highlight');
        }, 5000);
    };
    ...
    function populateOrderForm(order, items, loadedPayments) {
        ...
        if (order && order.id > 0) {
            var pfdRefreshUrl = 'ajax/get-old-jewelry-scrap-invoice.php?id=' + encodeURIComponent(order.id) + '&purpose=stock_remaining&_=' + (Date.now ? Date.now() : 0);
            ...
            try {
                ...
                if (!added) {
                    ...
                    if (lastRow) {
                        ...
                        if (item.id) {
                            lastRow.setAttribute('data-scrap-item-id', item.id);
                        }
        ...
        if (typeof window.ojdSyncScrapStockCapFromInvoiceItems === 'function') {
            window.ojdSyncScrapStockCapFromInvoiceItems(itemList);
        }
        ...
        setTimeout(function () {
            if (typeof window.ojdFocusScrapItemRow === 'function') window.ojdFocusScrapItemRow();
        }, 400);
    }
    ...
    (function() {
        ...
        function doPopulateFromEmbed() {
            ...
            try {
                ...
                if (window.history && window.history.replaceState) window.history.replaceState({}, '', 'old-jewellery-scrap-stock-in.php?id=' + editId);
                window.isPurchaseInvoiceEditMode = true;
                return true;
            } catch (e) {
...
<script>
    ...
    function showPrintInvoiceModal(invoiceId) {
        ...
        setTimeout(function() {
            ...
            if (modal) {
                ...
                if (confirm('Scrap stock-in saved successfully! Do you want to print?')) {
                    window.open('old-jewelry-scrap-invoice-print.php?id=' + invoiceId, '_blank', 'width=1200,height=800');
                }
    ...
    function confirmPrintInvoice() {
        if (savedInvoiceId) {
            window.open('old-jewelry-scrap-invoice-print.php?id=' + savedInvoiceId, '_blank', 'width=1200,height=800');
        }
