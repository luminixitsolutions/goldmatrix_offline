...
$preview_sample = isset($_GET['preview_sample']) && $_GET['preview_sample'] === '1';
$use_pos_tables = isset($_GET['pos']) && (string) $_GET['pos'] === '1';
$t_inv = $use_pos_tables ? 'tbl_pos_sale_invoices' : 'tbl_sale_invoices';
$t_items = $use_pos_tables ? 'tbl_pos_sale_invoice_items' : 'tbl_sale_invoice_items';
$t_payments = $use_pos_tables ? 'tbl_pos_sale_invoice_payments' : 'tbl_sale_invoice_payments';

if ($preview_sample) {
    $preview_uid = (int) ($_SESSION['user_id'] ?? 0);
    $preview_admin = (int) ($_SESSION['Admin']['id'] ?? 0);
    if ($preview_uid <= 0 && $preview_admin <= 0) {
        die('Please log in to preview.');
    }
    $invoice = [
        'invoice_no' => 'DEMO-001',
    ...
    $items = [
        [
            'product_name' => 'UNCUT DIAMOND BANGLE',
            'design_no' => 'TAG-1001',
            'barcode' => 'HSN7113',
            'gross_weight' => 365.553,
            'less_weight' => 0.2,
            'stone_weight' => 127.720,
            'diamond_carat' => 115.530,
            'carat' => 115.530,
            'net_weight' => 340.009,
            'final_weight' => 340.009,
            'rate' => 513.35,
            'metal_value' => 160579.785,
            'quantity' => 22,
            'metal_qty' => 22,
            'making_amount' => 13090.743,
            'diamond_amount' => 40323.436,
            'stone_amount' => 503.902,
            'discount' => 0,
            'net_amount' => 214497.866,
            'amount' => 214497.866,
            'tax_amount' => 654.537,
            'net_amt_with_tax' => 215152.403,
            'purity' => '22',
            'category_name' => 'Gold',
        ],
        [
            'product_name' => 'UNCUT DIAMOND BRACELET',
            'design_no' => 'TAG-1002',
            'barcode' => 'HSN7114',
            'gross_weight' => 76.689,
            'less_weight' => 0,
            'stone_weight' => 21.510,
            'diamond_carat' => 20.880,
            'net_weight' => 72.387,
            'final_weight' => 72.387,
            'rate' => 513.35,
            'metal_value' => 34187.003,
            'quantity' => 8,
            'metal_qty' => 8,
            'making_amount' => 2786.984,
            'diamond_amount' => 7287.746,
            'stone_amount' => 31.045,
            'discount' => 0,
            'net_amount' => 44292.778,
            'amount' => 44292.778,
            'tax_amount' => 139.349,
            'net_amt_with_tax' => 44432.127,
            'purity' => '22',
            'category_name' => 'Gold',
        ],
        [
            'product_name' => 'Silver Ring (sample)',
            'design_no' => 'TAG-1002',
            'barcode' => 'HSN7114',
            'gross_weight' => 12.0,
            'less_weight' => 0,
            ...
            'amount' => 5000.00,
            'tax_amount' => 250.00,
            'net_amt_with_tax' => 5250.00,
            'purity' => '925',
            'category_name' => 'Silver',
        ],
    ];
    $payments = [
        ['payment_type' => 'bank', 'amount' => 15000.00],
        ['payment_type' => 'cash', 'amount' => 5000.00],
    ];
} elseif ($invoice_id <= 0) {
    die('Invalid invoice ID');
} else {
    try {
        $invoice = getRecord("SELECT * FROM $t_inv WHERE id = $invoice_id");
        if (!$invoice) {
    ...
    $payments = getList("SELECT * FROM $t_payments WHERE invoice_id = $invoice_id ORDER BY id ASC");
}

$print_settings_branch_id = function_exists('auragold_resolve_invoice_print_settings_branch_id')
    ? auragold_resolve_invoice_print_settings_branch_id($invoice ?? null)
    : (function_exists('auragold_settings_branch_id') ? auragold_settings_branch_id() : 0);

// Language from Invoice Print Settings (branch-scoped)
$invoice_secondary = function_exists('getInvoicePrintSettingsForDocument')
    ? (getInvoicePrintSettingsForDocument('sale_invoice', $print_settings_branch_id > 0 ? $print_settings_branch_id : null)['invoice_secondary_language'] ?? '')
    : '';
if (!in_array($invoice_secondary, ['hi', 'mr', 'ar'], true)) {
    $invoice_secondary = '';
}
$allowed_langs = function_exists('getInvoicePrintAllowedLanguages') ? getInvoicePrintAllowedLanguages('sale_invoice') : ['en'];
if (isset($_GET['lang'])) {
    ...
    $lang = ($invoice_secondary === 'ar') ? 'ar' : 'en';
}
...
foreach ($payments as $payment) {
    ...
    if (strpos($pt, 'cash') !== false) {
        $pt = 'cash';
    } elseif (strpos($pt, 'bank') !== false || strpos($pt, 'transfer') !== false) {
        $pt = 'bank';
    } elseif (strpos($pt, 'cheque') !== false || strpos($pt, 'check') !== false) {
        $pt = 'cheque';
    } elseif (strpos($pt, 'upi') !== false || strpos($pt, 'mobile') !== false) {
        $pt = 'upi';
    } elseif (strpos($pt, 'card') !== false) {
        $pt = 'card';
    } elseif (strpos($pt, 'metal') !== false || strpos($pt, 'exch') !== false) {
        $pt = 'metal_exchange';
    } elseif (strpos($pt, 'scrap') !== false) {
        $pt = 'scrap';
    }
    $amt = (float)($payment['amount'] ?? 0);
    if (isset($payment_totals[$pt])) {
        $payment_totals[$pt] += $amt;
    } else {
        $payment_totals['cash'] += $amt;
    }
}

$__money_sum = $payment_totals['cash'] + $payment_totals['bank'] + $payment_totals['cheque'] + $payment_totals['upi'] + $payment_totals['card'] + $payment_totals['metal_exchange'];
$__paid = (float) ($invoice['paid_amt'] ?? 0);
if ($__paid > 0.0001 && $__money_sum < 0.0001 && (float) ($payment_totals['scrap'] ?? 0) < 0.0001) {
    $payment_totals['cash'] = $__paid;
}

$currency = !empty($invoice['currency']) ? trim((string) $invoice['currency']) : 'AED';
$currency_symbol = invoice_print_currency_symbol(isset($conn) ? $conn : null, $currency);

$invoice_date_raw = !empty($invoice['invoice_date']) ? strtotime($invoice['invoice_date']) : time();
$invoice_date = date('d/m/Y', $invoice_date_raw);
$invoice_time = date('h:i A', $invoice_date_raw);
...
$additional_amt = (float)($invoice['additional_amt'] ?? 0);
$tax_amount = 0;
...
$advance_amt = (float)($invoice['advance_payment'] ?? 0);
...
$customer_ref = $invoice['ref_no'] ?? ($invoice['customer_id'] ?? '');
$customer_identity_no = '';
if (!empty($invoice['customer_id'])) {
    $cust = getRecord("SELECT * FROM tbl_customers WHERE id = " . (int)$invoice['customer_id'] . " LIMIT 1");
    if ($cust) {
        $customer_identity_no = trim((string)($cust['identity_no'] ?? ''));
        if (empty($customer_ref)) {
            $customer_ref = $cust['ref_no'] ?? $cust['mobile_no'] ?? $cust['phone'] ?? $cust['mobile'] ?? $invoice['customer_id'];
        }
    }
}
...
$L = [
    'en' => [
        'tax_invoice' => 'TAX INVOICE', 'invoice_no' => 'Invoice No.', 'date' => 'Date', 'salesman' => 'Salesman',
        ...
        'less_scrap' => 'Less: Scrap Purchased', 'balance_amount' => 'BALANCE AMOUNT',
        'amount_in_words' => 'Amount in Words', 'card' => 'Card', 'cash' => 'Cash', 'bank' => 'Bank', 'upi' => 'UPI', 'cheque' => 'Cheque', 'metal_exchange' => 'Metal Exchange', 'comment' => 'Comment',
    ...
    'hi' => [
        'tax_invoice' => 'कर चालान', 'invoice_no' => 'चालान सं.', 'date' => 'तारीख', 'salesman' => 'विक्रेता',
        ...
        'less_scrap' => 'घटाया: स्क्रैप खरीदा', 'balance_amount' => 'शेष राशि',
        'amount_in_words' => 'शब्दों में राशि', 'card' => 'कार्ड', 'cash' => 'नकद', 'bank' => 'बैंक', 'upi' => 'UPI', 'cheque' => 'चेक', 'metal_exchange' => 'मेटल एक्सचेंज', 'comment' => 'टिप्पणी',
    ...
    'mr' => [
        'tax_invoice' => 'कर चालान', 'invoice_no' => 'चालान क्र.', 'date' => 'तारीख', 'salesman' => 'विक्रेता',
        ...
        'less_scrap' => 'वजा: स्क्रॅप खरेदी', 'balance_amount' => 'शिल्लक रक्कम',
        'amount_in_words' => 'शब्दांमध्ये रक्कम', 'card' => 'कार्ड', 'cash' => 'रोख', 'bank' => 'बँक', 'upi' => 'UPI', 'cheque' => 'चेक', 'metal_exchange' => 'मेटल एक्सचेंज', 'comment' => 'टिप्पणी',
    ...
    'ar' => [
        'tax_invoice' => 'فاتورة ضريبية', 'invoice_no' => 'رقم الفاتورة', 'date' => 'التاريخ', 'salesman' => 'البائع',
        ...
        'net_wt' => 'الصافي الوزن', 'net_amt' => 'إجمالي المبلغ', 'vat_5' => 'المضافة ضريبة القيمة', 'total_amount' => 'المبلغ',
        'total' => 'المجموع', 'advance_amount' => 'المبلغ المسبق', 'total_before_vat' => 'المجموع قبل الضريبة',
        'vat_5_label' => 'ضريبة القيمة المضافة %5', 'total_including_vat' => 'المجموع مع الضريبة',
        'less_scrap' => 'ناقص الخردة', 'balance_amount' => 'مبلغ الرصيد',
        'amount_in_words' => 'المبلغ بالكلمات', 'card' => 'البطاقة', 'cash' => 'نقدي', 'bank' => 'بنك', 'upi' => 'UPI', 'cheque' => 'شيك', 'metal_exchange' => 'صرف معدني', 'comment' => 'تعليق',
...
$t = $L['en'];
$t2 = [];
if ($bilingual && isset($L[$invoice_secondary])) {
    $t2 = array_merge($L['en'], $L[$invoice_secondary]);
} elseif (!$bilingual) {
    $t = array_merge($L['en'], $L[$lang] ?? []);
}

function numberToWords($num, $currency = 'AED') {
...
$amount_words = numberToWords($grand_total, $currency);

// Invoice print settings (columns, header/footer toggles, layout type) — document type sale_invoice, fallback to default
$print_settings = (function_exists('getInvoicePrintSettingsForDocument')
    ? getInvoicePrintSettingsForDocument('sale_invoice', $print_settings_branch_id > 0 ? $print_settings_branch_id : null)
    : null) ?: (function_exists('getInvoicePrintSettingsDefaults') ? getInvoicePrintSettingsDefaults() : [
    ...
    'header_company_logo' => '1', 'header_company_name' => '1', 'header_gst_number' => '1', 'header_phone' => '1', 'header_invoice_title' => '1',
    'footer_terms_conditions' => '1', 'footer_authorized_signature' => '1', 'footer_thank_you_message' => '1', 'layout_type' => 'A4',
]);
if (!is_array($print_settings)) {
    $print_settings = function_exists('getInvoicePrintSettingsDefaults') ? getInvoicePrintSettingsDefaults() : [];
}
$selected_columns = isset($print_settings['sale_invoice_columns']) && is_array($print_settings['sale_invoice_columns']) ? $print_settings['sale_invoice_columns'] : ['sr_no','item_name','gross_weight','net_weight','rate','amount'];
$catalog_col_labels = function_exists('getInvoicePrintSaleInvoiceColumnLabels') ? getInvoicePrintSaleInvoiceColumnLabels() : [];
if (function_exists('auragold_invoice_print_extra_field_column_labels') && isset($conn)) {
    $catalog_col_labels = array_merge(
        $catalog_col_labels,
        auragold_invoice_print_extra_field_column_labels($conn, (int) $print_settings_branch_id)
    );
}
$col_labels = array_merge($catalog_col_labels, [
    'sr_no' => $t['sr'],
...
foreach ($selected_columns as $_sel_col) {
    if (!isset($col_labels[$_sel_col])) {
        $col_labels[$_sel_col] = ucwords(str_replace('_', ' ', (string) $_sel_col));
    }
}
...
if ($bilingual && !empty($t2)) {
    $col_labels_secondary = [
        'sr_no' => $t2['sr'] ?? '',
        'item_name' => $t2['description'] ?? '',
        'design_no' => $t2['design_no'] ?? '',
        'huid' => $t2['huid'] ?? '',
        'category' => $t2['category'] ?? '',
        'gross_weight' => $t2['weight'] ?? '',
        'less_weight' => $t2['less_weight'] ?? '',
        'net_weight' => $t2['net_wt'] ?? '',
        'purity_karat' => $t2['purity_karat'] ?? '',
        'rate' => $t2['rate'] ?? '',
        'making_charge' => $t2['making_charge'] ?? '',
        'diamond_amount' => $t2['diamond_amount'] ?? '',
        'stone_amount' => $t2['stone_amount'] ?? '',
...
if (isset($_GET['design_preview']) && function_exists('getInvoicePrintDesignTemplates')) {
    $valid_templates = array_column(getInvoicePrintDesignTemplates(), 'id');
    if (in_array($_GET['design_preview'], $valid_templates, true)) {
        $design_template = $_GET['design_preview'];
    }
}

// Resolve company header: print settings → branch profile → constants / e-Way GST
if (function_exists('auragold_resolve_invoice_print_company_header')) {
    $company_header = auragold_resolve_invoice_print_company_header($print_settings, (int) $print_settings_branch_id, [
    ...
    $company_name = $company_header['company_name'];
    $company_address = $company_header['company_address'];
    $company_trn = $company_header['company_trn'];
    $company_phone = $company_header['company_phone'];
    $company_landline = $company_header['company_landline'] ?? '';
    $company_email = $company_header['company_email'];
} else {
    ...
    if (!empty($print_settings['company_phone'])) {
        $company_phone = $print_settings['company_phone'];
    }
    if (!empty($print_settings['company_email'])) {
...
// Build item rows for template (each row = array of column values)
$item_rows = [];
$print_item_context = ['customer_identity_no' => $customer_identity_no];
$sr = 0;
foreach ($items as $item) {
    $sr++;
    if (function_exists('buildInvoicePrintItemRowFromSaleItem')) {
        $item_rows[] = buildInvoicePrintItemRowFromSaleItem($item, $sr, $print_item_context);
        continue;
    }
...
// Normalized document vars for templates
$doc_no = $invoice['invoice_no'] ?? '';
$doc_date = $invoice_date;
$doc_time = $invoice_time;
$party_name = $invoice['customer_name'] ?? '';
$party_ref = $customer_ref;
$person_label = $t['salesman'];
$person_value = $invoice['sales_person'] ?? '';
$doc_comment = $invoice['comment'] ?? '';
$doc_title = !empty($print_settings['invoice_title']) ? $print_settings['invoice_title'] : $t['tax_invoice'];
$back_url = 'sale-invoice.php';
$document = $invoice;

// Structural template: from settings or template_preview GET (validated)
$template = isset($_GET['template_preview']) ? trim((string)$_GET['template_preview']) : (function_exists('getInvoiceTemplateForDocument') ? getInvoiceTemplateForDocument('sale_invoice', $print_settings_branch_id > 0 ? $print_settings_branch_id : null) : 'template_classic');
...
<head>
  ...
  <title><?php echo $t['tax_invoice']; ?> - <?php echo htmlspecialchars($invoice['invoice_no']); ?></title>
  <style>
  ...
  <?php
    // Formal Naveen layout (structure template_6) uses .inv-naveen rules from design template_6; load them if design differs.
    ...
    if (($template ?? '') === 'template_8' && ($design_template ?? '') !== 'template_8') {
        echo '<style>' . getInvoicePrintTemplateCss('template_8') . '</style>';
    }
    if (($template ?? '') === 'template_9' && ($design_template ?? '') !== 'template_9') {
        echo '<style>' . getInvoicePrintTemplateCss('template_9') . '</style>';
    }
    if (($template ?? '') === 'template_10' && ($design_template ?? '') !== 'template_10') {
        echo '<style>' . getInvoicePrintTemplateCss('template_10') . '</style>';
    }
    if (($template ?? '') === 'template_11' && ($design_template ?? '') !== 'template_11') {
        echo '<style>' . getInvoicePrintTemplateCss('template_11') . '</style>';
    }
    ?>
  <?php endif; ?>
  <?php if (function_exists('invoicePrintEmitCustomCss')) {
      invoicePrintEmitCustomCss($print_settings);
  } ?>
</head>
