...
$items = getList("
    ...
    FROM tbl_receipt_voucher_items rvi
    LEFT JOIN tbl_products p ON rvi.product_id = p.id
    LEFT JOIN tbl_metal m ON rvi.metal_id = m.id
    WHERE rvi.voucher_id = $voucher_id
    ORDER BY rvi.id ASC
");
...
<head>
    ...
    <title><?php echo h($voucher['voucher_no'] ?? 'RV'); ?> — Receipt Voucher</title>
    <style>
...
<body<?php if (function_exists('invoicePrintBodyPaddingAttr')) echo invoicePrintBodyPaddingAttr($print_settings); ?>>
    ...
    <div class="invoice <?php echo h($design_template); ?>">
        ...
        <div class="inv-tax-badge"><?php echo h(!empty($print_settings['invoice_title']) ? $print_settings['invoice_title'] : 'Receipt Voucher'); ?> — <?php echo h($voucher['voucher_no'] ?? ''); ?></div>
        <?php endif; ?>

        <div class="sheet">
            <?php if (($print_settings['header_company_name'] ?? '1') !== '1'): ?>
            <h1 style="margin: 0 0 8px; font-size: 22px; color: #11294b;"><?php echo h($company_name); ?></h1>
            <p style="color:#64748b;font-size:14px;margin-bottom:24px;">Receipt Voucher — <?php echo h($voucher['voucher_no'] ?? ''); ?></p>
            <?php endif; ?>
