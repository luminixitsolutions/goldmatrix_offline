...
$mi_id = isset($_GET['id']) ? (int)$_GET['id'] : 0;

if ($mi_id <= 0) {
    die('Invalid Material Receive ID');
}

$mi = getRecord("SELECT * FROM tbl_material_receives WHERE id = $mi_id");
if (!$mi) {
    die('Material Receive not found');
}

$items = getList("SELECT * FROM tbl_material_receive_items WHERE material_receive_id = $mi_id ORDER BY id ASC");

$currency_symbol = 'AED';
$invoice_date = !empty($mi['order_date']) ? date('d F Y', strtotime($mi['order_date'])) : '';
$due_date = !empty($mi['due_date']) ? date('d F Y', strtotime($mi['due_date'])) : '';
$grand_total = (float)($mi['grand_total'] ?? 0);
...
<head>
  ...
  <title>Material Receive - <?php echo htmlspecialchars($mi['material_receive_no']); ?></title>
        ...
        <div class="tm_invoice_in">
          <div class="tm_invoice_head tm_top_head tm_mb20">
            <div class="tm_invoice_left">
              <div class="tm_logo">
                ...
                <p style="margin: 5px 0 0 0; color: #64748b; font-size: 12px;">Material Receive</p>
              </div>
            </div>
          ...
          <div class="tm_invoice_info tm_mb10">
            <div class="tm_invoice_info_left">
              ...
              <p>
                <b class="tm_f16 tm_primary_color"><?php echo htmlspecialchars($mi['customer_name'] ?? ''); ?></b><br>
                <span>Sale Order: #<?php echo htmlspecialchars($mi['sale_order_no'] ?? ''); ?></span>
              </p>
            </div>
            <div class="tm_invoice_info_right">
              <div class="tm_ternary_color tm_f50 tm_text_uppercase tm_text_center tm_invoice_title tm_mb15 tm_mobile_hide">Material Receive</div>
              <div class="tm_grid_row tm_col_3 tm_invoice_info_in tm_accent_bg">
                ...
                <div><span class="tm_white_color_60">Material Receive No:</span><br><b class="tm_f16 tm_white_color"><?php echo htmlspecialchars($mi['material_receive_no']); ?></b></div>
              </div>
