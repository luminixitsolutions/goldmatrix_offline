<?php
/**
 * Imitation / Watches stock — serial stock list (barcode-level) with full journal / cost columns.
 * Scoped to tbl_metal rows with display_name "Imitation Or Watches" (same as imitation-analysis.php).
 */
session_start();
...
require_once __DIR__ . '/includes/imitation_watches_stock_list_sql_include.php';

$sql = $imitation_watches_stock_list_sql . ' LIMIT 5000';
...
if ($gas_user_id > 0) {
    ...
    $gas_pref_rows = getList(
        "SELECT column_key, column_order, is_visible$wsel
         ...
         WHERE user_id = $gas_user_id AND page_name = 'imitation-and-watches' AND tab_key = '$gas_tab_esc'
...
$gas_js_prefs = [
    'pageName' => 'imitation-and-watches',
    'tabKey' => $tab,
...
<head>
    <title>Imitation / Watches - <?php echo htmlspecialchars(auragold_app_name(), ENT_QUOTES, 'UTF-8'); ?></title>
    <meta charset="utf-8">
            ...
            <div class="layout-content">
                <div class="container-fluid flex-grow-1" style="padding-top: 0;">
                    ...
                    <div class="gas-wrap">
                        <div class="gas-head">
                            <div>
                                <h1 class="gas-title">Imitation / Watches</h1>
                            </div>
                        </div>
...
(function () {
    ...
    btn.addEventListener('click', function () {
        window.location.href = 'ajax/export-imitation-watches-stock-excel.php';
    });
})();
</script>
<script>
...
(function () {
    ...
    function gasSaveState() {
        ...
        var fd = new FormData();
        fd.append('page_name', prefs.pageName || 'imitation-and-watches');
