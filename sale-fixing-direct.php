...
try {
    ...
    if (isset($conn) && $conn) {
        $table_check = @mysqli_query($conn, "SHOW TABLES LIKE 'tbl_sale_fixing_direct'");
        if ($table_check && mysqli_num_rows($table_check) > 0) {
            // Table exists, try to get last invoice
            $last_invoice = @getRecord("SELECT invoice_no FROM tbl_sale_fixing_direct ORDER BY id DESC LIMIT 1");
            if ($last_invoice && isset($last_invoice['invoice_no']) && $last_invoice['invoice_no']) {
                $last_num = (int)str_replace('SFD-', '', $last_invoice['invoice_no']);
                $next_order_no = 'SFD-' . ($last_num + 1);
            }
        }
    }
} catch (Exception $e) {
    // Table doesn't exist or has issues - use default
    error_log("Error checking tbl_sale_fixing_direct table: " . $e->getMessage());
} catch (Error $e) {
    // Handle fatal errors
    error_log("Fatal error checking tbl_sale_fixing_direct table: " . $e->getMessage());
}
...
if ($edit_order_id > 0) {
    try {
        $edit_order = @getRecord("SELECT * FROM tbl_sale_fixing_direct WHERE id = $edit_order_id");
        if ($edit_order) {
            $edit_items = @getList("SELECT * FROM tbl_sale_fixing_direct_items WHERE fixing_id = $edit_order_id");
            // Map invoice fields to order fields for JavaScript compatibility
            $edit_order['order_no'] = $edit_order['invoice_no'] ?? '';
            $edit_order['order_date'] = $edit_order['invoice_date'] ?? $edit_order['fixing_date'] ?? date('Y-m-d');
            $edit_order['order_id'] = $edit_order['id'];
            $next_order_no = $edit_order['invoice_no'] ?? $next_order_no;
        }
    } catch (Exception $e) {
        error_log("Error loading edit order: " . $e->getMessage());
    } catch (Error $e) {
        error_log("Fatal error loading edit order: " . $e->getMessage());
    }
}

// Get list of saved invoices for dropdown
try {
    $saved_orders = @getList("SELECT id, invoice_no as order_no, customer_name, COALESCE(invoice_date, fixing_date) as order_date, COALESCE(grand_total, total_amount) as grand_total FROM tbl_sale_fixing_direct ORDER BY id DESC LIMIT 50");
    if (!is_array($saved_orders)) {
        $saved_orders = [];
    }
} catch (Exception $e) {
    error_log("Error loading saved orders: " . $e->getMessage());
    $saved_orders = [];
} catch (Error $e) {
    error_log("Fatal error loading saved orders: " . $e->getMessage());
    $saved_orders = [];
}
?>
<!DOCTYPE html>
                            ...
                            <div class="card mb-4">
                                <div class="card-body billing-form">
                                        <div class="row">
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label>Sale Fixing Direct Invoice *</label>
                                                    <div style="position: relative;">
                            ...
                            <div class="summary-panel" >
                                ...
                                <div class="invoice-header" style="margin: -1rem -1rem 1rem -1rem; border-radius: 10px 10px 0 0; display: flex; align-items: center; justify-content: space-between;">
                                    <div style="display: flex; align-items: center; gap: 0.5rem; flex: 1;">
                                        ...
                                        <h5 class="mb-0" style="font-size: 0.9rem;">Sales Invoice No: <span id="currentOrderNo"><?php echo htmlspecialchars($next_order_no); ?></span></h5>
                                    </div>
                                    <div class="invoice-header-actions">
                                        <button class="btn-new-invoice btn-sm" onclick="resetOrder()">New +</button>
                                        <button class="btn-save-invoice btn-sm">Save</button>
                                    </div>
                                </div>
                                <!-- Previous Balance -->
                                <div class="summary-section">
                                    ...
                                    <div class="summary-row" id="advanceAmountRow" style="display: none;">
                                        <span class="summary-label" style="color: #16a34a;">Advance (-)</span>
                                        <span class="summary-value" id="advanceAmount" style="color: #16a34a;">0.00</span>
                                    </div>
                                    <div class="summary-row" id="adjustedBalanceRow" style="display: none; border-top: 1px solid #e2e8f0; margin-top: 4px; padding-top: 4px;">
                                        <span class="summary-label" style="font-weight: 600;">Adjusted Balance</span>
                                        <span class="summary-value" id="adjustedBalanceAmount" style="font-weight: 600;">0.00</span>
                                    </div>
                                    <div class="summary-row">
                                ...
                                <div class="summary-section">
                                    ...
                                    <div class="summary-row">
                                        ...
                                        <span class="summary-value" id="summaryTotal" style="color: #c5a864;">0.00</span>
                                    </div>
                                    <div class="summary-row">
                                        <span class="summary-label">Grand Total (AED)</span>
                                        <span class="summary-value" id="summaryGrandTotal" style="color: #c5a864; font-weight: 700;">0.00</span>
                                    </div>
                                    <div class="summary-row">
...
<script>
    ...
    $(document).ready(function() {
        ...
        // ================== SALE FIXING DIRECT INVOICE SEARCH FUNCTIONALITY ==================
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status === 'success' && response.invoices && response.invoices.length > 0) {
                            ...
                            saleInvoiceSuggestions.html('<div style="padding: 0.75rem; color: #94a3b8; text-align: center;">No invoices found</div>').show();
                        }
                    },
                    error: function() {
                        saleInvoiceSuggestions.hide();
    ...
    function initCategoryTabs() {
        document.querySelectorAll('.category-tab-btn').forEach(function(btn) {
            btn.addEventListener('click', function() {
                ...
                // Just filter existing products in the table by metal
                filterProductsByMetal(metalId);
            });
        });
        
        // Set initial metal (first tab) - but don't load products automatically
    ...
    // Fetch barcode details from stock journal and add to Product Selection table
        ...
        fetch('ajax/get-barcode-details.php?barcode=' + encodeURIComponent(barcode.trim()))
            ...
            .then(data => {
                if (data.success && data.data) {
                    // Add row to Product Selection table with fetched data
                    ...
                    alert(data.message || 'Barcode not found in stock journal');
                    if (barcodeInput) {
    ...
    // Open product search modal for main table row
        ...
        fetch(url)
            ...
            .then(data => {
                ...
                if (data.status === 'success' && data.balance) {
                    const originalAmount = Math.abs(parseFloat(data.original_balance?.amount || data.balance.amount) || 0);
                    const adjustedAmount = Math.abs(parseFloat(data.balance.amount) || 0);
                    const advanceAmount = Math.abs(parseFloat(data.advance?.amount || 0) || 0);
                    // Use original_balance for gold/silver so Previous Balance matches ledger
                    const gold = Math.abs(parseFloat(data.original_balance?.gold ?? data.balance.gold) || 0);
                    const silver = Math.abs(parseFloat(data.original_balance?.silver ?? data.balance.silver) || 0);
                    ...
                    if (prevBalanceAmtEl) {
                        prevBalanceAmtEl.textContent = originalAmount.toFixed(2);
                        // Store ORIGINAL balance (before advance deduction) for calculations
                        // The advance will be deducted separately in Balance Amt calculation
                        prevBalanceAmtEl.setAttribute('data-original-balance', originalAmount.toFixed(2));
    ...
    function updateSummaryPanel() {
        ...
        // Balance Amt = Grand Total (Net Amt+Tax) - Paid Current Order Amount - Adjusted Balance Amount
        // Adjusted Balance should be DEDUCTED from the balance
        const balanceAmt = totalNetAmtTax - paidCurrentOrderAmt - adjustedBalanceAmount;
        ...
        if (summaryBalanceAmt) {
            ...
            console.log('Balance Amt Calculation:', {
                totalNetAmtTax: totalNetAmtTax.toFixed(2),
                paidCurrentOrderAmt: paidCurrentOrderAmt.toFixed(2),
                adjustedBalanceAmount: adjustedBalanceAmount.toFixed(2),
                balanceAmt: balanceAmt.toFixed(2),
                formula: 'Balance Amt = Grand Total - Paid Current Order - Adjusted Balance'
            });
        }
    }
    ...
    // Save invoice to database (tbl_sale_fixing_direct)
    function saveOrder() {
        ...
        if (typeof jQuery !== 'undefined' && jQuery.ajax) {
            jQuery.ajax({
                url: 'ajax/save-sale-fixing-direct.php',
                type: 'POST',
                data: postData,
                dataType: 'json',
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        const invoiceId = response.order_id || response.invoice_id;
                        if (invoiceId) {
                            // Store invoice ID for print modal
                            ...
                            alert('Invoice saved successfully! Invoice No: ' + response.order_no);
            ...
            fetch('ajax/save-sale-fixing-direct.php', {
                method: 'POST',
                body: formData
            })
            .then(response => {
            ...
            .then(data => {
                ...
                if (data.status === 'success') {
                    const invoiceId = data.order_id || data.invoice_id;
                    if (invoiceId) {
                        // Store invoice ID for print modal
                        savedInvoiceId = invoiceId;
                        // Show print confirmation modal
                        ...
                        alert('Invoice saved successfully! Invoice No: ' + data.order_no);
                        // Clear form after save
                        setTimeout(function() {
                            clearInvoiceForm();
                        }, 500);
    ...
    // Reset invoice form (with confirmation)
    function resetOrder() {
        ...
        if (typeof jQuery !== 'undefined' && jQuery.ajax) {
            jQuery.ajax({
                ...
                error: function(xhr, status, error) {
                    ...
                    console.error('Load invoice error:', xhr.responseText);
                }
            });
        } else {
...
<script>
    ...
    function showPrintInvoiceModal(invoiceId) {
        ...
        setTimeout(function() {
            ...
            if (modal) {
                ...
                if (confirm('Sale Fixing Direct Invoice saved successfully! Do you want to print the invoice?')) {
    ...
    function confirmPrintInvoice() {
        if (savedInvoiceId) {
            // Open sale fixing print page
            window.open('sale-fixing-print.php?id=' + savedInvoiceId, '_blank', 'width=1200,height=800');
        }
    ...
    // Function to print sale fixing direct invoice
